...
fn main() -> ExitCode {
    eprintln!(
        "zkai_attention_kv_native_d128_single_head_seq16_fused_softmax_table_proof requires --features stwo-backend"
    );
    ExitCode::from(2)
}
    ...
    I: IntoIterator<Item = OsString>,
{
    ...
    if args.is_empty() {
        return Err("usage: zkai_attention_kv_native_d128_single_head_seq16_fused_softmax_table_proof prove <source-input.json> <fused-envelope.json> | verify <fused-envelope.json>".to_string());
    }
    ...
    match mode.as_str() {
        "prove" => {
            ...
            let envelope =
                ...
                .map_err(|error| error.to_string())?;
            let verified =
                verify_zkai_attention_kv_native_d128_single_head_seq16_fused_softmax_table_envelope(&envelope)
                    .map_err(|error| error.to_string())?;
            if !verified {
                return Err("fused Softmax-table envelope verification returned false".to_string());
            }
            ...
            Ok(serde_json::json!({
                "schema": "zkai-attention-kv-stwo-native-d128-single-head-seq16-fused-softmax-table-cli-summary-v1",
                "mode": "prove",
        ...
        "verify" => {
            ...
            let envelope =
                zkai_attention_kv_native_d128_single_head_seq16_fused_softmax_table_envelope_from_json_slice(&raw)
                    .map_err(|error| error.to_string())?;
            let verified =
                verify_zkai_attention_kv_native_d128_single_head_seq16_fused_softmax_table_envelope(&envelope)
                    .map_err(|error| error.to_string())?;
...
mod tests {
    ...
    fn temp_path(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "ptvm-d128-single-head-seq16-fused-softmax-cli-{label}-{}-{nonce}.json",
            std::process::id()
        ))
    }
    ...
    fn rejects_missing_or_unknown_cli_args() {
        ...
        assert_eq!(
            ...
            "usage: prove <source-input.json> <fused-envelope.json>"
        );
        assert_eq!(
            run_with_args(vec![arg("verify")]).expect_err("bad verify args must reject"),
            "usage: verify <fused-envelope.json>"
        );
    }
    ...
    fn rejects_oversized_envelope_before_deserialization() {
        let envelope = write_temp(
            ...
            &vec![
                b'0';
                ZKAI_ATTENTION_KV_NATIVE_D128_SINGLE_HEAD_SEQ16_FUSED_SOFTMAX_TABLE_MAX_ENVELOPE_JSON_BYTES + 1
            ],
        );
        let error = run_with_args(vec![arg("verify"), envelope.as_os_str().to_os_string()])
            .expect_err("oversized envelope must reject");
        assert!(error.contains("fused envelope JSON exceeds max size"));
        let _ = fs::remove_file(envelope);
    }
    ...
    fn rejects_tampered_envelope_proof_bytes() {
        let mut envelope: serde_json::Value = serde_json::from_slice(include_bytes!(
            "../../docs/engineering/evidence/zkai-attention-kv-stwo-native-d128-single-head-seq16-fused-softmax-table-proof-2026-05.envelope.json"
        ))
        .expect("checked fused envelope fixture");
        let proof = envelope
            .get_mut("proof")
