...
fn run() -> Result<String, String> {
    ...
    match mode.as_str() {
        ...
        "build-input-preprocessed-anchor" => {
            if args.len() != 3 {
                return Err("usage: build-input-preprocessed-anchor <attention-source.json> <mlp-input.json> <single-input.json>".to_string());
            }
            let attention_path = PathBuf::from(&args[0]);
            let mlp_path = PathBuf::from(&args[1]);
            let output_path = PathBuf::from(&args[2]);
            ...
            let bytes = pretty_json_bytes_with_trailing_newline(
                &input,
                ZKAI_NATIVE_ATTENTION_MLP_SINGLE_PROOF_MAX_INPUT_JSON_BYTES,
                "single proof preprocessed-anchor input JSON",
            )?;
            atomic_write_file(
                &output_path,
                &bytes,
                "single proof preprocessed-anchor input",
            )?;
            Ok(serde_json::json!({
                "schema": "zkai-native-attention-mlp-single-proof-cli-summary-v1",
                "mode": "build-input-preprocessed-anchor",
                "input_path": output_path.display().to_string(),
                "input_size_bytes": bytes.len(),
                "statement_commitment": input.statement_commitment,
                "public_instance_commitment": input.public_instance_commitment,
        ...
        "build-input-rmsnorm-fused" => {
            if args.len() != 3 {
                return Err("usage: build-input-rmsnorm-fused <attention-source.json> <mlp-input.json> <single-input.json>".to_string());
            }
            let attention_path = PathBuf::from(&args[0]);
            let mlp_path = PathBuf::from(&args[1]);
            let output_path = PathBuf::from(&args[2]);
            ...
            let bytes = pretty_json_bytes_with_trailing_newline(
                &input,
                ZKAI_NATIVE_ATTENTION_MLP_SINGLE_PROOF_MAX_INPUT_JSON_BYTES,
                "single proof RMSNorm-fused input JSON",
            )?;
            atomic_write_file(&output_path, &bytes, "single proof RMSNorm-fused input")?;
            Ok(serde_json::json!({
                "schema": "zkai-native-attention-mlp-single-proof-cli-summary-v1",
                "mode": "build-input-rmsnorm-fused",
                "input_path": output_path.display().to_string(),
                "input_size_bytes": bytes.len(),
                "statement_commitment": input.statement_commitment,
                "public_instance_commitment": input.public_instance_commitment,
        ...
        "build-input-rmsnorm-fused-label-probe-a" => build_input_with_adapter_mode(
            &args,
            ZkAiNativeAttentionMlpAdapterMode::RmsnormInputFusedLabelProbeA,
            "build-input-rmsnorm-fused-label-probe-a",
            "single proof RMSNorm-fused label probe A input",
        ),
        "build-input-rmsnorm-fused-adjacent" => build_input_with_adapter_mode(
            ...
            "single proof RMSNorm-fused adjacent fixed-column input",
        ),
        "build-input-rmsnorm-fused-adjacent-label-probe-a" => build_input_with_adapter_mode(
            &args,
            ZkAiNativeAttentionMlpAdapterMode::RmsnormInputFusedAdjacentLabelProbeA,
            "build-input-rmsnorm-fused-adjacent-label-probe-a",
            "single proof RMSNorm-fused adjacent label probe A input",
        ),
        "build-input-rmsnorm-fused-adjacent-label-probe-b" => build_input_with_adapter_mode(
            &args,
        ...
        "build-input-rmsnorm-fused-label-probe-b" => build_input_with_adapter_mode(
            ...
            "single proof RMSNorm-fused label probe B input",
        ),
        "prove" => {
            if args.len() != 2 {
...
fn build_input_with_adapter_mode(
    ...
    if args.len() != 3 {
        return Err(format!(
            "usage: {cli_mode} <attention-source.json> <mlp-input.json> <single-input.json>"
        ));
    }
    let attention_path = PathBuf::from(&args[0]);
    let mlp_path = PathBuf::from(&args[1]);
    ...
    let attention =
        zkai_attention_kv_native_d8_fused_softmax_table_source_input_from_json_str(&attention_raw)
            .map_err(|error| error.to_string())?;
    let mlp_raw = read_bounded_utf8(
        &mlp_path,
        ZKAI_D128_RMSNORM_MLP_FUSED_MAX_JSON_BYTES,
    ...
    atomic_write_file(&output_path, &bytes, label)?;
    Ok(serde_json::json!({
        "schema": "zkai-native-attention-mlp-single-proof-cli-summary-v1",
        "mode": cli_mode,
        "input_path": output_path.display().to_string(),
