...
fn main() -> ExitCode {
    eprintln!(
        "zkai_attention_kv_native_d128_four_head_seq64_softmax_table_lookup_proof requires --features stwo-backend"
    );
    ExitCode::from(2)
}
...
fn run_with_args(mut args: Vec<OsString>) -> Result<String, String> {
    if args.is_empty() {
        return Err("usage: zkai_attention_kv_native_d128_four_head_seq64_softmax_table_lookup_proof prove <source-input.json> <lookup-envelope.json> | verify <lookup-envelope.json>".to_string());
    }
    ...
    match mode.as_str() {
        "prove" => {
            ...
            require_verified(
                ...
                "lookup d128 four-head seq64 envelope",
            )?;
            ...
            Ok(serde_json::json!({
                "schema": "zkai-attention-kv-stwo-native-d128-four-head-seq64-softmax-table-logup-sidecar-cli-summary-v1",
                "mode": "prove",
...
mod tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "zkai-d128-four-head-seq64-lookup-cli-{label}-{nonce}-{}",
            std::process::id()
        ));
