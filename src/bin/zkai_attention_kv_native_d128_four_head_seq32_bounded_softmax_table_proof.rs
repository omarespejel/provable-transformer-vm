...
fn main() -> ExitCode {
    eprintln!(
        "zkai_attention_kv_native_d128_four_head_seq32_bounded_softmax_table_proof requires --features stwo-backend"
    );
    ExitCode::from(2)
}
...
fn run_with_args(mut args: Vec<std::ffi::OsString>) -> Result<String, String> {
    if args.is_empty() {
        return Err("usage: zkai_attention_kv_native_d128_four_head_seq32_bounded_softmax_table_proof prove <input.json> <envelope.json> | verify <envelope.json>".to_string());
    }
    ...
    match mode.as_str() {
        "prove" => {
            ...
            require_verified(
                ...
                "bounded d128 four-head seq32 envelope",
            )?;
            ...
            atomic_write_file(&envelope_path, &envelope_bytes, "envelope")?;
            let input_path_json = input_path.to_string_lossy().into_owned();
            let envelope_path_json = envelope_path.to_string_lossy().into_owned();
            Ok(serde_json::json!({
                "schema": "zkai-attention-kv-stwo-native-d128-four-head-seq32-bounded-softmax-table-cli-summary-v1",
                "mode": "prove",
...
mod tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "zkai-seq64-bounded-cli-{label}-{}-{nonce}",
            process::id()
        ))
    }
    ...
    fn require_verified_rejects_false_verifier_result() {
        let error = require_verified(Ok::<bool, &str>(false), "fixture envelope")
            .expect_err("false verifier result rejected");
        assert!(error.contains("fixture envelope proof verification returned false"));
    }
}
