...
fn main() -> ExitCode {
    eprintln!(
        "zkai_attention_kv_native_d64_single_head_longseq_fused_softmax_table_proof requires --features stwo-backend"
    );
    ExitCode::from(2)
}
    ...
    I: IntoIterator<Item = OsString>,
{
    ...
    if args.is_empty() {
        return Err("usage: zkai_attention_kv_native_d64_single_head_longseq_fused_softmax_table_proof prove <source-input.json> <fused-envelope.json> | verify <fused-envelope.json>".to_string());
    }
    ...
    match mode.as_str() {
        "prove" => {
            ...
            let envelope =
                prove_zkai_attention_kv_native_d64_single_head_longseq_fused_softmax_table_envelope(&source_input)
                    .map_err(|error| error.to_string())?;
            let verified =
                verify_zkai_attention_kv_native_d64_single_head_longseq_fused_softmax_table_envelope(&envelope)
                    .map_err(|error| error.to_string())?;
            if !verified {
            ...
            Ok(serde_json::json!({
                "schema": "zkai-attention-kv-stwo-native-d64-single-head-longseq-fused-softmax-table-cli-summary-v1",
                "mode": "prove",
        ...
        "verify" => {
            ...
            let envelope =
                zkai_attention_kv_native_d64_single_head_longseq_fused_softmax_table_envelope_from_json_slice(&raw)
                    .map_err(|error| error.to_string())?;
            let verified =
                verify_zkai_attention_kv_native_d64_single_head_longseq_fused_softmax_table_envelope(&envelope)
                    .map_err(|error| error.to_string())?;
...
mod tests {
    ...
    fn temp_path(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "ptvm-d64-single-head-longseq-fused-softmax-cli-{label}-{}-{nonce}.json",
            std::process::id()
        ))
    }
    ...
    fn rejects_oversized_envelope_before_deserialization() {
        let envelope = write_temp(
            ...
            &vec![
                b'0';
                ZKAI_ATTENTION_KV_NATIVE_D64_SINGLE_HEAD_LONGSEQ_FUSED_SOFTMAX_TABLE_MAX_ENVELOPE_JSON_BYTES + 1
            ],
        );
        let error = run_with_args(vec![arg("verify"), envelope.as_os_str().to_os_string()])
    ...
    fn rejects_tampered_envelope_proof_bytes() {
        let mut envelope: serde_json::Value = serde_json::from_slice(include_bytes!(
            "../../docs/engineering/evidence/zkai-attention-kv-stwo-native-d64-single-head-longseq-fused-softmax-table-proof-2026-05.envelope.json"
        ))
