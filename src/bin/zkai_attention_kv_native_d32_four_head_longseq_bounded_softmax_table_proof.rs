...
fn main() -> ExitCode {
    eprintln!(
        "zkai_attention_kv_native_d32_four_head_longseq_bounded_softmax_table_proof requires --features stwo-backend"
    );
    ExitCode::from(2)
}
...
fn run_with_args(mut args: Vec<std::ffi::OsString>) -> Result<String, String> {
    if args.is_empty() {
        return Err("usage: zkai_attention_kv_native_d32_four_head_longseq_bounded_softmax_table_proof prove <input.json> <envelope.json> | verify <envelope.json>".to_string());
    }
    ...
    match mode.as_str() {
        "prove" => {
            ...
            Ok(serde_json::json!({
                "schema": "zkai-attention-kv-stwo-native-d32-four-head-longseq-bounded-softmax-table-cli-summary-v1",
                "mode": "prove",
