...
fn main() -> ExitCode {
    eprintln!(
        "zkai_attention_kv_native_d64_single_head_longseq_softmax_table_lookup_proof requires --features stwo-backend"
    );
    ExitCode::from(2)
}
    ...
    I: IntoIterator<Item = OsString>,
{
    ...
    if args.is_empty() {
        return Err("usage: zkai_attention_kv_native_d64_single_head_longseq_softmax_table_lookup_proof prove <source-input.json> <lookup-envelope.json> | verify <lookup-envelope.json>".to_string());
    }
    ...
    match mode.as_str() {
        "prove" => {
            ...
            let envelope =
                prove_zkai_attention_kv_native_d64_single_head_longseq_softmax_table_lookup_envelope(&source_input)
                    .map_err(|error| error.to_string())?;
            let verified =
                verify_zkai_attention_kv_native_d64_single_head_longseq_softmax_table_lookup_envelope(&envelope)
                    .map_err(|error| error.to_string())?;
            if !verified {
            ...
            Ok(serde_json::json!({
                "schema": "zkai-attention-kv-stwo-native-d64-single-head-longseq-softmax-table-logup-sidecar-cli-summary-v1",
                "mode": "prove",
        ...
        "verify" => {
            ...
            let envelope =
                zkai_attention_kv_native_d64_single_head_longseq_softmax_table_lookup_envelope_from_json_slice(&raw)
                    .map_err(|error| error.to_string())?;
            let verified =
                verify_zkai_attention_kv_native_d64_single_head_longseq_softmax_table_lookup_envelope(&envelope)
                    .map_err(|error| error.to_string())?;
...
mod tests {
    ...
    fn temp_path(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "ptvm-d64-single-head-longseq-softmax-table-lookup-cli-{label}-{}-{nonce}.json",
            std::process::id()
        ))
    }
