...
fn main() -> ExitCode {
    eprintln!(
        "zkai_attention_kv_native_d256_two_head_seq32_fused_softmax_table_proof requires --features stwo-backend"
    );
    ExitCode::from(2)
}
...
fn run_with_args(mut args: Vec<std::ffi::OsString>) -> Result<String, String> {
    if args.is_empty() {
        return Err("usage: zkai_attention_kv_native_d256_two_head_seq32_fused_softmax_table_proof prove <source-input.json> <fused-envelope.json> | verify <fused-envelope.json>".to_string());
    }
    ...
    match mode.as_str() {
        "prove" => {
            ...
            require_verified(
                ...
                "fused d256 two-head seq32 envelope",
            )?;
            ...
            Ok(serde_json::json!({
                "schema": "zkai-attention-kv-stwo-native-d256-two-head-seq32-fused-softmax-table-cli-summary-v1",
                "mode": "prove",
