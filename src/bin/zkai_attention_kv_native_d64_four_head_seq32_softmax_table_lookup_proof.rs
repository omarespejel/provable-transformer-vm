...
fn main() -> ExitCode {
    eprintln!(
        "zkai_attention_kv_native_d64_four_head_seq32_softmax_table_lookup_proof requires --features stwo-backend"
    );
    ExitCode::from(2)
}
...
fn run_with_args(mut args: Vec<OsString>) -> Result<String, String> {
    if args.is_empty() {
        return Err("usage: zkai_attention_kv_native_d64_four_head_seq32_softmax_table_lookup_proof prove <source-input.json> <lookup-envelope.json> | verify <lookup-envelope.json>".to_string());
    }
    ...
    match mode.as_str() {
        "prove" => {
            ...
            Ok(serde_json::json!({
                "schema": "zkai-attention-kv-stwo-native-d64-four-head-seq32-softmax-table-logup-sidecar-cli-summary-v1",
                "mode": "prove",
...
mod tests {
    ...
    fn temp_dir(label: &str) -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "zkai-d64-four-head-seq32-lookup-cli-{label}-{nonce}-{}",
            std::process::id()
        ));
