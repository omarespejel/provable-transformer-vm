...
use super::attention_kv_native_two_head_longseq_bounded_softmax_table_proof::{
    validate_zkai_attention_kv_native_two_head_longseq_bounded_softmax_table_input,
...
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_LONGSEQ_FUSED_SOFTMAX_TABLE_PROOF_VERSION: &str =
    "stwo-attention-kv-two-head-longseq-fused-bounded-softmax-table-logup-proof-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_LONGSEQ_FUSED_SOFTMAX_TABLE_BACKEND_VERSION: &str =
    "stwo-attention-kv-two-head-longseq-fused-bounded-softmax-table-logup-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_LONGSEQ_FUSED_SOFTMAX_TABLE_STATEMENT_VERSION: &str =
    "zkai-attention-kv-stwo-native-two-head-longseq-fused-softmax-table-logup-statement-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_LONGSEQ_FUSED_SOFTMAX_TABLE_SEMANTIC_SCOPE: &str =
    "two_head_longseq_d8_bounded_softmax_table_attention_arithmetic_and_logup_membership_fused_in_one_native_stwo_proof";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_LONGSEQ_FUSED_SOFTMAX_TABLE_DECISION: &str =
    "GO_NATIVE_STWO_FUSED_ATTENTION_ARITHMETIC_AND_SOFTMAX_TABLE_LOGUP_MEMBERSHIP";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_LONGSEQ_FUSED_SOFTMAX_TABLE_TARGET_ID: &str =
    "attention-kv-two-head-longseq-d8-causal-mask-fused-bounded-softmax-table-logup-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_LONGSEQ_FUSED_SOFTMAX_TABLE_VERIFIER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-longseq-fused-bounded-softmax-table-logup:v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_LONGSEQ_FUSED_SOFTMAX_TABLE_MAX_ENVELOPE_JSON_BYTES:
    usize = 2_097_152;
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_LONGSEQ_FUSED_SOFTMAX_TABLE_MAX_PROOF_BYTES: usize =
    131_072;

const ISSUE: usize = 498;
const SOURCE_ISSUE: usize = 498;
const SIDECAR_ISSUE: usize = 500;
const SOURCE_PLUS_SIDECAR_RAW_PROOF_BYTES: usize = 79_444;
...
const NON_FUSED_STATUS: &str =
    "GO_MATCHED_TWO_HEAD_LONGSEQ_SOURCE_PLUS_LOGUP_SIDECAR_COMPARATOR_RECORDED";
...
const PREPROCESSED_TABLE_GAP: &str =
    "zkai/attention-kv/native-two-head-longseq-fused-softmax-table-logup/table-gap";
const PREPROCESSED_TABLE_WEIGHT: &str =
    "zkai/attention-kv/native-two-head-longseq-fused-softmax-table-logup/table-weight";
const PREPROCESSED_TABLE_MULTIPLICITY: &str =
    "zkai/attention-kv/native-two-head-longseq-fused-softmax-table-logup/table-multiplicity";
const FUSED_COLUMN_PREFIX: &str =
    "zkai/attention-kv/native-two-head-longseq-fused-softmax-table-logup";
...
relation!(AttentionKvTwoHeadLongseqFusedSoftmaxTableRelation, 2);

#[derive(Debug, Clone)]
struct AttentionKvNativeTwoHeadLongseqFusedSoftmaxTableEval {
    lookup_elements: AttentionKvTwoHeadLongseqFusedSoftmaxTableRelation,
}

impl FrameworkEval for AttentionKvNativeTwoHeadLongseqFusedSoftmaxTableEval {
...
pub struct AttentionKvTwoHeadLongseqFusedSoftmaxTableMultiplicity {
    pub gap: usize,
    pub weight: i64,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeTwoHeadLongseqFusedSoftmaxTableSummary {
    pub issue: usize,
    ...
    pub table_multiplicities: Vec<AttentionKvTwoHeadLongseqFusedSoftmaxTableMultiplicity>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeTwoHeadLongseqFusedSoftmaxTableEnvelope {
    ...
    pub fused_summary: ZkAiAttentionKvNativeTwoHeadLongseqFusedSoftmaxTableSummary,
...
#[serde(deny_unknown_fields)]
struct AttentionKvNativeTwoHeadLongseqFusedSoftmaxTableProofPayload {
...
struct FusedBundle {
    log_size: u32,
    summary: ZkAiAttentionKvNativeTwoHeadLongseqFusedSoftmaxTableSummary,
...
pub fn zkai_attention_kv_native_two_head_longseq_fused_softmax_table_source_input_from_json_str(
    raw_json: &str,
...
pub fn prove_zkai_attention_kv_native_two_head_longseq_fused_softmax_table_envelope(
    source_input: &ZkAiAttentionKvNativeTwoHeadLongseqBoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeTwoHeadLongseqFusedSoftmaxTableEnvelope> {
    validate_source_input(source_input)?;
    let bundle = build_fused_bundle(source_input)?;
    let proof = prove_fused(&bundle)?;
    let envelope = ZkAiAttentionKvNativeTwoHeadLongseqFusedSoftmaxTableEnvelope {
        proof_backend: StarkProofBackend::Stwo,
        proof_backend_version:
            ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_LONGSEQ_FUSED_SOFTMAX_TABLE_BACKEND_VERSION
                .to_string(),
        proof_schema_version:
            ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_LONGSEQ_FUSED_SOFTMAX_TABLE_PROOF_VERSION.to_string(),
        statement_version:
            ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_LONGSEQ_FUSED_SOFTMAX_TABLE_STATEMENT_VERSION
                .to_string(),
        semantic_scope:
            ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_LONGSEQ_FUSED_SOFTMAX_TABLE_SEMANTIC_SCOPE.to_string(),
        decision: ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_LONGSEQ_FUSED_SOFTMAX_TABLE_DECISION
            .to_string(),
        target_id: ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_LONGSEQ_FUSED_SOFTMAX_TABLE_TARGET_ID
            .to_string(),
        verifier_domain:
...
pub fn zkai_attention_kv_native_two_head_longseq_fused_softmax_table_envelope_from_json_slice(
    raw_json: &[u8],
) -> Result<ZkAiAttentionKvNativeTwoHeadLongseqFusedSoftmaxTableEnvelope> {
    if raw_json.len()
...
pub fn verify_zkai_attention_kv_native_two_head_longseq_fused_softmax_table_envelope(
    envelope: &ZkAiAttentionKvNativeTwoHeadLongseqFusedSoftmaxTableEnvelope,
) -> Result<bool> {
    validate_envelope(envelope)?;
...
fn validate_source_input(
    ...
    validate_zkai_attention_kv_native_two_head_longseq_bounded_softmax_table_input(input)
}

fn fused_summary(
    input: &ZkAiAttentionKvNativeTwoHeadLongseqBoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeTwoHeadLongseqFusedSoftmaxTableSummary> {
    let mut multiplicities = input
        .weight_table
        .iter()
        .map(
            |entry| AttentionKvTwoHeadLongseqFusedSoftmaxTableMultiplicity {
                gap: entry.gap,
                weight: entry.weight,
                multiplicity: 0,
            },
    ...
    Ok(
        ZkAiAttentionKvNativeTwoHeadLongseqFusedSoftmaxTableSummary {
            ...
            lookup_relation: "AttentionKvTwoHeadLongseqFusedSoftmaxTableRelation".to_string(),
...
fn prove_fused(bundle: &FusedBundle) -> Result<Vec<u8>> {
    let component = fused_component(AttentionKvTwoHeadLongseqFusedSoftmaxTableRelation::dummy());
    ...
    let lookup_elements = AttentionKvTwoHeadLongseqFusedSoftmaxTableRelation::draw(channel);
    ...
    serde_json::to_vec(
        &AttentionKvNativeTwoHeadLongseqFusedSoftmaxTableProofPayload { stark_proof },
    )
    .map_err(|error| VmError::Serialization(error.to_string()))
}
...
fn verify_fused(
    ...
    let payload: AttentionKvNativeTwoHeadLongseqFusedSoftmaxTableProofPayload =
    ...
    let component_placeholder =
        fused_component(AttentionKvTwoHeadLongseqFusedSoftmaxTableRelation::dummy());
...
fn fused_component(
    lookup_elements: AttentionKvTwoHeadLongseqFusedSoftmaxTableRelation,
) -> FrameworkComponent<AttentionKvNativeTwoHeadLongseqFusedSoftmaxTableEval> {
    FrameworkComponent::new(
        &mut TraceLocationAllocator::new_with_preprocessed_columns(&fused_preprocessed_column_ids()),
        AttentionKvNativeTwoHeadLongseqFusedSoftmaxTableEval { lookup_elements },
        SecureField::zero(),
    )
}
...
mod tests {
    ...
    #[test]
    fn attention_kv_two_head_longseq_fused_softmax_table_round_trips_real_proof() {
        let input = source_input();
        let envelope =
            prove_zkai_attention_kv_native_two_head_longseq_fused_softmax_table_envelope(&input)
    ...
    #[test]
    fn attention_kv_two_head_longseq_fused_softmax_table_summary_counts_claims() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_two_head_longseq_fused_softmax_table_derives_logup_indices_from_column_ids() {
        let row_ids = fused_row_column_ids();
        let preprocessed_ids = fused_preprocessed_column_ids();
        let indices = fused_trace_column_indices();
    ...
    #[test]
    fn attention_kv_two_head_longseq_fused_softmax_table_rejects_summary_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_two_head_longseq_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
        envelope.fused_summary.lookup_claims += 1;
        let error = verify_zkai_attention_kv_native_two_head_longseq_fused_softmax_table_envelope(
            &envelope,
        )
    ...
    #[test]
    fn attention_kv_two_head_longseq_fused_softmax_table_rejects_source_weight_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_two_head_longseq_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_two_head_longseq_fused_softmax_table_rejects_negative_bit_material() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_two_head_longseq_fused_softmax_table_rejects_output_remainder_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_two_head_longseq_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_two_head_longseq_fused_softmax_table_rejects_proof_byte_tamper() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_two_head_longseq_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_two_head_longseq_fused_softmax_table_rejects_backend_version_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_two_head_longseq_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_two_head_longseq_fused_softmax_table_rejects_proof_schema_version_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_two_head_longseq_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_two_head_longseq_fused_softmax_table_rejects_unknown_envelope_field() {
        let input = source_input();
        let envelope =
            prove_zkai_attention_kv_native_two_head_longseq_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
