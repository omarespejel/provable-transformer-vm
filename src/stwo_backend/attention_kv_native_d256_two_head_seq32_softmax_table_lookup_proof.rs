...
pub const ZKAI_ATTENTION_KV_NATIVE_D256_TWO_HEAD_SEQ32_SOFTMAX_TABLE_LOOKUP_SEMANTIC_SCOPE: &str =
    "d256_two_head_seq32_bounded_softmax_table_membership_constrained_by_native_stwo_logup_sidecar";
pub const ZKAI_ATTENTION_KV_NATIVE_D256_TWO_HEAD_SEQ32_SOFTMAX_TABLE_LOOKUP_DECISION: &str =
    "GO_NATIVE_STWO_AIR_CONSTRAINED_SOFTMAX_TABLE_LOOKUP_RELATION_SIDECAR";
pub const ZKAI_ATTENTION_KV_NATIVE_D256_TWO_HEAD_SEQ32_SOFTMAX_TABLE_LOOKUP_TARGET_ID: &str =
    "attention-kv-d256-two-head-seq32-causal-mask-bounded-softmax-table-logup-sidecar-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D256_TWO_HEAD_SEQ32_SOFTMAX_TABLE_LOOKUP_VERIFIER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d256-two-head-seq32-softmax-table-logup-sidecar:v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D256_TWO_HEAD_SEQ32_SOFTMAX_TABLE_LOOKUP_MAX_ENVELOPE_JSON_BYTES:
    usize = 134_217_728;
...
const PREPROCESSED_TABLE_GAP: &str =
    "zkai/attention-kv/native-d256-two-head-seq32-softmax-table-logup/table-gap";
const PREPROCESSED_TABLE_WEIGHT: &str =
    "zkai/attention-kv/native-d256-two-head-seq32-softmax-table-logup/table-weight";
const PREPROCESSED_TABLE_MULTIPLICITY: &str =
    "zkai/attention-kv/native-d256-two-head-seq32-softmax-table-logup/table-multiplicity";

relation!(AttentionKvD256TwoHeadSeq32SoftmaxTableLookupRelation, 2);

#[derive(Debug, Clone)]
struct AttentionKvD256TwoHeadSeq32SoftmaxTableLookupEval {
    lookup_elements: AttentionKvD256TwoHeadSeq32SoftmaxTableLookupRelation,
}

impl FrameworkEval for AttentionKvD256TwoHeadSeq32SoftmaxTableLookupEval {
...
pub struct AttentionKvD256TwoHeadSeq32SoftmaxTableLookupMultiplicity {
    pub gap: usize,
    pub weight: i64,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeD256TwoHeadSeq32SoftmaxTableLookupSummary {
    ...
    pub table_multiplicities: Vec<AttentionKvD256TwoHeadSeq32SoftmaxTableLookupMultiplicity>,
}
...
pub struct ZkAiAttentionKvNativeD256TwoHeadSeq32SoftmaxTableLookupEnvelope {
    ...
    pub lookup_summary: ZkAiAttentionKvNativeD256TwoHeadSeq32SoftmaxTableLookupSummary,
...
#[serde(deny_unknown_fields)]
struct AttentionKvD256TwoHeadSeq32SoftmaxTableLookupProofPayload {
...
struct LookupBundle {
    log_size: u32,
    summary: ZkAiAttentionKvNativeD256TwoHeadSeq32SoftmaxTableLookupSummary,
...
pub fn zkai_attention_kv_native_d256_two_head_seq32_softmax_table_lookup_source_input_from_json_str(
    raw_json: &str,
...
pub fn prove_zkai_attention_kv_native_d256_two_head_seq32_softmax_table_lookup_envelope(
    source_input: &ZkAiAttentionKvNativeD256TwoHeadSeq32BoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeD256TwoHeadSeq32SoftmaxTableLookupEnvelope> {
    ...
    let envelope = ZkAiAttentionKvNativeD256TwoHeadSeq32SoftmaxTableLookupEnvelope {
        ...
        proof_schema_version:
            ZKAI_ATTENTION_KV_NATIVE_D256_TWO_HEAD_SEQ32_SOFTMAX_TABLE_LOOKUP_PROOF_VERSION
                .to_string(),
        statement_version:
            ZKAI_ATTENTION_KV_NATIVE_D256_TWO_HEAD_SEQ32_SOFTMAX_TABLE_LOOKUP_STATEMENT_VERSION
                .to_string(),
        semantic_scope:
            ZKAI_ATTENTION_KV_NATIVE_D256_TWO_HEAD_SEQ32_SOFTMAX_TABLE_LOOKUP_SEMANTIC_SCOPE
                .to_string(),
        decision: ZKAI_ATTENTION_KV_NATIVE_D256_TWO_HEAD_SEQ32_SOFTMAX_TABLE_LOOKUP_DECISION
            .to_string(),
        verifier_domain:
            ZKAI_ATTENTION_KV_NATIVE_D256_TWO_HEAD_SEQ32_SOFTMAX_TABLE_LOOKUP_VERIFIER_DOMAIN
                .to_string(),
...
pub fn zkai_attention_kv_native_d256_two_head_seq32_softmax_table_lookup_envelope_from_json_slice(
    raw_json: &[u8],
) -> Result<ZkAiAttentionKvNativeD256TwoHeadSeq32SoftmaxTableLookupEnvelope> {
    if raw_json.len()
...
pub fn verify_zkai_attention_kv_native_d256_two_head_seq32_softmax_table_lookup_envelope(
    envelope: &ZkAiAttentionKvNativeD256TwoHeadSeq32SoftmaxTableLookupEnvelope,
) -> Result<bool> {
    validate_envelope(envelope)?;
...
fn validate_envelope(
    ...
    expect_eq(
        &envelope.decision,
        ZKAI_ATTENTION_KV_NATIVE_D256_TWO_HEAD_SEQ32_SOFTMAX_TABLE_LOOKUP_DECISION,
        "lookup decision",
    )?;
    expect_eq(
    ...
    if envelope.proof.is_empty()
        || envelope.proof.len()
            > ZKAI_ATTENTION_KV_NATIVE_D256_TWO_HEAD_SEQ32_SOFTMAX_TABLE_LOOKUP_MAX_PROOF_BYTES
    {
...
fn validate_source_input(
    ...
    zkai_attention_kv_native_d256_two_head_seq32_bounded_softmax_table_input_from_json_str(&raw)?;
    Ok(())
}

fn lookup_summary(
    input: &ZkAiAttentionKvNativeD256TwoHeadSeq32BoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeD256TwoHeadSeq32SoftmaxTableLookupSummary> {
    let mut multiplicities = input
        .weight_table
        .iter()
        .map(
            |entry| AttentionKvD256TwoHeadSeq32SoftmaxTableLookupMultiplicity {
                gap: entry.gap,
                weight: entry.weight,
                multiplicity: 0,
            },
    ...
    Ok(
        ZkAiAttentionKvNativeD256TwoHeadSeq32SoftmaxTableLookupSummary {
            ...
            lookup_relation: "AttentionKvD256TwoHeadSeq32SoftmaxTableLookupRelation".to_string(),
...
fn prove_lookup(bundle: &LookupBundle) -> Result<Vec<u8>> {
    let component =
        lookup_component(AttentionKvD256TwoHeadSeq32SoftmaxTableLookupRelation::dummy());
    ...
    let lookup_elements = AttentionKvD256TwoHeadSeq32SoftmaxTableLookupRelation::draw(channel);
    ...
    serde_json::to_vec(&AttentionKvD256TwoHeadSeq32SoftmaxTableLookupProofPayload { stark_proof })
        .map_err(|error| VmError::Serialization(error.to_string()))
}
...
fn verify_lookup(
    ...
    let payload: AttentionKvD256TwoHeadSeq32SoftmaxTableLookupProofPayload =
...
fn lookup_component(
    lookup_elements: AttentionKvD256TwoHeadSeq32SoftmaxTableLookupRelation,
) -> FrameworkComponent<AttentionKvD256TwoHeadSeq32SoftmaxTableLookupEval> {
    FrameworkComponent::new(
        ...
        AttentionKvD256TwoHeadSeq32SoftmaxTableLookupEval { lookup_elements },
        SecureField::zero(),
    )
}
...
mod tests {
    ...
    #[test]
    fn attention_kv_d256_two_head_seq32_softmax_table_lookup_sidecar_round_trips_real_proof() {
    ...
    #[test]
    fn attention_kv_d256_two_head_seq32_softmax_table_lookup_summary_counts_claims() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_d256_two_head_seq32_softmax_table_lookup_rejects_empty_table_without_panic() {
    ...
    #[test]
    fn attention_kv_d256_two_head_seq32_softmax_table_lookup_rejects_preprocessed_overflow_without_panic(
    ) {
        let input = source_input();
        let mut summary = lookup_summary(&input).expect("summary");
    ...
    #[test]
    fn attention_kv_d256_two_head_seq32_softmax_table_lookup_masks_inactive_denominators() {
        let guarded =
    ...
    #[test]
    fn attention_kv_d256_two_head_seq32_softmax_table_lookup_preserves_one_sided_contributions() {
    ...
    #[test]
    fn attention_kv_d256_two_head_seq32_softmax_table_lookup_rejects_summary_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d256_two_head_seq32_softmax_table_lookup_envelope(
                &input,
    ...
    #[test]
    fn attention_kv_d256_two_head_seq32_softmax_table_lookup_rejects_source_weight_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d256_two_head_seq32_softmax_table_lookup_envelope(
                &input,
    ...
    #[test]
    fn attention_kv_d256_two_head_seq32_softmax_table_lookup_rejects_proof_byte_tamper() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d256_two_head_seq32_softmax_table_lookup_envelope(
                &input,
