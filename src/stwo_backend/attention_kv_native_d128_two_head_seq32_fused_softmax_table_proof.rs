...
use super::attention_kv_native_d128_two_head_seq32_bounded_softmax_table_proof::{
    validate_zkai_attention_kv_native_d128_two_head_seq32_bounded_softmax_table_input,
    ...
    ZkAiAttentionKvNativeD128TwoHeadSeq32BoundedSoftmaxTableProofInput,
...
pub const ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_PROOF_VERSION: &str =
    "stwo-attention-kv-d128-two-head-seq32-fused-bounded-softmax-table-logup-proof-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_BACKEND_VERSION: &str =
    "stwo-attention-kv-d128-two-head-seq32-fused-bounded-softmax-table-logup-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_STATEMENT_VERSION: &str =
    "zkai-attention-kv-stwo-native-d128-two-head-seq32-fused-softmax-table-logup-statement-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_SEMANTIC_SCOPE: &str =
    "d128_two_head_seq32_bounded_softmax_table_attention_arithmetic_and_logup_membership_fused_in_one_native_stwo_proof";
pub const ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_DECISION: &str =
    "GO_NATIVE_STWO_FUSED_ATTENTION_ARITHMETIC_AND_SOFTMAX_TABLE_LOGUP_MEMBERSHIP";
pub const ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_TARGET_ID: &str =
    "attention-kv-d128-two-head-seq32-causal-mask-fused-bounded-softmax-table-logup-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_VERIFIER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d128-two-head-seq32-fused-bounded-softmax-table-logup:v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_MAX_ENVELOPE_JSON_BYTES: usize =
    67_108_864;
pub const ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_MAX_PROOF_BYTES: usize =
    524_288;

const ISSUE: usize = 715;
const SOURCE_ISSUE: usize = 715;
const SIDECAR_ISSUE: usize = 715;
const SOURCE_PLUS_SIDECAR_RAW_PROOF_BYTES: usize = 443266 + 35010;
...
const NON_FUSED_STATUS: &str =
    "GO_MATCHED_D128_TWO_HEAD_SEQ32_SOURCE_PLUS_LOGUP_SIDECAR_COMPARATOR_RECORDED";
...
const PREPROCESSED_TABLE_GAP: &str =
    "zkai/attention-kv/native-d128-two-head-seq32-fused-softmax-table-logup/table-gap";
const PREPROCESSED_TABLE_WEIGHT: &str =
    "zkai/attention-kv/native-d128-two-head-seq32-fused-softmax-table-logup/table-weight";
const PREPROCESSED_TABLE_MULTIPLICITY: &str =
    "zkai/attention-kv/native-d128-two-head-seq32-fused-softmax-table-logup/table-multiplicity";
const FUSED_COLUMN_PREFIX: &str =
    "zkai/attention-kv/native-d128-two-head-seq32-fused-softmax-table-logup";
...
#[derive(Default)]
struct AttentionKvD128TwoHeadSeq32FusedSoftmaxTableLayoutCounter {
    trace_masks: usize,
    preprocessed_masks: usize,
}

impl EvalAtRow for AttentionKvD128TwoHeadSeq32FusedSoftmaxTableLayoutCounter {
    type F = BaseField;
    type EF = SecureField;
...
impl FrameworkEval for AttentionKvNativeD128TwoHeadSeq32FusedSoftmaxTableEval {
...
pub struct AttentionKvD128TwoHeadSeq32FusedSoftmaxTableMultiplicity {
    pub gap: usize,
    pub weight: i64,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeD128TwoHeadSeq32FusedSoftmaxTableSummary {
    pub issue: usize,
    ...
    pub table_multiplicities: Vec<AttentionKvD128TwoHeadSeq32FusedSoftmaxTableMultiplicity>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeD128TwoHeadSeq32FusedSoftmaxTableEnvelope {
    ...
    pub fused_summary: ZkAiAttentionKvNativeD128TwoHeadSeq32FusedSoftmaxTableSummary,
    pub source_input: ZkAiAttentionKvNativeD128TwoHeadSeq32BoundedSoftmaxTableProofInput,
    pub proof: Vec<u8>,
}

#[derive(Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
struct AttentionKvNativeD128TwoHeadSeq32FusedSoftmaxTableProofPayload {
...
pub fn zkai_attention_kv_native_d128_two_head_seq32_fused_softmax_table_source_input_from_json_str(
    raw_json: &str,
    ...
    zkai_attention_kv_native_d128_two_head_seq32_bounded_softmax_table_input_from_json_str(raw_json)
}

pub fn prove_zkai_attention_kv_native_d128_two_head_seq32_fused_softmax_table_envelope(
    source_input: &ZkAiAttentionKvNativeD128TwoHeadSeq32BoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeD128TwoHeadSeq32FusedSoftmaxTableEnvelope> {
    validate_source_input(source_input)?;
    let bundle = build_fused_bundle(source_input)?;
    let proof = prove_fused(&bundle)?;
    let envelope = ZkAiAttentionKvNativeD128TwoHeadSeq32FusedSoftmaxTableEnvelope {
        proof_backend: StarkProofBackend::Stwo,
        proof_backend_version:
            ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_BACKEND_VERSION
                .to_string(),
        proof_schema_version:
            ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_PROOF_VERSION
                .to_string(),
        statement_version:
            ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_STATEMENT_VERSION
                .to_string(),
        semantic_scope:
            ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_SEMANTIC_SCOPE
                .to_string(),
        decision: ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_DECISION
            .to_string(),
        target_id: ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_TARGET_ID
            .to_string(),
        verifier_domain:
            ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_VERIFIER_DOMAIN
                .to_string(),
...
pub fn zkai_attention_kv_native_d128_two_head_seq32_fused_softmax_table_envelope_from_json_slice(
    raw_json: &[u8],
) -> Result<ZkAiAttentionKvNativeD128TwoHeadSeq32FusedSoftmaxTableEnvelope> {
    if raw_json.len()
...
pub fn verify_zkai_attention_kv_native_d128_two_head_seq32_fused_softmax_table_envelope(
    envelope: &ZkAiAttentionKvNativeD128TwoHeadSeq32FusedSoftmaxTableEnvelope,
) -> Result<bool> {
    validate_envelope(envelope)?;
...
fn validate_envelope(
    ...
    expect_eq(
        &envelope.decision,
        ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_DECISION,
        "fused decision",
    )?;
    expect_eq(
        &envelope.target_id,
        ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_TARGET_ID,
        "fused target id",
    )?;
    expect_eq(
    ...
    if envelope.proof.is_empty()
        || envelope.proof.len()
            > ZKAI_ATTENTION_KV_NATIVE_D128_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_MAX_PROOF_BYTES
    {
...
pub(super) fn fused_summary(
    ...
    let mut multiplicities = input
        ...
        .iter()
        .map(
            |entry| AttentionKvD128TwoHeadSeq32FusedSoftmaxTableMultiplicity {
                gap: entry.gap,
                weight: entry.weight,
                multiplicity: 0,
            },
    ...
    Ok(
        ZkAiAttentionKvNativeD128TwoHeadSeq32FusedSoftmaxTableSummary {
            ...
            lookup_relation: "AttentionKvD128TwoHeadSeq32FusedSoftmaxTableRelation".to_string(),
...
fn prove_fused(bundle: &FusedBundle) -> Result<Vec<u8>> {
    let component = fused_component(AttentionKvD128TwoHeadSeq32FusedSoftmaxTableRelation::dummy());
    ...
    serde_json::to_vec(
        &AttentionKvNativeD128TwoHeadSeq32FusedSoftmaxTableProofPayload { stark_proof },
    )
    .map_err(|error| VmError::Serialization(error.to_string()))
}
...
fn verify_fused(
    ...
    let payload: AttentionKvNativeD128TwoHeadSeq32FusedSoftmaxTableProofPayload =
    ...
    let component_placeholder =
        fused_component(AttentionKvD128TwoHeadSeq32FusedSoftmaxTableRelation::dummy());
...
fn validate_static_fused_air_layout() -> Result<()> {
    ...
    let counter = AttentionKvNativeD128TwoHeadSeq32FusedSoftmaxTableEval {
        lookup_elements: AttentionKvD128TwoHeadSeq32FusedSoftmaxTableRelation::dummy(),
    }
    .evaluate(AttentionKvD128TwoHeadSeq32FusedSoftmaxTableLayoutCounter::default());
...
mod tests {
    ...
    #[test]
    fn attention_kv_d128_two_head_seq32_fused_softmax_table_round_trips_real_proof() {
        let input = source_input();
        let envelope =
            prove_zkai_attention_kv_native_d128_two_head_seq32_fused_softmax_table_envelope(&input)
    ...
    #[test]
    fn attention_kv_d128_two_head_seq32_fused_softmax_table_summary_counts_claims() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_d128_two_head_seq32_fused_softmax_table_derives_logup_indices_from_column_ids()
    {
        let row_ids = fused_row_column_ids();
        let preprocessed_ids = fused_preprocessed_column_ids();
    ...
    #[test]
    fn attention_kv_d128_two_head_seq32_fused_softmax_table_rejects_summary_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d128_two_head_seq32_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d128_two_head_seq32_fused_softmax_table_rejects_source_weight_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d128_two_head_seq32_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d128_two_head_seq32_fused_softmax_table_rejects_negative_bit_material() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_d128_two_head_seq32_fused_softmax_table_rejects_output_remainder_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d128_two_head_seq32_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d128_two_head_seq32_fused_softmax_table_rejects_proof_byte_tamper() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d128_two_head_seq32_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d128_two_head_seq32_fused_softmax_table_rejects_backend_version_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d128_two_head_seq32_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d128_two_head_seq32_fused_softmax_table_rejects_proof_schema_version_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d128_two_head_seq32_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d128_two_head_seq32_fused_softmax_table_rejects_unknown_envelope_field() {
        let input = source_input();
        let envelope =
            prove_zkai_attention_kv_native_d128_two_head_seq32_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
