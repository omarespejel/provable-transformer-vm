...
use super::attention_kv_native_d32_two_head_bounded_softmax_table_proof::{
    validate_zkai_attention_kv_native_d32_two_head_bounded_softmax_table_input,
    ...
    ZkAiAttentionKvNativeD32TwoHeadBoundedSoftmaxTableProofInput,
...
pub const ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_PROOF_VERSION: &str =
    "stwo-attention-kv-d32-two-head-fused-bounded-softmax-table-logup-proof-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_BACKEND_VERSION: &str =
    "stwo-attention-kv-d32-two-head-fused-bounded-softmax-table-logup-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_STATEMENT_VERSION: &str =
    "zkai-attention-kv-stwo-native-d32-two-head-fused-softmax-table-logup-statement-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_SEMANTIC_SCOPE: &str =
    "d32_two_head_bounded_softmax_table_attention_arithmetic_and_logup_membership_fused_in_one_native_stwo_proof";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_DECISION: &str =
    "GO_NATIVE_STWO_FUSED_ATTENTION_ARITHMETIC_AND_SOFTMAX_TABLE_LOGUP_MEMBERSHIP";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_TARGET_ID: &str =
    "attention-kv-d32-two-head-causal-mask-fused-bounded-softmax-table-logup-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_VERIFIER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d32-two-head-fused-bounded-softmax-table-logup:v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_MAX_ENVELOPE_JSON_BYTES: usize =
    4_194_304;
pub const ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_MAX_PROOF_BYTES: usize =
    262_144;

const ISSUE: usize = 715;
const SOURCE_ISSUE: usize = 715;
const SIDECAR_ISSUE: usize = 715;
const SOURCE_PLUS_SIDECAR_RAW_PROOF_BYTES: usize = 123926 + 18137;
...
const NON_FUSED_STATUS: &str =
    "GO_MATCHED_D32_TWO_HEAD_SOURCE_PLUS_LOGUP_SIDECAR_COMPARATOR_RECORDED";
...
const PREPROCESSED_TABLE_GAP: &str =
    "zkai/attention-kv/native-d32-two-head-fused-softmax-table-logup/table-gap";
const PREPROCESSED_TABLE_WEIGHT: &str =
    "zkai/attention-kv/native-d32-two-head-fused-softmax-table-logup/table-weight";
const PREPROCESSED_TABLE_MULTIPLICITY: &str =
    "zkai/attention-kv/native-d32-two-head-fused-softmax-table-logup/table-multiplicity";
const FUSED_COLUMN_PREFIX: &str = "zkai/attention-kv/native-d32-two-head-fused-softmax-table-logup";
...
relation!(AttentionKvD32TwoHeadFusedSoftmaxTableRelation, 2);
...
pub struct AttentionKvD32TwoHeadFusedSoftmaxTableMultiplicity {
    pub gap: usize,
    pub weight: i64,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeD32TwoHeadFusedSoftmaxTableSummary {
    pub issue: usize,
    ...
    pub table_multiplicities: Vec<AttentionKvD32TwoHeadFusedSoftmaxTableMultiplicity>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeD32TwoHeadFusedSoftmaxTableEnvelope {
    ...
    pub fused_summary: ZkAiAttentionKvNativeD32TwoHeadFusedSoftmaxTableSummary,
    pub source_input: ZkAiAttentionKvNativeD32TwoHeadBoundedSoftmaxTableProofInput,
    pub proof: Vec<u8>,
}

#[derive(Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
struct AttentionKvNativeD32TwoHeadFusedSoftmaxTableProofPayload {
...
struct FusedBundle {
    log_size: u32,
    summary: ZkAiAttentionKvNativeD32TwoHeadFusedSoftmaxTableSummary,
...
pub fn zkai_attention_kv_native_d32_two_head_fused_softmax_table_source_input_from_json_str(
    raw_json: &str,
    ...
    zkai_attention_kv_native_d32_two_head_bounded_softmax_table_input_from_json_str(raw_json)
}

pub fn prove_zkai_attention_kv_native_d32_two_head_fused_softmax_table_envelope(
    source_input: &ZkAiAttentionKvNativeD32TwoHeadBoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeD32TwoHeadFusedSoftmaxTableEnvelope> {
    validate_source_input(source_input)?;
    let bundle = build_fused_bundle(source_input)?;
    let proof = prove_fused(&bundle)?;
    let envelope = ZkAiAttentionKvNativeD32TwoHeadFusedSoftmaxTableEnvelope {
        proof_backend: StarkProofBackend::Stwo,
        proof_backend_version:
            ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_BACKEND_VERSION.to_string(),
        proof_schema_version:
            ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_PROOF_VERSION.to_string(),
        statement_version:
            ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_STATEMENT_VERSION.to_string(),
        semantic_scope: ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_SEMANTIC_SCOPE
            .to_string(),
        decision: ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_DECISION.to_string(),
        target_id: ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_TARGET_ID.to_string(),
        verifier_domain: ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_VERIFIER_DOMAIN
            .to_string(),
...
pub fn zkai_attention_kv_native_d32_two_head_fused_softmax_table_envelope_from_json_slice(
    raw_json: &[u8],
) -> Result<ZkAiAttentionKvNativeD32TwoHeadFusedSoftmaxTableEnvelope> {
    if raw_json.len()
...
pub fn verify_zkai_attention_kv_native_d32_two_head_fused_softmax_table_envelope(
    envelope: &ZkAiAttentionKvNativeD32TwoHeadFusedSoftmaxTableEnvelope,
) -> Result<bool> {
    validate_envelope(envelope)?;
...
fn validate_envelope(
    ...
    expect_eq(
        ...
        ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_BACKEND_VERSION,
        "fused proof backend version",
    )?;
    expect_eq(
        &envelope.proof_schema_version,
        ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_PROOF_VERSION,
        "fused proof schema version",
    )?;
    expect_eq(
        &envelope.statement_version,
        ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_STATEMENT_VERSION,
        "fused statement version",
    )?;
    expect_eq(
        &envelope.semantic_scope,
        ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_SEMANTIC_SCOPE,
        "fused semantic scope",
    )?;
    expect_eq(
        &envelope.decision,
        ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_DECISION,
        "fused decision",
    )?;
    expect_eq(
        &envelope.target_id,
        ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_TARGET_ID,
        "fused target id",
    )?;
    expect_eq(
        &envelope.verifier_domain,
        ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_VERIFIER_DOMAIN,
    ...
    if envelope.proof.is_empty()
        || envelope.proof.len()
            > ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_MAX_PROOF_BYTES
    {
...
fn validate_source_input(
    ...
    validate_zkai_attention_kv_native_d32_two_head_bounded_softmax_table_input(input)
}

fn fused_summary(
    input: &ZkAiAttentionKvNativeD32TwoHeadBoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeD32TwoHeadFusedSoftmaxTableSummary> {
    let mut multiplicities = input
        .weight_table
        .iter()
        .map(|entry| AttentionKvD32TwoHeadFusedSoftmaxTableMultiplicity {
            gap: entry.gap,
            weight: entry.weight,
            multiplicity: 0,
        })
    ...
    Ok(ZkAiAttentionKvNativeD32TwoHeadFusedSoftmaxTableSummary {
        issue: ISSUE,
        ...
        lookup_relation: "AttentionKvD32TwoHeadFusedSoftmaxTableRelation".to_string(),
...
fn prove_fused(bundle: &FusedBundle) -> Result<Vec<u8>> {
    let component = fused_component(AttentionKvD32TwoHeadFusedSoftmaxTableRelation::dummy());
    ...
    let lookup_elements = AttentionKvD32TwoHeadFusedSoftmaxTableRelation::draw(channel);
    ...
    serde_json::to_vec(&AttentionKvNativeD32TwoHeadFusedSoftmaxTableProofPayload { stark_proof })
        .map_err(|error| VmError::Serialization(error.to_string()))
}
...
fn verify_fused(
    ...
    let payload: AttentionKvNativeD32TwoHeadFusedSoftmaxTableProofPayload =
    ...
    let component_placeholder =
        fused_component(AttentionKvD32TwoHeadFusedSoftmaxTableRelation::dummy());
...
fn fused_component(
    lookup_elements: AttentionKvD32TwoHeadFusedSoftmaxTableRelation,
) -> FrameworkComponent<AttentionKvNativeD32TwoHeadFusedSoftmaxTableEval> {
    FrameworkComponent::new(
        &mut TraceLocationAllocator::new_with_preprocessed_columns(&fused_preprocessed_column_ids()),
        AttentionKvNativeD32TwoHeadFusedSoftmaxTableEval { lookup_elements },
        SecureField::zero(),
    )
}
...
mod tests {
    ...
    fn source_input() -> ZkAiAttentionKvNativeD32TwoHeadBoundedSoftmaxTableProofInput {
        let raw = include_str!(
            "../../docs/engineering/evidence/zkai-attention-kv-stwo-native-d32-two-head-bounded-softmax-table-proof-2026-05.json"
        );
        zkai_attention_kv_native_d32_two_head_fused_softmax_table_source_input_from_json_str(raw)
            .expect("source input")
    }

    #[test]
    fn attention_kv_d32_two_head_fused_softmax_table_round_trips_real_proof() {
        let input = source_input();
        let envelope =
            prove_zkai_attention_kv_native_d32_two_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
        assert_eq!(
            envelope.decision,
            ZKAI_ATTENTION_KV_NATIVE_D32_TWO_HEAD_FUSED_SOFTMAX_TABLE_DECISION
        );
        assert_eq!(envelope.fused_summary.lookup_claims, 104);
    ...
    #[test]
    fn attention_kv_d32_two_head_fused_softmax_table_summary_counts_claims() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_d32_two_head_fused_softmax_table_rejects_summary_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d32_two_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d32_two_head_fused_softmax_table_rejects_source_weight_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d32_two_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d32_two_head_fused_softmax_table_rejects_negative_bit_material() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_d32_two_head_fused_softmax_table_rejects_output_remainder_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d32_two_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d32_two_head_fused_softmax_table_rejects_proof_byte_tamper() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d32_two_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d32_two_head_fused_softmax_table_rejects_backend_version_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d32_two_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d32_two_head_fused_softmax_table_rejects_proof_schema_version_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d32_two_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d32_two_head_fused_softmax_table_rejects_unknown_envelope_field() {
        let input = source_input();
        let envelope =
            prove_zkai_attention_kv_native_d32_two_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
