...
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_INPUT_SCHEMA: &str =
    "zkai-attention-kv-stwo-native-two-head-bounded-weighted-air-proof-input-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_INPUT_DECISION: &str =
    "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_TWO_HEAD_BOUNDED_WEIGHTED_AIR_PROOF";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_PROOF_VERSION: &str =
    "stwo-attention-kv-d8-causal-mask-two-head-bounded-weighted-air-proof-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_STATEMENT_VERSION: &str =
    "zkai-attention-kv-stwo-native-two-head-bounded-weighted-statement-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_SEMANTIC_SCOPE: &str =
    "two_head_d8_bounded_power2_weighted_attention_kv_causal_mask_rows_bound_to_statement_receipt";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_DECISION: &str =
    "GO_STWO_NATIVE_ATTENTION_KV_TWO_HEAD_BOUNDED_WEIGHTED_AIR_PROOF";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_TARGET_ID: &str =
    "attention-kv-d8-causal-mask-two-head-bounded-weighted-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_REQUIRED_BACKEND_VERSION: &str =
    "stwo-attention-kv-d8-causal-mask-two-head-bounded-weighted-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_VERIFIER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-bounded-weighted:v1";

const ISSUE: usize = 461;
const SOURCE_ISSUE: usize = 460;
...
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_MAX_INPUT_JSON_BYTES: usize =
    1_048_576;
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_MAX_ENVELOPE_JSON_BYTES: usize =
    1_048_576;
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_MAX_PROOF_BYTES: usize = 8_388_608;

const ROW_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-bounded-weighted-score-rows:v1";
const INITIAL_KV_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-bounded-weighted-initial-kv:v1";
const INPUT_STEPS_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-bounded-weighted-input-steps:v1";
const FINAL_KV_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-bounded-weighted-final-kv:v1";
const OUTPUTS_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-bounded-weighted-outputs:v1";
const PUBLIC_INSTANCE_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-bounded-weighted-public-instance:v1";
const PROOF_NATIVE_PARAMETER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-bounded-weighted-proof-parameters:v1";
...
#[derive(Debug, Clone)]
struct AttentionKvNativeTwoHeadBoundedWeightedEval;

impl FrameworkEval for AttentionKvNativeTwoHeadBoundedWeightedEval {
...
pub struct AttentionKvTwoHeadBoundedWeightedEntry {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AttentionKvTwoHeadBoundedWeightedInputStep {
    pub head_index: usize,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AttentionKvTwoHeadBoundedWeightedScoreRow {
    pub row_index: usize,
    pub head_index: usize,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeTwoHeadBoundedWeightedProofInput {
    pub schema: String,
    pub decision: String,
    pub issue: usize,
    ...
    pub initial_kv_cache: Vec<AttentionKvTwoHeadBoundedWeightedEntry>,
    pub input_steps: Vec<AttentionKvTwoHeadBoundedWeightedInputStep>,
    pub final_kv_cache: Vec<AttentionKvTwoHeadBoundedWeightedEntry>,
    pub attention_outputs: Vec<Vec<i64>>,
    pub score_rows: Vec<AttentionKvTwoHeadBoundedWeightedScoreRow>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeTwoHeadBoundedWeightedEnvelope {
    pub proof_backend: StarkProofBackend,
    pub proof_backend_version: String,
    pub statement_version: String,
    pub semantic_scope: String,
    pub decision: String,
    pub input: ZkAiAttentionKvNativeTwoHeadBoundedWeightedProofInput,
    pub proof: Vec<u8>,
}

#[derive(Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
struct AttentionKvNativeTwoHeadBoundedWeightedProofPayload {
    stark_proof: StarkProof<Blake2sM31MerkleHasher>,
}

pub fn zkai_attention_kv_native_two_head_bounded_weighted_input_from_json_str(
    raw_json: &str,
) -> Result<ZkAiAttentionKvNativeTwoHeadBoundedWeightedProofInput> {
    if raw_json.len() > ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_MAX_INPUT_JSON_BYTES {
    ...
    let input: ZkAiAttentionKvNativeTwoHeadBoundedWeightedProofInput =
...
pub fn prove_zkai_attention_kv_native_two_head_bounded_weighted_envelope(
    input: &ZkAiAttentionKvNativeTwoHeadBoundedWeightedProofInput,
) -> Result<ZkAiAttentionKvNativeTwoHeadBoundedWeightedEnvelope> {
    validate_input(input)?;
    Ok(ZkAiAttentionKvNativeTwoHeadBoundedWeightedEnvelope {
        proof_backend: StarkProofBackend::Stwo,
        proof_backend_version:
            ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_REQUIRED_BACKEND_VERSION.to_string(),
        statement_version: ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_STATEMENT_VERSION
            .to_string(),
        semantic_scope: ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_SEMANTIC_SCOPE
            .to_string(),
        decision: ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_DECISION.to_string(),
        input: input.clone(),
        proof: prove_rows(input)?,
    })
}

pub fn zkai_attention_kv_native_two_head_bounded_weighted_envelope_from_json_slice(
    raw_json: &[u8],
) -> Result<ZkAiAttentionKvNativeTwoHeadBoundedWeightedEnvelope> {
    if raw_json.len() > ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_MAX_ENVELOPE_JSON_BYTES {
        return Err(weighted_error(format!(
    ...
    let envelope: ZkAiAttentionKvNativeTwoHeadBoundedWeightedEnvelope =
...
pub fn verify_zkai_attention_kv_native_two_head_bounded_weighted_envelope(
    envelope: &ZkAiAttentionKvNativeTwoHeadBoundedWeightedEnvelope,
) -> Result<bool> {
    validate_envelope(envelope)?;
    verify_rows(&envelope.input, &envelope.proof)
}

fn validate_envelope(envelope: &ZkAiAttentionKvNativeTwoHeadBoundedWeightedEnvelope) -> Result<()> {
    ...
    if envelope.proof.len() > ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_BOUNDED_WEIGHTED_MAX_PROOF_BYTES {
        return Err(weighted_error(format!(
...
fn validate_input(input: &ZkAiAttentionKvNativeTwoHeadBoundedWeightedProofInput) -> Result<()> {
    expect_eq(
        &input.schema,
...
fn validate_sequence(input: &ZkAiAttentionKvNativeTwoHeadBoundedWeightedProofInput) -> Result<()> {
    ...
    for (global_step_index, step) in input.input_steps.iter().enumerate() {
        ...
        let next_item = AttentionKvTwoHeadBoundedWeightedEntry {
        ...
        let scored: Vec<(AttentionKvTwoHeadBoundedWeightedEntry, i64)> = next_cache
            .iter()
            .filter(|candidate| {
            ...
            scored.iter().zip(weights.iter()).enumerate()
        {
            ...
            expected_rows.push(AttentionKvTwoHeadBoundedWeightedScoreRow {
...
fn validate_kv_entry(entry: &AttentionKvTwoHeadBoundedWeightedEntry) -> Result<()> {
    if entry.head_index >= HEAD_COUNT {
...
fn validate_input_step(step: &AttentionKvTwoHeadBoundedWeightedInputStep) -> Result<()> {
    if step.head_index >= HEAD_COUNT {
        return Err(weighted_error("input step head index out of range"));
    }
...
fn validate_score_row(
    row: &AttentionKvTwoHeadBoundedWeightedScoreRow,
...
fn prove_rows(input: &ZkAiAttentionKvNativeTwoHeadBoundedWeightedProofInput) -> Result<Vec<u8>> {
    validate_input(input)?;
        ...
        prove::<SimdBackend, Blake2sM31MerkleChannel>(&[&component], channel, commitment_scheme)
            .map_err(|error| {
                VmError::UnsupportedProof(format!(
                    "attention/KV native two-head bounded weighted AIR proving failed: {error}"
                ))
            })?;
    serde_json::to_vec(&AttentionKvNativeTwoHeadBoundedWeightedProofPayload { stark_proof })
        .map_err(|error| VmError::Serialization(error.to_string()))
}
...
fn verify_rows(
    ...
    validate_input(input)?;
    let payload: AttentionKvNativeTwoHeadBoundedWeightedProofPayload =
    ...
    verify(&[&component], channel, commitment_scheme, stark_proof)
        ...
        .map_err(|error| {
            weighted_error(format!(
                "attention/KV native two-head bounded weighted proof rejected: {error}"
            ))
        })
}
...
fn attention_component() -> FrameworkComponent<AttentionKvNativeTwoHeadBoundedWeightedEval> {
    FrameworkComponent::new(
        &mut TraceLocationAllocator::new_with_preprocessed_columns(&preprocessed_column_ids()),
        AttentionKvNativeTwoHeadBoundedWeightedEval,
        SecureField::zero(),
    )
}
...
fn padding_row(row_index: usize) -> AttentionKvTwoHeadBoundedWeightedScoreRow {
    AttentionKvTwoHeadBoundedWeightedScoreRow {
        row_index,
        head_index: 0,
        step_index: 0,
        candidate_index: 0,
...
fn column_ids() -> Vec<String> {
    ...
    .into_iter()
    .map(|suffix| format!("zkai/attention-kv/native-two-head-bounded-weighted/{suffix}"))
    .collect::<Vec<_>>();
    for prefix in [
        "query",
        "key",
        ...
        for index in 0..width {
            ids.push(format!(
                "zkai/attention-kv/native-two-head-bounded-weighted/{prefix}-{index:02}"
            ));
        }
    }
    for index in 0..SCORE_GAP_BITS {
        ids.push(format!(
            "zkai/attention-kv/native-two-head-bounded-weighted/score-gap-bit-{index:02}"
        ));
    }
    for index in 0..CAUSAL_GAP_BITS {
        ids.push(format!(
            "zkai/attention-kv/native-two-head-bounded-weighted/causal-gap-bit-{index:02}"
        ));
    }
    for index in 0..WEIGHT_BITS {
        ids.push(format!(
            "zkai/attention-kv/native-two-head-bounded-weighted/weight-bit-{index:02}"
        ));
    }
    for dim in 0..VALUE_WIDTH {
        for index in 0..OUTPUT_REMAINDER_BITS {
            ids.push(format!(
                "zkai/attention-kv/native-two-head-bounded-weighted/output-remainder-{dim:02}-bit-{index:02}"
            ));
        }
    }
    ids
...
fn kv_commitment(cache: &[AttentionKvTwoHeadBoundedWeightedEntry], domain: &str) -> Result<String> {
    let material = cache
        .iter()
        .map(|entry| {
...
fn input_steps_commitment(steps: &[AttentionKvTwoHeadBoundedWeightedInputStep]) -> Result<String> {
    let material = steps
        .iter()
        .map(|step| {
...
fn rows_commitment(rows: &[AttentionKvTwoHeadBoundedWeightedScoreRow]) -> Result<String> {
    let material = rows.iter().map(score_row_material).collect::<Vec<_>>();
    commitment_from_parts(
        &[
            (
                "encoding",
                json_string("attention_kv_stwo_native_two_head_bounded_weighted_score_rows_v1")?,
            ),
            (
                "shape",
...
fn score_row_material(row: &AttentionKvTwoHeadBoundedWeightedScoreRow) -> Vec<i64> {
    let mut out = vec![
...
fn outputs_commitment(
    steps: &[AttentionKvTwoHeadBoundedWeightedInputStep],
    outputs: &[Vec<i64>],
) -> Result<String> {
    ...
    commitment_from_parts(
        &[
            (
                "encoding",
                json_string("bounded_weighted_attention_outputs_with_head_v1")?,
            ),
            (
                "shape",
...
fn weighted_error(message: impl Into<String>) -> VmError {
    VmError::InvalidConfig(format!(
        "attention/KV native two-head bounded weighted proof: {}",
        message.into()
    ))
}
...
mod tests {
    ...
    fn input() -> ZkAiAttentionKvNativeTwoHeadBoundedWeightedProofInput {
        zkai_attention_kv_native_two_head_bounded_weighted_input_from_json_str(INPUT_JSON)
            .expect("two-head bounded weighted attention input")
    }

    #[test]
    fn attention_kv_native_two_head_bounded_weighted_input_validates_checked_rows() {
        let input = input();
        ...
        assert_eq!(input.attention_outputs[0], vec![2, -4, 1, -5, 0, 3, -1, 2]);
        assert_eq!(
            input.attention_outputs[15],
            vec![-1, 1, -3, 3, 0, -3, -2, -1]
        );
        assert_eq!(input.score_rows[0].head_index, 0);
        assert_eq!(input.score_rows[0].attention_weight, 4);
        assert_eq!(input.score_rows[3].head_index, 1);
        assert_eq!(input.score_rows[4].attention_weight, 16);
    }

    #[test]
    fn attention_kv_native_two_head_bounded_weighted_uses_floor_division_for_negative_numerators() {
        assert_eq!(
    ...
    #[test]
    fn attention_kv_native_two_head_bounded_weighted_air_proof_round_trips() {
        let input = input();
        let envelope = prove_zkai_attention_kv_native_two_head_bounded_weighted_envelope(&input)
            .expect("two-head bounded weighted attention proof");
    ...
    #[test]
    fn attention_kv_native_two_head_bounded_weighted_rejects_weight_relabeling() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
        value["score_rows"][0]["attention_weight"] = Value::from(15);
        let error = zkai_attention_kv_native_two_head_bounded_weighted_input_from_json_str(
    ...
    #[test]
    fn attention_kv_native_two_head_bounded_weighted_rejects_head_relabeling() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_two_head_bounded_weighted_rejects_output_relabeling() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_two_head_bounded_weighted_rejects_quotient_remainder_drift() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_two_head_bounded_weighted_rejects_commitment_drift() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_two_head_bounded_weighted_rejects_proof_byte_tamper() {
        let input = input();
        let mut envelope =
        ...
        *last ^= 1;
        assert!(
            verify_zkai_attention_kv_native_two_head_bounded_weighted_envelope(&envelope).is_err()
        );
    }

    #[test]
    fn attention_kv_native_two_head_bounded_weighted_rejects_unknown_envelope_field() {
        let input = input();
        let envelope = prove_zkai_attention_kv_native_two_head_bounded_weighted_envelope(&input)
            .expect("two-head bounded weighted attention proof");
