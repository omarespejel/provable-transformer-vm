...
pub const ZKAI_ATTENTION_KV_NATIVE_D32_SOFTMAX_TABLE_LOOKUP_PROOF_VERSION: &str =
    "stwo-attention-kv-d32-softmax-table-logup-sidecar-proof-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_SOFTMAX_TABLE_LOOKUP_STATEMENT_VERSION: &str =
    "zkai-attention-kv-stwo-native-d32-softmax-table-logup-sidecar-statement-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_SOFTMAX_TABLE_LOOKUP_SEMANTIC_SCOPE: &str =
    "d32_bounded_softmax_table_membership_constrained_by_native_stwo_logup_sidecar";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_SOFTMAX_TABLE_LOOKUP_DECISION: &str =
    "GO_NATIVE_STWO_AIR_CONSTRAINED_SOFTMAX_TABLE_LOOKUP_RELATION_SIDECAR";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_SOFTMAX_TABLE_LOOKUP_TARGET_ID: &str =
    "attention-kv-d32-causal-mask-bounded-softmax-table-logup-sidecar-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_SOFTMAX_TABLE_LOOKUP_VERIFIER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d32-softmax-table-logup-sidecar:v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_SOFTMAX_TABLE_LOOKUP_MAX_ENVELOPE_JSON_BYTES: usize =
    1_048_576;
pub const ZKAI_ATTENTION_KV_NATIVE_D32_SOFTMAX_TABLE_LOOKUP_MAX_PROOF_BYTES: usize = 65_536;
...
const PREPROCESSED_TABLE_GAP: &str = "zkai/attention-kv/native-d32-softmax-table-logup/table-gap";
const PREPROCESSED_TABLE_WEIGHT: &str =
    "zkai/attention-kv/native-d32-softmax-table-logup/table-weight";
const PREPROCESSED_TABLE_MULTIPLICITY: &str =
    "zkai/attention-kv/native-d32-softmax-table-logup/table-multiplicity";

relation!(AttentionKvD32SoftmaxTableLookupRelation, 2);

#[derive(Debug, Clone)]
struct AttentionKvD32SoftmaxTableLookupEval {
    lookup_elements: AttentionKvD32SoftmaxTableLookupRelation,
}

impl FrameworkEval for AttentionKvD32SoftmaxTableLookupEval {
...
pub struct AttentionKvD32SoftmaxTableLookupMultiplicity {
    pub gap: usize,
    pub weight: i64,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeD32SoftmaxTableLookupSummary {
    ...
    pub table_multiplicities: Vec<AttentionKvD32SoftmaxTableLookupMultiplicity>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeD32SoftmaxTableLookupEnvelope {
...
#[serde(deny_unknown_fields)]
struct AttentionKvD32SoftmaxTableLookupProofPayload {
...
struct LookupBundle {
    log_size: u32,
    summary: ZkAiAttentionKvNativeD32SoftmaxTableLookupSummary,
...
pub fn zkai_attention_kv_native_d32_softmax_table_lookup_source_input_from_json_str(
    raw_json: &str,
...
pub fn prove_zkai_attention_kv_native_d32_softmax_table_lookup_envelope(
    source_input: &ZkAiAttentionKvNativeD32BoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeD32SoftmaxTableLookupEnvelope> {
    validate_source_input(source_input)?;
    let bundle = build_lookup_bundle(source_input)?;
    let proof = prove_lookup(&bundle)?;
    let envelope = ZkAiAttentionKvNativeD32SoftmaxTableLookupEnvelope {
        proof_backend: StarkProofBackend::Stwo,
        proof_backend_version: ZKAI_ATTENTION_KV_NATIVE_D32_SOFTMAX_TABLE_LOOKUP_PROOF_VERSION
            .to_string(),
        statement_version: ZKAI_ATTENTION_KV_NATIVE_D32_SOFTMAX_TABLE_LOOKUP_STATEMENT_VERSION
            .to_string(),
...
pub fn zkai_attention_kv_native_d32_softmax_table_lookup_envelope_from_json_slice(
    raw_json: &[u8],
) -> Result<ZkAiAttentionKvNativeD32SoftmaxTableLookupEnvelope> {
    if raw_json.len() > ZKAI_ATTENTION_KV_NATIVE_D32_SOFTMAX_TABLE_LOOKUP_MAX_ENVELOPE_JSON_BYTES {
...
pub fn verify_zkai_attention_kv_native_d32_softmax_table_lookup_envelope(
    envelope: &ZkAiAttentionKvNativeD32SoftmaxTableLookupEnvelope,
) -> Result<bool> {
    validate_envelope(envelope)?;
    verify_lookup(&envelope.source_input, &envelope.proof)
}

fn validate_envelope(envelope: &ZkAiAttentionKvNativeD32SoftmaxTableLookupEnvelope) -> Result<()> {
    ...
    if envelope.proof.is_empty()
        || envelope.proof.len() > ZKAI_ATTENTION_KV_NATIVE_D32_SOFTMAX_TABLE_LOOKUP_MAX_PROOF_BYTES
    {
...
fn lookup_summary(
    input: &ZkAiAttentionKvNativeD32BoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeD32SoftmaxTableLookupSummary> {
    let mut multiplicities = input
        .weight_table
        .iter()
        .map(|entry| AttentionKvD32SoftmaxTableLookupMultiplicity {
            gap: entry.gap,
            weight: entry.weight,
            multiplicity: 0,
        })
    ...
    Ok(ZkAiAttentionKvNativeD32SoftmaxTableLookupSummary {
        ...
        lookup_relation: "AttentionKvD32SoftmaxTableLookupRelation".to_string(),
...
fn prove_lookup(bundle: &LookupBundle) -> Result<Vec<u8>> {
    let component = lookup_component(AttentionKvD32SoftmaxTableLookupRelation::dummy());
    ...
    let lookup_elements = AttentionKvD32SoftmaxTableLookupRelation::draw(channel);
    ...
    serde_json::to_vec(&AttentionKvD32SoftmaxTableLookupProofPayload { stark_proof })
        .map_err(|error| VmError::Serialization(error.to_string()))
}
...
fn verify_lookup(
    ...
    validate_source_input(source_input)?;
    if proof.is_empty()
        || proof.len() > ZKAI_ATTENTION_KV_NATIVE_D32_SOFTMAX_TABLE_LOOKUP_MAX_PROOF_BYTES
    {
        return Err(lookup_error("lookup proof byte length outside bounded cap"));
    }
    let payload: AttentionKvD32SoftmaxTableLookupProofPayload =
    ...
    let component_placeholder = lookup_component(AttentionKvD32SoftmaxTableLookupRelation::dummy());
...
fn lookup_component(
    lookup_elements: AttentionKvD32SoftmaxTableLookupRelation,
) -> FrameworkComponent<AttentionKvD32SoftmaxTableLookupEval> {
    FrameworkComponent::new(
        ...
        AttentionKvD32SoftmaxTableLookupEval { lookup_elements },
        SecureField::zero(),
    )
}
...
mod tests {
    ...
    #[test]
    fn attention_kv_d32_softmax_table_lookup_sidecar_round_trips_real_proof() {
        let input = source_input();
        let envelope = prove_zkai_attention_kv_native_d32_softmax_table_lookup_envelope(&input)
    ...
    #[test]
    fn attention_kv_d32_softmax_table_lookup_summary_counts_claims() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_d32_softmax_table_lookup_rejects_empty_table_without_panic() {
    ...
    #[test]
    fn attention_kv_d32_softmax_table_lookup_rejects_preprocessed_overflow_without_panic() {
        let input = source_input();
        let mut summary = lookup_summary(&input).expect("summary");
    ...
    #[test]
    fn attention_kv_d32_softmax_table_lookup_masks_inactive_denominators() {
        let guarded =
    ...
    #[test]
    fn attention_kv_d32_softmax_table_lookup_preserves_one_sided_contributions() {
    ...
    #[test]
    fn attention_kv_d32_softmax_table_lookup_rejects_summary_drift() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_d32_softmax_table_lookup_transcript_binds_claim_strings() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_d32_softmax_table_lookup_rejects_source_weight_drift() {
        let input = source_input();
        let mut envelope = prove_zkai_attention_kv_native_d32_softmax_table_lookup_envelope(&input)
            .expect("prove lookup sidecar");
    ...
    #[test]
    fn attention_kv_d32_softmax_table_lookup_rejects_proof_byte_tamper() {
        let input = source_input();
        let mut envelope = prove_zkai_attention_kv_native_d32_softmax_table_lookup_envelope(&input)
            .expect("prove lookup sidecar");
