...
use super::attention_kv_native_d32_bounded_softmax_table_proof::{
    validate_zkai_attention_kv_native_d32_bounded_softmax_table_input,
...
pub const ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_PROOF_VERSION: &str =
    "stwo-attention-kv-d32-fused-bounded-softmax-table-logup-proof-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_BACKEND_VERSION: &str =
    "stwo-attention-kv-d32-fused-bounded-softmax-table-logup-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_STATEMENT_VERSION: &str =
    "zkai-attention-kv-stwo-native-d32-fused-softmax-table-logup-statement-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_SEMANTIC_SCOPE: &str =
    "d32_bounded_softmax_table_attention_arithmetic_and_logup_membership_fused_in_one_native_stwo_proof";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_DECISION: &str =
    "GO_NATIVE_STWO_FUSED_ATTENTION_ARITHMETIC_AND_SOFTMAX_TABLE_LOGUP_MEMBERSHIP";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_TARGET_ID: &str =
    "attention-kv-d32-causal-mask-fused-bounded-softmax-table-logup-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_VERIFIER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d32-fused-bounded-softmax-table-logup:v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_MAX_ENVELOPE_JSON_BYTES: usize =
    2_097_152;
pub const ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_MAX_PROOF_BYTES: usize = 131_072;

const ISSUE: usize = 6;
const SOURCE_ISSUE: usize = 6;
const SOURCE_PLUS_SIDECAR_RAW_PROOF_BYTES: usize = 101_120 + 15_562;
const FUSION_STATUS: &str =
    "GO_ONE_NATIVE_STWO_PROOF_OBJECT_WITH_ATTENTION_ARITHMETIC_AND_LOGUP_MEMBERSHIP";
const NON_FUSED_STATUS: &str = "GO_MATCHED_D32_SOURCE_PLUS_LOGUP_SIDECAR_COMPARATOR_RECORDED";
...
const PREPROCESSED_TABLE_GAP: &str =
    "zkai/attention-kv/native-d32-fused-softmax-table-logup/table-gap";
const PREPROCESSED_TABLE_WEIGHT: &str =
    "zkai/attention-kv/native-d32-fused-softmax-table-logup/table-weight";
const PREPROCESSED_TABLE_MULTIPLICITY: &str =
    "zkai/attention-kv/native-d32-fused-softmax-table-logup/table-multiplicity";
const FUSED_COLUMN_PREFIX: &str = "zkai/attention-kv/native-d32-fused-softmax-table-logup";
...
relation!(AttentionKvD32FusedSoftmaxTableRelation, 2);

#[derive(Debug, Clone)]
struct AttentionKvNativeD32FusedSoftmaxTableEval {
    lookup_elements: AttentionKvD32FusedSoftmaxTableRelation,
}

impl FrameworkEval for AttentionKvNativeD32FusedSoftmaxTableEval {
...
pub struct AttentionKvD32FusedSoftmaxTableMultiplicity {
    pub gap: usize,
    pub weight: i64,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeD32FusedSoftmaxTableSummary {
    pub issue: usize,
    ...
    pub table_multiplicities: Vec<AttentionKvD32FusedSoftmaxTableMultiplicity>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeD32FusedSoftmaxTableEnvelope {
    ...
    pub fused_summary: ZkAiAttentionKvNativeD32FusedSoftmaxTableSummary,
...
#[serde(deny_unknown_fields)]
struct AttentionKvNativeD32FusedSoftmaxTableProofPayload {
...
struct FusedBundle {
    log_size: u32,
    summary: ZkAiAttentionKvNativeD32FusedSoftmaxTableSummary,
...
pub fn zkai_attention_kv_native_d32_fused_softmax_table_source_input_from_json_str(
    raw_json: &str,
    ...
    zkai_attention_kv_native_d32_bounded_softmax_table_input_from_json_str(raw_json)
}

pub fn prove_zkai_attention_kv_native_d32_fused_softmax_table_envelope(
    source_input: &ZkAiAttentionKvNativeD32BoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeD32FusedSoftmaxTableEnvelope> {
    validate_source_input(source_input)?;
    let bundle = build_fused_bundle(source_input)?;
    let proof = prove_fused(&bundle)?;
    let envelope = ZkAiAttentionKvNativeD32FusedSoftmaxTableEnvelope {
        proof_backend: StarkProofBackend::Stwo,
        proof_backend_version: ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_BACKEND_VERSION
            .to_string(),
        proof_schema_version: ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_PROOF_VERSION
            .to_string(),
        statement_version: ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_STATEMENT_VERSION
            .to_string(),
        semantic_scope: ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_SEMANTIC_SCOPE.to_string(),
        decision: ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_DECISION.to_string(),
        target_id: ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_TARGET_ID.to_string(),
        verifier_domain: ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_VERIFIER_DOMAIN
            .to_string(),
...
pub fn zkai_attention_kv_native_d32_fused_softmax_table_envelope_from_json_slice(
    raw_json: &[u8],
) -> Result<ZkAiAttentionKvNativeD32FusedSoftmaxTableEnvelope> {
    if raw_json.len() > ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_MAX_ENVELOPE_JSON_BYTES {
        return Err(fused_error(format!(
...
pub fn verify_zkai_attention_kv_native_d32_fused_softmax_table_envelope(
    envelope: &ZkAiAttentionKvNativeD32FusedSoftmaxTableEnvelope,
) -> Result<bool> {
    validate_envelope(envelope)?;
    verify_fused(&envelope.source_input, &envelope.proof)
}

fn validate_envelope(envelope: &ZkAiAttentionKvNativeD32FusedSoftmaxTableEnvelope) -> Result<()> {
    ...
    if envelope.proof.is_empty()
        || envelope.proof.len() > ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_MAX_PROOF_BYTES
    {
...
fn validate_source_input(
    ...
    validate_zkai_attention_kv_native_d32_bounded_softmax_table_input(input)?;
...
fn fused_summary(
    input: &ZkAiAttentionKvNativeD32BoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeD32FusedSoftmaxTableSummary> {
    let mut multiplicities = input
        .weight_table
        .iter()
        .map(|entry| AttentionKvD32FusedSoftmaxTableMultiplicity {
            gap: entry.gap,
            weight: entry.weight,
            multiplicity: 0,
        })
    ...
    Ok(ZkAiAttentionKvNativeD32FusedSoftmaxTableSummary {
        issue: ISSUE,
        ...
        lookup_relation: "AttentionKvD32FusedSoftmaxTableRelation".to_string(),
...
fn prove_fused(bundle: &FusedBundle) -> Result<Vec<u8>> {
    let component = fused_component(AttentionKvD32FusedSoftmaxTableRelation::dummy());
    ...
    let lookup_elements = AttentionKvD32FusedSoftmaxTableRelation::draw(channel);
    ...
    serde_json::to_vec(&AttentionKvNativeD32FusedSoftmaxTableProofPayload { stark_proof })
        .map_err(|error| VmError::Serialization(error.to_string()))
}
...
fn verify_fused(
    ...
    validate_source_input(source_input)?;
    if proof.is_empty()
        || proof.len() > ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_MAX_PROOF_BYTES
    {
        return Err(fused_error("fused proof byte length outside bounded cap"));
    }
    let payload: AttentionKvNativeD32FusedSoftmaxTableProofPayload =
    ...
    let component_placeholder = fused_component(AttentionKvD32FusedSoftmaxTableRelation::dummy());
...
fn fused_component(
    lookup_elements: AttentionKvD32FusedSoftmaxTableRelation,
) -> FrameworkComponent<AttentionKvNativeD32FusedSoftmaxTableEval> {
    FrameworkComponent::new(
        &mut TraceLocationAllocator::new_with_preprocessed_columns(&fused_preprocessed_column_ids()),
        AttentionKvNativeD32FusedSoftmaxTableEval { lookup_elements },
        SecureField::zero(),
    )
}
...
fn fused_error(message: impl Into<String>) -> VmError {
    VmError::UnsupportedProof(format!(
        "attention/KV native d32 fused bounded Softmax-table proof: {}",
        message.into()
    ))
}
...
mod tests {
    ...
    fn source_input() -> ZkAiAttentionKvNativeD32BoundedSoftmaxTableProofInput {
        let raw = include_str!(
            "../../docs/engineering/evidence/zkai-attention-kv-stwo-native-d32-bounded-softmax-table-proof-2026-05.json"
        );
        zkai_attention_kv_native_d32_fused_softmax_table_source_input_from_json_str(raw)
            .expect("source input")
    }

    #[test]
    fn attention_kv_d32_fused_softmax_table_round_trips_real_proof() {
        let input = source_input();
        let envelope = prove_zkai_attention_kv_native_d32_fused_softmax_table_envelope(&input)
            .expect("prove fused attention/lookup");
        assert_eq!(
            envelope.decision,
            ZKAI_ATTENTION_KV_NATIVE_D32_FUSED_SOFTMAX_TABLE_DECISION
        );
        assert_eq!(envelope.fused_summary.lookup_claims, 52);
        assert_eq!(envelope.fused_summary.table_rows, 9);
        assert!(
            verify_zkai_attention_kv_native_d32_fused_softmax_table_envelope(&envelope)
                .expect("verify fused attention/lookup")
        );
    }

    #[test]
    fn attention_kv_d32_fused_softmax_table_summary_counts_claims() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_d32_fused_softmax_table_derives_logup_indices_from_column_ids() {
        let row_ids = fused_row_column_ids();
        let preprocessed_ids = fused_preprocessed_column_ids();
        let indices = fused_trace_column_indices();
    ...
    #[test]
    fn attention_kv_d32_fused_softmax_table_rejects_summary_drift() {
        let input = source_input();
        let mut envelope = prove_zkai_attention_kv_native_d32_fused_softmax_table_envelope(&input)
            .expect("prove fused attention/lookup");
        envelope.fused_summary.lookup_claims += 1;
        let error = verify_zkai_attention_kv_native_d32_fused_softmax_table_envelope(&envelope)
            .expect_err("summary drift must reject");
        assert!(error.to_string().contains("fused summary"));
    }

    #[test]
    fn attention_kv_d32_fused_softmax_table_rejects_source_weight_drift() {
        let input = source_input();
        let mut envelope = prove_zkai_attention_kv_native_d32_fused_softmax_table_envelope(&input)
            .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d32_fused_softmax_table_rejects_negative_bit_material() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_d32_fused_softmax_table_rejects_output_remainder_drift() {
        let input = source_input();
        let mut envelope = prove_zkai_attention_kv_native_d32_fused_softmax_table_envelope(&input)
            .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d32_fused_softmax_table_rejects_proof_byte_tamper() {
        let input = source_input();
        let mut envelope = prove_zkai_attention_kv_native_d32_fused_softmax_table_envelope(&input)
            .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d32_fused_softmax_table_rejects_backend_version_drift() {
        let input = source_input();
        let mut envelope = prove_zkai_attention_kv_native_d32_fused_softmax_table_envelope(&input)
            .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d32_fused_softmax_table_rejects_proof_schema_version_drift() {
        let input = source_input();
        let mut envelope = prove_zkai_attention_kv_native_d32_fused_softmax_table_envelope(&input)
            .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d32_fused_softmax_table_rejects_unknown_envelope_field() {
        let input = source_input();
        let envelope = prove_zkai_attention_kv_native_d32_fused_softmax_table_envelope(&input)
            .expect("prove fused attention/lookup");
        ...
        let error = zkai_attention_kv_native_d32_fused_softmax_table_envelope_from_json_slice(&raw)
