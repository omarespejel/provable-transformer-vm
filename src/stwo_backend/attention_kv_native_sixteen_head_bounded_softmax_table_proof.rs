...
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_BOUNDED_SOFTMAX_TABLE_INPUT_SCHEMA: &str =
    "zkai-attention-kv-stwo-native-sixteen-head-bounded-softmax-table-air-proof-input-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_BOUNDED_SOFTMAX_TABLE_INPUT_DECISION: &str =
    "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_SIXTEEN_HEAD_BOUNDED_SOFTMAX_TABLE_AIR_PROOF";
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_BOUNDED_SOFTMAX_TABLE_PROOF_VERSION: &str =
    "stwo-attention-kv-d8-causal-mask-sixteen-head-bounded-softmax-table-air-proof-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_BOUNDED_SOFTMAX_TABLE_STATEMENT_VERSION: &str =
    "zkai-attention-kv-stwo-native-sixteen-head-bounded-softmax-table-statement-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_BOUNDED_SOFTMAX_TABLE_SEMANTIC_SCOPE: &str =
    "sixteen_head_d8_bounded_table_softmax_approx_attention_kv_causal_mask_rows_bound_to_statement_receipt";
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_BOUNDED_SOFTMAX_TABLE_DECISION: &str =
    "GO_STWO_NATIVE_ATTENTION_KV_SIXTEEN_HEAD_BOUNDED_SOFTMAX_TABLE_AIR_PROOF";
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_BOUNDED_SOFTMAX_TABLE_TARGET_ID: &str =
    "attention-kv-d8-causal-mask-sixteen-head-bounded-softmax-table-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_BOUNDED_SOFTMAX_TABLE_REQUIRED_BACKEND_VERSION:
    &str = "stwo-attention-kv-d8-causal-mask-sixteen-head-bounded-softmax-table-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_BOUNDED_SOFTMAX_TABLE_VERIFIER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-sixteen-head-bounded-softmax-table:v1";

const ISSUE: usize = 516;
const SOURCE_ISSUE: usize = 496;
...
const SCORE_ROW_COUNT: usize = 832;
...
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_BOUNDED_SOFTMAX_TABLE_MAX_INPUT_JSON_BYTES: usize =
    4_194_304;
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_BOUNDED_SOFTMAX_TABLE_MAX_ENVELOPE_JSON_BYTES:
    usize = 8_388_608;
...
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_BOUNDED_SOFTMAX_TABLE_MAX_PROOF_BYTES: usize =
    262_144;

const ROW_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-sixteen-head-bounded-softmax-table-score-rows:v1";
const INITIAL_KV_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-sixteen-head-bounded-softmax-table-initial-kv:v1";
const INPUT_STEPS_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-sixteen-head-bounded-softmax-table-input-steps:v1";
const FINAL_KV_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-sixteen-head-bounded-softmax-table-final-kv:v1";
const OUTPUTS_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-sixteen-head-bounded-softmax-table-outputs:v1";
const PUBLIC_INSTANCE_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-sixteen-head-bounded-softmax-table-public-instance:v1";
const PROOF_NATIVE_PARAMETER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-sixteen-head-bounded-softmax-table-proof-parameters:v1";
const WEIGHT_TABLE_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-sixteen-head-bounded-softmax-table-weight-table:v1";
...
const NEXT_BACKEND_STEP: &str =
    "test whether the sixteen-head LogUp sidecar keeps proof bytes compact when lookup claims double again";
...
const EXPECTED_VALIDATION_COMMANDS: &[&str] = &[
    ...
    "python3 -m unittest scripts.tests.test_zkai_attention_kv_stwo_native_sixteen_head_bounded_softmax_table_proof_input",
...
#[derive(Debug, Clone)]
struct AttentionKvNativeSixteenHeadBoundedSoftmaxTableEval;

impl FrameworkEval for AttentionKvNativeSixteenHeadBoundedSoftmaxTableEval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        assert_eq!(
            column_ids.len(),
            trace_values.len(),
            "sixteen-head bounded Softmax-table AIR column arity drift"
        );
...
pub struct AttentionKvSixteenHeadBoundedSoftmaxTableEntry {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AttentionKvSixteenHeadBoundedSoftmaxTableInputStep {
    pub head_index: usize,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AttentionKvSixteenHeadBoundedSoftmaxTableWeightEntry {
    pub gap: usize,
    pub weight: i64,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AttentionKvSixteenHeadBoundedSoftmaxTableScoreRow {
    pub row_index: usize,
    pub head_index: usize,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeSixteenHeadBoundedSoftmaxTableProofInput {
    pub schema: String,
    pub decision: String,
    pub issue: usize,
    ...
    pub weight_table: Vec<AttentionKvSixteenHeadBoundedSoftmaxTableWeightEntry>,
    ...
    pub initial_kv_cache: Vec<AttentionKvSixteenHeadBoundedSoftmaxTableEntry>,
    pub input_steps: Vec<AttentionKvSixteenHeadBoundedSoftmaxTableInputStep>,
    pub final_kv_cache: Vec<AttentionKvSixteenHeadBoundedSoftmaxTableEntry>,
    pub attention_outputs: Vec<Vec<i64>>,
    pub score_rows: Vec<AttentionKvSixteenHeadBoundedSoftmaxTableScoreRow>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeSixteenHeadBoundedSoftmaxTableEnvelope {
    pub proof_backend: StarkProofBackend,
    pub proof_backend_version: String,
    pub statement_version: String,
    pub semantic_scope: String,
    pub decision: String,
    pub input: ZkAiAttentionKvNativeSixteenHeadBoundedSoftmaxTableProofInput,
    pub proof: Vec<u8>,
}

#[derive(Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
struct AttentionKvNativeSixteenHeadBoundedSoftmaxTableProofPayload {
    stark_proof: StarkProof<Blake2sM31MerkleHasher>,
}

pub fn zkai_attention_kv_native_sixteen_head_bounded_softmax_table_input_from_json_str(
    raw_json: &str,
) -> Result<ZkAiAttentionKvNativeSixteenHeadBoundedSoftmaxTableProofInput> {
    if raw_json.len()
    ...
    let input: ZkAiAttentionKvNativeSixteenHeadBoundedSoftmaxTableProofInput =
...
pub fn prove_zkai_attention_kv_native_sixteen_head_bounded_softmax_table_envelope(
    input: &ZkAiAttentionKvNativeSixteenHeadBoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeSixteenHeadBoundedSoftmaxTableEnvelope> {
    validate_input(input)?;
    Ok(
        ZkAiAttentionKvNativeSixteenHeadBoundedSoftmaxTableEnvelope {
            ...
            semantic_scope:
                ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_BOUNDED_SOFTMAX_TABLE_SEMANTIC_SCOPE
                    .to_string(),
            decision: ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_BOUNDED_SOFTMAX_TABLE_DECISION
                .to_string(),
            input: input.clone(),
...
pub fn zkai_attention_kv_native_sixteen_head_bounded_softmax_table_envelope_from_json_slice(
    raw_json: &[u8],
) -> Result<ZkAiAttentionKvNativeSixteenHeadBoundedSoftmaxTableEnvelope> {
    if raw_json.len()
    ...
    let envelope: ZkAiAttentionKvNativeSixteenHeadBoundedSoftmaxTableEnvelope =
...
pub fn verify_zkai_attention_kv_native_sixteen_head_bounded_softmax_table_envelope(
    envelope: &ZkAiAttentionKvNativeSixteenHeadBoundedSoftmaxTableEnvelope,
) -> Result<bool> {
    validate_envelope(envelope)?;
...
pub(crate) fn validate_zkai_attention_kv_native_sixteen_head_bounded_softmax_table_input(
    input: &ZkAiAttentionKvNativeSixteenHeadBoundedSoftmaxTableProofInput,
) -> Result<()> {
    validate_input(input)
}
...
fn validate_sequence(
    ...
    for (global_step_index, step) in input.input_steps.iter().enumerate() {
        ...
        let next_item = AttentionKvSixteenHeadBoundedSoftmaxTableEntry {
        ...
        let scored: Vec<(AttentionKvSixteenHeadBoundedSoftmaxTableEntry, i64)> = next_cache
            .iter()
            .filter(|candidate| {
            ...
            scored.iter().zip(weights.iter()).enumerate()
        {
            ...
            expected_rows.push(AttentionKvSixteenHeadBoundedSoftmaxTableScoreRow {
...
fn validate_per_head_strict_positions(
    entries: &[AttentionKvSixteenHeadBoundedSoftmaxTableEntry],
    label: &str,
) -> Result<()> {
...
fn validate_final_kv_append_order(
    ...
    for step in &input.input_steps {
        expected.push(AttentionKvSixteenHeadBoundedSoftmaxTableEntry {
            head_index: step.head_index,
            position: step.token_position,
            key: step.new_key.clone(),
            value: step.new_value.clone(),
...
fn validate_kv_entry(entry: &AttentionKvSixteenHeadBoundedSoftmaxTableEntry) -> Result<()> {
    if entry.head_index >= HEAD_COUNT {
...
fn validate_input_step(step: &AttentionKvSixteenHeadBoundedSoftmaxTableInputStep) -> Result<()> {
    if step.head_index >= HEAD_COUNT {
        return Err(weighted_error("input step head index out of range"));
    }
...
fn validate_score_row(
    row: &AttentionKvSixteenHeadBoundedSoftmaxTableScoreRow,
    ...
    let stark_proof = match stark_proof_result {
        ...
        Err(error) => {
            return Err(VmError::UnsupportedProof(format!(
                "attention/KV native sixteen-head bounded Softmax-table AIR proving failed: {error}"
            )));
        }
    };
    serde_json::to_vec(&AttentionKvNativeSixteenHeadBoundedSoftmaxTableProofPayload { stark_proof })
        .map_err(|error| VmError::Serialization(error.to_string()))
}
...
fn verify_rows(
    ...
    validate_input(input)?;
    let payload: AttentionKvNativeSixteenHeadBoundedSoftmaxTableProofPayload =
    ...
    verify(&[&component], channel, commitment_scheme, stark_proof)
        ...
        .map_err(|error| {
            weighted_error(format!(
                "attention/KV native sixteen-head bounded Softmax-table proof rejected: {error}"
            ))
        })
}
...
fn attention_component() -> FrameworkComponent<AttentionKvNativeSixteenHeadBoundedSoftmaxTableEval>
{
    FrameworkComponent::new(
        &mut TraceLocationAllocator::new_with_preprocessed_columns(&preprocessed_column_ids()),
        AttentionKvNativeSixteenHeadBoundedSoftmaxTableEval,
        SecureField::zero(),
    )
}
...
fn attention_trace(
    ...
    for (real_index, row) in rows.iter().enumerate() {
        ...
        assert_eq!(
            values.len(),
            columns.len(),
            "sixteen-head bounded Softmax-table trace width drift"
        );
...
fn padding_row(row_index: usize) -> AttentionKvSixteenHeadBoundedSoftmaxTableScoreRow {
...
fn column_ids() -> Vec<String> {
    ...
    .into_iter()
    .map(|suffix| format!("zkai/attention-kv/native-sixteen-head-bounded-softmax-table/{suffix}"))
    .collect::<Vec<_>>();
    for prefix in [
        "query",
        "key",
        ...
        for index in 0..width {
            ids.push(format!(
                "zkai/attention-kv/native-sixteen-head-bounded-softmax-table/{prefix}-{index:02}"
            ));
        }
    }
    for index in 0..SCORE_GAP_BITS {
        ids.push(format!(
            "zkai/attention-kv/native-sixteen-head-bounded-softmax-table/score-gap-bit-{index:02}"
        ));
    }
    for index in 0..CAUSAL_GAP_BITS {
        ids.push(format!(
            "zkai/attention-kv/native-sixteen-head-bounded-softmax-table/causal-gap-bit-{index:02}"
        ));
    }
    for index in 0..WEIGHT_BITS {
        ids.push(format!(
            "zkai/attention-kv/native-sixteen-head-bounded-softmax-table/weight-bit-{index:02}"
        ));
    }
    for dim in 0..VALUE_WIDTH {
        for index in 0..OUTPUT_REMAINDER_BITS {
            ids.push(format!(
                "zkai/attention-kv/native-sixteen-head-bounded-softmax-table/output-remainder-{dim:02}-bit-{index:02}"
            ));
        }
    }
    ids
...
fn expected_weight_table_entries() -> Vec<AttentionKvSixteenHeadBoundedSoftmaxTableWeightEntry> {
    WEIGHT_TABLE
        .iter()
        .map(
            |(gap, weight)| AttentionKvSixteenHeadBoundedSoftmaxTableWeightEntry {
                gap: *gap,
                weight: *weight,
            },
        )
...
fn validate_weight_table(
    table: &[AttentionKvSixteenHeadBoundedSoftmaxTableWeightEntry],
) -> Result<()> {
...
fn kv_commitment(
    cache: &[AttentionKvSixteenHeadBoundedSoftmaxTableEntry],
    domain: &str,
) -> Result<String> {
    let material = cache
        .iter()
...
fn input_steps_commitment(
    steps: &[AttentionKvSixteenHeadBoundedSoftmaxTableInputStep],
) -> Result<String> {
    let material = steps
        .iter()
        .map(|step| {
...
fn rows_commitment(rows: &[AttentionKvSixteenHeadBoundedSoftmaxTableScoreRow]) -> Result<String> {
    ...
    commitment_from_parts(
        &[
            (
                "encoding",
                json_string(
                    "attention_kv_stwo_native_sixteen_head_bounded_softmax_table_score_rows_v1",
                )?,
            ),
            (
                "shape",
...
fn score_row_material(row: &AttentionKvSixteenHeadBoundedSoftmaxTableScoreRow) -> Vec<i64> {
    let mut out = vec![
...
fn weighted_error(message: impl Into<String>) -> VmError {
    VmError::InvalidConfig(format!(
        "attention/KV native sixteen-head bounded Softmax-table proof: {}",
        message.into()
    ))
}
...
mod tests {
    ...
    fn input() -> ZkAiAttentionKvNativeSixteenHeadBoundedSoftmaxTableProofInput {
        zkai_attention_kv_native_sixteen_head_bounded_softmax_table_input_from_json_str(INPUT_JSON)
            .expect("sixteen-head bounded Softmax-table attention input")
    }

    #[test]
    fn attention_kv_native_sixteen_head_bounded_softmax_table_input_validates_checked_rows() {
        let input = input();
        ...
        assert_eq!(
            ...
            vec![-4, -3, 8, -10, 1, 8, -6, 1]
        );
        assert_eq!(
            input.attention_outputs[32],
            vec![-14, -1, 13, -12, 2, 16, -9, -5]
        );
        assert_eq!(
            input.attention_outputs[127],
            vec![-26, 3, 24, -21, 1, 23, -24, -1]
        );
    ...
    #[test]
    fn attention_kv_native_sixteen_head_bounded_softmax_table_uses_floor_division_for_negative_numerators(
    ) {
        assert_eq!(
    ...
    #[test]
    fn attention_kv_native_sixteen_head_bounded_softmax_table_air_proof_round_trips() {
        let input = input();
        let envelope =
            prove_zkai_attention_kv_native_sixteen_head_bounded_softmax_table_envelope(&input)
                .expect("sixteen-head bounded Softmax-table attention proof");
        assert!(!envelope.proof.is_empty());
        assert!(
            verify_zkai_attention_kv_native_sixteen_head_bounded_softmax_table_envelope(&envelope)
                .expect("verify")
        );
    }

    #[test]
    fn attention_kv_native_sixteen_head_bounded_softmax_table_rejects_weight_relabeling() {
    ...
    #[test]
    fn attention_kv_native_sixteen_head_bounded_softmax_table_rejects_head_relabeling() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_sixteen_head_bounded_softmax_table_rejects_output_relabeling() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_sixteen_head_bounded_softmax_table_rejects_quotient_remainder_drift() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_sixteen_head_bounded_softmax_table_rejects_initial_kv_position_reorder()
    {
        let mut input = input();
    ...
    #[test]
    fn attention_kv_native_sixteen_head_bounded_softmax_table_rejects_final_kv_append_reorder() {
        let mut input = input();
        input.final_kv_cache.swap(32, 33);
        let error = validate_sequence(&input).unwrap_err();
        assert!(error
            .to_string()
    ...
    #[test]
    fn attention_kv_native_sixteen_head_bounded_softmax_table_rejects_commitment_drift() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_sixteen_head_bounded_softmax_table_rejects_proof_byte_tamper() {
        let input = input();
        let mut envelope =
            prove_zkai_attention_kv_native_sixteen_head_bounded_softmax_table_envelope(&input)
                .expect("sixteen-head bounded Softmax-table attention proof");
    ...
    #[test]
    fn attention_kv_native_sixteen_head_bounded_softmax_table_rejects_unknown_envelope_field() {
        let input = input();
        let envelope =
            prove_zkai_attention_kv_native_sixteen_head_bounded_softmax_table_envelope(&input)
                .expect("sixteen-head bounded Softmax-table attention proof");
