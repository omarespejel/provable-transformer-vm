...
use super::attention_kv_native_eight_head_bounded_softmax_table_proof::{
    validate_zkai_attention_kv_native_eight_head_bounded_softmax_table_input,
...
pub const ZKAI_ATTENTION_KV_NATIVE_EIGHT_HEAD_FUSED_SOFTMAX_TABLE_PROOF_VERSION: &str =
    "stwo-attention-kv-eight-head-fused-bounded-softmax-table-logup-proof-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_EIGHT_HEAD_FUSED_SOFTMAX_TABLE_BACKEND_VERSION: &str =
    "stwo-attention-kv-eight-head-fused-bounded-softmax-table-logup-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_EIGHT_HEAD_FUSED_SOFTMAX_TABLE_STATEMENT_VERSION: &str =
    "zkai-attention-kv-stwo-native-eight-head-fused-softmax-table-logup-statement-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_EIGHT_HEAD_FUSED_SOFTMAX_TABLE_SEMANTIC_SCOPE: &str =
    "eight_head_d8_bounded_softmax_table_attention_arithmetic_and_logup_membership_fused_in_one_native_stwo_proof";
pub const ZKAI_ATTENTION_KV_NATIVE_EIGHT_HEAD_FUSED_SOFTMAX_TABLE_DECISION: &str =
    "GO_NATIVE_STWO_FUSED_ATTENTION_ARITHMETIC_AND_SOFTMAX_TABLE_LOGUP_MEMBERSHIP";
pub const ZKAI_ATTENTION_KV_NATIVE_EIGHT_HEAD_FUSED_SOFTMAX_TABLE_TARGET_ID: &str =
    "attention-kv-eight-head-d8-causal-mask-fused-bounded-softmax-table-logup-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_EIGHT_HEAD_FUSED_SOFTMAX_TABLE_VERIFIER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-eight-head-fused-bounded-softmax-table-logup:v1";
pub const ZKAI_ATTENTION_KV_NATIVE_EIGHT_HEAD_FUSED_SOFTMAX_TABLE_MAX_ENVELOPE_JSON_BYTES: usize =
    4_194_304;
pub const ZKAI_ATTENTION_KV_NATIVE_EIGHT_HEAD_FUSED_SOFTMAX_TABLE_MAX_PROOF_BYTES: usize = 262_144;

const ISSUE: usize = 496;
const SOURCE_ISSUE: usize = 496;
const SIDECAR_ISSUE: usize = 514;
const SOURCE_PLUS_SIDECAR_RAW_PROOF_BYTES: usize = 74_086;
...
const NON_FUSED_STATUS: &str =
    "GO_MATCHED_EIGHT_HEAD_SOURCE_PLUS_LOGUP_SIDECAR_COMPARATOR_RECORDED";
...
const PREPROCESSED_TABLE_GAP: &str =
    "zkai/attention-kv/native-eight-head-fused-softmax-table-logup/table-gap";
const PREPROCESSED_TABLE_WEIGHT: &str =
    "zkai/attention-kv/native-eight-head-fused-softmax-table-logup/table-weight";
const PREPROCESSED_TABLE_MULTIPLICITY: &str =
    "zkai/attention-kv/native-eight-head-fused-softmax-table-logup/table-multiplicity";
const FUSED_COLUMN_PREFIX: &str = "zkai/attention-kv/native-eight-head-fused-softmax-table-logup";
...
relation!(AttentionKvEightHeadFusedSoftmaxTableRelation, 2);

#[derive(Debug, Clone)]
struct AttentionKvNativeEightHeadFusedSoftmaxTableEval {
    lookup_elements: AttentionKvEightHeadFusedSoftmaxTableRelation,
}

impl FrameworkEval for AttentionKvNativeEightHeadFusedSoftmaxTableEval {
...
pub struct AttentionKvEightHeadFusedSoftmaxTableMultiplicity {
    pub gap: usize,
    pub weight: i64,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeEightHeadFusedSoftmaxTableSummary {
    pub issue: usize,
    ...
    pub table_multiplicities: Vec<AttentionKvEightHeadFusedSoftmaxTableMultiplicity>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeEightHeadFusedSoftmaxTableEnvelope {
    ...
    pub fused_summary: ZkAiAttentionKvNativeEightHeadFusedSoftmaxTableSummary,
    pub source_input: ZkAiAttentionKvNativeEightHeadBoundedSoftmaxTableProofInput,
    pub proof: Vec<u8>,
}

#[derive(Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
struct AttentionKvNativeEightHeadFusedSoftmaxTableProofPayload {
...
struct FusedBundle {
    log_size: u32,
    summary: ZkAiAttentionKvNativeEightHeadFusedSoftmaxTableSummary,
...
pub fn zkai_attention_kv_native_eight_head_fused_softmax_table_source_input_from_json_str(
    raw_json: &str,
    ...
    zkai_attention_kv_native_eight_head_bounded_softmax_table_input_from_json_str(raw_json)
}

pub fn prove_zkai_attention_kv_native_eight_head_fused_softmax_table_envelope(
    source_input: &ZkAiAttentionKvNativeEightHeadBoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeEightHeadFusedSoftmaxTableEnvelope> {
    validate_source_input(source_input)?;
    let bundle = build_fused_bundle(source_input)?;
    let proof = prove_fused(&bundle)?;
    let envelope = ZkAiAttentionKvNativeEightHeadFusedSoftmaxTableEnvelope {
        proof_backend: StarkProofBackend::Stwo,
        proof_backend_version:
            ZKAI_ATTENTION_KV_NATIVE_EIGHT_HEAD_FUSED_SOFTMAX_TABLE_BACKEND_VERSION.to_string(),
        proof_schema_version: ZKAI_ATTENTION_KV_NATIVE_EIGHT_HEAD_FUSED_SOFTMAX_TABLE_PROOF_VERSION
            .to_string(),
        statement_version:
            ZKAI_ATTENTION_KV_NATIVE_EIGHT_HEAD_FUSED_SOFTMAX_TABLE_STATEMENT_VERSION.to_string(),
        semantic_scope: ZKAI_ATTENTION_KV_NATIVE_EIGHT_HEAD_FUSED_SOFTMAX_TABLE_SEMANTIC_SCOPE
            .to_string(),
        decision: ZKAI_ATTENTION_KV_NATIVE_EIGHT_HEAD_FUSED_SOFTMAX_TABLE_DECISION.to_string(),
        target_id: ZKAI_ATTENTION_KV_NATIVE_EIGHT_HEAD_FUSED_SOFTMAX_TABLE_TARGET_ID.to_string(),
        verifier_domain: ZKAI_ATTENTION_KV_NATIVE_EIGHT_HEAD_FUSED_SOFTMAX_TABLE_VERIFIER_DOMAIN
            .to_string(),
...
pub fn zkai_attention_kv_native_eight_head_fused_softmax_table_envelope_from_json_slice(
    raw_json: &[u8],
) -> Result<ZkAiAttentionKvNativeEightHeadFusedSoftmaxTableEnvelope> {
    if raw_json.len()
...
pub fn verify_zkai_attention_kv_native_eight_head_fused_softmax_table_envelope(
    envelope: &ZkAiAttentionKvNativeEightHeadFusedSoftmaxTableEnvelope,
) -> Result<bool> {
    validate_envelope(envelope)?;
...
fn parse_fused_proof_payload(
    proof: &[u8],
) -> Result<AttentionKvNativeEightHeadFusedSoftmaxTableProofPayload> {
    let payload: AttentionKvNativeEightHeadFusedSoftmaxTableProofPayload =
...
fn validate_source_input(
    ...
    validate_zkai_attention_kv_native_eight_head_bounded_softmax_table_input(input)
}

fn fused_summary(
    input: &ZkAiAttentionKvNativeEightHeadBoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeEightHeadFusedSoftmaxTableSummary> {
    let mut multiplicities = input
        .weight_table
        .iter()
        .map(|entry| AttentionKvEightHeadFusedSoftmaxTableMultiplicity {
            gap: entry.gap,
            weight: entry.weight,
            multiplicity: 0,
        })
    ...
    Ok(ZkAiAttentionKvNativeEightHeadFusedSoftmaxTableSummary {
        issue: ISSUE,
        ...
        lookup_relation: "AttentionKvEightHeadFusedSoftmaxTableRelation".to_string(),
...
fn prove_fused(bundle: &FusedBundle) -> Result<Vec<u8>> {
    let component = fused_component(AttentionKvEightHeadFusedSoftmaxTableRelation::dummy());
    ...
    let lookup_elements = AttentionKvEightHeadFusedSoftmaxTableRelation::draw(channel);
    ...
    serde_json::to_vec(&AttentionKvNativeEightHeadFusedSoftmaxTableProofPayload { stark_proof })
        .map_err(|error| VmError::Serialization(error.to_string()))
}
...
fn verify_fused(
    ...
    validate_source_input(source_input)?;
    if proof.is_empty()
        || proof.len() > ZKAI_ATTENTION_KV_NATIVE_EIGHT_HEAD_FUSED_SOFTMAX_TABLE_MAX_PROOF_BYTES
    {
        return Err(fused_error("fused proof byte length outside bounded cap"));
    }
    ...
    let component_placeholder =
        fused_component(AttentionKvEightHeadFusedSoftmaxTableRelation::dummy());
...
fn fused_component(
    lookup_elements: AttentionKvEightHeadFusedSoftmaxTableRelation,
) -> FrameworkComponent<AttentionKvNativeEightHeadFusedSoftmaxTableEval> {
    FrameworkComponent::new(
        &mut TraceLocationAllocator::new_with_preprocessed_columns(&fused_preprocessed_column_ids()),
        AttentionKvNativeEightHeadFusedSoftmaxTableEval { lookup_elements },
        SecureField::zero(),
    )
}
...
fn fused_error(message: impl Into<String>) -> VmError {
    VmError::UnsupportedProof(format!(
        "attention/KV native eight-head fused bounded Softmax-table proof: {}",
        message.into()
    ))
}
...
mod tests {
    ...
    fn source_input() -> ZkAiAttentionKvNativeEightHeadBoundedSoftmaxTableProofInput {
        let raw = include_str!(
            "../../docs/engineering/evidence/zkai-attention-kv-stwo-native-eight-head-bounded-softmax-table-proof-2026-05.json"
        );
        zkai_attention_kv_native_eight_head_fused_softmax_table_source_input_from_json_str(raw)
            .expect("source input")
    }

    #[test]
    fn attention_kv_eight_head_fused_softmax_table_round_trips_real_proof() {
        let input = source_input();
        let envelope =
            prove_zkai_attention_kv_native_eight_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
        assert_eq!(
            envelope.decision,
            ZKAI_ATTENTION_KV_NATIVE_EIGHT_HEAD_FUSED_SOFTMAX_TABLE_DECISION
        );
        assert_eq!(envelope.fused_summary.lookup_claims, 416);
        assert_eq!(envelope.fused_summary.table_rows, 9);
        assert!(
            verify_zkai_attention_kv_native_eight_head_fused_softmax_table_envelope(&envelope)
                .expect("verify fused attention/lookup")
        );
    }

    #[test]
    fn attention_kv_eight_head_fused_softmax_table_summary_counts_claims() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_eight_head_fused_softmax_table_transcript_binds_commitments() {
        let input = source_input();
        let summary = fused_summary(&input).expect("summary");
    ...
    #[test]
    fn attention_kv_eight_head_fused_softmax_table_derives_logup_indices_from_column_ids() {
        let row_ids = fused_row_column_ids();
        let preprocessed_ids = fused_preprocessed_column_ids();
    ...
    #[test]
    fn attention_kv_eight_head_fused_softmax_table_returns_error_on_column_id_drift() {
    ...
    #[test]
    fn attention_kv_eight_head_fused_softmax_table_rejects_summary_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_eight_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_eight_head_fused_softmax_table_rejects_source_weight_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_eight_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_eight_head_fused_softmax_table_rejects_negative_bit_material() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_eight_head_fused_softmax_table_rejects_output_remainder_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_eight_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_eight_head_fused_softmax_table_rejects_proof_byte_tamper() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_eight_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_eight_head_fused_softmax_table_parse_rejects_malformed_proof_payload() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_eight_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
        ...
        let error =
            zkai_attention_kv_native_eight_head_fused_softmax_table_envelope_from_json_slice(&raw)
                .expect_err("malformed proof payload must reject at parse");
        assert!(error.to_string().contains("malformed fused proof payload"));
    }

    #[test]
    fn attention_kv_eight_head_fused_softmax_table_rejects_backend_version_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_eight_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_eight_head_fused_softmax_table_rejects_proof_schema_version_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_eight_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_eight_head_fused_softmax_table_rejects_unknown_envelope_field() {
        let input = source_input();
        let envelope =
            prove_zkai_attention_kv_native_eight_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
