...
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_INPUT_SCHEMA: &str =
    "zkai-attention-kv-stwo-native-two-head-seq32-bounded-softmax-table-air-proof-input-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_INPUT_DECISION: &str =
    "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_AIR_PROOF";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_PROOF_VERSION: &str =
    "stwo-attention-kv-d8-causal-mask-two-head-seq32-bounded-softmax-table-air-proof-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_STATEMENT_VERSION: &str =
    "zkai-attention-kv-stwo-native-two-head-seq32-bounded-softmax-table-statement-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_SEMANTIC_SCOPE: &str =
    "two_head_seq32_d8_bounded_table_softmax_approx_attention_kv_causal_mask_rows_bound_to_statement_receipt";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_DECISION: &str =
    "GO_STWO_NATIVE_ATTENTION_KV_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_AIR_PROOF";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_TARGET_ID: &str =
    "attention-kv-d8-causal-mask-two-head-seq32-bounded-softmax-table-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_REQUIRED_BACKEND_VERSION:
    &str = "stwo-attention-kv-d8-causal-mask-two-head-seq32-bounded-softmax-table-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_VERIFIER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-seq32-bounded-softmax-table:v1";

const ISSUE: usize = 537;
const SOURCE_ISSUE: usize = 498;
...
const MAX_POSITION_PER_HEAD: usize = FINAL_KV_ITEMS_PER_HEAD - 1;
...
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_MAX_INPUT_JSON_BYTES:
    usize = 4_194_304;
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_MAX_ENVELOPE_JSON_BYTES:
    usize = 8_388_608;
...
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_MAX_PROOF_BYTES: usize =
    524_288;

const ROW_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-seq32-bounded-softmax-table-score-rows:v1";
const INITIAL_KV_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-seq32-bounded-softmax-table-initial-kv:v1";
const INPUT_STEPS_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-seq32-bounded-softmax-table-input-steps:v1";
const FINAL_KV_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-seq32-bounded-softmax-table-final-kv:v1";
const OUTPUTS_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-seq32-bounded-softmax-table-outputs:v1";
const PUBLIC_INSTANCE_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-seq32-bounded-softmax-table-public-instance:v1";
const PROOF_NATIVE_PARAMETER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-seq32-bounded-softmax-table-proof-parameters:v1";
const WEIGHT_TABLE_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-seq32-bounded-softmax-table-weight-table:v1";
...
const NEXT_BACKEND_STEP: &str = "fuse the same two-head seq32 bounded Softmax-table arithmetic with LogUp table membership in one native Stwo proof";
...
const EXPECTED_VALIDATION_COMMANDS: &[&str] = &[
    ...
    "python3 -m unittest scripts.tests.test_zkai_attention_kv_stwo_native_two_head_seq32_bounded_softmax_table_proof_input",
    "cargo +nightly-2025-07-14 test attention_kv_native_two_head_seq32_bounded_softmax_table_proof --lib --features stwo-backend",
    "cargo +nightly-2025-07-14 run --features stwo-backend --bin zkai_attention_kv_native_two_head_seq32_bounded_softmax_table_proof -- prove docs/engineering/evidence/zkai-attention-kv-stwo-native-two-head-seq32-bounded-softmax-table-proof-2026-05.json docs/engineering/evidence/zkai-attention-kv-stwo-native-two-head-seq32-bounded-softmax-table-proof-2026-05.envelope.json",
    "cargo +nightly-2025-07-14 run --features stwo-backend --bin zkai_attention_kv_native_two_head_seq32_bounded_softmax_table_proof -- verify docs/engineering/evidence/zkai-attention-kv-stwo-native-two-head-seq32-bounded-softmax-table-proof-2026-05.envelope.json",
...
#[derive(Debug, Clone)]
struct AttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableEval;

impl FrameworkEval for AttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableEval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        assert_eq!(
            column_ids.len(),
            trace_values.len(),
            "bounded Softmax-table AIR column layout drift"
        );
...
pub struct AttentionKvTwoHeadSeq32BoundedSoftmaxTableEntry {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AttentionKvTwoHeadSeq32BoundedSoftmaxTableInputStep {
    pub head_index: usize,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AttentionKvTwoHeadSeq32BoundedSoftmaxTableWeightEntry {
    pub gap: usize,
    pub weight: i64,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AttentionKvTwoHeadSeq32BoundedSoftmaxTableScoreRow {
    pub row_index: usize,
    pub head_index: usize,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableProofInput {
    pub schema: String,
    pub decision: String,
    pub issue: usize,
    ...
    pub weight_table: Vec<AttentionKvTwoHeadSeq32BoundedSoftmaxTableWeightEntry>,
    ...
    pub initial_kv_cache: Vec<AttentionKvTwoHeadSeq32BoundedSoftmaxTableEntry>,
    pub input_steps: Vec<AttentionKvTwoHeadSeq32BoundedSoftmaxTableInputStep>,
    pub final_kv_cache: Vec<AttentionKvTwoHeadSeq32BoundedSoftmaxTableEntry>,
    pub attention_outputs: Vec<Vec<i64>>,
    pub score_rows: Vec<AttentionKvTwoHeadSeq32BoundedSoftmaxTableScoreRow>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableEnvelope {
    pub proof_backend: StarkProofBackend,
    pub proof_backend_version: String,
    pub statement_version: String,
    pub semantic_scope: String,
    pub decision: String,
    pub input: ZkAiAttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableProofInput,
    pub proof: Vec<u8>,
}

#[derive(Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
struct AttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableProofPayload {
    stark_proof: StarkProof<Blake2sM31MerkleHasher>,
}

pub fn zkai_attention_kv_native_two_head_seq32_bounded_softmax_table_input_from_json_str(
    raw_json: &str,
) -> Result<ZkAiAttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableProofInput> {
    if raw_json.len()
    ...
    let input: ZkAiAttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableProofInput =
...
pub fn prove_zkai_attention_kv_native_two_head_seq32_bounded_softmax_table_envelope(
    input: &ZkAiAttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableEnvelope> {
    validate_input(input)?;
    let envelope = ZkAiAttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableEnvelope {
        ...
        statement_version:
            ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_STATEMENT_VERSION
                .to_string(),
        semantic_scope:
            ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_SEMANTIC_SCOPE.to_string(),
        decision: ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_DECISION
            .to_string(),
        input: input.clone(),
...
pub fn zkai_attention_kv_native_two_head_seq32_bounded_softmax_table_envelope_from_json_slice(
    raw_json: &[u8],
) -> Result<ZkAiAttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableEnvelope> {
    if raw_json.len()
...
pub fn verify_zkai_attention_kv_native_two_head_seq32_bounded_softmax_table_envelope(
    envelope: &ZkAiAttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableEnvelope,
) -> Result<bool> {
    validate_envelope(envelope)?;
...
pub(crate) fn validate_zkai_attention_kv_native_two_head_seq32_bounded_softmax_table_input(
    input: &ZkAiAttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableProofInput,
) -> Result<()> {
    validate_input(input)
}
...
fn validate_sequence(
    ...
    for (global_step_index, step) in input.input_steps.iter().enumerate() {
        ...
        let next_item = AttentionKvTwoHeadSeq32BoundedSoftmaxTableEntry {
        ...
        let scored: Vec<(AttentionKvTwoHeadSeq32BoundedSoftmaxTableEntry, i64)> = next_cache
            .iter()
            .filter(|candidate| {
            ...
            scored.iter().zip(weights.iter()).enumerate()
        {
            ...
            let causal_gap = usize_to_i64(step.token_position, "input step token position")?
                .checked_sub(usize_to_i64(candidate.position, "candidate position")?)
                .ok_or_else(|| weighted_error("causal gap subtraction overflow"))?;
            expected_rows.push(AttentionKvTwoHeadSeq32BoundedSoftmaxTableScoreRow {
...
fn validate_per_head_strict_positions(
    entries: &[AttentionKvTwoHeadSeq32BoundedSoftmaxTableEntry],
    label: &str,
) -> Result<()> {
...
fn validate_final_kv_append_order(
    ...
    for step in &input.input_steps {
        expected.push(AttentionKvTwoHeadSeq32BoundedSoftmaxTableEntry {
            head_index: step.head_index,
            position: step.token_position,
            key: step.new_key.clone(),
            value: step.new_value.clone(),
...
fn validate_kv_entry(entry: &AttentionKvTwoHeadSeq32BoundedSoftmaxTableEntry) -> Result<()> {
    if entry.head_index >= HEAD_COUNT {
        return Err(weighted_error("KV head index out of range"));
    }
    expect_usize_at_most(entry.position, MAX_POSITION_PER_HEAD, "KV position")?;
...
fn validate_input_step(step: &AttentionKvTwoHeadSeq32BoundedSoftmaxTableInputStep) -> Result<()> {
    if step.head_index >= HEAD_COUNT {
        return Err(weighted_error("input step head index out of range"));
    }
    expect_usize_at_most(
        step.token_position,
        MAX_POSITION_PER_HEAD,
        "input step token position",
    )?;
...
fn validate_score_row(
    row: &AttentionKvTwoHeadSeq32BoundedSoftmaxTableScoreRow,
    ...
    expect_usize_less_than(
        row.candidate_index,
        FINAL_KV_ITEMS_PER_HEAD,
        "score row candidate index",
    )?;
    expect_usize_at_most(
        row.token_position,
        MAX_POSITION_PER_HEAD,
        "score row token position",
    )?;
    expect_usize_at_most(
        row.candidate_position,
        MAX_POSITION_PER_HEAD,
        "score row candidate position",
    )?;
    expect_usize(
        row.token_position,
        INITIAL_KV_ITEMS_PER_HEAD + row.step_index,
        "score row token position",
    )?;
    if row.candidate_position > row.token_position {
        return Err(weighted_error("candidate position exceeds token position"));
    }
    ...
    let token_position = usize_to_i64(row.token_position, "score row token position")?;
    let candidate_position = usize_to_i64(row.candidate_position, "score row candidate position")?;
    let causal_gap = token_position
        .checked_sub(candidate_position)
        .ok_or_else(|| weighted_error("causal gap subtraction overflow"))?;
    expect_i64(row.causal_gap, causal_gap, "causal gap")?;
...
fn prove_rows(
    ...
    let stark_proof =
        prove::<SimdBackend, Blake2sM31MerkleChannel>(&[&component], channel, commitment_scheme)
            ...
            "attention/KV native two-head seq32 bounded Softmax-table AIR proving failed: {error}"
        ))
            })?;
    serde_json::to_vec(
        &AttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableProofPayload { stark_proof },
    )
    .map_err(|error| VmError::Serialization(error.to_string()))
}
...
fn verify_rows(
    ...
    validate_input(input)?;
    let payload: AttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableProofPayload =
    ...
    verify(&[&component], channel, commitment_scheme, stark_proof)
        ...
        .map_err(|error| {
            weighted_error(format!(
                "attention/KV native two-head seq32 bounded Softmax-table proof rejected: {error}"
            ))
        })
}
...
fn attention_component() -> FrameworkComponent<AttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableEval>
{
    FrameworkComponent::new(
        &mut TraceLocationAllocator::new_with_preprocessed_columns(&preprocessed_column_ids()),
        AttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableEval,
        SecureField::zero(),
    )
}
...
fn attention_trace(
    ...
    for (real_index, row) in rows.iter().enumerate() {
        ...
        if values.len() != columns.len() {
            return Err(weighted_error(format!(
                "trace column layout drift: got {} values, expected {}",
                values.len(),
                columns.len()
            )));
        }
...
fn padding_row(row_index: usize) -> AttentionKvTwoHeadSeq32BoundedSoftmaxTableScoreRow {
...
fn column_ids() -> Vec<String> {
    ...
    .into_iter()
    .map(|suffix| format!("zkai/attention-kv/native-two-head-seq32-bounded-softmax-table/{suffix}"))
    .collect::<Vec<_>>();
    for prefix in [
        "query",
        "key",
        ...
        for index in 0..width {
            ids.push(format!(
                "zkai/attention-kv/native-two-head-seq32-bounded-softmax-table/{prefix}-{index:02}"
            ));
        }
    }
    for index in 0..SCORE_GAP_BITS {
        ids.push(format!(
            "zkai/attention-kv/native-two-head-seq32-bounded-softmax-table/score-gap-bit-{index:02}"
        ));
    }
    for index in 0..CAUSAL_GAP_BITS {
        ids.push(format!(
            "zkai/attention-kv/native-two-head-seq32-bounded-softmax-table/causal-gap-bit-{index:02}"
        ));
    }
    for index in 0..WEIGHT_BITS {
        ids.push(format!(
            "zkai/attention-kv/native-two-head-seq32-bounded-softmax-table/weight-bit-{index:02}"
        ));
    }
    for dim in 0..VALUE_WIDTH {
        for index in 0..OUTPUT_REMAINDER_BITS {
            ids.push(format!(
                "zkai/attention-kv/native-two-head-seq32-bounded-softmax-table/output-remainder-{dim:02}-bit-{index:02}"
            ));
        }
    }
    ids
...
fn expected_weight_table_entries() -> Vec<AttentionKvTwoHeadSeq32BoundedSoftmaxTableWeightEntry> {
    WEIGHT_TABLE
        .iter()
        .map(
            |(gap, weight)| AttentionKvTwoHeadSeq32BoundedSoftmaxTableWeightEntry {
                gap: *gap,
                weight: *weight,
            },
        )
...
fn validate_weight_table(
    table: &[AttentionKvTwoHeadSeq32BoundedSoftmaxTableWeightEntry],
) -> Result<()> {
...
fn bounded_weight(score_gap: i64) -> Result<i64> {
    ...
    let clipped = std::cmp::min(gap, SCORE_GAP_CLIP);
    WEIGHT_TABLE
        .iter()
...
fn kv_commitment(
    cache: &[AttentionKvTwoHeadSeq32BoundedSoftmaxTableEntry],
    domain: &str,
) -> Result<String> {
    let material = cache
        .iter()
...
fn input_steps_commitment(
    steps: &[AttentionKvTwoHeadSeq32BoundedSoftmaxTableInputStep],
) -> Result<String> {
    let material = steps
        .iter()
        .map(|step| {
...
fn rows_commitment(rows: &[AttentionKvTwoHeadSeq32BoundedSoftmaxTableScoreRow]) -> Result<String> {
    ...
    commitment_from_parts(
        &[
            (
                "encoding",
                json_string(
                    "attention_kv_stwo_native_two_head_seq32_bounded_softmax_table_score_rows_v1",
                )?,
            ),
            (
                "shape",
...
fn score_row_material(row: &AttentionKvTwoHeadSeq32BoundedSoftmaxTableScoreRow) -> Vec<i64> {
    let mut out = vec![
...
fn expect_usize_at_most(actual: usize, max: usize, label: &str) -> Result<()> {
    if actual > max {
        return Err(weighted_error(format!(
            "{label} exceeds bound: got {actual}, max {max}"
        )));
    }
    Ok(())
}

fn expect_usize_less_than(actual: usize, max_exclusive: usize, label: &str) -> Result<()> {
    if actual >= max_exclusive {
        return Err(weighted_error(format!(
            "{label} exceeds bound: got {actual}, max exclusive {max_exclusive}"
        )));
    }
    Ok(())
}
...
fn usize_to_i64(value: usize, label: &str) -> Result<i64> {
    i64::try_from(value).map_err(|_| weighted_error(format!("{label} exceeds bounded i64 range")))
}
...
fn weighted_error(message: impl Into<String>) -> VmError {
    VmError::InvalidConfig(format!(
        "attention/KV native two-head seq32 bounded Softmax-table proof: {}",
        message.into()
    ))
}
...
mod tests {
    ...
    fn input() -> ZkAiAttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableProofInput {
        zkai_attention_kv_native_two_head_seq32_bounded_softmax_table_input_from_json_str(
            INPUT_JSON,
        )
        .expect("two-head seq32 bounded Softmax-table attention input")
    }

    #[test]
    fn attention_kv_native_two_head_seq32_bounded_softmax_table_input_validates_checked_rows() {
        let input = input();
        ...
        assert_eq!(input.attention_outputs[0], vec![2, -3, 1, -4, 1, 2, 0, 1]);
        assert_eq!(
    ...
    #[test]
    fn attention_kv_native_two_head_seq32_bounded_softmax_table_uses_floor_division_for_negative_numerators(
    ) {
        assert_eq!(
    ...
    #[test]
    fn attention_kv_native_two_head_seq32_bounded_softmax_table_air_proof_round_trips() {
        let input = input();
        let envelope =
            prove_zkai_attention_kv_native_two_head_seq32_bounded_softmax_table_envelope(&input)
                .expect("two-head seq32 bounded Softmax-table attention proof");
    ...
    #[test]
    fn attention_kv_native_two_head_seq32_bounded_softmax_table_rejects_weight_relabeling() {
    ...
    #[test]
    fn attention_kv_native_two_head_seq32_bounded_softmax_table_rejects_head_relabeling() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_two_head_seq32_bounded_softmax_table_rejects_output_relabeling() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_two_head_seq32_bounded_softmax_table_rejects_quotient_remainder_drift() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_two_head_seq32_bounded_softmax_table_rejects_initial_kv_position_reorder(
    ) {
        let mut input = input();
    ...
    #[test]
    fn attention_kv_native_two_head_seq32_bounded_softmax_table_rejects_final_kv_append_reorder() {
        let mut input = input();
    ...
    #[test]
    fn attention_kv_native_two_head_seq32_bounded_softmax_table_rejects_out_of_range_score_row_positions(
    ) {
        let input = input();
        let mut row = input.score_rows[0].clone();

        row.candidate_index = FINAL_KV_ITEMS_PER_HEAD;
        let error = validate_score_row(&row, 0).unwrap_err();
        assert!(error.to_string().contains("candidate index exceeds bound"));

        row = input.score_rows[0].clone();
        row.token_position = usize::MAX;
        let error = validate_score_row(&row, 0).unwrap_err();
        assert!(error.to_string().contains("token position exceeds bound"));

        row = input.score_rows[0].clone();
        row.candidate_position = usize::MAX;
        let error = validate_score_row(&row, 0).unwrap_err();
        assert!(error
            .to_string()
            .contains("candidate position exceeds bound"));
    }

    #[test]
    fn attention_kv_native_two_head_seq32_bounded_softmax_table_trace_conversion_is_fallible() {
        let mut input = input();
        input.score_rows[0].candidate_index = usize::MAX;
        let error = attention_trace(&input).unwrap_err();
        assert!(error
            .to_string()
            .contains("candidate index exceeds bounded u32 range"));
    }

    #[test]
    fn attention_kv_native_two_head_seq32_bounded_softmax_table_rejects_commitment_drift() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_two_head_seq32_bounded_softmax_table_rejects_proof_byte_tamper() {
        let input = input();
        let mut envelope =
            prove_zkai_attention_kv_native_two_head_seq32_bounded_softmax_table_envelope(&input)
                .expect("two-head seq32 bounded Softmax-table attention proof");
    ...
    #[test]
    fn attention_kv_native_two_head_seq32_bounded_softmax_table_rejects_unknown_envelope_field() {
        let input = input();
        let envelope =
            prove_zkai_attention_kv_native_two_head_seq32_bounded_softmax_table_envelope(&input)
                .expect("two-head seq32 bounded Softmax-table attention proof");
