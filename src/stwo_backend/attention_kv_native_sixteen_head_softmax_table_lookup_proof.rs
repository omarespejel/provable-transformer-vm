...
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_SOFTMAX_TABLE_LOOKUP_PROOF_VERSION: &str =
    "stwo-attention-kv-sixteen-head-softmax-table-logup-sidecar-proof-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_SOFTMAX_TABLE_LOOKUP_STATEMENT_VERSION: &str =
    "zkai-attention-kv-stwo-native-sixteen-head-softmax-table-logup-sidecar-statement-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_SOFTMAX_TABLE_LOOKUP_SEMANTIC_SCOPE: &str =
    "sixteen_head_bounded_softmax_table_membership_constrained_by_native_stwo_logup_sidecar";
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_SOFTMAX_TABLE_LOOKUP_DECISION: &str =
    "GO_NATIVE_STWO_AIR_CONSTRAINED_SOFTMAX_TABLE_LOOKUP_RELATION_SIDECAR";
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_SOFTMAX_TABLE_LOOKUP_TARGET_ID: &str =
    "attention-kv-sixteen-head-d8-causal-mask-bounded-softmax-table-logup-sidecar-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_SOFTMAX_TABLE_LOOKUP_VERIFIER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-sixteen-head-softmax-table-logup-sidecar:v1";
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_SOFTMAX_TABLE_LOOKUP_MAX_ENVELOPE_JSON_BYTES:
    usize = 4_194_304;
pub const ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_SOFTMAX_TABLE_LOOKUP_MAX_PROOF_BYTES: usize =
    262_144;
...
const PREPROCESSED_TABLE_GAP: &str =
    "zkai/attention-kv/native-sixteen-head-softmax-table-logup/table-gap";
const PREPROCESSED_TABLE_WEIGHT: &str =
    "zkai/attention-kv/native-sixteen-head-softmax-table-logup/table-weight";
const PREPROCESSED_TABLE_MULTIPLICITY: &str =
    "zkai/attention-kv/native-sixteen-head-softmax-table-logup/table-multiplicity";

relation!(AttentionKvSixteenHeadSoftmaxTableLookupRelation, 2);

#[derive(Debug, Clone)]
struct AttentionKvSixteenHeadSoftmaxTableLookupEval {
    lookup_elements: AttentionKvSixteenHeadSoftmaxTableLookupRelation,
}

impl FrameworkEval for AttentionKvSixteenHeadSoftmaxTableLookupEval {
...
pub struct AttentionKvSixteenHeadSoftmaxTableLookupMultiplicity {
    pub gap: usize,
    pub weight: i64,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeSixteenHeadSoftmaxTableLookupSummary {
    ...
    pub table_multiplicities: Vec<AttentionKvSixteenHeadSoftmaxTableLookupMultiplicity>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeSixteenHeadSoftmaxTableLookupEnvelope {
    ...
    pub lookup_summary: ZkAiAttentionKvNativeSixteenHeadSoftmaxTableLookupSummary,
...
#[serde(deny_unknown_fields)]
struct AttentionKvSixteenHeadSoftmaxTableLookupProofPayload {
...
pub fn zkai_attention_kv_native_sixteen_head_softmax_table_lookup_source_input_from_json_str(
    raw_json: &str,
...
pub fn prove_zkai_attention_kv_native_sixteen_head_softmax_table_lookup_envelope(
    source_input: &ZkAiAttentionKvNativeSixteenHeadBoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeSixteenHeadSoftmaxTableLookupEnvelope> {
    validate_source_input(source_input)?;
    let bundle = build_lookup_bundle(source_input)?;
    let proof = prove_lookup(&bundle)?;
    let envelope = ZkAiAttentionKvNativeSixteenHeadSoftmaxTableLookupEnvelope {
        proof_backend: StarkProofBackend::Stwo,
        proof_backend_version:
            ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_SOFTMAX_TABLE_LOOKUP_PROOF_VERSION.to_string(),
        statement_version:
            ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_SOFTMAX_TABLE_LOOKUP_STATEMENT_VERSION.to_string(),
        semantic_scope: ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_SOFTMAX_TABLE_LOOKUP_SEMANTIC_SCOPE
            .to_string(),
        decision: ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_SOFTMAX_TABLE_LOOKUP_DECISION.to_string(),
        verifier_domain: ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_SOFTMAX_TABLE_LOOKUP_VERIFIER_DOMAIN
            .to_string(),
...
pub fn zkai_attention_kv_native_sixteen_head_softmax_table_lookup_envelope_from_json_slice(
    raw_json: &[u8],
) -> Result<ZkAiAttentionKvNativeSixteenHeadSoftmaxTableLookupEnvelope> {
    if raw_json.len()
...
pub fn verify_zkai_attention_kv_native_sixteen_head_softmax_table_lookup_envelope(
    envelope: &ZkAiAttentionKvNativeSixteenHeadSoftmaxTableLookupEnvelope,
) -> Result<bool> {
    validate_envelope(envelope)?;
...
fn lookup_summary(
    input: &ZkAiAttentionKvNativeSixteenHeadBoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeSixteenHeadSoftmaxTableLookupSummary> {
    let mut multiplicities = input
        .weight_table
        .iter()
        .map(
            |entry| AttentionKvSixteenHeadSoftmaxTableLookupMultiplicity {
                gap: entry.gap,
                weight: entry.weight,
                multiplicity: 0,
            },
    ...
    Ok(ZkAiAttentionKvNativeSixteenHeadSoftmaxTableLookupSummary {
        ...
        lookup_relation: "AttentionKvSixteenHeadSoftmaxTableLookupRelation".to_string(),
...
fn prove_lookup(bundle: &LookupBundle) -> Result<Vec<u8>> {
    let component = lookup_component(AttentionKvSixteenHeadSoftmaxTableLookupRelation::dummy());
    ...
    let lookup_elements = AttentionKvSixteenHeadSoftmaxTableLookupRelation::draw(channel);
    ...
    serde_json::to_vec(&AttentionKvSixteenHeadSoftmaxTableLookupProofPayload { stark_proof })
        .map_err(|error| VmError::Serialization(error.to_string()))
}
...
fn verify_lookup(
    ...
    validate_source_input(source_input)?;
    if proof.is_empty()
        || proof.len() > ZKAI_ATTENTION_KV_NATIVE_SIXTEEN_HEAD_SOFTMAX_TABLE_LOOKUP_MAX_PROOF_BYTES
    {
        return Err(lookup_error("lookup proof byte length outside bounded cap"));
    }
    let payload: AttentionKvSixteenHeadSoftmaxTableLookupProofPayload =
    ...
    let component_placeholder =
        lookup_component(AttentionKvSixteenHeadSoftmaxTableLookupRelation::dummy());
...
fn lookup_component(
    lookup_elements: AttentionKvSixteenHeadSoftmaxTableLookupRelation,
) -> FrameworkComponent<AttentionKvSixteenHeadSoftmaxTableLookupEval> {
    FrameworkComponent::new(
        ...
        AttentionKvSixteenHeadSoftmaxTableLookupEval { lookup_elements },
        SecureField::zero(),
    )
}
...
mod tests {
    ...
    fn source_input() -> ZkAiAttentionKvNativeSixteenHeadBoundedSoftmaxTableProofInput {
        ...
        zkai_attention_kv_native_sixteen_head_softmax_table_lookup_source_input_from_json_str(raw)
            .expect("source input")
    }

    #[test]
    fn attention_kv_sixteen_head_softmax_table_lookup_sidecar_round_trips_real_proof() {
        ...
        assert_eq!(envelope.lookup_summary.source_head_count, 16);
        assert_eq!(envelope.lookup_summary.score_rows, 832);
        assert_eq!(envelope.lookup_summary.lookup_claims, 832);
    ...
    #[test]
    fn attention_kv_sixteen_head_softmax_table_lookup_summary_counts_claims() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_sixteen_head_softmax_table_lookup_rejects_empty_table_without_panic() {
    ...
    #[test]
    fn attention_kv_sixteen_head_softmax_table_lookup_rejects_preprocessed_overflow_without_panic()
    {
        let input = source_input();
        let mut summary = lookup_summary(&input).expect("summary");
    ...
    #[test]
    fn attention_kv_sixteen_head_softmax_table_lookup_masks_inactive_denominators() {
        let guarded =
    ...
    #[test]
    fn attention_kv_sixteen_head_softmax_table_lookup_preserves_one_sided_contributions() {
    ...
    #[test]
    fn attention_kv_sixteen_head_softmax_table_lookup_rejects_summary_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_sixteen_head_softmax_table_lookup_envelope(&input)
                .expect("prove lookup sidecar");
    ...
    #[test]
    fn attention_kv_sixteen_head_softmax_table_lookup_rejects_source_weight_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_sixteen_head_softmax_table_lookup_envelope(&input)
                .expect("prove lookup sidecar");
    ...
    #[test]
    fn attention_kv_sixteen_head_softmax_table_lookup_rejects_proof_byte_tamper() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_sixteen_head_softmax_table_lookup_envelope(&input)
                .expect("prove lookup sidecar");
