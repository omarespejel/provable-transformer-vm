...
pub const ZKAI_ATTENTION_KV_NATIVE_D8_FUSED_SOFTMAX_TABLE_SEMANTIC_SCOPE: &str =
    "d8_bounded_softmax_table_attention_arithmetic_and_logup_membership_fused_in_one_native_stwo_proof";
pub const ZKAI_ATTENTION_KV_NATIVE_D8_FUSED_SOFTMAX_TABLE_DECISION: &str =
    "GO_NATIVE_STWO_FUSED_ATTENTION_ARITHMETIC_AND_SOFTMAX_TABLE_LOGUP_MEMBERSHIP";
pub const ZKAI_ATTENTION_KV_NATIVE_D8_FUSED_SOFTMAX_TABLE_TARGET_ID: &str =
    "attention-kv-d8-causal-mask-fused-bounded-softmax-table-logup-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D8_FUSED_SOFTMAX_TABLE_VERIFIER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d8-fused-bounded-softmax-table-logup:v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D8_FUSED_SOFTMAX_TABLE_MAX_ENVELOPE_JSON_BYTES: usize =
    1_048_576;
pub const ZKAI_ATTENTION_KV_NATIVE_D8_FUSED_SOFTMAX_TABLE_MAX_PROOF_BYTES: usize = 131_072;

const ISSUE: usize = 478;
const SOURCE_ISSUE: usize = 463;
const SOURCE_PLUS_SIDECAR_RAW_PROOF_BYTES: usize = 44_692 + 14_745;
...
const PREPROCESSED_TABLE_GAP: &str =
    "zkai/attention-kv/native-d8-fused-softmax-table-logup/table-gap";
...
impl FrameworkEval for AttentionKvNativeD8FusedSoftmaxTableEval {
...
pub struct AttentionKvD8FusedSoftmaxTableMultiplicity {
    pub gap: usize,
    pub weight: i64,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeD8FusedSoftmaxTableSummary {
    pub issue: usize,
    ...
    pub table_multiplicities: Vec<AttentionKvD8FusedSoftmaxTableMultiplicity>,
...
pub struct ZkAiAttentionKvNativeD8FusedSoftmaxTableEnvelope {
    ...
    pub fused_summary: ZkAiAttentionKvNativeD8FusedSoftmaxTableSummary,
...
#[serde(deny_unknown_fields)]
struct AttentionKvNativeD8FusedSoftmaxTableProofPayload {
...
pub fn prove_zkai_attention_kv_native_d8_fused_softmax_table_envelope(
    source_input: &ZkAiAttentionKvNativeD8BoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeD8FusedSoftmaxTableEnvelope> {
    ...
    let envelope = ZkAiAttentionKvNativeD8FusedSoftmaxTableEnvelope {
        ...
        statement_version: ZKAI_ATTENTION_KV_NATIVE_D8_FUSED_SOFTMAX_TABLE_STATEMENT_VERSION
            .to_string(),
        semantic_scope: ZKAI_ATTENTION_KV_NATIVE_D8_FUSED_SOFTMAX_TABLE_SEMANTIC_SCOPE.to_string(),
        decision: ZKAI_ATTENTION_KV_NATIVE_D8_FUSED_SOFTMAX_TABLE_DECISION.to_string(),
        target_id: ZKAI_ATTENTION_KV_NATIVE_D8_FUSED_SOFTMAX_TABLE_TARGET_ID.to_string(),
        verifier_domain: ZKAI_ATTENTION_KV_NATIVE_D8_FUSED_SOFTMAX_TABLE_VERIFIER_DOMAIN
            .to_string(),
...
pub fn zkai_attention_kv_native_d8_fused_softmax_table_envelope_from_json_slice(
    raw_json: &[u8],
) -> Result<ZkAiAttentionKvNativeD8FusedSoftmaxTableEnvelope> {
    if raw_json.len() > ZKAI_ATTENTION_KV_NATIVE_D8_FUSED_SOFTMAX_TABLE_MAX_ENVELOPE_JSON_BYTES {
        return Err(fused_error(format!(
...
pub fn verify_zkai_attention_kv_native_d8_fused_softmax_table_envelope(
    envelope: &ZkAiAttentionKvNativeD8FusedSoftmaxTableEnvelope,
) -> Result<bool> {
    validate_envelope(envelope)?;
    verify_fused(&envelope.source_input, &envelope.proof)
}

fn validate_envelope(envelope: &ZkAiAttentionKvNativeD8FusedSoftmaxTableEnvelope) -> Result<()> {
    ...
    if envelope.proof.is_empty()
        || envelope.proof.len() > ZKAI_ATTENTION_KV_NATIVE_D8_FUSED_SOFTMAX_TABLE_MAX_PROOF_BYTES
    {
...
fn fused_summary(
    ...
    let mut multiplicities = input
        .weight_table
        .iter()
        .map(|entry| AttentionKvD8FusedSoftmaxTableMultiplicity {
            gap: entry.gap,
            weight: entry.weight,
            multiplicity: 0,
        })
    ...
    Ok(ZkAiAttentionKvNativeD8FusedSoftmaxTableSummary {
        issue: ISSUE,
        ...
        lookup_relation: "AttentionKvD8FusedSoftmaxTableRelation".to_string(),
...
fn prove_fused(bundle: &FusedBundle) -> Result<Vec<u8>> {
    let component = fused_component(AttentionKvD8FusedSoftmaxTableRelation::dummy());
    ...
    serde_json::to_vec(&AttentionKvNativeD8FusedSoftmaxTableProofPayload { stark_proof })
        .map_err(|error| VmError::Serialization(error.to_string()))
}
...
fn verify_fused(
    ...
    validate_source_input(source_input)?;
    if proof.is_empty()
        || proof.len() > ZKAI_ATTENTION_KV_NATIVE_D8_FUSED_SOFTMAX_TABLE_MAX_PROOF_BYTES
    {
        return Err(fused_error("fused proof byte length outside bounded cap"));
    }
    let payload: AttentionKvNativeD8FusedSoftmaxTableProofPayload =
    ...
    let component_placeholder = fused_component(AttentionKvD8FusedSoftmaxTableRelation::dummy());
...
fn fused_error(message: impl Into<String>) -> VmError {
    VmError::UnsupportedProof(format!(
        "attention/KV native d8 fused bounded Softmax-table proof: {}",
        message.into()
    ))
}
...
mod tests {
    ...
    fn source_input() -> ZkAiAttentionKvNativeD8BoundedSoftmaxTableProofInput {
        ...
        zkai_attention_kv_native_d8_fused_softmax_table_source_input_from_json_str(raw)
            .expect("source input")
    }

    #[test]
    fn attention_kv_d8_fused_softmax_table_round_trips_real_proof() {
        let input = source_input();
        let envelope = prove_zkai_attention_kv_native_d8_fused_softmax_table_envelope(&input)
        ...
        assert!(
            verify_zkai_attention_kv_native_d8_fused_softmax_table_envelope(&envelope)
                .expect("verify fused attention/lookup")
        );
    }

    #[test]
    fn attention_kv_d8_fused_softmax_table_summary_counts_claims() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_d8_fused_softmax_table_rejects_summary_drift() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_d8_fused_softmax_table_rejects_source_weight_drift() {
        let input = source_input();
        let mut envelope = prove_zkai_attention_kv_native_d8_fused_softmax_table_envelope(&input)
            .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d8_fused_softmax_table_rejects_negative_bit_material() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_d8_fused_softmax_table_rejects_output_remainder_drift() {
        let input = source_input();
        let mut envelope = prove_zkai_attention_kv_native_d8_fused_softmax_table_envelope(&input)
            .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d8_fused_softmax_table_rejects_proof_byte_tamper() {
        let input = source_input();
        let mut envelope = prove_zkai_attention_kv_native_d8_fused_softmax_table_envelope(&input)
            .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d8_fused_softmax_table_rejects_unknown_envelope_field() {
        let input = source_input();
        let envelope = prove_zkai_attention_kv_native_d8_fused_softmax_table_envelope(&input)
            .expect("prove fused attention/lookup");
        ...
        let error = zkai_attention_kv_native_d8_fused_softmax_table_envelope_from_json_slice(&raw)
