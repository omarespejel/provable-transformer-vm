...
pub const ZKAI_ATTENTION_KV_NATIVE_D8_BOUNDED_SOFTMAX_TABLE_INPUT_SCHEMA: &str =
    "zkai-attention-kv-stwo-native-d8-bounded-softmax-table-air-proof-input-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D8_BOUNDED_SOFTMAX_TABLE_INPUT_DECISION: &str =
    "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_D8_BOUNDED_SOFTMAX_TABLE_AIR_PROOF";
pub const ZKAI_ATTENTION_KV_NATIVE_D8_BOUNDED_SOFTMAX_TABLE_PROOF_VERSION: &str =
    "stwo-attention-kv-d8-causal-mask-bounded-softmax-table-air-proof-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D8_BOUNDED_SOFTMAX_TABLE_STATEMENT_VERSION: &str =
    "zkai-attention-kv-stwo-native-d8-bounded-softmax-table-statement-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D8_BOUNDED_SOFTMAX_TABLE_SEMANTIC_SCOPE: &str =
    "d8_bounded_table_softmax_approx_attention_kv_causal_mask_rows_bound_to_statement_receipt";
pub const ZKAI_ATTENTION_KV_NATIVE_D8_BOUNDED_SOFTMAX_TABLE_DECISION: &str =
    "GO_STWO_NATIVE_ATTENTION_KV_BOUNDED_SOFTMAX_TABLE_AIR_PROOF";
pub const ZKAI_ATTENTION_KV_NATIVE_D8_BOUNDED_SOFTMAX_TABLE_TARGET_ID: &str =
    "attention-kv-d8-causal-mask-bounded-softmax-table-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D8_BOUNDED_SOFTMAX_TABLE_REQUIRED_BACKEND_VERSION: &str =
    "stwo-attention-kv-d8-causal-mask-bounded-softmax-table-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D8_BOUNDED_SOFTMAX_TABLE_VERIFIER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d8-bounded-softmax-table:v1";

const ISSUE: usize = 463;
...
const INITIAL_KV_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d8-bounded-softmax-table-initial-kv:v1";
const INPUT_STEPS_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d8-bounded-softmax-table-input-steps:v1";
const FINAL_KV_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d8-bounded-softmax-table-final-kv:v1";
const OUTPUTS_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d8-bounded-softmax-table-outputs:v1";
const PUBLIC_INSTANCE_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d8-bounded-softmax-table-public-instance:v1";
const PROOF_NATIVE_PARAMETER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d8-bounded-softmax-table-proof-parameters:v1";
const WEIGHT_TABLE_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d8-bounded-softmax-table-weight-table:v1";
...
const NEXT_BACKEND_STEP: &str = "either add AIR-private lookup/table columns for the same policy or combine this bounded Softmax-table policy with two-head state binding after preserving table, quotient/remainder, carry, mask, and relabeling rejection surfaces";
...
#[derive(Debug, Clone)]
struct AttentionKvNativeD8BoundedSoftmaxTableEval;

impl FrameworkEval for AttentionKvNativeD8BoundedSoftmaxTableEval {
...
pub struct AttentionKvD8BoundedSoftmaxTableEntry {
    pub position: usize,
    pub key: Vec<i64>,
    pub value: Vec<i64>,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AttentionKvD8BoundedSoftmaxTableInputStep {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AttentionKvD8BoundedSoftmaxTableWeightEntry {
    pub gap: usize,
    pub weight: i64,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AttentionKvD8BoundedSoftmaxTableScoreRow {
    pub row_index: usize,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeD8BoundedSoftmaxTableProofInput {
    pub schema: String,
    pub decision: String,
    pub issue: usize,
    ...
    pub weight_table: Vec<AttentionKvD8BoundedSoftmaxTableWeightEntry>,
    ...
    pub initial_kv_cache: Vec<AttentionKvD8BoundedSoftmaxTableEntry>,
    pub input_steps: Vec<AttentionKvD8BoundedSoftmaxTableInputStep>,
    pub final_kv_cache: Vec<AttentionKvD8BoundedSoftmaxTableEntry>,
    pub attention_outputs: Vec<Vec<i64>>,
    pub score_rows: Vec<AttentionKvD8BoundedSoftmaxTableScoreRow>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeD8BoundedSoftmaxTableEnvelope {
    pub proof_backend: StarkProofBackend,
    pub proof_backend_version: String,
    pub statement_version: String,
    pub semantic_scope: String,
    pub decision: String,
    pub input: ZkAiAttentionKvNativeD8BoundedSoftmaxTableProofInput,
    pub proof: Vec<u8>,
}

#[derive(Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
struct AttentionKvNativeD8BoundedSoftmaxTableProofPayload {
    stark_proof: StarkProof<Blake2sM31MerkleHasher>,
}

pub fn zkai_attention_kv_native_d8_bounded_softmax_table_input_from_json_str(
    raw_json: &str,
) -> Result<ZkAiAttentionKvNativeD8BoundedSoftmaxTableProofInput> {
    if raw_json.len() > ZKAI_ATTENTION_KV_NATIVE_D8_BOUNDED_SOFTMAX_TABLE_MAX_INPUT_JSON_BYTES {
    ...
    let input: ZkAiAttentionKvNativeD8BoundedSoftmaxTableProofInput =
...
pub fn prove_zkai_attention_kv_native_d8_bounded_softmax_table_envelope(
    input: &ZkAiAttentionKvNativeD8BoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeD8BoundedSoftmaxTableEnvelope> {
    validate_input(input)?;
    Ok(ZkAiAttentionKvNativeD8BoundedSoftmaxTableEnvelope {
        proof_backend: StarkProofBackend::Stwo,
        proof_backend_version:
            ZKAI_ATTENTION_KV_NATIVE_D8_BOUNDED_SOFTMAX_TABLE_REQUIRED_BACKEND_VERSION.to_string(),
...
pub fn zkai_attention_kv_native_d8_bounded_softmax_table_envelope_from_json_slice(
    raw_json: &[u8],
) -> Result<ZkAiAttentionKvNativeD8BoundedSoftmaxTableEnvelope> {
    if raw_json.len() > ZKAI_ATTENTION_KV_NATIVE_D8_BOUNDED_SOFTMAX_TABLE_MAX_ENVELOPE_JSON_BYTES {
        return Err(weighted_error(format!(
...
pub fn verify_zkai_attention_kv_native_d8_bounded_softmax_table_envelope(
    envelope: &ZkAiAttentionKvNativeD8BoundedSoftmaxTableEnvelope,
) -> Result<bool> {
    validate_envelope(envelope)?;
    verify_rows(&envelope.input, &envelope.proof)
}

fn validate_envelope(envelope: &ZkAiAttentionKvNativeD8BoundedSoftmaxTableEnvelope) -> Result<()> {
    ...
    if envelope.proof.len() > ZKAI_ATTENTION_KV_NATIVE_D8_BOUNDED_SOFTMAX_TABLE_MAX_PROOF_BYTES {
        return Err(weighted_error(format!(
...
fn validate_input(input: &ZkAiAttentionKvNativeD8BoundedSoftmaxTableProofInput) -> Result<()> {
    expect_eq(
        &input.schema,
...
fn validate_sequence(input: &ZkAiAttentionKvNativeD8BoundedSoftmaxTableProofInput) -> Result<()> {
    ...
    for (step_index, step) in input.input_steps.iter().enumerate() {
        ...
        let next_item = AttentionKvD8BoundedSoftmaxTableEntry {
        ...
        let scored: Vec<(AttentionKvD8BoundedSoftmaxTableEntry, i64)> = next_cache
            .iter()
            ...
            scored.iter().zip(weights.iter()).enumerate()
        {
            ...
            expected_rows.push(AttentionKvD8BoundedSoftmaxTableScoreRow {
...
fn validate_kv_entry(entry: &AttentionKvD8BoundedSoftmaxTableEntry) -> Result<()> {
...
fn validate_input_step(
    step: &AttentionKvD8BoundedSoftmaxTableInputStep,
    step_index: usize,
) -> Result<()> {
    expect_usize(
        step.token_position,
...
fn validate_score_row(
    row: &AttentionKvD8BoundedSoftmaxTableScoreRow,
...
fn prove_rows(input: &ZkAiAttentionKvNativeD8BoundedSoftmaxTableProofInput) -> Result<Vec<u8>> {
    validate_input(input)?;
        ...
        prove::<SimdBackend, Blake2sM31MerkleChannel>(&[&component], channel, commitment_scheme)
            .map_err(|error| {
                VmError::UnsupportedProof(format!(
                    "attention/KV native d8 bounded Softmax-table AIR proving failed: {error}"
                ))
            })?;
    serde_json::to_vec(&AttentionKvNativeD8BoundedSoftmaxTableProofPayload { stark_proof })
        .map_err(|error| VmError::Serialization(error.to_string()))
}
...
fn verify_rows(
    ...
    validate_input(input)?;
    let payload: AttentionKvNativeD8BoundedSoftmaxTableProofPayload =
    ...
    verify(&[&component], channel, commitment_scheme, stark_proof)
        ...
        .map_err(|error| {
            weighted_error(format!(
                "attention/KV native d8 bounded Softmax-table proof rejected: {error}"
            ))
        })
}
...
fn attention_component() -> FrameworkComponent<AttentionKvNativeD8BoundedSoftmaxTableEval> {
    FrameworkComponent::new(
        &mut TraceLocationAllocator::new_with_preprocessed_columns(&preprocessed_column_ids()),
        AttentionKvNativeD8BoundedSoftmaxTableEval,
        SecureField::zero(),
    )
}
...
fn padding_row(row_index: usize) -> AttentionKvD8BoundedSoftmaxTableScoreRow {
    AttentionKvD8BoundedSoftmaxTableScoreRow {
        row_index,
        step_index: 0,
        candidate_index: 0,
        token_position: 0,
...
fn column_ids() -> Vec<String> {
    ...
    .into_iter()
    .map(|suffix| format!("zkai/attention-kv/native-d8-bounded-softmax-table/{suffix}"))
    .collect::<Vec<_>>();
    for prefix in [
        "query",
        "key",
        ...
        for index in 0..width {
            ids.push(format!(
                "zkai/attention-kv/native-d8-bounded-softmax-table/{prefix}-{index:02}"
            ));
        }
    }
    for index in 0..SCORE_GAP_BITS {
        ids.push(format!(
            "zkai/attention-kv/native-d8-bounded-softmax-table/score-gap-bit-{index:02}"
        ));
    }
    for index in 0..CAUSAL_GAP_BITS {
        ids.push(format!(
            "zkai/attention-kv/native-d8-bounded-softmax-table/causal-gap-bit-{index:02}"
        ));
    }
    for index in 0..WEIGHT_BITS {
        ids.push(format!(
            "zkai/attention-kv/native-d8-bounded-softmax-table/weight-bit-{index:02}"
        ));
    }
    for dim in 0..VALUE_WIDTH {
        for index in 0..OUTPUT_REMAINDER_BITS {
            ids.push(format!(
                "zkai/attention-kv/native-d8-bounded-softmax-table/output-remainder-{dim:02}-bit-{index:02}"
            ));
        }
    }
    ids
...
fn expected_weight_table_entries() -> Vec<AttentionKvD8BoundedSoftmaxTableWeightEntry> {
    WEIGHT_TABLE
        .iter()
        .map(
            |(gap, weight)| AttentionKvD8BoundedSoftmaxTableWeightEntry {
                gap: *gap,
                weight: *weight,
            },
        )
...
fn validate_weight_table(table: &[AttentionKvD8BoundedSoftmaxTableWeightEntry]) -> Result<()> {
...
fn kv_commitment(cache: &[AttentionKvD8BoundedSoftmaxTableEntry], domain: &str) -> Result<String> {
    let material = cache
        .iter()
        .map(|entry| {
...
fn input_steps_commitment(steps: &[AttentionKvD8BoundedSoftmaxTableInputStep]) -> Result<String> {
    let material = steps
        .iter()
        .map(|step| {
...
fn rows_commitment(rows: &[AttentionKvD8BoundedSoftmaxTableScoreRow]) -> Result<String> {
...
fn score_row_material(row: &AttentionKvD8BoundedSoftmaxTableScoreRow) -> Vec<i64> {
    let mut out = vec![
...
fn weighted_error(message: impl Into<String>) -> VmError {
    VmError::InvalidConfig(format!(
        "attention/KV native d8 bounded Softmax-table proof: {}",
        message.into()
    ))
}
...
mod tests {
    ...
    fn input() -> ZkAiAttentionKvNativeD8BoundedSoftmaxTableProofInput {
        zkai_attention_kv_native_d8_bounded_softmax_table_input_from_json_str(INPUT_JSON)
            .expect("d8 bounded Softmax-table attention input")
    }

    #[test]
    fn attention_kv_native_d8_bounded_softmax_table_input_validates_checked_rows() {
        let input = input();
        ...
        assert_eq!(input.attention_outputs[0], vec![1, 1, 0, -1, 1, -1, 2, 1]);
        assert_eq!(input.attention_outputs[7], vec![-3, 3, 0, -2, 3, 1, -1, 0]);
        assert_eq!(input.score_rows[0].attention_weight, 256);
        assert_eq!(input.score_rows[2].attention_weight, 45);
        assert_eq!(input.score_gap_clip, SCORE_GAP_CLIP);
    }

    #[test]
    fn attention_kv_native_d8_bounded_softmax_table_uses_floor_division_for_negative_numerators() {
        assert_eq!(
    ...
    #[test]
    fn attention_kv_native_d8_bounded_softmax_table_air_proof_round_trips() {
        let input = input();
        let envelope = prove_zkai_attention_kv_native_d8_bounded_softmax_table_envelope(&input)
    ...
    #[test]
    fn attention_kv_native_d8_bounded_softmax_table_rejects_weight_relabeling() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
        value["score_rows"][0]["attention_weight"] = Value::from(15);
        let error = zkai_attention_kv_native_d8_bounded_softmax_table_input_from_json_str(
    ...
    #[test]
    fn attention_kv_native_d8_bounded_softmax_table_rejects_table_relabeling() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_d8_bounded_softmax_table_rejects_output_relabeling() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_d8_bounded_softmax_table_rejects_quotient_remainder_drift() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_d8_bounded_softmax_table_rejects_commitment_drift() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_d8_bounded_softmax_table_rejects_proof_byte_tamper() {
        let input = input();
    ...
    #[test]
    fn attention_kv_native_d8_bounded_softmax_table_rejects_unknown_envelope_field() {
        let input = input();
        let envelope = prove_zkai_attention_kv_native_d8_bounded_softmax_table_envelope(&input)
            .expect("d8 bounded Softmax-table attention proof");
