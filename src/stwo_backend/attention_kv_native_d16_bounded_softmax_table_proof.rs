...
pub const ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_INPUT_SCHEMA: &str =
    "zkai-attention-kv-stwo-native-d16-bounded-softmax-table-air-proof-input-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_INPUT_DECISION: &str =
    "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_D16_BOUNDED_SOFTMAX_TABLE_AIR_PROOF";
pub const ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_PROOF_VERSION: &str =
    "stwo-attention-kv-d16-causal-mask-bounded-softmax-table-air-proof-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_STATEMENT_VERSION: &str =
    "zkai-attention-kv-stwo-native-d16-bounded-softmax-table-statement-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_SEMANTIC_SCOPE: &str =
    "d16_bounded_table_softmax_approx_attention_kv_causal_mask_rows_bound_to_statement_receipt";
pub const ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_DECISION: &str =
    "GO_STWO_NATIVE_ATTENTION_KV_BOUNDED_SOFTMAX_TABLE_AIR_PROOF";
pub const ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_TARGET_ID: &str =
    "attention-kv-d16-causal-mask-bounded-softmax-table-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_REQUIRED_BACKEND_VERSION: &str =
    "stwo-attention-kv-d16-causal-mask-bounded-softmax-table-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_VERIFIER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d16-bounded-softmax-table:v1";

const ISSUE: usize = 501;
const SOURCE_ISSUE: usize = 453;
...
pub const ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_MAX_INPUT_JSON_BYTES: usize =
    1_048_576;
pub const ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_MAX_ENVELOPE_JSON_BYTES: usize =
    2_097_152;
...
pub const ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_MAX_PROOF_BYTES: usize = 131_072;

const ROW_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d16-bounded-softmax-table-score-rows:v1";
const INITIAL_KV_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d16-bounded-softmax-table-initial-kv:v1";
const INPUT_STEPS_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d16-bounded-softmax-table-input-steps:v1";
const FINAL_KV_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d16-bounded-softmax-table-final-kv:v1";
const OUTPUTS_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d16-bounded-softmax-table-outputs:v1";
const PUBLIC_INSTANCE_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d16-bounded-softmax-table-public-instance:v1";
const PROOF_NATIVE_PARAMETER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d16-bounded-softmax-table-proof-parameters:v1";
const WEIGHT_TABLE_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d16-bounded-softmax-table-weight-table:v1";
...
const NEXT_BACKEND_STEP: &str = "fuse this d16 bounded Softmax-table source with AIR-private LogUp table membership, then compare against a matched d16 source-plus-sidecar route";

const EXPECTED_VALIDATION_COMMANDS: &[&str] = &[
    "python3 scripts/zkai_attention_kv_stwo_native_d16_bounded_softmax_table_proof_input.py --write-json docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-bounded-softmax-table-proof-2026-05.json --write-tsv docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-bounded-softmax-table-proof-2026-05.tsv",
    "python3 -m unittest scripts.tests.test_zkai_attention_kv_stwo_native_d16_bounded_softmax_table_proof_input",
    "cargo +nightly-2025-07-14 test attention_kv_native_d16_bounded_softmax_table_proof --lib --features stwo-backend",
    "cargo +nightly-2025-07-14 run --features stwo-backend --bin zkai_attention_kv_native_d16_bounded_softmax_table_proof -- prove docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-bounded-softmax-table-proof-2026-05.json docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-bounded-softmax-table-proof-2026-05.envelope.json",
    "cargo +nightly-2025-07-14 run --features stwo-backend --bin zkai_attention_kv_native_d16_bounded_softmax_table_proof -- verify docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-bounded-softmax-table-proof-2026-05.envelope.json",
    "python3 scripts/zkai_attention_kv_d16_bounded_softmax_table_native_gate.py --write-json docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-bounded-softmax-table-gate-2026-05.json --write-tsv docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-bounded-softmax-table-gate-2026-05.tsv",
    "python3 -m unittest scripts.tests.test_zkai_attention_kv_d16_bounded_softmax_table_native_gate",
    "just lib",
    "just gate-fast",
    "just gate",
];
...
#[derive(Debug, Clone)]
struct AttentionKvNativeD16BoundedSoftmaxTableEval;

impl FrameworkEval for AttentionKvNativeD16BoundedSoftmaxTableEval {
...
pub struct AttentionKvD16BoundedSoftmaxTableEntry {
    pub position: usize,
    pub key: Vec<i64>,
    pub value: Vec<i64>,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AttentionKvD16BoundedSoftmaxTableInputStep {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AttentionKvD16BoundedSoftmaxTableWeightEntry {
    pub gap: usize,
    pub weight: i64,
}
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct AttentionKvD16BoundedSoftmaxTableScoreRow {
    pub row_index: usize,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeD16BoundedSoftmaxTableProofInput {
    pub schema: String,
    pub decision: String,
    pub issue: usize,
    ...
    pub weight_table: Vec<AttentionKvD16BoundedSoftmaxTableWeightEntry>,
    ...
    pub initial_kv_cache: Vec<AttentionKvD16BoundedSoftmaxTableEntry>,
    pub input_steps: Vec<AttentionKvD16BoundedSoftmaxTableInputStep>,
    pub final_kv_cache: Vec<AttentionKvD16BoundedSoftmaxTableEntry>,
    pub attention_outputs: Vec<Vec<i64>>,
    pub score_rows: Vec<AttentionKvD16BoundedSoftmaxTableScoreRow>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeD16BoundedSoftmaxTableEnvelope {
    pub proof_backend: StarkProofBackend,
    pub proof_backend_version: String,
    pub statement_version: String,
    pub semantic_scope: String,
    pub decision: String,
    pub input: ZkAiAttentionKvNativeD16BoundedSoftmaxTableProofInput,
    pub proof: Vec<u8>,
}

#[derive(Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
struct AttentionKvNativeD16BoundedSoftmaxTableProofPayload {
    stark_proof: StarkProof<Blake2sM31MerkleHasher>,
}

pub fn zkai_attention_kv_native_d16_bounded_softmax_table_input_from_json_str(
    raw_json: &str,
) -> Result<ZkAiAttentionKvNativeD16BoundedSoftmaxTableProofInput> {
    if raw_json.len() > ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_MAX_INPUT_JSON_BYTES {
    ...
    let input: ZkAiAttentionKvNativeD16BoundedSoftmaxTableProofInput =
...
pub fn prove_zkai_attention_kv_native_d16_bounded_softmax_table_envelope(
    input: &ZkAiAttentionKvNativeD16BoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeD16BoundedSoftmaxTableEnvelope> {
    validate_input(input)?;
    Ok(ZkAiAttentionKvNativeD16BoundedSoftmaxTableEnvelope {
        proof_backend: StarkProofBackend::Stwo,
        proof_backend_version:
            ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_REQUIRED_BACKEND_VERSION.to_string(),
        statement_version: ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_STATEMENT_VERSION
            .to_string(),
        semantic_scope: ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_SEMANTIC_SCOPE
            .to_string(),
        decision: ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_DECISION.to_string(),
        input: input.clone(),
        proof: prove_rows(input)?,
    })
}

pub fn zkai_attention_kv_native_d16_bounded_softmax_table_envelope_from_json_slice(
    raw_json: &[u8],
) -> Result<ZkAiAttentionKvNativeD16BoundedSoftmaxTableEnvelope> {
    if raw_json.len() > ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_MAX_ENVELOPE_JSON_BYTES {
        return Err(weighted_error(format!(
    ...
    let envelope: ZkAiAttentionKvNativeD16BoundedSoftmaxTableEnvelope =
...
pub fn verify_zkai_attention_kv_native_d16_bounded_softmax_table_envelope(
    envelope: &ZkAiAttentionKvNativeD16BoundedSoftmaxTableEnvelope,
) -> Result<bool> {
    validate_envelope(envelope)?;
    verify_rows(&envelope.input, &envelope.proof)
}

fn validate_envelope(envelope: &ZkAiAttentionKvNativeD16BoundedSoftmaxTableEnvelope) -> Result<()> {
    ...
    if envelope.proof.len() > ZKAI_ATTENTION_KV_NATIVE_D16_BOUNDED_SOFTMAX_TABLE_MAX_PROOF_BYTES {
        return Err(weighted_error(format!(
...
fn validate_input(input: &ZkAiAttentionKvNativeD16BoundedSoftmaxTableProofInput) -> Result<()> {
    expect_eq(
        &input.schema,
...
pub(crate) fn validate_zkai_attention_kv_native_d16_bounded_softmax_table_input(
    input: &ZkAiAttentionKvNativeD16BoundedSoftmaxTableProofInput,
) -> Result<()> {
    validate_input(input)
}

fn validate_sequence(input: &ZkAiAttentionKvNativeD16BoundedSoftmaxTableProofInput) -> Result<()> {
    ...
    for (step_index, step) in input.input_steps.iter().enumerate() {
        ...
        let next_item = AttentionKvD16BoundedSoftmaxTableEntry {
        ...
        let scored: Vec<(AttentionKvD16BoundedSoftmaxTableEntry, i64)> = next_cache
            .iter()
            ...
            scored.iter().zip(weights.iter()).enumerate()
        {
            ...
            expected_rows.push(AttentionKvD16BoundedSoftmaxTableScoreRow {
...
fn validate_kv_entry(entry: &AttentionKvD16BoundedSoftmaxTableEntry) -> Result<()> {
...
fn validate_input_step(
    step: &AttentionKvD16BoundedSoftmaxTableInputStep,
    step_index: usize,
) -> Result<()> {
    expect_usize(
        step.token_position,
...
fn validate_score_row(
    row: &AttentionKvD16BoundedSoftmaxTableScoreRow,
...
fn prove_rows(input: &ZkAiAttentionKvNativeD16BoundedSoftmaxTableProofInput) -> Result<Vec<u8>> {
    validate_input(input)?;
        ...
        prove::<SimdBackend, Blake2sM31MerkleChannel>(&[&component], channel, commitment_scheme)
            .map_err(|error| {
                VmError::UnsupportedProof(format!(
                    "attention/KV native d16 bounded Softmax-table AIR proving failed: {error}"
                ))
            })?;
    serde_json::to_vec(&AttentionKvNativeD16BoundedSoftmaxTableProofPayload { stark_proof })
        .map_err(|error| VmError::Serialization(error.to_string()))
}
...
fn verify_rows(
    ...
    validate_input(input)?;
    let payload: AttentionKvNativeD16BoundedSoftmaxTableProofPayload =
    ...
    verify(&[&component], channel, commitment_scheme, stark_proof)
        ...
        .map_err(|error| {
            weighted_error(format!(
                "attention/KV native d16 bounded Softmax-table proof rejected: {error}"
            ))
        })
}
...
fn attention_component() -> FrameworkComponent<AttentionKvNativeD16BoundedSoftmaxTableEval> {
    FrameworkComponent::new(
        &mut TraceLocationAllocator::new_with_preprocessed_columns(&preprocessed_column_ids()),
        AttentionKvNativeD16BoundedSoftmaxTableEval,
        SecureField::zero(),
    )
}
...
fn padding_row(row_index: usize) -> AttentionKvD16BoundedSoftmaxTableScoreRow {
    AttentionKvD16BoundedSoftmaxTableScoreRow {
        row_index,
        step_index: 0,
        candidate_index: 0,
        token_position: 0,
...
fn column_ids() -> Vec<String> {
    ...
    .into_iter()
    .map(|suffix| format!("zkai/attention-kv/native-d16-bounded-softmax-table/{suffix}"))
    .collect::<Vec<_>>();
    for prefix in [
        "query",
        "key",
        ...
        for index in 0..width {
            ids.push(format!(
                "zkai/attention-kv/native-d16-bounded-softmax-table/{prefix}-{index:02}"
            ));
        }
    }
    for index in 0..SCORE_GAP_BITS {
        ids.push(format!(
            "zkai/attention-kv/native-d16-bounded-softmax-table/score-gap-bit-{index:02}"
        ));
    }
    for index in 0..CAUSAL_GAP_BITS {
        ids.push(format!(
            "zkai/attention-kv/native-d16-bounded-softmax-table/causal-gap-bit-{index:02}"
        ));
    }
    for index in 0..WEIGHT_BITS {
        ids.push(format!(
            "zkai/attention-kv/native-d16-bounded-softmax-table/weight-bit-{index:02}"
        ));
    }
    for dim in 0..VALUE_WIDTH {
        for index in 0..OUTPUT_REMAINDER_BITS {
            ids.push(format!(
                "zkai/attention-kv/native-d16-bounded-softmax-table/output-remainder-{dim:02}-bit-{index:02}"
            ));
        }
    }
    ids
...
fn expected_weight_table_entries() -> Vec<AttentionKvD16BoundedSoftmaxTableWeightEntry> {
    WEIGHT_TABLE
        .iter()
        .map(
            |(gap, weight)| AttentionKvD16BoundedSoftmaxTableWeightEntry {
                gap: *gap,
                weight: *weight,
            },
        )
...
fn validate_weight_table(table: &[AttentionKvD16BoundedSoftmaxTableWeightEntry]) -> Result<()> {
...
fn kv_commitment(cache: &[AttentionKvD16BoundedSoftmaxTableEntry], domain: &str) -> Result<String> {
    let material = cache
        .iter()
        .map(|entry| {
...
fn input_steps_commitment(steps: &[AttentionKvD16BoundedSoftmaxTableInputStep]) -> Result<String> {
    let material = steps
        .iter()
        .map(|step| {
...
fn rows_commitment(rows: &[AttentionKvD16BoundedSoftmaxTableScoreRow]) -> Result<String> {
...
fn score_row_material(row: &AttentionKvD16BoundedSoftmaxTableScoreRow) -> Vec<i64> {
    let mut out = vec![
...
fn weighted_error(message: impl Into<String>) -> VmError {
    VmError::InvalidConfig(format!(
        "attention/KV native d16 bounded Softmax-table proof: {}",
        message.into()
    ))
}
...
mod tests {
    ...
    fn input() -> ZkAiAttentionKvNativeD16BoundedSoftmaxTableProofInput {
        zkai_attention_kv_native_d16_bounded_softmax_table_input_from_json_str(INPUT_JSON)
            .expect("d16 bounded Softmax-table attention input")
    }

    #[test]
    fn attention_kv_native_d16_bounded_softmax_table_input_validates_checked_rows() {
        let input = input();
        ...
        assert_eq!(
            ...
            vec![1, -4, 1, -5, 0, 4, -1, 3, -2, 2, -3, 1, -4, 1, -5, 0]
        );
        assert_eq!(
            input.attention_outputs[7],
            vec![0, -3, 2, -1, -4, 1, -2, 3, 0, -3, 2, 0, -3, 2, -1, -4]
        );
    ...
    #[test]
    fn attention_kv_native_d16_bounded_softmax_table_uses_floor_division_for_negative_numerators() {
        assert_eq!(
    ...
    #[test]
    fn attention_kv_native_d16_bounded_softmax_table_air_proof_round_trips() {
        let input = input();
        let envelope = prove_zkai_attention_kv_native_d16_bounded_softmax_table_envelope(&input)
            .expect("d16 bounded Softmax-table attention proof");
        assert!(!envelope.proof.is_empty());
        assert!(
            verify_zkai_attention_kv_native_d16_bounded_softmax_table_envelope(&envelope)
                .expect("verify")
        );
    }

    #[test]
    fn attention_kv_native_d16_bounded_softmax_table_rejects_weight_relabeling() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
        value["score_rows"][0]["attention_weight"] = Value::from(15);
        let error = zkai_attention_kv_native_d16_bounded_softmax_table_input_from_json_str(
    ...
    #[test]
    fn attention_kv_native_d16_bounded_softmax_table_rejects_table_relabeling() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_d16_bounded_softmax_table_rejects_output_relabeling() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_d16_bounded_softmax_table_rejects_quotient_remainder_drift() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_d16_bounded_softmax_table_rejects_commitment_drift() {
        let mut value: Value = serde_json::from_str(INPUT_JSON).expect("json");
    ...
    #[test]
    fn attention_kv_native_d16_bounded_softmax_table_rejects_proof_byte_tamper() {
        let input = input();
        let mut envelope =
            prove_zkai_attention_kv_native_d16_bounded_softmax_table_envelope(&input)
                .expect("d16 bounded Softmax-table attention proof");
        let last = envelope.proof.last_mut().expect("proof byte");
        *last ^= 1;
        assert!(
            verify_zkai_attention_kv_native_d16_bounded_softmax_table_envelope(&envelope).is_err()
        );
    }

    #[test]
    fn attention_kv_native_d16_bounded_softmax_table_rejects_unknown_envelope_field() {
        let input = input();
        let envelope = prove_zkai_attention_kv_native_d16_bounded_softmax_table_envelope(&input)
            .expect("d16 bounded Softmax-table attention proof");
        ...
        let error =
            zkai_attention_kv_native_d16_bounded_softmax_table_envelope_from_json_slice(&raw)
                .unwrap_err();
