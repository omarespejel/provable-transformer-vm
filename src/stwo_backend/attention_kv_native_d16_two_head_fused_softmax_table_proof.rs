...
use super::attention_kv_native_d16_two_head_bounded_softmax_table_proof::{
    validate_zkai_attention_kv_native_d16_two_head_bounded_softmax_table_input,
...
pub const ZKAI_ATTENTION_KV_NATIVE_D16_TWO_HEAD_FUSED_SOFTMAX_TABLE_PROOF_VERSION: &str =
    "stwo-attention-kv-d16-two-head-fused-bounded-softmax-table-logup-proof-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D16_TWO_HEAD_FUSED_SOFTMAX_TABLE_BACKEND_VERSION: &str =
    "stwo-attention-kv-d16-two-head-fused-bounded-softmax-table-logup-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D16_TWO_HEAD_FUSED_SOFTMAX_TABLE_STATEMENT_VERSION: &str =
    "zkai-attention-kv-stwo-native-d16-two-head-fused-softmax-table-logup-statement-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D16_TWO_HEAD_FUSED_SOFTMAX_TABLE_SEMANTIC_SCOPE: &str =
    "d16_two_head_bounded_softmax_table_attention_arithmetic_and_logup_membership_fused_in_one_native_stwo_proof";
pub const ZKAI_ATTENTION_KV_NATIVE_D16_TWO_HEAD_FUSED_SOFTMAX_TABLE_DECISION: &str =
    "GO_NATIVE_STWO_FUSED_ATTENTION_ARITHMETIC_AND_SOFTMAX_TABLE_LOGUP_MEMBERSHIP";
pub const ZKAI_ATTENTION_KV_NATIVE_D16_TWO_HEAD_FUSED_SOFTMAX_TABLE_TARGET_ID: &str =
    "attention-kv-d16-two-head-causal-mask-fused-bounded-softmax-table-logup-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D16_TWO_HEAD_FUSED_SOFTMAX_TABLE_VERIFIER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-d16-two-head-fused-bounded-softmax-table-logup:v1";
pub const ZKAI_ATTENTION_KV_NATIVE_D16_TWO_HEAD_FUSED_SOFTMAX_TABLE_MAX_ENVELOPE_JSON_BYTES: usize =
    2_097_152;
pub const ZKAI_ATTENTION_KV_NATIVE_D16_TWO_HEAD_FUSED_SOFTMAX_TABLE_MAX_PROOF_BYTES: usize =
    131_072;

const ISSUE: usize = 521;
const SOURCE_ISSUE: usize = 521;
const SIDECAR_ISSUE: usize = 521;
const SOURCE_PLUS_SIDECAR_RAW_PROOF_BYTES: usize = 73508 + 18088;
...
const NON_FUSED_STATUS: &str =
    "GO_MATCHED_D16_TWO_HEAD_SOURCE_PLUS_LOGUP_SIDECAR_COMPARATOR_RECORDED";
...
const PREPROCESSED_TABLE_GAP: &str =
    "zkai/attention-kv/native-d16-two-head-fused-softmax-table-logup/table-gap";
const PREPROCESSED_TABLE_WEIGHT: &str =
    "zkai/attention-kv/native-d16-two-head-fused-softmax-table-logup/table-weight";
const PREPROCESSED_TABLE_MULTIPLICITY: &str =
    "zkai/attention-kv/native-d16-two-head-fused-softmax-table-logup/table-multiplicity";
const FUSED_COLUMN_PREFIX: &str = "zkai/attention-kv/native-d16-two-head-fused-softmax-table-logup";
...
relation!(AttentionKvD16TwoHeadFusedSoftmaxTableRelation, 2);

#[derive(Debug, Clone)]
struct AttentionKvNativeD16TwoHeadFusedSoftmaxTableEval {
    lookup_elements: AttentionKvD16TwoHeadFusedSoftmaxTableRelation,
}

impl FrameworkEval for AttentionKvNativeD16TwoHeadFusedSoftmaxTableEval {
...
pub struct AttentionKvD16TwoHeadFusedSoftmaxTableMultiplicity {
    pub gap: usize,
    pub weight: i64,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeD16TwoHeadFusedSoftmaxTableSummary {
    pub issue: usize,
    ...
    pub table_multiplicities: Vec<AttentionKvD16TwoHeadFusedSoftmaxTableMultiplicity>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeD16TwoHeadFusedSoftmaxTableEnvelope {
    ...
    pub fused_summary: ZkAiAttentionKvNativeD16TwoHeadFusedSoftmaxTableSummary,
    pub source_input: ZkAiAttentionKvNativeD16TwoHeadBoundedSoftmaxTableProofInput,
    pub proof: Vec<u8>,
}

#[derive(Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
struct AttentionKvNativeD16TwoHeadFusedSoftmaxTableProofPayload {
...
struct FusedBundle {
    log_size: u32,
    summary: ZkAiAttentionKvNativeD16TwoHeadFusedSoftmaxTableSummary,
...
pub fn zkai_attention_kv_native_d16_two_head_fused_softmax_table_source_input_from_json_str(
    raw_json: &str,
    ...
    zkai_attention_kv_native_d16_two_head_bounded_softmax_table_input_from_json_str(raw_json)
}

pub fn prove_zkai_attention_kv_native_d16_two_head_fused_softmax_table_envelope(
    source_input: &ZkAiAttentionKvNativeD16TwoHeadBoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeD16TwoHeadFusedSoftmaxTableEnvelope> {
    validate_source_input(source_input)?;
    let bundle = build_fused_bundle(source_input)?;
    let proof = prove_fused(&bundle)?;
    let envelope = ZkAiAttentionKvNativeD16TwoHeadFusedSoftmaxTableEnvelope {
        proof_backend: StarkProofBackend::Stwo,
        proof_backend_version:
            ZKAI_ATTENTION_KV_NATIVE_D16_TWO_HEAD_FUSED_SOFTMAX_TABLE_BACKEND_VERSION.to_string(),
        proof_schema_version:
            ZKAI_ATTENTION_KV_NATIVE_D16_TWO_HEAD_FUSED_SOFTMAX_TABLE_PROOF_VERSION.to_string(),
        statement_version:
            ZKAI_ATTENTION_KV_NATIVE_D16_TWO_HEAD_FUSED_SOFTMAX_TABLE_STATEMENT_VERSION.to_string(),
        semantic_scope: ZKAI_ATTENTION_KV_NATIVE_D16_TWO_HEAD_FUSED_SOFTMAX_TABLE_SEMANTIC_SCOPE
            .to_string(),
        decision: ZKAI_ATTENTION_KV_NATIVE_D16_TWO_HEAD_FUSED_SOFTMAX_TABLE_DECISION.to_string(),
        target_id: ZKAI_ATTENTION_KV_NATIVE_D16_TWO_HEAD_FUSED_SOFTMAX_TABLE_TARGET_ID.to_string(),
        verifier_domain: ZKAI_ATTENTION_KV_NATIVE_D16_TWO_HEAD_FUSED_SOFTMAX_TABLE_VERIFIER_DOMAIN
            .to_string(),
...
pub fn zkai_attention_kv_native_d16_two_head_fused_softmax_table_envelope_from_json_slice(
    raw_json: &[u8],
) -> Result<ZkAiAttentionKvNativeD16TwoHeadFusedSoftmaxTableEnvelope> {
    if raw_json.len()
...
pub fn verify_zkai_attention_kv_native_d16_two_head_fused_softmax_table_envelope(
    envelope: &ZkAiAttentionKvNativeD16TwoHeadFusedSoftmaxTableEnvelope,
) -> Result<bool> {
    validate_envelope(envelope)?;
...
fn validate_source_input(
    ...
    validate_zkai_attention_kv_native_d16_two_head_bounded_softmax_table_input(input)
}

fn fused_summary(
    input: &ZkAiAttentionKvNativeD16TwoHeadBoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeD16TwoHeadFusedSoftmaxTableSummary> {
    let mut multiplicities = input
        .weight_table
        .iter()
        .map(|entry| AttentionKvD16TwoHeadFusedSoftmaxTableMultiplicity {
            gap: entry.gap,
            weight: entry.weight,
            multiplicity: 0,
        })
    ...
    Ok(ZkAiAttentionKvNativeD16TwoHeadFusedSoftmaxTableSummary {
        issue: ISSUE,
        ...
        lookup_relation: "AttentionKvD16TwoHeadFusedSoftmaxTableRelation".to_string(),
...
fn prove_fused(bundle: &FusedBundle) -> Result<Vec<u8>> {
    let component = fused_component(AttentionKvD16TwoHeadFusedSoftmaxTableRelation::dummy());
    ...
    let lookup_elements = AttentionKvD16TwoHeadFusedSoftmaxTableRelation::draw(channel);
    ...
    serde_json::to_vec(&AttentionKvNativeD16TwoHeadFusedSoftmaxTableProofPayload { stark_proof })
        .map_err(|error| VmError::Serialization(error.to_string()))
}
...
fn verify_fused(
    ...
    validate_source_input(source_input)?;
    if proof.is_empty()
        || proof.len() > ZKAI_ATTENTION_KV_NATIVE_D16_TWO_HEAD_FUSED_SOFTMAX_TABLE_MAX_PROOF_BYTES
    {
        return Err(fused_error("fused proof byte length outside bounded cap"));
    }
    let payload: AttentionKvNativeD16TwoHeadFusedSoftmaxTableProofPayload =
    ...
    let component_placeholder =
        fused_component(AttentionKvD16TwoHeadFusedSoftmaxTableRelation::dummy());
...
fn fused_component(
    lookup_elements: AttentionKvD16TwoHeadFusedSoftmaxTableRelation,
) -> FrameworkComponent<AttentionKvNativeD16TwoHeadFusedSoftmaxTableEval> {
    FrameworkComponent::new(
        &mut TraceLocationAllocator::new_with_preprocessed_columns(&fused_preprocessed_column_ids()),
        AttentionKvNativeD16TwoHeadFusedSoftmaxTableEval { lookup_elements },
        SecureField::zero(),
    )
}
...
fn fused_error(message: impl Into<String>) -> VmError {
    VmError::UnsupportedProof(format!(
        "attention/KV native d16-two-head fused bounded Softmax-table proof: {}",
        message.into()
    ))
}
...
mod tests {
    ...
    fn source_input() -> ZkAiAttentionKvNativeD16TwoHeadBoundedSoftmaxTableProofInput {
        let raw = include_str!(
            "../../docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-two-head-bounded-softmax-table-proof-2026-05.json"
        );
        zkai_attention_kv_native_d16_two_head_fused_softmax_table_source_input_from_json_str(raw)
            .expect("source input")
    }

    #[test]
    fn attention_kv_d16_two_head_fused_softmax_table_round_trips_real_proof() {
        let input = source_input();
        let envelope =
            prove_zkai_attention_kv_native_d16_two_head_fused_softmax_table_envelope(&input)
        ...
        assert!(
            verify_zkai_attention_kv_native_d16_two_head_fused_softmax_table_envelope(&envelope)
                .expect("verify fused attention/lookup")
        );
    }

    #[test]
    fn attention_kv_d16_two_head_fused_softmax_table_summary_counts_claims() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_d16_two_head_fused_softmax_table_derives_logup_indices_from_column_ids() {
        let row_ids = fused_row_column_ids();
        let preprocessed_ids = fused_preprocessed_column_ids();
        let indices = fused_trace_column_indices();
    ...
    #[test]
    fn attention_kv_d16_two_head_fused_softmax_table_rejects_summary_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d16_two_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d16_two_head_fused_softmax_table_rejects_source_weight_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d16_two_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d16_two_head_fused_softmax_table_rejects_negative_bit_material() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_d16_two_head_fused_softmax_table_rejects_output_remainder_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d16_two_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d16_two_head_fused_softmax_table_rejects_proof_byte_tamper() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d16_two_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d16_two_head_fused_softmax_table_rejects_backend_version_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d16_two_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d16_two_head_fused_softmax_table_rejects_proof_schema_version_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_d16_two_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_d16_two_head_fused_softmax_table_rejects_unknown_envelope_field() {
        let input = source_input();
        let envelope =
            prove_zkai_attention_kv_native_d16_two_head_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
