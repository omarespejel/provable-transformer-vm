...
use super::attention_kv_native_two_head_seq32_bounded_softmax_table_proof::{
    validate_zkai_attention_kv_native_two_head_seq32_bounded_softmax_table_input,
...
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_PROOF_VERSION: &str =
    "stwo-attention-kv-two-head-seq32-fused-bounded-softmax-table-logup-proof-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_BACKEND_VERSION: &str =
    "stwo-attention-kv-two-head-seq32-fused-bounded-softmax-table-logup-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_STATEMENT_VERSION: &str =
    "zkai-attention-kv-stwo-native-two-head-seq32-fused-softmax-table-logup-statement-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_SEMANTIC_SCOPE: &str =
    "two_head_seq32_d8_bounded_softmax_table_attention_arithmetic_and_logup_membership_fused_in_one_native_stwo_proof";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_DECISION: &str =
    "GO_NATIVE_STWO_FUSED_ATTENTION_ARITHMETIC_AND_SOFTMAX_TABLE_LOGUP_MEMBERSHIP";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_TARGET_ID: &str =
    "attention-kv-two-head-seq32-d8-causal-mask-fused-bounded-softmax-table-logup-v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_VERIFIER_DOMAIN: &str =
    "ptvm:zkai:attention-kv-stwo-native-two-head-seq32-fused-bounded-softmax-table-logup:v1";
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_MAX_ENVELOPE_JSON_BYTES:
    usize = 8_388_608;
pub const ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_MAX_PROOF_BYTES: usize =
    524_288;

const ISSUE: usize = 537;
const SOURCE_ISSUE: usize = 537;
const SIDECAR_ISSUE: usize = 537;
const SOURCE_PLUS_SIDECAR_RAW_PROOF_BYTES: usize = 98_012;
...
const NON_FUSED_STATUS: &str =
    "GO_MATCHED_TWO_HEAD_SEQ32_SOURCE_PLUS_LOGUP_SIDECAR_COMPARATOR_RECORDED";
...
const PREPROCESSED_TABLE_GAP: &str =
    "zkai/attention-kv/native-two-head-seq32-fused-softmax-table-logup/table-gap";
const PREPROCESSED_TABLE_WEIGHT: &str =
    "zkai/attention-kv/native-two-head-seq32-fused-softmax-table-logup/table-weight";
const PREPROCESSED_TABLE_MULTIPLICITY: &str =
    "zkai/attention-kv/native-two-head-seq32-fused-softmax-table-logup/table-multiplicity";
const FUSED_COLUMN_PREFIX: &str =
    "zkai/attention-kv/native-two-head-seq32-fused-softmax-table-logup";
...
impl FrameworkEval for AttentionKvNativeTwoHeadSeq32FusedSoftmaxTableEval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        assert_eq!(
            row_column_ids.len(),
            trace_values.len(),
            "fused AIR row-column layout drift"
        );
...
pub struct AttentionKvTwoHeadSeq32FusedSoftmaxTableMultiplicity {
    pub gap: usize,
    pub weight: i64,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeTwoHeadSeq32FusedSoftmaxTableSummary {
    pub issue: usize,
    ...
    pub table_multiplicities: Vec<AttentionKvTwoHeadSeq32FusedSoftmaxTableMultiplicity>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct ZkAiAttentionKvNativeTwoHeadSeq32FusedSoftmaxTableEnvelope {
    ...
    pub fused_summary: ZkAiAttentionKvNativeTwoHeadSeq32FusedSoftmaxTableSummary,
    pub source_input: ZkAiAttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableProofInput,
    pub proof: Vec<u8>,
}

#[derive(Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
struct AttentionKvNativeTwoHeadSeq32FusedSoftmaxTableProofPayload {
...
pub fn zkai_attention_kv_native_two_head_seq32_fused_softmax_table_source_input_from_json_str(
    raw_json: &str,
    ...
    zkai_attention_kv_native_two_head_seq32_bounded_softmax_table_input_from_json_str(raw_json)
}

pub fn prove_zkai_attention_kv_native_two_head_seq32_fused_softmax_table_envelope(
    source_input: &ZkAiAttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableProofInput,
) -> Result<ZkAiAttentionKvNativeTwoHeadSeq32FusedSoftmaxTableEnvelope> {
    validate_source_input(source_input)?;
    let bundle = build_fused_bundle(source_input)?;
    let proof = prove_fused(&bundle)?;
    let envelope = ZkAiAttentionKvNativeTwoHeadSeq32FusedSoftmaxTableEnvelope {
        proof_backend: StarkProofBackend::Stwo,
        proof_backend_version:
            ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_BACKEND_VERSION.to_string(),
        ...
        statement_version:
            ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_STATEMENT_VERSION
                .to_string(),
        semantic_scope: ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_SEMANTIC_SCOPE
            .to_string(),
        decision: ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_DECISION.to_string(),
        target_id: ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_TARGET_ID
            .to_string(),
        verifier_domain:
            ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_VERIFIER_DOMAIN.to_string(),
...
pub fn zkai_attention_kv_native_two_head_seq32_fused_softmax_table_envelope_from_json_slice(
    raw_json: &[u8],
) -> Result<ZkAiAttentionKvNativeTwoHeadSeq32FusedSoftmaxTableEnvelope> {
    if raw_json.len()
...
pub fn verify_zkai_attention_kv_native_two_head_seq32_fused_softmax_table_envelope(
    envelope: &ZkAiAttentionKvNativeTwoHeadSeq32FusedSoftmaxTableEnvelope,
) -> Result<bool> {
    validate_envelope(envelope)?;
...
pub(super) fn zkai_attention_kv_native_two_head_seq32_fused_softmax_table_summary(
    ...
    let mut multiplicities = input
        ...
        .iter()
        .map(
            |entry| AttentionKvTwoHeadSeq32FusedSoftmaxTableMultiplicity {
                gap: entry.gap,
                weight: entry.weight,
                multiplicity: 0,
            },
    ...
    for row in &input.score_rows {
        ...
        let clipped_gap = std::cmp::min(
            score_gap_usize(row.score_gap, "source row score gap")?,
            input.score_gap_clip,
        );
    ...
    Ok(ZkAiAttentionKvNativeTwoHeadSeq32FusedSoftmaxTableSummary {
        issue: ISSUE,
        ...
        lookup_relation: "AttentionKvTwoHeadSeq32FusedSoftmaxTableRelation".to_string(),
...
pub(super) fn zkai_attention_kv_native_two_head_seq32_fused_softmax_table_preprocessed_trace(
    ...
    for (real_index, row) in rows.iter().enumerate() {
        ...
        if values.len() != row_column_count {
            return Err(fused_error(format!(
                "fused preprocessed row layout drift: got {} values, expected {}",
                values.len(),
                row_column_count
            )));
        }
...
pub(super) fn zkai_attention_kv_native_two_head_seq32_fused_softmax_table_base_trace(
    ...
    for (real_index, row) in rows.iter().enumerate() {
        ...
        if values.len() != columns.len() {
            return Err(fused_error(format!(
                "fused base row layout drift: got {} values, expected {}",
                values.len(),
                columns.len()
            )));
        }
...
fn row_values(
    ...
    let clipped_gap = if row.score_gap < 0 {
        ...
        std::cmp::min(score_gap_usize(row.score_gap, "score gap")?, SCORE_GAP_CLIP)
    };
    let mut values = vec![
...
fn prove_fused(bundle: &FusedBundle) -> Result<Vec<u8>> {
    let component = fused_component(AttentionKvTwoHeadSeq32FusedSoftmaxTableRelation::dummy());
    ...
    serde_json::to_vec(&AttentionKvNativeTwoHeadSeq32FusedSoftmaxTableProofPayload { stark_proof })
        .map_err(|error| VmError::Serialization(error.to_string()))
}
...
fn verify_fused(
    ...
    validate_source_input(source_input)?;
    if proof.is_empty()
        || proof.len() > ZKAI_ATTENTION_KV_NATIVE_TWO_HEAD_SEQ32_FUSED_SOFTMAX_TABLE_MAX_PROOF_BYTES
    {
        return Err(fused_error("fused proof byte length outside bounded cap"));
    }
    let payload: AttentionKvNativeTwoHeadSeq32FusedSoftmaxTableProofPayload =
    ...
    let component_placeholder =
        fused_component(AttentionKvTwoHeadSeq32FusedSoftmaxTableRelation::dummy());
...
fn score_gap_usize(score_gap: i64, label: &str) -> Result<usize> {
    usize::try_from(score_gap).map_err(|_| fused_error(format!("{label} conversion overflow")))
}
...
fn fused_error(message: impl Into<String>) -> VmError {
    VmError::UnsupportedProof(format!(
        "attention/KV native two-head fused bounded Softmax-table proof: {}",
        message.into()
    ))
}
...
mod tests {
    ...
    fn source_input() -> ZkAiAttentionKvNativeTwoHeadSeq32BoundedSoftmaxTableProofInput {
        let raw = include_str!(
            "../../docs/engineering/evidence/zkai-attention-kv-stwo-native-two-head-seq32-bounded-softmax-table-proof-2026-05.json"
        );
        zkai_attention_kv_native_two_head_seq32_fused_softmax_table_source_input_from_json_str(raw)
            .expect("source input")
    }

    #[test]
    fn attention_kv_two_head_seq32_fused_softmax_table_round_trips_real_proof() {
        let input = source_input();
        let envelope =
            prove_zkai_attention_kv_native_two_head_seq32_fused_softmax_table_envelope(&input)
    ...
    #[test]
    fn attention_kv_two_head_seq32_fused_softmax_table_summary_counts_claims() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_two_head_seq32_fused_softmax_table_derives_logup_indices_from_column_ids() {
        let row_ids = fused_row_column_ids();
        let preprocessed_ids = fused_preprocessed_column_ids();
        let indices = fused_trace_column_indices().expect("fused trace indices");
    ...
    #[test]
    fn attention_kv_two_head_seq32_fused_softmax_table_rejects_missing_column_indices_without_panic(
    ) {
        let row_ids = fused_row_column_ids()
            .into_iter()
            .filter(|id| id != &fused_column_id("enabled"))
            .collect::<Vec<_>>();
        let error = fused_row_column_index(&row_ids, "enabled").unwrap_err();
        assert!(error.to_string().contains("missing fused trace column id"));

        let preprocessed_ids = fused_preprocessed_column_ids()
            .into_iter()
            .filter(|id| id.id != PREPROCESSED_TABLE_WEIGHT)
            .collect::<Vec<_>>();
        let error = fused_preprocessed_column_index(&preprocessed_ids, PREPROCESSED_TABLE_WEIGHT)
            .unwrap_err();
        assert!(error
            .to_string()
    ...
    #[test]
    fn attention_kv_two_head_seq32_fused_softmax_table_rejects_source_weight_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_two_head_seq32_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_two_head_seq32_fused_softmax_table_rejects_negative_bit_material() {
        let input = source_input();
    ...
    #[test]
    fn attention_kv_two_head_seq32_fused_softmax_table_rejects_output_remainder_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_two_head_seq32_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_two_head_seq32_fused_softmax_table_rejects_proof_byte_tamper() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_two_head_seq32_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_two_head_seq32_fused_softmax_table_rejects_backend_version_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_two_head_seq32_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_two_head_seq32_fused_softmax_table_rejects_proof_schema_version_drift() {
        let input = source_input();
        let mut envelope =
            prove_zkai_attention_kv_native_two_head_seq32_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
    ...
    #[test]
    fn attention_kv_two_head_seq32_fused_softmax_table_rejects_unknown_envelope_field() {
        let input = source_input();
        let envelope =
            prove_zkai_attention_kv_native_two_head_seq32_fused_softmax_table_envelope(&input)
                .expect("prove fused attention/lookup");
