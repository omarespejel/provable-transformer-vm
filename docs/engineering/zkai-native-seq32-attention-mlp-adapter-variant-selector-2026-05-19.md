...
The gate rejects `12 / 12` mutation cases: frontier promotion drift, gap
erasure, current champion drift, variant metric drift, inventory drift,
overclaim drift, source artifact drift, validation-command drift, and payload
commitment drift.
...
# Repeat the same build/prove/verify pattern for output-anchor, rmsnorm-fused, adjacent, and post-tail modes listed in the gate JSON.
