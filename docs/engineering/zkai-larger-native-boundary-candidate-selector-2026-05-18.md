...
The gate rejects `14 / 14` mutations covering selected-candidate drift, selected
metric drift, d8 baseline drift, bytes-per-lookup overclaim, NANOZK overclaim,
full-block overclaim, source digest drift, accounting row removal, non-claim
removal/addition, validation-command drift, interpretation overclaim, and
