...
rejects `13 / 13` mutation attacks. Backend, backend version, and timing mode
are part of the result, not footnotes: every statement row pins `backend`,
`backend_version`, and `timing_policy`.
...
Evidence was regenerated by the validation command below. The full JSON and TSV
artifacts are:
...
- Atlas unavailable binding relabeled as bound
