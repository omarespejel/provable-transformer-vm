...
as a production-security profile. Under the repository's simple query-blowup
floor label, q3 has `3` query-blowup bits before proof-of-work.
...
`.tsv` companions. The release gate byte-diffs the deterministic `.tsv`
companions; rendered `.pdf`, `.png`, and `.svg` files are preview artifacts
because their bytes can depend on local font and rendering libraries.
