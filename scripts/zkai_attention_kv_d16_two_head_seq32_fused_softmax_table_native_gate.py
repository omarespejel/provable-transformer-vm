#!/usr/bin/env python3
"""Checked fused native Stwo d16-two-head-seq32 bounded Softmax-table gate for issue #715.
...
SOURCE_INPUT_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-two-head-seq32-bounded-softmax-table-proof-2026-05.json"
SOURCE_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-two-head-seq32-bounded-softmax-table-proof-2026-05.envelope.json"
SIDECAR_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-two-head-seq32-softmax-table-logup-sidecar-proof-2026-05.envelope.json"
FUSED_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-two-head-seq32-fused-softmax-table-proof-2026-05.envelope.json"
SOURCE_INPUT_SCRIPT = ROOT / "scripts" / "zkai_attention_kv_stwo_native_d16_two_head_seq32_bounded_softmax_table_proof_input.py"
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-two-head-seq32-fused-softmax-table-gate-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d16-two-head-seq32-fused-softmax-table-gate-2026-05.tsv"
...
SCHEMA = "zkai-attention-kv-stwo-native-d16-two-head-seq32-fused-softmax-table-gate-v1"
ISSUE = 715
SOURCE_ISSUE = 715
SOURCE_INPUT_ISSUE = 715
SIDECAR_ISSUE = 715
DECISION = "GO_NATIVE_STWO_FUSED_ATTENTION_ARITHMETIC_AND_SOFTMAX_TABLE_LOGUP_MEMBERSHIP"
ROUTE_ID = "local_stwo_attention_kv_d16_two_head_seq32_fused_bounded_softmax_table_logup_proof"
CLAIM_BOUNDARY = (
    "ONE_NATIVE_STWO_D16_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_ATTENTION_PROOF_WITH_LOGUP_TABLE_MEMBERSHIP_"
...
NON_FUSED_STATUS = "GO_MATCHED_D16_TWO_HEAD_SEQ32_SOURCE_PLUS_LOGUP_SIDECAR_COMPARATOR_RECORDED"
TIMING_POLICY = "proof_existence_and_byte_accounting_only_not_public_benchmark"

SOURCE_PROOF_SIZE_BYTES = 90_754
SOURCE_ENVELOPE_SIZE_BYTES = 3_750_112
SIDECAR_PROOF_SIZE_BYTES = 36_453
SIDECAR_ENVELOPE_SIZE_BYTES = 3_317_663
SOURCE_PLUS_SIDECAR_RAW_PROOF_BYTES = SOURCE_PROOF_SIZE_BYTES + SIDECAR_PROOF_SIZE_BYTES
FUSED_PROOF_SIZE_BYTES = 92_363
FUSED_ENVELOPE_SIZE_BYTES = 3_766_025
...
SOURCE_STATEMENT_COMMITMENT = "blake2b-256:56b76de659918a80a0249d038329e0eb97eaabd8f17e36b6b4475f5133049ebf"
SOURCE_PUBLIC_INSTANCE_COMMITMENT = "blake2b-256:5a2f8123600f927e83c7a0cb361a7196796c74a94f656a441ace2000ce203540"
SOURCE_SCORE_ROW_COMMITMENT = "blake2b-256:fa5db08f15a8ddf991fdab5e24623de53afd3d5b89cd582f57d4e0130ae8a1fe"
SOURCE_FINAL_KV_CACHE_COMMITMENT = "blake2b-256:88a8b64462752b399b4f0a196dbd30fac73227ba54db1b209a48c9a4344c11f4"
SOURCE_OUTPUTS_COMMITMENT = "blake2b-256:c3b37c3fc34f9a38046948863b4040d3b29eb2cf36d75c36a839eaf2bfd7db17"
SOURCE_WEIGHT_TABLE_COMMITMENT = "blake2b-256:833b8b8915ffd5f1fac65eb067eda7bfc178ff88ac60c565640ed98b5c0cc30f"
SOURCE_HEAD_COUNT = 2
...
SOURCE_TABLE_ROWS = 9
SOURCE_BACKEND_VERSION = "stwo-attention-kv-d16-two-head-seq32-causal-mask-bounded-softmax-table-v1"
SOURCE_STATEMENT_VERSION = "zkai-attention-kv-stwo-native-d16-two-head-seq32-bounded-softmax-table-statement-v1"
SOURCE_SEMANTIC_SCOPE = "d16_two_head_seq32_bounded_table_softmax_approx_attention_kv_causal_mask_rows_bound_to_statement_receipt"
SOURCE_DECISION = "GO_STWO_NATIVE_ATTENTION_KV_D16_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
SOURCE_INPUT_DECISION = "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_D16_TWO_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
SOURCE_TARGET_ID = "attention-kv-d16-two-head-seq32-causal-mask-bounded-softmax-table-v1"
SOURCE_PROOF_VERSION = "stwo-attention-kv-d16-two-head-seq32-causal-mask-bounded-softmax-table-air-proof-v1"
SOURCE_VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-two-head-seq32-bounded-softmax-table:v1"
LOOKUP_RELATION = "AttentionKvD16TwoHeadSeq32FusedSoftmaxTableRelation"
SIDECAR_LOOKUP_RELATION = "AttentionKvD16TwoHeadSeq32SoftmaxTableLookupRelation"
LOOKUP_RELATION_WIDTH = 2
SOURCE_PROOF_COMMITMENTS = 3
SIDECAR_PROOF_COMMITMENTS = 4

FUSED_BACKEND_VERSION = "stwo-attention-kv-d16-two-head-seq32-fused-bounded-softmax-table-logup-v1"
FUSED_PROOF_SCHEMA_VERSION = "stwo-attention-kv-d16-two-head-seq32-fused-bounded-softmax-table-logup-proof-v1"
FUSED_STATEMENT_VERSION = "zkai-attention-kv-stwo-native-d16-two-head-seq32-fused-softmax-table-logup-statement-v1"
FUSED_SEMANTIC_SCOPE = "d16_two_head_seq32_bounded_softmax_table_attention_arithmetic_and_logup_membership_fused_in_one_native_stwo_proof"
FUSED_TARGET_ID = "attention-kv-d16-two-head-seq32-causal-mask-fused-bounded-softmax-table-logup-v1"
FUSED_VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d16-two-head-seq32-fused-bounded-softmax-table-logup:v1"
FUSED_PROOF_COMMITMENTS = 4
FUSED_TRACE_COMMITMENTS = 3

TABLE_MULTIPLICITIES = ({"gap": 0, "weight": 256, "multiplicity": 74}, {"gap": 1, "weight": 181, "multiplicity": 5}, {"gap": 2, "weight": 128, "multiplicity": 5}, {"gap": 3, "weight": 91, "multiplicity": 7}, {"gap": 4, "weight": 64, "multiplicity": 1}, {"gap": 5, "weight": 45, "multiplicity": 4}, {"gap": 6, "weight": 32, "multiplicity": 1}, {"gap": 7, "weight": 23, "multiplicity": 3}, {"gap": 8, "weight": 16, "multiplicity": 1084})
NON_CLAIMS = (
...
VALIDATION_COMMANDS = (
    ...
    "cargo +nightly-2025-07-14 run --locked --features stwo-backend --bin zkai_attention_kv_native_d16_two_head_seq32_fused_softmax_table_proof -- prove docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-two-head-seq32-bounded-softmax-table-proof-2026-05.json docs/engineering/evidence/zkai-attention-kv-stwo-native-d16-two-head-seq32-fused-softmax-table-proof-2026-05.envelope.json",
...
class AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(ValueError):
    pass
...
def load_script_module(path: pathlib.Path, module_name: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"failed to load {module_name}: {path}")
...
SOURCE_INPUT_MODULE = load_script_module(
    SOURCE_INPUT_SCRIPT, "zkai_attention_kv_stwo_native_d16_two_head_seq32_bounded_softmax_table_proof_input"
)


def read_bounded_bytes(path: pathlib.Path, max_bytes: int, label: str) -> bytes:
    if not path.is_file():
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"missing {label}: {path}")
    size = path.stat().st_size
    if size <= 0 or size > max_bytes:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"{label} size drift: got {size}, max {max_bytes}")
    try:
        raw = path.read_bytes()
    except OSError as err:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"failed to read {label}: {err}") from err
    if len(raw) != size:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"{label} read size drift: stat={size}, read={len(raw)}")
    return raw
...
def parse_bounded_json_bytes(raw: bytes, label: str) -> Any:
    try:
        ...
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"failed to decode {label}: {err}") from err


def expect_artifact_size(raw: bytes, expected_size: int, label: str) -> None:
    if len(raw) != expected_size:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(
...
def parse_stark_proof(
    ...
    *,
    ...
    if not isinstance(proof_bytes, list):
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"{label} proof must be a byte list")
    if len(proof_bytes) != expected_bytes:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"{label} proof byte length drift")
    if any(not isinstance(byte, int) or isinstance(byte, bool) or byte < 0 or byte > 255 for byte in proof_bytes):
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"{label} proof bytes must be uint8 values")
    try:
        payload = json.loads(bytes(proof_bytes).decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as err:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"failed to decode {label} proof payload: {err}") from err
    if set(payload) != {"stark_proof"}:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"{label} proof payload schema drift")
    stark_proof = payload["stark_proof"]
    if not isinstance(stark_proof, dict):
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"{label} stark_proof must be an object")
    commitments = stark_proof.get("commitments")
    if not isinstance(commitments, list) or len(commitments) != expected_commitments:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"{label} proof commitment count drift")
    return stark_proof
...
def same_digit_int_mutation(value: int, label: str) -> int:
    for candidate in (value + 1, value - 1):
        if 0 <= candidate <= 255 and len(str(candidate)) == len(str(value)):
            return candidate
    raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"no same-width byte mutation for {label}")
...
def mutate_same_size_stark_proof_commitment(envelope: dict[str, Any]) -> None:
    ...
    if len(proof_bytes) != len(envelope["proof"]):
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError("same-size proof mutation changed byte length")
...
def assert_strict_equal(actual: Any, expected: Any, label: str) -> None:
    if not type_strict_equal(actual, expected):
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"{label} drift: got {describe_value(actual)}")


def assert_fields(mapping: dict[str, Any], expected: dict[str, Any], label: str) -> None:
    if not isinstance(mapping, dict):
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"{label} must be an object")
...
def proof_bytes(envelope: dict[str, Any]) -> bytes:
    ...
    if not isinstance(proof, list):
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError("fused proof must be a byte list")
    if len(proof) != FUSED_PROOF_SIZE_BYTES:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError("fused proof byte length drift")
    if any(not isinstance(byte, int) or isinstance(byte, bool) or byte < 0 or byte > 255 for byte in proof):
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError("fused proof bytes must be uint8 values")
    return bytes(proof)
...
def verify_fused_envelope_bytes_with_native_cli(envelope_bytes: bytes, label: str) -> None:
    ...
    if completed.returncode != 0:
        detail = (completed.stderr or completed.stdout).strip().splitlines()
        suffix = detail[-1] if detail else f"exit code {completed.returncode}"
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"native fused verifier rejected {label}: {suffix}")
    try:
        summary = json.loads(completed.stdout)
    except json.JSONDecodeError as err:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"native fused verifier emitted malformed JSON: {err}") from err
    expected = {
        "mode": "verify",
        "verified": True,
...
def validate_source_input_contract(source_input: dict[str, Any]) -> None:
    ...
    if len(source_input.get("score_rows", [])) != SOURCE_SCORE_ROWS:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError("source score row count drift")
    if len(source_input.get("weight_table", [])) != SOURCE_TABLE_ROWS:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError("source weight table row count drift")
...
def validate_source_artifacts(
    ...
    *,
    ...
    try:
        ...
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"source input validation drift: {err}") from err
    ...
    verify_envelope_bytes_with_native_cli(
        ...
        binary="zkai_attention_kv_native_d16_two_head_seq32_bounded_softmax_table_proof",
        expected_summary={
            "mode": "verify",
            "verified": True,
    ...
    assert_fields(
        ...
        {
            "proof_backend": "stwo",
            "proof_backend_version": "stwo-attention-kv-d16-two-head-seq32-softmax-table-logup-sidecar-proof-v1",
            "statement_version": "zkai-attention-kv-stwo-native-d16-two-head-seq32-softmax-table-logup-sidecar-statement-v1",
            "semantic_scope": "d16_two_head_seq32_bounded_softmax_table_membership_constrained_by_native_stwo_logup_sidecar",
            "decision": "GO_NATIVE_STWO_AIR_CONSTRAINED_SOFTMAX_TABLE_LOOKUP_RELATION_SIDECAR",
            "verifier_domain": "ptvm:zkai:attention-kv-stwo-native-d16-two-head-seq32-softmax-table-logup-sidecar:v1",
        },
        "sidecar envelope",
    )
    ...
    if not isinstance(lookup_summary, dict):
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError("sidecar lookup summary missing")
    expected_sidecar = {
        "source_statement_commitment": SOURCE_STATEMENT_COMMITMENT,
    ...
    verify_envelope_bytes_with_native_cli(
        ...
        binary="zkai_attention_kv_native_d16_two_head_seq32_softmax_table_lookup_proof",
        expected_summary={
            "mode": "verify",
            "verified": True,
...
def validate_fused_envelope(envelope: dict[str, Any], source_input: dict[str, Any], *, run_native: bool) -> None:
    ...
    if extra_keys:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"unknown fused envelope field(s): {sorted(extra_keys)}")
    assert_fields(
        envelope,
        {
            "proof_backend": "stwo",
...
def run_gate() -> dict[str, Any]:
    ...
    if mutation_names != EXPECTED_MUTATION_NAMES:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError("mutation case order/name drift")
    rejected = sum(1 for result in mutation_results if result["rejected"])
    if rejected != EXPECTED_MUTATION_COUNT:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"mutation rejection drift: got {rejected}")
...
def evaluate_mutation_results(fused_envelope: dict[str, Any], source_input: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for name, mutated, run_native in mutation_cases(fused_envelope):
        ...
        except AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError as err:
...
def validate_result(result: dict[str, Any]) -> None:
    if not isinstance(result, dict):
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError("result must be an object")
    ...
    if missing:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"missing result keys: {sorted(missing)}")
    extra = set(result) - required
    if extra:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"unknown result keys: {sorted(extra)}")
    for key in ("fused_envelope_commitment", "fused_proof_commitment"):
        ...
        if not isinstance(value, str) or not value.startswith("blake2b-256:"):
            raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError(f"{key} drift")
    ...
    if not isinstance(mutation_results, list) or len(mutation_results) != EXPECTED_MUTATION_COUNT:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError("mutation result shape drift")
    for item in mutation_results:
        if not isinstance(item, dict):
            raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError("mutation result shape drift")
        if set(item) != {"name", "rejected", "error"}:
            raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError("mutation result schema drift")
    mutation_names = tuple(item["name"] for item in mutation_results)
    if mutation_names != EXPECTED_MUTATION_NAMES:
        raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError("mutation result name drift")
    for item in mutation_results:
        if (
            ...
            raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError("mutation result rejection drift")
...
def write_tsv(path: pathlib.Path, result: dict[str, Any]) -> None:
    ...
    try:
        ...
        if rows != [expected_row]:
            raise AttentionKvD16TwoHeadSeq32FusedSoftmaxTableGateError("TSV round-trip drift")
        tmp_path.replace(path)
    except Exception:
        tmp_path.unlink(missing_ok=True)
        raise
