...
DECISION = "GO_WIDE_GRID_SELECTOR_PROMOTE_D128_HEAD_AXIS_WITH_D256_STILL_FALSIFICATION_TARGET"
...
CLAIM_BOUNDARY = (
    ...
    "D64_HAS_PARTIAL_SOURCE_BACKED_ROWS_D128_HAS_TWO_SOURCE_BACKED_TWO_HEAD_ROWS_D256_HAS_NO_ATTENTION_PROOF_ROWS_YET;"
...
ROUTE_MATRIX_SHA256 = "11b59056f5a13551eb86b6ab81023ca96fbf486bb1c9536b064d066d1ffe8bed"
FULLER_GRID_SHA256 = "aee51fbeda5ca8c258d2a3671f883ed4b1def393710059b19ca8405a3763cc49"
CLAIM_PACK_SHA256 = "1d28730e245064f11992d755e1693640ca314e7a74e723e6adfdd013cf14aaa8"
...
NON_CLAIMS = (
    ...
    "does not widen the claim beyond checked d8, d16, d32, partial d64, and two d128 two-head attention rows",
)
...
def build_requested_grid_signal(current_signal: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "requested_rows": rows,
        "selector_status": "PARTIAL_D64_AND_TWO_D128_SOURCE_BACKED_D256_MISSING",
    }
...
def build_candidate_order() -> list[dict[str, Any]]:
    return [
        {
            "priority": 1,
            "profile_id": "d128_h4_seq32",
            "key_width": 128,
            "head_count": 4,
            "steps_per_head": 32,
            "selector_status": "NEXT_D128_HEAD_AXIS_AFTER_D128_SEQ64_GO",
            "why_this_row": "tests whether the positive d128 two-head rows survive head-axis pressure before jumping to d256",
            "go_gate": "d128 four-head seq32 source, sidecar, and fused rows validate and fused still beats matched split",
            "no_go_gate": "d128 four-head seq32 proof generation is too heavy, fails mutation guards, or loses the matched fused-vs-split saving",
        },
        {
            "priority": 2,
            ...
            "why_this_row": "anchors d128 width pressure with lower lookup load if the four-head row is too heavy",
        ...
        },
        {
            ...
            "head_count": 2,
            "steps_per_head": 32,
            "selector_status": "ONLY_AFTER_D128_HEAD_AXIS_OR_GENERATED_BACKEND_GO",
            "why_this_row": "records the next width stress test but keeps it behind d128 head-axis evidence or a generated backend",
            "go_gate": "generic or generated backend exists and d128 head-axis evidence stays structurally positive",
            "no_go_gate": "copy-per-width engineering dominates research signal or d128 head-axis pressure fails",
        },
    ]
...
def build_interpretation() -> dict[str, Any]:
    return {
        "human_read": (
            ...
            "The d128 two-head seq32 and seq64 rows are now positive, but D128 is not a victory "
            "lap; the next check is whether it survives head-axis pressure before D256."
        ),
        "paper_relevance": (
            "A paper-grade result needs the fused-vs-split saving to survive beyond isolated rows. "
            "The selector therefore promotes d128 head-axis pressure after d128 two-head seq32 and seq64 stayed positive."
        ),
        "research_recommendation": (
            "Do d128_h4_seq32 next. If it is too heavy, use d128_h1_seq16 as the lower-pressure width anchor."
        ),
    }
...
def validate_payload(payload: dict[str, Any], expected_source_artifacts: list[dict[str, Any]]) -> None:
    ...
    if current.get("checked_attention_route_rows") != 26:
        raise ProofPressureWideGridSelectorError("current route row count drift")
    ...
    if current.get("raw_fused_bytes_total") != 4_234_408:
        raise ProofPressureWideGridSelectorError("raw fused total drift")
    if current.get("raw_split_bytes_total") != 4_871_057:
        raise ProofPressureWideGridSelectorError("raw split total drift")
    if current.get("raw_saving_bytes_total") != 636_649:
    ...
    if accounting.get("attention_raw_proof_savings_bytes_total") != 636_649:
    ...
    if requested.get("selector_status") != "PARTIAL_D64_AND_TWO_D128_SOURCE_BACKED_D256_MISSING":
    ...
    if requested.get("source_backed_requested_cell_count") != 9:
        raise ProofPressureWideGridSelectorError("wide row smuggling")
    if requested.get("missing_requested_cell_count") != 18:
...
def mutation_cases(payload: dict[str, Any]) -> tuple[tuple[str, Any], ...]:
    return (
        ...
        ("wide_row_smuggling", lambda p: p["requested_grid_signal"].__setitem__("source_backed_requested_cell_count", 10)),
