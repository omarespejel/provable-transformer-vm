#!/usr/bin/env python3
"""Checked AIR-constrained d32 two-head Softmax-table lookup sidecar gate for issue #715.

This gate records the narrow positive result: the existing native d32 two-head
...
LOOKUP_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d32-two-head-softmax-table-logup-sidecar-proof-2026-05.envelope.json"
SOURCE_INPUT_SCRIPT = ROOT / "scripts" / "zkai_attention_kv_stwo_native_d32_two_head_bounded_softmax_table_proof_input.py"
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d32-two-head-softmax-table-logup-sidecar-gate-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d32-two-head-softmax-table-logup-sidecar-gate-2026-05.tsv"

MAX_SOURCE_INPUT_JSON_BYTES = 4_194_304
MAX_LOOKUP_ENVELOPE_JSON_BYTES = 2_097_152

SCHEMA = "zkai-attention-kv-stwo-native-d32-two-head-softmax-table-logup-sidecar-gate-v1"
ISSUE = 715
SOURCE_ISSUE = 715
DECISION = "GO_NATIVE_STWO_AIR_CONSTRAINED_D32_TWO_HEAD_SOFTMAX_TABLE_LOOKUP_RELATION_SIDECAR"
CLAIM_BOUNDARY = (
    "NATIVE_STWO_D32_TWO_HEAD_BOUNDED_SOFTMAX_TABLE_LOOKUP_MEMBERSHIP_CONSTRAINED_BY_LOGUP_SIDECAR_"
...
NEXT_BACKEND_STEP = (
    ...
    "the sidecar now reproduces across single-head and d32-two-head source families"
)
...
LOOKUP_PROOF_VERSION = "stwo-attention-kv-d32-two-head-softmax-table-logup-sidecar-proof-v1"
LOOKUP_STATEMENT_VERSION = "zkai-attention-kv-stwo-native-d32-two-head-softmax-table-logup-sidecar-statement-v1"
LOOKUP_SEMANTIC_SCOPE = "d32_two_head_bounded_softmax_table_membership_constrained_by_native_stwo_logup_sidecar"
LOOKUP_VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d32-two-head-softmax-table-logup-sidecar:v1"
LOOKUP_TARGET_ID = "attention-kv-d32-two-head-causal-mask-bounded-softmax-table-logup-sidecar-v1"
LOOKUP_RELATION = "AttentionKvD32TwoHeadSoftmaxTableLookupRelation"
LOOKUP_RELATION_WIDTH = 2
LOOKUP_PROOF_SIZE_BYTES = 18137
LOOKUP_ENVELOPE_SIZE_BYTES = 649773
LOOKUP_PROOF_COMMITMENTS = 4
LOOKUP_TRACE_COMMITMENTS = 3
LOOKUP_TABLE_MULTIPLICITIES = ({"gap": 0, "weight": 256, "multiplicity": 23}, {"gap": 1, "weight": 181, "multiplicity": 0}, {"gap": 2, "weight": 128, "multiplicity": 0}, {"gap": 3, "weight": 91, "multiplicity": 0}, {"gap": 4, "weight": 64, "multiplicity": 0}, {"gap": 5, "weight": 45, "multiplicity": 0}, {"gap": 6, "weight": 32, "multiplicity": 0}, {"gap": 7, "weight": 23, "multiplicity": 2}, {"gap": 8, "weight": 16, "multiplicity": 79})
EXPECTED_LOOKUP_TABLE_MULTIPLICITIES = LOOKUP_TABLE_MULTIPLICITIES

SINGLE_HEAD_LOOKUP_CLAIMS = 52
SINGLE_HEAD_LOOKUP_PROOF_SIZE_BYTES = 15562
SINGLE_HEAD_LOOKUP_ENVELOPE_SIZE_BYTES = 379293
CLAIM_COUNT_RATIO = "2.000000"
PROOF_SIZE_RATIO = "1.165467"
ENVELOPE_SIZE_RATIO = "1.713116"
...
VALIDATION_COMMANDS = (
    "cargo +nightly-2025-07-14 test attention_kv_d32_two_head_softmax_table_lookup --lib --features stwo-backend",
    "cargo +nightly-2025-07-14 run --features stwo-backend --bin zkai_attention_kv_native_d32_two_head_softmax_table_lookup_proof -- prove docs/engineering/evidence/zkai-attention-kv-stwo-native-d32-two-head-bounded-softmax-table-proof-2026-05.json docs/engineering/evidence/zkai-attention-kv-stwo-native-d32-two-head-softmax-table-logup-sidecar-proof-2026-05.envelope.json",
    "cargo +nightly-2025-07-14 run --features stwo-backend --bin zkai_attention_kv_native_d32_two_head_softmax_table_lookup_proof -- verify docs/engineering/evidence/zkai-attention-kv-stwo-native-d32-two-head-softmax-table-logup-sidecar-proof-2026-05.envelope.json",
    "python3 scripts/zkai_attention_kv_d32_two_head_air_private_softmax_table_lookup_gate.py --write-json docs/engineering/evidence/zkai-attention-kv-stwo-native-d32-two-head-softmax-table-logup-sidecar-gate-2026-05.json --write-tsv docs/engineering/evidence/zkai-attention-kv-stwo-native-d32-two-head-softmax-table-logup-sidecar-gate-2026-05.tsv",
    "python3 -m unittest scripts.tests.test_zkai_attention_kv_d32_two_head_air_private_softmax_table_lookup_gate",
    "just gate-fast",
    "just gate",
)
...
def verify_lookup_envelope_bytes_with_native_cli(envelope_bytes: bytes, label: str) -> None:
    ...
    command = [
        ...
        "stwo-backend",
        "--bin",
        "zkai_attention_kv_native_d32_two_head_softmax_table_lookup_proof",
        "--",
        "verify",
        str(tmp_path),
    ]
...
def single_head_comparison() -> dict[str, Any]:
    return {
        ...
        "d32_two_head_lookup_claims": SOURCE_SCORE_ROWS,
        "d32_two_head_lookup_proof_size_bytes": LOOKUP_PROOF_SIZE_BYTES,
        "d32_two_head_lookup_envelope_size_bytes": LOOKUP_ENVELOPE_SIZE_BYTES,
...
def build_payload() -> dict[str, Any]:
    source_input = read_bounded_json(SOURCE_INPUT_JSON, MAX_SOURCE_INPUT_JSON_BYTES, "source d32-two-head bounded Softmax-table input")
    validate_source_input(source_input)
    ...
    payload["gate_commitment"] = blake2b_commitment(
        ...
        "ptvm:zkai:attention-kv-d32-two-head-softmax-table-logup-sidecar-gate:v1",
    )
