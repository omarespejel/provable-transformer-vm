#!/usr/bin/env python3
"""Build the native Stwo d128 single-head seq16 bounded Softmax-table input.

This issue #715 row isolates the d128 single-head seq16 slope before the grid
...
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d128-single-head-seq16-bounded-softmax-table-proof-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d128-single-head-seq16-bounded-softmax-table-proof-2026-05.tsv"
SOURCE_SCRIPT = ROOT / "scripts" / "zkai_attention_kv_stwo_native_d64_single_head_longseq_bounded_softmax_table_proof_input.py"

SOURCE_SCHEMA = "zkai-attention-kv-stwo-native-d64-single-head-longseq-bounded-softmax-table-air-proof-input-v1"
SOURCE_DECISION = "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_D64_SINGLE_HEAD_LONGSEQ_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
SOURCE_TARGET_ID = "attention-kv-d64-single-head-longseq-causal-mask-bounded-softmax-table-v1"
SOURCE_BACKEND_VERSION = "stwo-attention-kv-d64-single-head-longseq-causal-mask-bounded-softmax-table-v1"
SOURCE_PROOF_VERSION = "stwo-attention-kv-d64-single-head-longseq-causal-mask-bounded-softmax-table-air-proof-v1"
SOURCE_STATEMENT_VERSION = "zkai-attention-kv-stwo-native-d64-single-head-longseq-bounded-softmax-table-statement-v1"
SOURCE_PAYLOAD_SHA256 = "f7d1c543bb985649ee4495fd18ae6bf3b8948c4f2849362dabe19aab92af4f21"

SCHEMA = "zkai-attention-kv-stwo-native-d128-single-head-seq16-bounded-softmax-table-air-proof-input-v1"
DECISION = "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_D128_SINGLE_HEAD_SEQ16_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
ISSUE = 715
SOURCE_ISSUE = 715
TARGET_ID = "attention-kv-d128-single-head-seq16-causal-mask-bounded-softmax-table-v1"
REQUIRED_BACKEND_VERSION = "stwo-attention-kv-d128-single-head-seq16-causal-mask-bounded-softmax-table-v1"
PROOF_VERSION = "stwo-attention-kv-d128-single-head-seq16-causal-mask-bounded-softmax-table-air-proof-v1"
STATEMENT_VERSION = "zkai-attention-kv-stwo-native-d128-single-head-seq16-bounded-softmax-table-statement-v1"
SEMANTIC_SCOPE = "d128_single_head_seq16_bounded_table_softmax_approx_attention_kv_causal_mask_rows_bound_to_statement_receipt"
VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-single-head-seq16-bounded-softmax-table:v1"
...
_SOURCE_PAYLOAD_CACHE: dict[str, Any] | None = None

ROW_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-single-head-seq16-bounded-softmax-table-score-rows:v1"
INITIAL_KV_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-single-head-seq16-bounded-softmax-table-initial-kv:v1"
INPUT_STEPS_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-single-head-seq16-bounded-softmax-table-input-steps:v1"
FINAL_KV_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-single-head-seq16-bounded-softmax-table-final-kv:v1"
OUTPUTS_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-single-head-seq16-bounded-softmax-table-outputs:v1"
PUBLIC_INSTANCE_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-single-head-seq16-bounded-softmax-table-public-instance:v1"
PROOF_NATIVE_PARAMETER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-single-head-seq16-bounded-softmax-table-proof-parameters:v1"
WEIGHT_TABLE_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-single-head-seq16-bounded-softmax-table-weight-table:v1"
...
NEXT_BACKEND_STEP = "fuse this d128 single-head seq16 bounded Softmax-table source with AIR-private LogUp table membership, then compare against a matched d128 single-head seq16 source-plus-sidecar route"
...
def _load_source_module() -> Any:
    spec = importlib.util.spec_from_file_location(
        "zkai_attention_kv_stwo_native_d64_single_head_longseq_bounded_softmax_table_proof_input",
        SOURCE_SCRIPT,
    )
    if spec is None or spec.loader is None:
        raise AttentionKvBoundedSoftmaxTableInputError(f"failed to load source script: {SOURCE_SCRIPT}")
...
def source_payload() -> dict[str, Any]:
    ...
    if _SOURCE_PAYLOAD_CACHE is not None:
        return _SOURCE_PAYLOAD_CACHE
    payload = SOURCE.build_payload()
    if not isinstance(payload, dict):
        raise AttentionKvBoundedSoftmaxTableInputError("source payload must be an object")
    expected_scalars = {
    ...
    for key, expected in expected_scalars.items():
        if payload.get(key) != expected:
            raise AttentionKvBoundedSoftmaxTableInputError(f"source payload {key} drift")
    observed = sha256_hex_bytes(canonical_json_bytes(payload))
    if observed != SOURCE_PAYLOAD_SHA256:
        raise AttentionKvBoundedSoftmaxTableInputError("source payload commitment drift")
    _SOURCE_PAYLOAD_CACHE = payload
    return payload
...
def widen_vector(
    ...
    *,
    ...
    for index in range(target_width - source_width):
        source_value = checked[(index + role_bias) % source_width]
        shift = ((local_step + index + role_bias) % 7) - 3
        widened.append(require_int(source_value + shift, f"{label}.widened[{index}]"))
    if len(widened) != target_width:
        raise AttentionKvBoundedSoftmaxTableInputError(f"{label} widened width drift")
    return widened


def initial_kv_cache() -> list[dict[str, Any]]:
    source_items = require_sequence(source_payload().get("initial_kv_cache"), label="source.initial_kv_cache")
    items = []
    for index, item in enumerate(source_items):
        if not isinstance(item, dict):
            raise AttentionKvBoundedSoftmaxTableInputError(f"source.initial_kv_cache[{index}] must be an object")
        position = require_int(item.get("position"), f"source.initial_kv_cache[{index}].position")
        items.append({
            "position": position,
            "key": widen_vector(
                item.get("key"),
                ...
                local_step=position,
                role_bias=11,
                label=f"source.initial_kv_cache[{index}].key",
            ),
            "value": widen_vector(
                item.get("value"),
                ...
                local_step=position,
                role_bias=17,
                label=f"source.initial_kv_cache[{index}].value",
            ),
        })
...
def input_step(step_index: int) -> dict[str, Any]:
    source_steps = require_sequence(source_payload().get("input_steps"), label="source.input_steps")
    if step_index < 0 or step_index >= len(source_steps):
        raise AttentionKvBoundedSoftmaxTableInputError("input step index outside source sequence")
    item = source_steps[step_index]
    if not isinstance(item, dict):
        raise AttentionKvBoundedSoftmaxTableInputError(f"source.input_steps[{step_index}] must be an object")
    token_position = require_int(item.get("token_position"), f"source.input_steps[{step_index}].token_position")
    if token_position != INITIAL_KV_ITEMS + step_index:
        raise AttentionKvBoundedSoftmaxTableInputError("source input step token position drift")
    return {
        ...
        "query": widen_vector(
            ...
            role_bias=23,
            label=f"source.input_steps[{step_index}].query",
        ),
        "new_key": widen_vector(
            ...
            local_step=step_index,
            role_bias=29,
            label=f"source.input_steps[{step_index}].new_key",
        ),
        "new_value": widen_vector(
            ...
            local_step=step_index,
            role_bias=31,
            label=f"source.input_steps[{step_index}].new_value",
        ),
    }
