...
- raw proof-byte fused-vs-split savings from the checked route matrix;
...
RESULT = "THIRTY_ATTENTION_ROUTE_ROWS_SCALE_RAW_PROOF_PRESSURE_WITH_D256_WIDTH_STRESS_POINT"
...
OPEN_FOLLOWUPS = (
    {
        ...
        "status": "OPEN_NEEDED",
        "reason": "Issue #715 asked for d64/d128/d256 where feasible. The checked attention route matrix now has one d256 two-head seq32 width-stress row, but d256 seq64 and d64/d128/d256 typed component-grid accounting are still missing.",
        "go_gate": "add a d256 seq64 attention route row or typed d64/d128/d256 component accounting without changing the claim boundary",
    },
    {
        "id": "seq64_attention_grid",
        "status": "OPEN_NEEDED",
        "reason": "d64 two-head, d64 four-head, d128 two-head, and d128 four-head seq64 rows are present, but the seq64 grid is not complete across width/head axes.",
        "go_gate": "produce the next checked seq64 row or record a concrete resource/no-go blocker with proof-byte attribution",
    },
    {
...
def build_route_matrix_signal(route_payload: dict[str, Any]) -> dict[str, Any]:
    ...
    d128_seq64_four_head = _row_by_id(rows, "d128_four_head_seq64")
    d256_seq32_two_head = _row_by_id(rows, "d256_two_head_seq32")
    ...
    d64_seq16_head_extension = require_dict(
    ...
    d64_two_head_sequence_extension = require_dict(
        axis_summary.get("combined_width_head_sequence_axis_d64_sequence_extension"),
        "d64 two-head sequence extension summary",
    )
    d128_width_frontier = require_dict(
        axis_summary.get("combined_width_head_sequence_axis_d128_seq32_width_frontier"),
        "d128 seq32 width frontier summary",
    )
    d128_sequence_frontier = require_dict(
        axis_summary.get("combined_width_head_sequence_axis_d128_sequence_frontier"),
        "d128 sequence frontier summary",
    )
    d128_seq32_head_frontier = require_dict(
        axis_summary.get("combined_width_head_sequence_axis_d128_seq32_head_frontier"),
        "d128 seq32 head frontier summary",
    )
    d128_four_head_sequence_frontier = require_dict(
        axis_summary.get("combined_width_head_sequence_axis_d128_four_head_sequence_frontier"),
        "d128 four-head sequence frontier summary",
    )
    d128_seq64_width_frontier = require_dict(
    ...
    d256_seq32_width_stress = require_dict(
        axis_summary.get("combined_width_head_sequence_axis_d256_seq32_width_stress"),
        "d256 seq32 width-stress summary",
    )

    return {
        "status": "GO_30_ROW_RAW_PROOF_ROUTE_MATRIX_WITH_D256_SEQ32_WIDTH_STRESS_POINT",
        ...
        "d64_seq16_head_extension": {
            ...
            "two_to_four_trace_row_growth": ratio_string(
                int_field(d64_seq16_four_head.get("trace_rows"), "d64 seq16 four-head trace rows"),
                int_field(d64_seq16_two_head.get("trace_rows"), "d64 seq16 two-head trace rows"),
            ),
            "two_to_four_fused_raw_proof_growth": ratio_string(
                int_field(d64_seq16_four_head.get("fused_proof_size_bytes"), "d64 seq16 four-head fused bytes"),
                int_field(d64_seq16_two_head.get("fused_proof_size_bytes"), "d64 seq16 two-head fused bytes"),
            ),
            "two_to_four_split_raw_proof_growth": ratio_string(
                int_field(
                    d64_seq16_four_head.get("source_plus_sidecar_raw_proof_bytes"),
                    "d64 seq16 four-head split bytes",
                ),
            ...
            "two_to_four_saving_growth": ratio_string(
                int_field(
                    d64_seq16_four_head.get("fused_saves_vs_source_plus_sidecar_bytes"),
                    "d64 seq16 four-head saving",
                ),
        ...
        "d64_two_head_seq64_decision_gate": {
            "profile_ids": list(d64_two_head_sequence_extension.get("profile_ids", [])),
            "seq32_lookup_claims": int_field(d64_seq32_two_head.get("lookup_claims"), "d64 seq32 two-head lookups"),
            "seq64_lookup_claims": int_field(d64_seq64_two_head.get("lookup_claims"), "d64 seq64 two-head lookups"),
            "seq32_trace_rows": int_field(d64_seq32_two_head.get("trace_rows"), "d64 seq32 two-head trace rows"),
            "seq64_trace_rows": int_field(d64_seq64_two_head.get("trace_rows"), "d64 seq64 two-head trace rows"),
            "seq32_source_raw_proof_bytes": int_field(
                d64_seq32_two_head.get("source_proof_size_bytes"), "d64 seq32 two-head source proof bytes"
            ),
            "seq64_source_raw_proof_bytes": int_field(
                d64_seq64_two_head.get("source_proof_size_bytes"), "d64 seq64 two-head source proof bytes"
            ),
            "seq32_sidecar_raw_proof_bytes": int_field(
                d64_seq32_two_head.get("sidecar_proof_size_bytes"), "d64 seq32 two-head sidecar proof bytes"
            ),
            "seq64_sidecar_raw_proof_bytes": int_field(
                d64_seq64_two_head.get("sidecar_proof_size_bytes"), "d64 seq64 two-head sidecar proof bytes"
            ),
            "seq32_fused_raw_proof_bytes": int_field(
                d64_seq32_two_head.get("fused_proof_size_bytes"), "d64 seq32 two-head fused proof bytes"
            ),
            "seq64_fused_raw_proof_bytes": int_field(
                d64_seq64_two_head.get("fused_proof_size_bytes"), "d64 seq64 two-head fused proof bytes"
            ),
            "seq64_source_plus_sidecar_raw_proof_bytes": int_field(
                d64_seq64_two_head.get("source_plus_sidecar_raw_proof_bytes"), "d64 seq64 two-head split proof bytes"
            ),
            "seq64_raw_saving_bytes": int_field(
                d64_seq64_two_head.get("fused_saves_vs_source_plus_sidecar_bytes"), "d64 seq64 two-head raw saving"
            ),
            "seq64_fused_to_split_ratio": d64_seq64_two_head.get("fused_to_source_plus_sidecar_ratio"),
            "seq32_to_seq64_lookup_claim_growth": ratio_string(
                int_field(d64_seq64_two_head.get("lookup_claims"), "d64 seq64 two-head lookups"),
                int_field(d64_seq32_two_head.get("lookup_claims"), "d64 seq32 two-head lookups"),
            ),
            "seq32_to_seq64_trace_row_growth": ratio_string(
                int_field(d64_seq64_two_head.get("trace_rows"), "d64 seq64 two-head trace rows"),
                int_field(d64_seq32_two_head.get("trace_rows"), "d64 seq32 two-head trace rows"),
            ),
            "seq32_to_seq64_source_raw_proof_growth": ratio_string(
                int_field(d64_seq64_two_head.get("source_proof_size_bytes"), "d64 seq64 two-head source bytes"),
                int_field(d64_seq32_two_head.get("source_proof_size_bytes"), "d64 seq32 two-head source bytes"),
            ),
            "seq32_to_seq64_sidecar_raw_proof_growth": ratio_string(
                int_field(d64_seq64_two_head.get("sidecar_proof_size_bytes"), "d64 seq64 two-head sidecar bytes"),
                int_field(d64_seq32_two_head.get("sidecar_proof_size_bytes"), "d64 seq32 two-head sidecar bytes"),
            ),
            "seq32_to_seq64_fused_raw_proof_growth": ratio_string(
                int_field(d64_seq64_two_head.get("fused_proof_size_bytes"), "d64 seq64 two-head fused bytes"),
                int_field(d64_seq32_two_head.get("fused_proof_size_bytes"), "d64 seq32 two-head fused bytes"),
            ),
            "seq32_to_seq64_split_raw_proof_growth": ratio_string(
                int_field(
                    d64_seq64_two_head.get("source_plus_sidecar_raw_proof_bytes"),
                    "d64 seq64 two-head split bytes",
                ),
                int_field(
                    d64_seq32_two_head.get("source_plus_sidecar_raw_proof_bytes"),
                    "d64 seq32 two-head split bytes",
                ),
            ),
            "seq32_to_seq64_saving_growth": ratio_string(
                int_field(
                    d64_seq64_two_head.get("fused_saves_vs_source_plus_sidecar_bytes"),
                    "d64 seq64 two-head saving",
                ),
                int_field(
                    d64_seq32_two_head.get("fused_saves_vs_source_plus_sidecar_bytes"),
                    "d64 seq32 two-head saving",
                ),
            ),
        },
        ...
        "d128_two_head_seq32_width_frontier": {
            "profile_ids": list(d128_width_frontier.get("profile_ids", [])),
            "d64_lookup_claims": int_field(d64_seq32_two_head.get("lookup_claims"), "d64 seq32 lookups"),
            "d128_lookup_claims": int_field(d128_seq32_two_head.get("lookup_claims"), "d128 seq32 lookups"),
            "d64_trace_rows": int_field(d64_seq32_two_head.get("trace_rows"), "d64 seq32 trace rows"),
            "d128_trace_rows": int_field(d128_seq32_two_head.get("trace_rows"), "d128 seq32 trace rows"),
            "d64_source_raw_proof_bytes": int_field(
                d64_seq32_two_head.get("source_proof_size_bytes"), "d64 seq32 source proof bytes"
            ),
            "d128_source_raw_proof_bytes": int_field(
                d128_seq32_two_head.get("source_proof_size_bytes"), "d128 seq32 source proof bytes"
            ),
            "d64_sidecar_raw_proof_bytes": int_field(
                d64_seq32_two_head.get("sidecar_proof_size_bytes"), "d64 seq32 sidecar proof bytes"
            ),
            "d128_sidecar_raw_proof_bytes": int_field(
                d128_seq32_two_head.get("sidecar_proof_size_bytes"), "d128 seq32 sidecar proof bytes"
            ),
            "d64_fused_raw_proof_bytes": int_field(
                d64_seq32_two_head.get("fused_proof_size_bytes"), "d64 seq32 fused proof bytes"
            ),
            "d128_fused_raw_proof_bytes": int_field(
                d128_seq32_two_head.get("fused_proof_size_bytes"), "d128 seq32 fused proof bytes"
            ),
            "d128_source_plus_sidecar_raw_proof_bytes": int_field(
                d128_seq32_two_head.get("source_plus_sidecar_raw_proof_bytes"), "d128 seq32 split proof bytes"
            ),
            "d128_raw_saving_bytes": int_field(
                d128_seq32_two_head.get("fused_saves_vs_source_plus_sidecar_bytes"), "d128 seq32 raw saving"
            ),
            "d128_fused_to_split_ratio": d128_seq32_two_head.get("fused_to_source_plus_sidecar_ratio"),
            "d64_to_d128_lookup_claim_growth": ratio_string(
                int_field(d128_seq32_two_head.get("lookup_claims"), "d128 seq32 lookups"),
                int_field(d64_seq32_two_head.get("lookup_claims"), "d64 seq32 lookups"),
            ),
            "d64_to_d128_trace_row_growth": ratio_string(
                int_field(d128_seq32_two_head.get("trace_rows"), "d128 seq32 trace rows"),
                int_field(d64_seq32_two_head.get("trace_rows"), "d64 seq32 trace rows"),
            ),
            "d64_to_d128_source_raw_proof_growth": ratio_string(
                int_field(d128_seq32_two_head.get("source_proof_size_bytes"), "d128 seq32 source bytes"),
                int_field(d64_seq32_two_head.get("source_proof_size_bytes"), "d64 seq32 source bytes"),
            ),
            "d64_to_d128_sidecar_raw_proof_growth": ratio_string(
                int_field(d128_seq32_two_head.get("sidecar_proof_size_bytes"), "d128 seq32 sidecar bytes"),
                int_field(d64_seq32_two_head.get("sidecar_proof_size_bytes"), "d64 seq32 sidecar bytes"),
            ),
            "d64_to_d128_fused_raw_proof_growth": ratio_string(
                int_field(d128_seq32_two_head.get("fused_proof_size_bytes"), "d128 seq32 fused bytes"),
                int_field(d64_seq32_two_head.get("fused_proof_size_bytes"), "d64 seq32 fused bytes"),
            ),
            "d64_to_d128_split_raw_proof_growth": ratio_string(
                int_field(
                    d128_seq32_two_head.get("source_plus_sidecar_raw_proof_bytes"),
                    "d128 seq32 split bytes",
                ),
                int_field(
                    d64_seq32_two_head.get("source_plus_sidecar_raw_proof_bytes"),
                    "d64 seq32 split bytes",
            ...
            "d64_to_d128_saving_growth": ratio_string(
                int_field(
                    d128_seq32_two_head.get("fused_saves_vs_source_plus_sidecar_bytes"),
                    "d128 seq32 saving",
                ),
                int_field(
                    d64_seq32_two_head.get("fused_saves_vs_source_plus_sidecar_bytes"),
        ...
        "d128_two_head_seq64_sequence_frontier": {
            "profile_ids": list(d128_sequence_frontier.get("profile_ids", [])),
            "seq32_lookup_claims": int_field(d128_seq32_two_head.get("lookup_claims"), "d128 seq32 lookups"),
            "seq64_lookup_claims": int_field(d128_seq64_two_head.get("lookup_claims"), "d128 seq64 lookups"),
            "seq32_trace_rows": int_field(d128_seq32_two_head.get("trace_rows"), "d128 seq32 trace rows"),
            "seq64_trace_rows": int_field(d128_seq64_two_head.get("trace_rows"), "d128 seq64 trace rows"),
            "seq32_source_raw_proof_bytes": int_field(
                d128_seq32_two_head.get("source_proof_size_bytes"), "d128 seq32 source proof bytes"
            ),
            "seq64_source_raw_proof_bytes": int_field(
                d128_seq64_two_head.get("source_proof_size_bytes"), "d128 seq64 source proof bytes"
            ),
            "seq32_sidecar_raw_proof_bytes": int_field(
                d128_seq32_two_head.get("sidecar_proof_size_bytes"), "d128 seq32 sidecar proof bytes"
            ),
            "seq64_sidecar_raw_proof_bytes": int_field(
                d128_seq64_two_head.get("sidecar_proof_size_bytes"), "d128 seq64 sidecar proof bytes"
            ),
            "seq32_fused_raw_proof_bytes": int_field(
                d128_seq32_two_head.get("fused_proof_size_bytes"), "d128 seq32 fused proof bytes"
            ),
            "seq64_fused_raw_proof_bytes": int_field(
                d128_seq64_two_head.get("fused_proof_size_bytes"), "d128 seq64 fused proof bytes"
            ),
            "seq64_source_plus_sidecar_raw_proof_bytes": int_field(
                d128_seq64_two_head.get("source_plus_sidecar_raw_proof_bytes"), "d128 seq64 split proof bytes"
            ),
            "seq64_raw_saving_bytes": int_field(
                d128_seq64_two_head.get("fused_saves_vs_source_plus_sidecar_bytes"), "d128 seq64 raw saving"
            ),
            "seq64_fused_to_split_ratio": d128_seq64_two_head.get("fused_to_source_plus_sidecar_ratio"),
            "seq32_to_seq64_lookup_claim_growth": ratio_string(
                int_field(d128_seq64_two_head.get("lookup_claims"), "d128 seq64 lookups"),
                int_field(d128_seq32_two_head.get("lookup_claims"), "d128 seq32 lookups"),
            ),
            "seq32_to_seq64_trace_row_growth": ratio_string(
                int_field(d128_seq64_two_head.get("trace_rows"), "d128 seq64 trace rows"),
                int_field(d128_seq32_two_head.get("trace_rows"), "d128 seq32 trace rows"),
            ),
            "seq32_to_seq64_source_raw_proof_growth": ratio_string(
                int_field(d128_seq64_two_head.get("source_proof_size_bytes"), "d128 seq64 source bytes"),
                int_field(d128_seq32_two_head.get("source_proof_size_bytes"), "d128 seq32 source bytes"),
            ),
            "seq32_to_seq64_sidecar_raw_proof_growth": ratio_string(
                int_field(d128_seq64_two_head.get("sidecar_proof_size_bytes"), "d128 seq64 sidecar bytes"),
                int_field(d128_seq32_two_head.get("sidecar_proof_size_bytes"), "d128 seq32 sidecar bytes"),
            ),
            "seq32_to_seq64_fused_raw_proof_growth": ratio_string(
                int_field(d128_seq64_two_head.get("fused_proof_size_bytes"), "d128 seq64 fused bytes"),
                int_field(d128_seq32_two_head.get("fused_proof_size_bytes"), "d128 seq32 fused bytes"),
            ),
            "seq32_to_seq64_split_raw_proof_growth": ratio_string(
                int_field(
                    d128_seq64_two_head.get("source_plus_sidecar_raw_proof_bytes"),
                    "d128 seq64 split bytes",
                ),
                int_field(
                    d128_seq32_two_head.get("source_plus_sidecar_raw_proof_bytes"),
                    "d128 seq32 split bytes",
            ...
            "seq32_to_seq64_saving_growth": ratio_string(
                int_field(
                    d128_seq64_two_head.get("fused_saves_vs_source_plus_sidecar_bytes"),
                    "d128 seq64 saving",
                ),
                int_field(
                    d128_seq32_two_head.get("fused_saves_vs_source_plus_sidecar_bytes"),
        ...
        "d128_four_head_seq32_head_frontier": {
            "profile_ids": list(d128_seq32_head_frontier.get("profile_ids", [])),
            "two_head_lookup_claims": int_field(
                d128_seq32_two_head.get("lookup_claims"), "d128 seq32 two-head lookups"
            ),
            "four_head_lookup_claims": int_field(
                d128_seq32_four_head.get("lookup_claims"), "d128 seq32 four-head lookups"
            ),
            "two_head_trace_rows": int_field(
                d128_seq32_two_head.get("trace_rows"), "d128 seq32 two-head trace rows"
            ),
            "four_head_trace_rows": int_field(
                d128_seq32_four_head.get("trace_rows"), "d128 seq32 four-head trace rows"
            ),
            "two_head_source_raw_proof_bytes": int_field(
                d128_seq32_two_head.get("source_proof_size_bytes"), "d128 seq32 two-head source proof bytes"
            ),
            "four_head_source_raw_proof_bytes": int_field(
                d128_seq32_four_head.get("source_proof_size_bytes"), "d128 seq32 four-head source proof bytes"
            ),
            "two_head_sidecar_raw_proof_bytes": int_field(
                d128_seq32_two_head.get("sidecar_proof_size_bytes"), "d128 seq32 two-head sidecar proof bytes"
            ),
            "four_head_sidecar_raw_proof_bytes": int_field(
                d128_seq32_four_head.get("sidecar_proof_size_bytes"), "d128 seq32 four-head sidecar proof bytes"
            ),
            "two_head_fused_raw_proof_bytes": int_field(
                d128_seq32_two_head.get("fused_proof_size_bytes"), "d128 seq32 two-head fused proof bytes"
            ),
            "four_head_fused_raw_proof_bytes": int_field(
                d128_seq32_four_head.get("fused_proof_size_bytes"), "d128 seq32 four-head fused proof bytes"
            ),
            "four_head_source_plus_sidecar_raw_proof_bytes": int_field(
                d128_seq32_four_head.get("source_plus_sidecar_raw_proof_bytes"),
                "d128 seq32 four-head split proof bytes",
            ),
            "four_head_raw_saving_bytes": int_field(
                d128_seq32_four_head.get("fused_saves_vs_source_plus_sidecar_bytes"),
                "d128 seq32 four-head raw saving",
            ),
            "four_head_fused_to_split_ratio": d128_seq32_four_head.get("fused_to_source_plus_sidecar_ratio"),
            "two_to_four_lookup_claim_growth": ratio_string(
                int_field(d128_seq32_four_head.get("lookup_claims"), "d128 seq32 four-head lookups"),
                int_field(d128_seq32_two_head.get("lookup_claims"), "d128 seq32 two-head lookups"),
            ),
            "two_to_four_trace_row_growth": ratio_string(
                int_field(d128_seq32_four_head.get("trace_rows"), "d128 seq32 four-head trace rows"),
                int_field(d128_seq32_two_head.get("trace_rows"), "d128 seq32 two-head trace rows"),
            ),
            "two_to_four_source_raw_proof_growth": ratio_string(
                int_field(d128_seq32_four_head.get("source_proof_size_bytes"), "d128 seq32 four-head source bytes"),
                int_field(d128_seq32_two_head.get("source_proof_size_bytes"), "d128 seq32 two-head source bytes"),
            ),
            "two_to_four_sidecar_raw_proof_growth": ratio_string(
                int_field(d128_seq32_four_head.get("sidecar_proof_size_bytes"), "d128 seq32 four-head sidecar bytes"),
                int_field(d128_seq32_two_head.get("sidecar_proof_size_bytes"), "d128 seq32 two-head sidecar bytes"),
            ),
            "two_to_four_fused_raw_proof_growth": ratio_string(
                int_field(d128_seq32_four_head.get("fused_proof_size_bytes"), "d128 seq32 four-head fused bytes"),
                int_field(d128_seq32_two_head.get("fused_proof_size_bytes"), "d128 seq32 two-head fused bytes"),
            ),
            "two_to_four_split_raw_proof_growth": ratio_string(
                int_field(
                    d128_seq32_four_head.get("source_plus_sidecar_raw_proof_bytes"),
                    "d128 seq32 four-head split bytes",
                ),
                int_field(
                    d128_seq32_two_head.get("source_plus_sidecar_raw_proof_bytes"),
                    "d128 seq32 two-head split bytes",
                ),
            ),
            "two_to_four_saving_growth": ratio_string(
                int_field(
                    d128_seq32_four_head.get("fused_saves_vs_source_plus_sidecar_bytes"),
                    "d128 seq32 four-head saving",
                ),
                int_field(
                    d128_seq32_two_head.get("fused_saves_vs_source_plus_sidecar_bytes"),
                    "d128 seq32 two-head saving",
                ),
            ),
        },
        "d128_four_head_seq64_decision_gate": {
            "profile_ids": list(d128_four_head_sequence_frontier.get("profile_ids", [])),
            "seq32_lookup_claims": int_field(d128_seq32_four_head.get("lookup_claims"), "d128 seq32 four-head lookups"),
            "seq64_lookup_claims": int_field(d128_seq64_four_head.get("lookup_claims"), "d128 seq64 four-head lookups"),
            "seq32_trace_rows": int_field(d128_seq32_four_head.get("trace_rows"), "d128 seq32 four-head trace rows"),
            "seq64_trace_rows": int_field(d128_seq64_four_head.get("trace_rows"), "d128 seq64 four-head trace rows"),
            "seq32_source_raw_proof_bytes": int_field(
                d128_seq32_four_head.get("source_proof_size_bytes"), "d128 seq32 four-head source proof bytes"
            ),
            "seq64_source_raw_proof_bytes": int_field(
                d128_seq64_four_head.get("source_proof_size_bytes"), "d128 seq64 four-head source proof bytes"
            ),
            "seq32_sidecar_raw_proof_bytes": int_field(
                d128_seq32_four_head.get("sidecar_proof_size_bytes"), "d128 seq32 four-head sidecar proof bytes"
            ),
            "seq64_sidecar_raw_proof_bytes": int_field(
                d128_seq64_four_head.get("sidecar_proof_size_bytes"), "d128 seq64 four-head sidecar proof bytes"
            ),
            "seq32_fused_raw_proof_bytes": int_field(
                d128_seq32_four_head.get("fused_proof_size_bytes"), "d128 seq32 four-head fused proof bytes"
            ),
            "seq64_fused_raw_proof_bytes": int_field(
                d128_seq64_four_head.get("fused_proof_size_bytes"), "d128 seq64 four-head fused proof bytes"
            ),
            "seq64_source_plus_sidecar_raw_proof_bytes": int_field(
                d128_seq64_four_head.get("source_plus_sidecar_raw_proof_bytes"), "d128 seq64 four-head split proof bytes"
            ),
            "seq64_raw_saving_bytes": int_field(
                d128_seq64_four_head.get("fused_saves_vs_source_plus_sidecar_bytes"), "d128 seq64 four-head raw saving"
            ),
            "seq64_fused_to_split_ratio": d128_seq64_four_head.get("fused_to_source_plus_sidecar_ratio"),
            "seq32_to_seq64_lookup_claim_growth": ratio_string(
                int_field(d128_seq64_four_head.get("lookup_claims"), "d128 seq64 four-head lookups"),
                int_field(d128_seq32_four_head.get("lookup_claims"), "d128 seq32 four-head lookups"),
            ),
            "seq32_to_seq64_trace_row_growth": ratio_string(
                int_field(d128_seq64_four_head.get("trace_rows"), "d128 seq64 four-head trace rows"),
                int_field(d128_seq32_four_head.get("trace_rows"), "d128 seq32 four-head trace rows"),
            ),
            "seq32_to_seq64_source_raw_proof_growth": ratio_string(
                int_field(d128_seq64_four_head.get("source_proof_size_bytes"), "d128 seq64 four-head source bytes"),
                int_field(d128_seq32_four_head.get("source_proof_size_bytes"), "d128 seq32 four-head source bytes"),
            ),
            "seq32_to_seq64_sidecar_raw_proof_growth": ratio_string(
                int_field(d128_seq64_four_head.get("sidecar_proof_size_bytes"), "d128 seq64 four-head sidecar bytes"),
                int_field(d128_seq32_four_head.get("sidecar_proof_size_bytes"), "d128 seq32 four-head sidecar bytes"),
            ),
            "seq32_to_seq64_fused_raw_proof_growth": ratio_string(
                int_field(d128_seq64_four_head.get("fused_proof_size_bytes"), "d128 seq64 four-head fused bytes"),
                int_field(d128_seq32_four_head.get("fused_proof_size_bytes"), "d128 seq32 four-head fused bytes"),
            ),
            "seq32_to_seq64_split_raw_proof_growth": ratio_string(
                int_field(
                    d128_seq64_four_head.get("source_plus_sidecar_raw_proof_bytes"),
                    "d128 seq64 four-head split bytes",
                ),
                int_field(
                    d128_seq32_four_head.get("source_plus_sidecar_raw_proof_bytes"),
                    "d128 seq32 four-head split bytes",
            ...
            "seq32_to_seq64_saving_growth": ratio_string(
                int_field(
                    d128_seq64_four_head.get("fused_saves_vs_source_plus_sidecar_bytes"),
                    "d128 seq64 four-head saving",
                ),
                int_field(
                    d128_seq32_four_head.get("fused_saves_vs_source_plus_sidecar_bytes"),
                    "d128 seq32 four-head saving",
        ...
        "d128_two_head_seq64_width_frontier": {
            "profile_ids": list(d128_seq64_width_frontier.get("profile_ids", [])),
            "d64_lookup_claims": int_field(d64_seq64_two_head.get("lookup_claims"), "d64 seq64 lookups"),
            "d128_lookup_claims": int_field(d128_seq64_two_head.get("lookup_claims"), "d128 seq64 lookups"),
            "d64_trace_rows": int_field(d64_seq64_two_head.get("trace_rows"), "d64 seq64 trace rows"),
            "d128_trace_rows": int_field(d128_seq64_two_head.get("trace_rows"), "d128 seq64 trace rows"),
            "d64_source_raw_proof_bytes": int_field(
                d64_seq64_two_head.get("source_proof_size_bytes"), "d64 seq64 source proof bytes"
            ),
            "d128_source_raw_proof_bytes": int_field(
                d128_seq64_two_head.get("source_proof_size_bytes"), "d128 seq64 source proof bytes"
            ),
            "d64_sidecar_raw_proof_bytes": int_field(
                d64_seq64_two_head.get("sidecar_proof_size_bytes"), "d64 seq64 sidecar proof bytes"
            ),
            "d128_sidecar_raw_proof_bytes": int_field(
                d128_seq64_two_head.get("sidecar_proof_size_bytes"), "d128 seq64 sidecar proof bytes"
            ),
            "d64_fused_raw_proof_bytes": int_field(
                d64_seq64_two_head.get("fused_proof_size_bytes"), "d64 seq64 fused proof bytes"
            ),
            "d128_fused_raw_proof_bytes": int_field(
                d128_seq64_two_head.get("fused_proof_size_bytes"), "d128 seq64 fused proof bytes"
            ),
            ...
            "d128_fused_to_split_ratio": d128_seq64_two_head.get("fused_to_source_plus_sidecar_ratio"),
            "d64_to_d128_lookup_claim_growth": ratio_string(
                int_field(d128_seq64_two_head.get("lookup_claims"), "d128 seq64 lookups"),
                int_field(d64_seq64_two_head.get("lookup_claims"), "d64 seq64 lookups"),
            ),
            "d64_to_d128_trace_row_growth": ratio_string(
                int_field(d128_seq64_two_head.get("trace_rows"), "d128 seq64 trace rows"),
                int_field(d64_seq64_two_head.get("trace_rows"), "d64 seq64 trace rows"),
            ),
            "d64_to_d128_source_raw_proof_growth": ratio_string(
                int_field(d128_seq64_two_head.get("source_proof_size_bytes"), "d128 seq64 source bytes"),
                int_field(d64_seq64_two_head.get("source_proof_size_bytes"), "d64 seq64 source bytes"),
            ),
            "d64_to_d128_sidecar_raw_proof_growth": ratio_string(
                int_field(d128_seq64_two_head.get("sidecar_proof_size_bytes"), "d128 seq64 sidecar bytes"),
                int_field(d64_seq64_two_head.get("sidecar_proof_size_bytes"), "d64 seq64 sidecar bytes"),
            ),
            "d64_to_d128_fused_raw_proof_growth": ratio_string(
                int_field(d128_seq64_two_head.get("fused_proof_size_bytes"), "d128 seq64 fused bytes"),
                int_field(d64_seq64_two_head.get("fused_proof_size_bytes"), "d64 seq64 fused bytes"),
            ),
            "d64_to_d128_split_raw_proof_growth": ratio_string(
                int_field(
                    d128_seq64_two_head.get("source_plus_sidecar_raw_proof_bytes"),
        ...
        "d256_two_head_seq32_width_stress": {
            "profile_ids": list(d256_seq32_width_stress.get("profile_ids", [])),
            "key_widths": list(d256_seq32_width_stress.get("key_widths", [])),
            "lookup_claims": list(d256_seq32_width_stress.get("lookup_claims", [])),
            "trace_rows": list(d256_seq32_width_stress.get("trace_rows", [])),
            "source_proof_size_bytes": list(d256_seq32_width_stress.get("source_proof_size_bytes", [])),
            "sidecar_proof_size_bytes": list(d256_seq32_width_stress.get("sidecar_proof_size_bytes", [])),
            "fused_proof_size_bytes": list(d256_seq32_width_stress.get("fused_proof_size_bytes", [])),
            "source_plus_sidecar_raw_proof_bytes": list(
                d256_seq32_width_stress.get("source_plus_sidecar_raw_proof_bytes", [])
            ),
            "fused_to_source_plus_sidecar_ratios": list(
                d256_seq32_width_stress.get("fused_to_source_plus_sidecar_ratios", [])
            ),
            "d256_lookup_claims": int_field(d256_seq32_two_head.get("lookup_claims"), "d256 seq32 lookups"),
            "d256_trace_rows": int_field(d256_seq32_two_head.get("trace_rows"), "d256 seq32 trace rows"),
            "d256_source_raw_proof_bytes": int_field(
                d256_seq32_two_head.get("source_proof_size_bytes"), "d256 seq32 source proof bytes"
            ),
            "d256_sidecar_raw_proof_bytes": int_field(
                d256_seq32_two_head.get("sidecar_proof_size_bytes"), "d256 seq32 sidecar proof bytes"
            ),
            "d256_fused_raw_proof_bytes": int_field(
                d256_seq32_two_head.get("fused_proof_size_bytes"), "d256 seq32 fused proof bytes"
            ),
            "d256_source_plus_sidecar_raw_proof_bytes": int_field(
                d256_seq32_two_head.get("source_plus_sidecar_raw_proof_bytes"),
                "d256 seq32 split proof bytes",
            ),
            "d256_raw_saving_bytes": int_field(
                d256_seq32_two_head.get("fused_saves_vs_source_plus_sidecar_bytes"), "d256 seq32 raw saving"
            ),
            "d256_fused_to_split_ratio": d256_seq32_two_head.get("fused_to_source_plus_sidecar_ratio"),
            "d128_to_d256_key_width_growth": ratio_string(
                int_field(d256_seq32_two_head.get("key_width"), "d256 seq32 width"),
                int_field(d128_seq32_two_head.get("key_width"), "d128 seq32 width"),
            ),
            "d128_to_d256_lookup_claim_growth": ratio_string(
                int_field(d256_seq32_two_head.get("lookup_claims"), "d256 seq32 lookups"),
                int_field(d128_seq32_two_head.get("lookup_claims"), "d128 seq32 lookups"),
            ),
            "d128_to_d256_trace_row_growth": ratio_string(
                int_field(d256_seq32_two_head.get("trace_rows"), "d256 seq32 trace rows"),
                int_field(d128_seq32_two_head.get("trace_rows"), "d128 seq32 trace rows"),
            ),
            "d128_to_d256_source_raw_proof_growth": ratio_string(
                int_field(d256_seq32_two_head.get("source_proof_size_bytes"), "d256 seq32 source bytes"),
                int_field(d128_seq32_two_head.get("source_proof_size_bytes"), "d128 seq32 source bytes"),
            ),
            "d128_to_d256_sidecar_raw_proof_growth": ratio_string(
                int_field(d256_seq32_two_head.get("sidecar_proof_size_bytes"), "d256 seq32 sidecar bytes"),
                int_field(d128_seq32_two_head.get("sidecar_proof_size_bytes"), "d128 seq32 sidecar bytes"),
            ),
            "d128_to_d256_fused_raw_proof_growth": ratio_string(
                int_field(d256_seq32_two_head.get("fused_proof_size_bytes"), "d256 seq32 fused bytes"),
                int_field(d128_seq32_two_head.get("fused_proof_size_bytes"), "d128 seq32 fused bytes"),
            ),
            "d128_to_d256_split_raw_proof_growth": ratio_string(
                int_field(
                    d256_seq32_two_head.get("source_plus_sidecar_raw_proof_bytes"),
                    "d256 seq32 split bytes",
                ),
                int_field(
                    d128_seq32_two_head.get("source_plus_sidecar_raw_proof_bytes"),
                    "d128 seq32 split bytes",
            ...
            "d128_to_d256_saving_growth": ratio_string(
                int_field(
                    d256_seq32_two_head.get("fused_saves_vs_source_plus_sidecar_bytes"),
                    "d256 seq32 saving",
                ),
                int_field(
                    d128_seq32_two_head.get("fused_saves_vs_source_plus_sidecar_bytes"),
...
def build_summary(
    ...
    return {
        ...
        "d64_seq16_two_to_four_lookup_growth": require_dict(
            route_signal.get("d64_seq16_head_extension"), "d64 seq16 route signal"
        )["two_to_four_lookup_claim_growth"],
        ...
        "d128_seq32_raw_saving_bytes": int_field(
            ...
            "d128 seq32 raw saving",
        ),
        "d64_to_d128_seq32_fused_raw_proof_growth": require_dict(
            route_signal.get("d128_two_head_seq32_width_frontier"), "d128 route signal"
        )["d64_to_d128_fused_raw_proof_growth"],
        "d128_seq64_raw_saving_bytes": int_field(
            require_dict(route_signal.get("d128_two_head_seq64_sequence_frontier"), "d128 seq64 route signal").get(
                "seq64_raw_saving_bytes"
            ),
            "d128 seq64 raw saving",
        ),
        "d128_seq32_to_seq64_lookup_growth": require_dict(
            route_signal.get("d128_two_head_seq64_sequence_frontier"), "d128 seq64 route signal"
        )["seq32_to_seq64_lookup_claim_growth"],
        "d128_seq32_to_seq64_fused_raw_proof_growth": require_dict(
            route_signal.get("d128_two_head_seq64_sequence_frontier"), "d128 seq64 route signal"
        )["seq32_to_seq64_fused_raw_proof_growth"],
        "d128_four_head_seq32_raw_saving_bytes": int_field(
            require_dict(route_signal.get("d128_four_head_seq32_head_frontier"), "d128 head route signal").get(
                "four_head_raw_saving_bytes"
            ),
            "d128 four-head seq32 raw saving",
        ),
        "d128_seq32_two_to_four_lookup_growth": require_dict(
            route_signal.get("d128_four_head_seq32_head_frontier"), "d128 head route signal"
        )["two_to_four_lookup_claim_growth"],
        "d128_seq32_two_to_four_fused_raw_proof_growth": require_dict(
            route_signal.get("d128_four_head_seq32_head_frontier"), "d128 head route signal"
        )["two_to_four_fused_raw_proof_growth"],
        "d128_four_head_seq64_raw_saving_bytes": int_field(
            require_dict(route_signal.get("d128_four_head_seq64_decision_gate"), "d128 four-head seq64 signal").get(
                "seq64_raw_saving_bytes"
            ),
            "d128 four-head seq64 raw saving",
        ),
        "d128_four_head_seq32_to_seq64_lookup_growth": require_dict(
            route_signal.get("d128_four_head_seq64_decision_gate"), "d128 four-head seq64 signal"
        )["seq32_to_seq64_lookup_claim_growth"],
        "d128_four_head_seq32_to_seq64_fused_raw_proof_growth": require_dict(
            route_signal.get("d128_four_head_seq64_decision_gate"), "d128 four-head seq64 signal"
        )["seq32_to_seq64_fused_raw_proof_growth"],
        "d64_to_d128_seq64_fused_raw_proof_growth": require_dict(
            route_signal.get("d128_two_head_seq64_width_frontier"), "d128 seq64 width route signal"
        )["d64_to_d128_fused_raw_proof_growth"],
        "d256_seq32_raw_saving_bytes": int_field(
            require_dict(route_signal.get("d256_two_head_seq32_width_stress"), "d256 seq32 width-stress signal").get(
                "d256_raw_saving_bytes"
            ),
            "d256 seq32 raw saving",
        ),
        "d128_to_d256_seq32_lookup_growth": require_dict(
            route_signal.get("d256_two_head_seq32_width_stress"), "d256 seq32 width-stress signal"
        )["d128_to_d256_lookup_claim_growth"],
        "d128_to_d256_seq32_trace_row_growth": require_dict(
            route_signal.get("d256_two_head_seq32_width_stress"), "d256 seq32 width-stress signal"
        )["d128_to_d256_trace_row_growth"],
        "d128_to_d256_seq32_fused_raw_proof_growth": require_dict(
            route_signal.get("d256_two_head_seq32_width_stress"), "d256 seq32 width-stress signal"
        )["d128_to_d256_fused_raw_proof_growth"],
...
def mutation_cases(payload: dict[str, Any]) -> list[tuple[str, dict[str, Any]]]:
    ...
    def mutate_d128_width_frontier_signal(item: dict[str, Any]) -> None:
        item["route_matrix_signal"]["d128_two_head_seq32_width_frontier"]["d128_raw_saving_bytes"] = 32_387

    def mutate_d128_sequence_frontier_signal(item: dict[str, Any]) -> None:
        item["route_matrix_signal"]["d128_two_head_seq64_sequence_frontier"]["seq64_raw_saving_bytes"] = 40_316

    def mutate_d128_head_frontier_signal(item: dict[str, Any]) -> None:
        item["route_matrix_signal"]["d128_four_head_seq32_head_frontier"]["four_head_raw_saving_bytes"] = 39_303
    ...
    def mutate_d256_seq32_width_stress_signal(item: dict[str, Any]) -> None:
        item["route_matrix_signal"]["d256_two_head_seq32_width_stress"]["d256_raw_saving_bytes"] = 30_142
    ...
    def mutate_summary_d256_seq32_width_stress_signal(item: dict[str, Any]) -> None:
        item["summary"]["d256_seq32_raw_saving_bytes"] = 30_142
    ...
    mutations: tuple[tuple[str, Callable[[dict[str, Any]], None]], ...] = (
        ...
        ("d256_seq32_width_stress_signal_drift", mutate_d256_seq32_width_stress_signal),
        ...
        ("summary_d256_seq32_width_stress_signal_drift", mutate_summary_d256_seq32_width_stress_signal),
...
def validate_route_matrix_signal(payload: dict[str, Any]) -> None:
    ...
    if (
        signal.get("status")
        != "GO_30_ROW_RAW_PROOF_ROUTE_MATRIX_WITH_D256_SEQ32_WIDTH_STRESS_POINT"
    ):
        raise ProofPressureScalingClaimPackError("route matrix status drift")
    if signal.get("profiles_checked") != 30 or signal.get("matched_comparator_profiles") != 30:
        raise ProofPressureScalingClaimPackError("route matrix profile count drift")
    if signal.get("widths") != [8, 16, 32, 64, 128, 256]:
    ...
    if signal.get("total_lookup_claims") != 45_652 or signal.get("total_trace_rows") != 80_704:
        raise ProofPressureScalingClaimPackError("route matrix work total drift")
    if signal.get("fused_raw_proof_bytes_total") != 6_397_632:
        raise ProofPressureScalingClaimPackError("route matrix fused total drift")
    if signal.get("source_plus_sidecar_raw_proof_bytes_total") != 7_164_515:
        raise ProofPressureScalingClaimPackError("route matrix split total drift")
    if signal.get("raw_proof_savings_bytes_total") != 766_883:
    ...
    if signal.get("max_fused_to_split_ratio") != 0.964602:
    ...
    if d64_head.get("two_to_four_trace_row_growth") != "2.000000":
        raise ProofPressureScalingClaimPackError("d64 seq16 trace growth drift")
    if d64_head.get("two_to_four_fused_raw_proof_growth") != "0.996193":
        raise ProofPressureScalingClaimPackError("d64 seq16 fused raw growth drift")
    if d64_head.get("two_to_four_split_raw_proof_growth") != "1.011485":
    ...
    if d64_two_seq64.get("seq32_lookup_claims") != 1_184 or d64_two_seq64.get("seq64_lookup_claims") != 4_416:
        raise ProofPressureScalingClaimPackError("d64 two-head seq64 lookup drift")
    if d64_two_seq64.get("seq32_trace_rows") != 2_048 or d64_two_seq64.get("seq64_trace_rows") != 8_192:
        raise ProofPressureScalingClaimPackError("d64 two-head seq64 trace row drift")
    if d64_two_seq64.get("seq32_source_raw_proof_bytes") != 248_702:
        raise ProofPressureScalingClaimPackError("d64 two-head seq32 source raw proof drift")
    if d64_two_seq64.get("seq64_source_raw_proof_bytes") != 264_403:
        raise ProofPressureScalingClaimPackError("d64 two-head seq64 source raw proof drift")
    if d64_two_seq64.get("seq32_sidecar_raw_proof_bytes") != 36_400:
        raise ProofPressureScalingClaimPackError("d64 two-head seq32 sidecar raw proof drift")
    if d64_two_seq64.get("seq64_sidecar_raw_proof_bytes") != 42_567:
        raise ProofPressureScalingClaimPackError("d64 two-head seq64 sidecar raw proof drift")
    if d64_two_seq64.get("seq32_fused_raw_proof_bytes") != 253_257:
        raise ProofPressureScalingClaimPackError("d64 two-head seq32 fused raw proof drift")
    if d64_two_seq64.get("seq64_fused_raw_proof_bytes") != 272_636:
        raise ProofPressureScalingClaimPackError("d64 two-head seq64 fused raw proof drift")
    if d64_two_seq64.get("seq64_source_plus_sidecar_raw_proof_bytes") != 306_970:
        raise ProofPressureScalingClaimPackError("d64 two-head seq64 split raw proof drift")
    if d64_two_seq64.get("seq64_raw_saving_bytes") != 34_334:
        raise ProofPressureScalingClaimPackError("d64 two-head seq64 raw saving drift")
    if d64_two_seq64.get("seq64_fused_to_split_ratio") != 0.888152:
        raise ProofPressureScalingClaimPackError("d64 two-head seq64 ratio drift")
    if d64_two_seq64.get("seq32_to_seq64_lookup_claim_growth") != "3.729730":
        raise ProofPressureScalingClaimPackError("d64 two-head seq64 lookup growth drift")
    if d64_two_seq64.get("seq32_to_seq64_trace_row_growth") != "4.000000":
        raise ProofPressureScalingClaimPackError("d64 two-head seq64 trace growth drift")
    if d64_two_seq64.get("seq32_to_seq64_source_raw_proof_growth") != "1.063132":
        raise ProofPressureScalingClaimPackError("d64 two-head seq64 source raw growth drift")
    if d64_two_seq64.get("seq32_to_seq64_sidecar_raw_proof_growth") != "1.169423":
        raise ProofPressureScalingClaimPackError("d64 two-head seq64 sidecar raw growth drift")
    if d64_two_seq64.get("seq32_to_seq64_fused_raw_proof_growth") != "1.076519":
        raise ProofPressureScalingClaimPackError("d64 two-head seq64 fused raw growth drift")
    if d64_two_seq64.get("seq32_to_seq64_split_raw_proof_growth") != "1.076702":
        raise ProofPressureScalingClaimPackError("d64 two-head seq64 split raw growth drift")
    if d64_two_seq64.get("seq32_to_seq64_saving_growth") != "1.078160":
        raise ProofPressureScalingClaimPackError("d64 two-head seq64 saving growth drift")
    ...
    d128_width = require_dict(signal.get("d128_two_head_seq32_width_frontier"), "d128 width frontier")
    if d128_width.get("profile_ids") != ["d32_two_head_seq32", "d64_two_head_seq32", "d128_two_head_seq32"]:
        raise ProofPressureScalingClaimPackError("d128 width frontier profile drift")
    if d128_width.get("d64_lookup_claims") != 1_184 or d128_width.get("d128_lookup_claims") != 1_184:
        raise ProofPressureScalingClaimPackError("d128 width frontier lookup drift")
    if d128_width.get("d64_trace_rows") != 2_048 or d128_width.get("d128_trace_rows") != 2_048:
        raise ProofPressureScalingClaimPackError("d128 width frontier trace row drift")
    if d128_width.get("d64_source_raw_proof_bytes") != 248_702:
        raise ProofPressureScalingClaimPackError("d128 width frontier d64 source drift")
    if d128_width.get("d128_source_raw_proof_bytes") != 443_266:
        raise ProofPressureScalingClaimPackError("d128 width frontier source drift")
    if d128_width.get("d64_sidecar_raw_proof_bytes") != 36_400:
        raise ProofPressureScalingClaimPackError("d128 width frontier d64 sidecar drift")
    if d128_width.get("d128_sidecar_raw_proof_bytes") != 35_010:
        raise ProofPressureScalingClaimPackError("d128 width frontier sidecar drift")
    if d128_width.get("d64_fused_raw_proof_bytes") != 253_257:
        raise ProofPressureScalingClaimPackError("d128 width frontier d64 fused drift")
    if d128_width.get("d128_fused_raw_proof_bytes") != 445_888:
        raise ProofPressureScalingClaimPackError("d128 width frontier fused drift")
    if d128_width.get("d128_source_plus_sidecar_raw_proof_bytes") != 478_276:
        raise ProofPressureScalingClaimPackError("d128 width frontier split drift")
    if d128_width.get("d128_raw_saving_bytes") != 32_388:
        raise ProofPressureScalingClaimPackError("d128 width frontier saving drift")
    if d128_width.get("d128_fused_to_split_ratio") != 0.932282:
        raise ProofPressureScalingClaimPackError("d128 width frontier ratio drift")
    if d128_width.get("d64_to_d128_lookup_claim_growth") != "1.000000":
        raise ProofPressureScalingClaimPackError("d128 width frontier lookup growth drift")
    if d128_width.get("d64_to_d128_trace_row_growth") != "1.000000":
        raise ProofPressureScalingClaimPackError("d128 width frontier trace growth drift")
    if d128_width.get("d64_to_d128_source_raw_proof_growth") != "1.782318":
        raise ProofPressureScalingClaimPackError("d128 width frontier source growth drift")
    if d128_width.get("d64_to_d128_sidecar_raw_proof_growth") != "0.961813":
        raise ProofPressureScalingClaimPackError("d128 width frontier sidecar growth drift")
    if d128_width.get("d64_to_d128_fused_raw_proof_growth") != "1.760615":
        raise ProofPressureScalingClaimPackError("d128 width frontier fused growth drift")
    if d128_width.get("d64_to_d128_split_raw_proof_growth") != "1.677561":
        raise ProofPressureScalingClaimPackError("d128 width frontier split growth drift")
    if d128_width.get("d64_to_d128_saving_growth") != "1.017051":
        raise ProofPressureScalingClaimPackError("d128 width frontier saving growth drift")
    d128_sequence = require_dict(
        signal.get("d128_two_head_seq64_sequence_frontier"), "d128 sequence frontier"
    )
    if d128_sequence.get("profile_ids") != ["d128_two_head_seq32", "d128_two_head_seq64"]:
        raise ProofPressureScalingClaimPackError("d128 sequence frontier profile drift")
    if d128_sequence.get("seq32_lookup_claims") != 1_184 or d128_sequence.get("seq64_lookup_claims") != 4_416:
        raise ProofPressureScalingClaimPackError("d128 sequence frontier lookup drift")
    if d128_sequence.get("seq32_trace_rows") != 2_048 or d128_sequence.get("seq64_trace_rows") != 8_192:
        raise ProofPressureScalingClaimPackError("d128 sequence frontier trace row drift")
    if d128_sequence.get("seq32_source_raw_proof_bytes") != 443_266:
        raise ProofPressureScalingClaimPackError("d128 sequence seq32 source raw proof drift")
    if d128_sequence.get("seq64_source_raw_proof_bytes") != 476_773:
        raise ProofPressureScalingClaimPackError("d128 sequence seq64 source raw proof drift")
    if d128_sequence.get("seq32_sidecar_raw_proof_bytes") != 35_010:
        raise ProofPressureScalingClaimPackError("d128 sequence seq32 sidecar raw proof drift")
    if d128_sequence.get("seq64_sidecar_raw_proof_bytes") != 45_414:
        raise ProofPressureScalingClaimPackError("d128 sequence seq64 sidecar raw proof drift")
    if d128_sequence.get("seq32_fused_raw_proof_bytes") != 445_888:
        raise ProofPressureScalingClaimPackError("d128 sequence seq32 fused raw proof drift")
    if d128_sequence.get("seq64_fused_raw_proof_bytes") != 481_870:
        raise ProofPressureScalingClaimPackError("d128 sequence seq64 fused raw proof drift")
    if d128_sequence.get("seq64_source_plus_sidecar_raw_proof_bytes") != 522_187:
        raise ProofPressureScalingClaimPackError("d128 sequence seq64 split raw proof drift")
    if d128_sequence.get("seq64_raw_saving_bytes") != 40_317:
        raise ProofPressureScalingClaimPackError("d128 sequence seq64 saving drift")
    if d128_sequence.get("seq64_fused_to_split_ratio") != 0.922792:
        raise ProofPressureScalingClaimPackError("d128 sequence ratio drift")
    if d128_sequence.get("seq32_to_seq64_lookup_claim_growth") != "3.729730":
        raise ProofPressureScalingClaimPackError("d128 sequence lookup growth drift")
    if d128_sequence.get("seq32_to_seq64_trace_row_growth") != "4.000000":
        raise ProofPressureScalingClaimPackError("d128 sequence trace growth drift")
    if d128_sequence.get("seq32_to_seq64_source_raw_proof_growth") != "1.075591":
        raise ProofPressureScalingClaimPackError("d128 sequence source growth drift")
    if d128_sequence.get("seq32_to_seq64_sidecar_raw_proof_growth") != "1.297172":
        raise ProofPressureScalingClaimPackError("d128 sequence sidecar growth drift")
    if d128_sequence.get("seq32_to_seq64_fused_raw_proof_growth") != "1.080697":
        raise ProofPressureScalingClaimPackError("d128 sequence fused growth drift")
    if d128_sequence.get("seq32_to_seq64_split_raw_proof_growth") != "1.091811":
        raise ProofPressureScalingClaimPackError("d128 sequence split growth drift")
    if d128_sequence.get("seq32_to_seq64_saving_growth") != "1.244813":
        raise ProofPressureScalingClaimPackError("d128 sequence saving growth drift")
    d128_head = require_dict(
        signal.get("d128_four_head_seq32_head_frontier"), "d128 head frontier"
    )
    if d128_head.get("profile_ids") != ["d128_two_head_seq32", "d128_four_head_seq32"]:
        raise ProofPressureScalingClaimPackError("d128 head frontier profile drift")
    if d128_head.get("two_head_lookup_claims") != 1_184 or d128_head.get("four_head_lookup_claims") != 2_368:
        raise ProofPressureScalingClaimPackError("d128 head frontier lookup drift")
    if d128_head.get("two_head_trace_rows") != 2_048 or d128_head.get("four_head_trace_rows") != 4_096:
        raise ProofPressureScalingClaimPackError("d128 head frontier trace row drift")
    if d128_head.get("two_head_source_raw_proof_bytes") != 443_266:
        raise ProofPressureScalingClaimPackError("d128 head two-head source raw proof drift")
    if d128_head.get("four_head_source_raw_proof_bytes") != 463_410:
        raise ProofPressureScalingClaimPackError("d128 head four-head source raw proof drift")
    if d128_head.get("two_head_sidecar_raw_proof_bytes") != 35_010:
        raise ProofPressureScalingClaimPackError("d128 head two-head sidecar raw proof drift")
    if d128_head.get("four_head_sidecar_raw_proof_bytes") != 41_524:
        raise ProofPressureScalingClaimPackError("d128 head four-head sidecar raw proof drift")
    if d128_head.get("two_head_fused_raw_proof_bytes") != 445_888:
        raise ProofPressureScalingClaimPackError("d128 head two-head fused raw proof drift")
    if d128_head.get("four_head_fused_raw_proof_bytes") != 465_630:
        raise ProofPressureScalingClaimPackError("d128 head four-head fused raw proof drift")
    if d128_head.get("four_head_source_plus_sidecar_raw_proof_bytes") != 504_934:
        raise ProofPressureScalingClaimPackError("d128 head four-head split raw proof drift")
    if d128_head.get("four_head_raw_saving_bytes") != 39_304:
        raise ProofPressureScalingClaimPackError("d128 head four-head saving drift")
    if d128_head.get("four_head_fused_to_split_ratio") != 0.92216:
        raise ProofPressureScalingClaimPackError("d128 head frontier ratio drift")
    if d128_head.get("two_to_four_lookup_claim_growth") != "2.000000":
        raise ProofPressureScalingClaimPackError("d128 head lookup growth drift")
    if d128_head.get("two_to_four_trace_row_growth") != "2.000000":
        raise ProofPressureScalingClaimPackError("d128 head trace growth drift")
    if d128_head.get("two_to_four_source_raw_proof_growth") != "1.045444":
        raise ProofPressureScalingClaimPackError("d128 head source raw growth drift")
    if d128_head.get("two_to_four_sidecar_raw_proof_growth") != "1.186061":
        raise ProofPressureScalingClaimPackError("d128 head sidecar raw growth drift")
    if d128_head.get("two_to_four_fused_raw_proof_growth") != "1.044276":
        raise ProofPressureScalingClaimPackError("d128 head fused raw growth drift")
    if d128_head.get("two_to_four_split_raw_proof_growth") != "1.055738":
        raise ProofPressureScalingClaimPackError("d128 head split raw growth drift")
    if d128_head.get("two_to_four_saving_growth") != "1.213536":
        raise ProofPressureScalingClaimPackError("d128 head saving growth drift")
    d128_four_seq64 = require_dict(
        signal.get("d128_four_head_seq64_decision_gate"), "d128 four-head seq64 decision gate"
    )
    if d128_four_seq64.get("profile_ids") != ["d128_four_head_seq32", "d128_four_head_seq64"]:
        raise ProofPressureScalingClaimPackError("d128 four-head seq64 profile drift")
    if d128_four_seq64.get("seq32_lookup_claims") != 2_368 or d128_four_seq64.get("seq64_lookup_claims") != 8_832:
        raise ProofPressureScalingClaimPackError("d128 four-head seq64 lookup drift")
    if d128_four_seq64.get("seq32_trace_rows") != 4_096 or d128_four_seq64.get("seq64_trace_rows") != 16_384:
        raise ProofPressureScalingClaimPackError("d128 four-head seq64 trace drift")
    if d128_four_seq64.get("seq32_source_raw_proof_bytes") != 463_410:
        raise ProofPressureScalingClaimPackError("d128 four-head seq32 source raw proof drift")
    if d128_four_seq64.get("seq64_source_raw_proof_bytes") != 490_307:
        raise ProofPressureScalingClaimPackError("d128 four-head seq64 source raw proof drift")
    if d128_four_seq64.get("seq32_sidecar_raw_proof_bytes") != 41_524:
        raise ProofPressureScalingClaimPackError("d128 four-head seq32 sidecar raw proof drift")
    if d128_four_seq64.get("seq64_sidecar_raw_proof_bytes") != 49_363:
        raise ProofPressureScalingClaimPackError("d128 four-head seq64 sidecar raw proof drift")
    if d128_four_seq64.get("seq32_fused_raw_proof_bytes") != 465_630:
        raise ProofPressureScalingClaimPackError("d128 four-head seq32 fused raw proof drift")
    if d128_four_seq64.get("seq64_fused_raw_proof_bytes") != 495_854:
        raise ProofPressureScalingClaimPackError("d128 four-head seq64 fused raw proof drift")
    if d128_four_seq64.get("seq64_source_plus_sidecar_raw_proof_bytes") != 539_670:
        raise ProofPressureScalingClaimPackError("d128 four-head seq64 split raw proof drift")
    if d128_four_seq64.get("seq64_raw_saving_bytes") != 43_816:
        raise ProofPressureScalingClaimPackError("d128 four-head seq64 saving drift")
    if d128_four_seq64.get("seq64_fused_to_split_ratio") != 0.91881:
        raise ProofPressureScalingClaimPackError("d128 four-head seq64 ratio drift")
    if d128_four_seq64.get("seq32_to_seq64_lookup_claim_growth") != "3.729730":
        raise ProofPressureScalingClaimPackError("d128 four-head seq64 lookup growth drift")
    if d128_four_seq64.get("seq32_to_seq64_trace_row_growth") != "4.000000":
        raise ProofPressureScalingClaimPackError("d128 four-head seq64 trace growth drift")
    if d128_four_seq64.get("seq32_to_seq64_source_raw_proof_growth") != "1.058041":
        raise ProofPressureScalingClaimPackError("d128 four-head seq64 source growth drift")
    if d128_four_seq64.get("seq32_to_seq64_sidecar_raw_proof_growth") != "1.188782":
        raise ProofPressureScalingClaimPackError("d128 four-head seq64 sidecar growth drift")
    if d128_four_seq64.get("seq32_to_seq64_fused_raw_proof_growth") != "1.064910":
        raise ProofPressureScalingClaimPackError("d128 four-head seq64 fused growth drift")
    if d128_four_seq64.get("seq32_to_seq64_split_raw_proof_growth") != "1.068793":
        raise ProofPressureScalingClaimPackError("d128 four-head seq64 split growth drift")
    if d128_four_seq64.get("seq32_to_seq64_saving_growth") != "1.114797":
        raise ProofPressureScalingClaimPackError("d128 four-head seq64 saving growth drift")
    d128_seq64_width = require_dict(
        signal.get("d128_two_head_seq64_width_frontier"), "d128 seq64 width frontier"
    )
    if d128_seq64_width.get("profile_ids") != ["d64_two_head_seq64", "d128_two_head_seq64"]:
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier profile drift")
    if d128_seq64_width.get("d64_lookup_claims") != 4_416 or d128_seq64_width.get("d128_lookup_claims") != 4_416:
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier lookup drift")
    if d128_seq64_width.get("d64_trace_rows") != 8_192 or d128_seq64_width.get("d128_trace_rows") != 8_192:
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier trace drift")
    if d128_seq64_width.get("d64_source_raw_proof_bytes") != 264_403:
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier d64 source drift")
    if d128_seq64_width.get("d128_source_raw_proof_bytes") != 476_773:
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier source drift")
    if d128_seq64_width.get("d64_sidecar_raw_proof_bytes") != 42_567:
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier d64 sidecar drift")
    if d128_seq64_width.get("d128_sidecar_raw_proof_bytes") != 45_414:
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier sidecar drift")
    if d128_seq64_width.get("d64_fused_raw_proof_bytes") != 272_636:
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier d64 fused drift")
    if d128_seq64_width.get("d128_fused_raw_proof_bytes") != 481_870:
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier fused drift")
    if d128_seq64_width.get("d128_source_plus_sidecar_raw_proof_bytes") != 522_187:
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier split drift")
    if d128_seq64_width.get("d128_raw_saving_bytes") != 40_317:
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier saving drift")
    if d128_seq64_width.get("d128_fused_to_split_ratio") != 0.922792:
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier ratio drift")
    if d128_seq64_width.get("d64_to_d128_lookup_claim_growth") != "1.000000":
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier lookup growth drift")
    if d128_seq64_width.get("d64_to_d128_trace_row_growth") != "1.000000":
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier trace growth drift")
    if d128_seq64_width.get("d64_to_d128_source_raw_proof_growth") != "1.803206":
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier source growth drift")
    if d128_seq64_width.get("d64_to_d128_sidecar_raw_proof_growth") != "1.066883":
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier sidecar growth drift")
    if d128_seq64_width.get("d64_to_d128_fused_raw_proof_growth") != "1.767448":
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier fused growth drift")
    if d128_seq64_width.get("d64_to_d128_split_raw_proof_growth") != "1.701101":
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier split growth drift")
    if d128_seq64_width.get("d64_to_d128_saving_growth") != "1.174259":
        raise ProofPressureScalingClaimPackError("d128 seq64 width frontier saving growth drift")
    d256_width = require_dict(
        signal.get("d256_two_head_seq32_width_stress"), "d256 two-head seq32 width stress"
    )
    if d256_width.get("profile_ids") != ["d64_two_head_seq32", "d128_two_head_seq32", "d256_two_head_seq32"]:
        raise ProofPressureScalingClaimPackError("d256 seq32 width-stress profile drift")
    if d256_width.get("key_widths") != [64, 128, 256]:
        raise ProofPressureScalingClaimPackError("d256 seq32 width-stress key-width drift")
    if d256_width.get("lookup_claims") != [1_184, 1_184, 1_184]:
        raise ProofPressureScalingClaimPackError("d256 seq32 width-stress lookup drift")
    if d256_width.get("trace_rows") != [2_048, 2_048, 2_048]:
        raise ProofPressureScalingClaimPackError("d256 seq32 width-stress trace drift")
    if d256_width.get("source_proof_size_bytes") != [248_702, 443_266, 816_627]:
        raise ProofPressureScalingClaimPackError("d256 seq32 width-stress source proof drift")
    if d256_width.get("sidecar_proof_size_bytes") != [36_400, 35_010, 34_914]:
        raise ProofPressureScalingClaimPackError("d256 seq32 width-stress sidecar proof drift")
    if d256_width.get("fused_proof_size_bytes") != [253_257, 445_888, 821_398]:
        raise ProofPressureScalingClaimPackError("d256 seq32 width-stress fused proof drift")
    if d256_width.get("source_plus_sidecar_raw_proof_bytes") != [285_102, 478_276, 851_541]:
        raise ProofPressureScalingClaimPackError("d256 seq32 width-stress split proof drift")
    if d256_width.get("fused_to_source_plus_sidecar_ratios") != [0.888303, 0.932282, 0.964602]:
        raise ProofPressureScalingClaimPackError("d256 seq32 width-stress ratio list drift")
    if d256_width.get("d256_lookup_claims") != 1_184 or d256_width.get("d256_trace_rows") != 2_048:
        raise ProofPressureScalingClaimPackError("d256 seq32 work drift")
    if d256_width.get("d256_source_raw_proof_bytes") != 816_627:
        raise ProofPressureScalingClaimPackError("d256 seq32 source raw proof drift")
    if d256_width.get("d256_sidecar_raw_proof_bytes") != 34_914:
        raise ProofPressureScalingClaimPackError("d256 seq32 sidecar raw proof drift")
    if d256_width.get("d256_fused_raw_proof_bytes") != 821_398:
        raise ProofPressureScalingClaimPackError("d256 seq32 fused raw proof drift")
    if d256_width.get("d256_source_plus_sidecar_raw_proof_bytes") != 851_541:
        raise ProofPressureScalingClaimPackError("d256 seq32 split raw proof drift")
    if d256_width.get("d256_raw_saving_bytes") != 30_143:
        raise ProofPressureScalingClaimPackError("d256 seq32 raw saving drift")
    if d256_width.get("d256_fused_to_split_ratio") != 0.964602:
        raise ProofPressureScalingClaimPackError("d256 seq32 fused ratio drift")
    if d256_width.get("d128_to_d256_key_width_growth") != "2.000000":
        raise ProofPressureScalingClaimPackError("d256 seq32 key-width growth drift")
    if d256_width.get("d128_to_d256_lookup_claim_growth") != "1.000000":
        raise ProofPressureScalingClaimPackError("d256 seq32 lookup growth drift")
    if d256_width.get("d128_to_d256_trace_row_growth") != "1.000000":
        raise ProofPressureScalingClaimPackError("d256 seq32 trace growth drift")
    if d256_width.get("d128_to_d256_source_raw_proof_growth") != "1.842296":
        raise ProofPressureScalingClaimPackError("d256 seq32 source growth drift")
    if d256_width.get("d128_to_d256_sidecar_raw_proof_growth") != "0.997258":
        raise ProofPressureScalingClaimPackError("d256 seq32 sidecar growth drift")
    if d256_width.get("d128_to_d256_fused_raw_proof_growth") != "1.842162":
        raise ProofPressureScalingClaimPackError("d256 seq32 fused growth drift")
    if d256_width.get("d128_to_d256_split_raw_proof_growth") != "1.780438":
        raise ProofPressureScalingClaimPackError("d256 seq32 split growth drift")
    if d256_width.get("d128_to_d256_saving_growth") != "0.930684":
        raise ProofPressureScalingClaimPackError("d256 seq32 saving growth drift")
...
def validate_payload(payload: dict[str, Any], *, check_mutations: bool = True) -> None:
    ...
    if summary.get("attention_route_rows_checked") != 30:
        raise ProofPressureScalingClaimPackError("summary route row count drift")
    if summary.get("attention_raw_proof_savings_bytes_total") != 766_883:
    ...
    d64_head = require_dict(route_signal.get("d64_seq16_head_extension"), "d64 seq16 head extension")
    if summary.get("d64_seq16_two_to_four_lookup_growth") != d64_head.get("two_to_four_lookup_claim_growth"):
        raise ProofPressureScalingClaimPackError("summary d64 lookup growth drift")
    if summary.get("d64_seq16_two_to_four_fused_raw_proof_growth") != d64_head.get(
        "two_to_four_fused_raw_proof_growth"
    ):
    ...
    if summary.get("d64_to_d128_seq32_fused_raw_proof_growth") != d128_width.get(
        "d64_to_d128_fused_raw_proof_growth"
    ):
        raise ProofPressureScalingClaimPackError("summary d128 fused growth drift")
    d128_sequence = require_dict(
        route_signal.get("d128_two_head_seq64_sequence_frontier"), "d128 sequence frontier"
    )
    if summary.get("d128_seq64_raw_saving_bytes") != d128_sequence.get("seq64_raw_saving_bytes"):
        raise ProofPressureScalingClaimPackError("summary d128 seq64 saving drift")
    if summary.get("d128_seq32_to_seq64_lookup_growth") != d128_sequence.get(
        "seq32_to_seq64_lookup_claim_growth"
    ):
        raise ProofPressureScalingClaimPackError("summary d128 seq64 lookup growth drift")
    if summary.get("d128_seq32_to_seq64_fused_raw_proof_growth") != d128_sequence.get(
        "seq32_to_seq64_fused_raw_proof_growth"
    ):
        raise ProofPressureScalingClaimPackError("summary d128 seq64 fused growth drift")
    d128_head = require_dict(route_signal.get("d128_four_head_seq32_head_frontier"), "d128 head frontier")
    if summary.get("d128_four_head_seq32_raw_saving_bytes") != d128_head.get("four_head_raw_saving_bytes"):
        raise ProofPressureScalingClaimPackError("summary d128 head saving drift")
    if summary.get("d128_seq32_two_to_four_lookup_growth") != d128_head.get("two_to_four_lookup_claim_growth"):
        raise ProofPressureScalingClaimPackError("summary d128 head lookup growth drift")
    if summary.get("d128_seq32_two_to_four_fused_raw_proof_growth") != d128_head.get(
        "two_to_four_fused_raw_proof_growth"
    ):
        raise ProofPressureScalingClaimPackError("summary d128 head fused growth drift")
    d128_four_seq64 = require_dict(
        route_signal.get("d128_four_head_seq64_decision_gate"), "d128 four-head seq64 decision gate"
    )
    if summary.get("d128_four_head_seq64_raw_saving_bytes") != d128_four_seq64.get("seq64_raw_saving_bytes"):
        raise ProofPressureScalingClaimPackError("summary d128 four-head seq64 saving drift")
    if summary.get("d128_four_head_seq32_to_seq64_lookup_growth") != d128_four_seq64.get(
        "seq32_to_seq64_lookup_claim_growth"
    ):
        raise ProofPressureScalingClaimPackError("summary d128 four-head seq64 lookup growth drift")
    if summary.get("d128_four_head_seq32_to_seq64_fused_raw_proof_growth") != d128_four_seq64.get(
        "seq32_to_seq64_fused_raw_proof_growth"
    ):
        raise ProofPressureScalingClaimPackError("summary d128 four-head seq64 fused growth drift")
    d128_seq64_width = require_dict(
        route_signal.get("d128_two_head_seq64_width_frontier"), "d128 seq64 width frontier"
    )
    if summary.get("d64_to_d128_seq64_fused_raw_proof_growth") != d128_seq64_width.get(
        "d64_to_d128_fused_raw_proof_growth"
    ):
        raise ProofPressureScalingClaimPackError("summary d128 seq64 width fused growth drift")
    d256_width = require_dict(
        route_signal.get("d256_two_head_seq32_width_stress"), "d256 seq32 width-stress signal"
    )
    if summary.get("d256_seq32_raw_saving_bytes") != d256_width.get("d256_raw_saving_bytes"):
        raise ProofPressureScalingClaimPackError("summary d256 seq32 saving drift")
    if summary.get("d128_to_d256_seq32_lookup_growth") != d256_width.get(
        "d128_to_d256_lookup_claim_growth"
    ):
        raise ProofPressureScalingClaimPackError("summary d256 seq32 lookup growth drift")
    if summary.get("d128_to_d256_seq32_trace_row_growth") != d256_width.get("d128_to_d256_trace_row_growth"):
        raise ProofPressureScalingClaimPackError("summary d256 seq32 trace growth drift")
    if summary.get("d128_to_d256_seq32_fused_raw_proof_growth") != d256_width.get(
        "d128_to_d256_fused_raw_proof_growth"
    ):
        raise ProofPressureScalingClaimPackError("summary d256 seq32 fused growth drift")
