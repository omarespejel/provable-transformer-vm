...
def build_boundary_examples(sources: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        boundary_statement(
            ...
            source_status="local_checked",
            model_binding=binding("bound", relative_source_path(MINIMAL_BENCHMARK), two_proof, "model/profile facts are bound through local benchmark contract"),
            input_binding=binding("bound", relative_source_path(MINIMAL_BENCHMARK), minimal_summary, "input facts are bound by source artifact digests"),
            output_binding=binding("bound", relative_source_path(MINIMAL_BENCHMARK), statement_chain, "output statement-chain facts are source-bound"),
            proof_object_binding=binding("bound", relative_source_path(MINIMAL_BENCHMARK), two_proof, "two local proof objects, not one native block proof"),
            approximation_policy="bounded quantized attention plus d128 RMSNorm/SwiGLU substitute; not exact Softmax, LayerNorm, or GELU",
            quantization_policy="quantized integer fixture policy from checked local evidence",
            verifier_semantics="local proof-size accounting over two verified Stwo proof objects",
            proof_size_policy="typed proof-field accounting, local only, not external comparable",
        ...
        boundary_statement(
            ...
            source_status="local_checked",
            model_binding=binding("bound", relative_source_path(MINIMAL_BENCHMARK), statement_chain, "statement chain carries model/workload contract metadata"),
            input_binding=binding("bound", relative_source_path(MINIMAL_BENCHMARK), statement_chain, "statement input facts are bound but not re-proven here"),
            output_binding=binding("bound", relative_source_path(MINIMAL_BENCHMARK), statement_chain, "statement output facts are bound but not native proof execution"),
            proof_object_binding=binding("bound", relative_source_path(MINIMAL_BENCHMARK), statement_chain, "compact statement artifact, not native proof object"),
            approximation_policy="inherits the local benchmark approximation policy; statement-validity only",
            quantization_policy="inherits checked local quantized fixture policy",
            verifier_semantics="statement binding and object classification only",
            proof_size_policy="not comparable to native proof bytes",
        ...
        boundary_statement(
            ...
            workload="tiny Gemm statement-envelope adapter",
            source_status="local_checked_external_fixture",
            model_binding=binding("bound", relative_source_path(JSTPROVE_STATEMENT_ENVELOPE), envelope_summary, "tiny model fixture metadata is statement-bound"),
            input_binding=binding("bound", relative_source_path(JSTPROVE_STATEMENT_ENVELOPE), envelope_summary, "input fixture metadata is statement-bound"),
            output_binding=binding("bound", relative_source_path(JSTPROVE_STATEMENT_ENVELOPE), envelope_summary, "output fixture metadata is statement-bound"),
            proof_object_binding=binding("bound", relative_source_path(JSTPROVE_STATEMENT_ENVELOPE), sources["jstprove"], "external proof-backed statement envelope"),
            approximation_policy="tiny Gemm external fixture, not transformer-block arithmetic",
            quantization_policy="external fixture policy, not model-faithful transformer quantization",
            verifier_semantics="statement-envelope rejects relabeling; does not make Tablero an external verifier",
            proof_size_policy="statement-envelope binding, not native Stwo proof-size comparison",
        ...
        boundary_statement(
            ...
            source_status="local_checked_external_fixture",
            model_binding=binding("bound", relative_source_path(GKR_BASELINE), gkr_tiny, "tiny Gemm fixture source-bound"),
            input_binding=binding("bound", relative_source_path(GKR_BASELINE), gkr_tiny, "tiny Gemm input fixture source-bound"),
            output_binding=binding("bound", relative_source_path(GKR_BASELINE), gkr_tiny, "tiny Gemm output fixture source-bound"),
            proof_object_binding=binding("bound", relative_source_path(GKR_BASELINE), gkr_tiny, "local external GKR fixture proof bytes"),
            approximation_policy="tiny projection-shaped dense arithmetic only",
            quantization_policy="fixture-local arithmetic policy, not d128 model-faithful attention",
            verifier_semantics="sidecar/baseline context, not Stwo replacement",
            proof_size_policy="local fixture proof bytes, not matched transformer-layer proof bytes",
        ...
        boundary_statement(
            ...
            model_binding=binding("source_reported", relative_source_path(JOLT_ATLAS_COMPARISON), atlas_self_attention, "repo command available; model/input details unavailable until local run"),
            input_binding=binding("unavailable_explicit", relative_source_path(JOLT_ATLAS_COMPARISON), atlas_self_attention, "input digest unavailable because Atlas was not locally reproduced"),
            output_binding=binding("unavailable_explicit", relative_source_path(JOLT_ATLAS_COMPARISON), atlas_self_attention, "output digest unavailable because Atlas was not locally reproduced"),
            proof_object_binding=binding("unavailable_explicit", relative_source_path(JOLT_ATLAS_COMPARISON), atlas_self_attention, "proof bytes unavailable until local self-attention run"),
            approximation_policy="external ONNX/tensor semantics unknown locally until reproduction",
            quantization_policy="external repository policy unavailable locally until reproduction",
            verifier_semantics="source-context row only; no local Atlas verifier execution",
            proof_size_policy="not reported until local run",
