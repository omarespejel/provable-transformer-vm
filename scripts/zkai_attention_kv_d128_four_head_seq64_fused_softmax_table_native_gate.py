#!/usr/bin/env python3
"""Checked fused native Stwo d128-four-head-seq64 bounded Softmax-table gate for issue #715.
...
SOURCE_INPUT_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d128-four-head-seq64-bounded-softmax-table-proof-2026-05.json"
SOURCE_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d128-four-head-seq64-bounded-softmax-table-proof-2026-05.envelope.json"
SIDECAR_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d128-four-head-seq64-softmax-table-logup-sidecar-proof-2026-05.envelope.json"
FUSED_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d128-four-head-seq64-fused-softmax-table-proof-2026-05.envelope.json"
SOURCE_INPUT_SCRIPT = ROOT / "scripts" / "zkai_attention_kv_stwo_native_d128_four_head_seq64_bounded_softmax_table_proof_input.py"
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d128-four-head-seq64-fused-softmax-table-gate-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d128-four-head-seq64-fused-softmax-table-gate-2026-05.tsv"

MAX_SOURCE_INPUT_JSON_BYTES = 268_435_456
MAX_SOURCE_ENVELOPE_JSON_BYTES = 268_435_456
MAX_SIDECAR_ENVELOPE_JSON_BYTES = 268_435_456
MAX_FUSED_ENVELOPE_JSON_BYTES = 268_435_456
NATIVE_VERIFY_TIMEOUT_SECONDS = 420
FUSED_VERIFY_TIMEOUT_SECONDS = 420

SCHEMA = "zkai-attention-kv-stwo-native-d128-four-head-seq64-fused-softmax-table-gate-v1"
ISSUE = 715
SOURCE_ISSUE = 715
SOURCE_INPUT_ISSUE = 715
SIDECAR_ISSUE = 715
DECISION = "GO_NATIVE_STWO_FUSED_ATTENTION_ARITHMETIC_AND_SOFTMAX_TABLE_LOGUP_MEMBERSHIP"
ROUTE_ID = "local_stwo_attention_kv_d128_four_head_seq64_fused_bounded_softmax_table_logup_proof"
CLAIM_BOUNDARY = (
    "ONE_NATIVE_STWO_D128_FOUR_HEAD_SEQ64_BOUNDED_SOFTMAX_TABLE_ATTENTION_PROOF_WITH_LOGUP_TABLE_MEMBERSHIP_"
...
NON_FUSED_STATUS = "GO_MATCHED_D128_FOUR_HEAD_SEQ64_SOURCE_PLUS_LOGUP_SIDECAR_COMPARATOR_RECORDED"
TIMING_POLICY = "proof_existence_and_byte_accounting_only_not_public_benchmark"

SOURCE_PROOF_SIZE_BYTES = 490_307
SOURCE_ENVELOPE_SIZE_BYTES = 140_928_819
SIDECAR_PROOF_SIZE_BYTES = 49_363
SIDECAR_ENVELOPE_SIZE_BYTES = 137_403_230
SOURCE_PLUS_SIDECAR_RAW_PROOF_BYTES = SOURCE_PROOF_SIZE_BYTES + SIDECAR_PROOF_SIZE_BYTES
FUSED_PROOF_SIZE_BYTES = 495_854
FUSED_ENVELOPE_SIZE_BYTES = 140_976_246
...
SOURCE_STATEMENT_COMMITMENT = "blake2b-256:9f7564f82bd44833784ef83bbe84058838b3abef13cf5db61444546e6d169953"
SOURCE_PUBLIC_INSTANCE_COMMITMENT = "blake2b-256:ab969d779814f4b0cca509b347693663e4731e9358fa30bed0c97c48c85be407"
SOURCE_SCORE_ROW_COMMITMENT = "blake2b-256:8edc2ffee7847b34425d7bc93ae6eabf5aa44a46c7f990af36348e1bb1784684"
SOURCE_FINAL_KV_CACHE_COMMITMENT = "blake2b-256:5ef15131f7e506f40bb1166a07ca44bdcb4ff6a8f48743b1a47de847af6ba57d"
SOURCE_OUTPUTS_COMMITMENT = "blake2b-256:1b07544046102909e63670e5e2839f10221e9006a48cd9921e6070716493353e"
SOURCE_WEIGHT_TABLE_COMMITMENT = "blake2b-256:b7f92b49df70772167c8a5ca9453de7a584b661d8a7f54d9b44cf2dd48ce5722"
SOURCE_HEAD_COUNT = 4
...
SOURCE_TABLE_ROWS = 9
SOURCE_BACKEND_VERSION = "stwo-attention-kv-d128-four-head-seq64-causal-mask-bounded-softmax-table-v1"
SOURCE_STATEMENT_VERSION = "zkai-attention-kv-stwo-native-d128-four-head-seq64-bounded-softmax-table-statement-v1"
SOURCE_SEMANTIC_SCOPE = "d128_four_head_seq64_bounded_table_softmax_approx_attention_kv_causal_mask_rows_bound_to_statement_receipt"
SOURCE_DECISION = "GO_STWO_NATIVE_ATTENTION_KV_D128_FOUR_HEAD_SEQ64_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
SOURCE_INPUT_DECISION = "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_D128_FOUR_HEAD_SEQ64_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
SOURCE_TARGET_ID = "attention-kv-d128-four-head-seq64-causal-mask-bounded-softmax-table-v1"
SOURCE_PROOF_VERSION = "stwo-attention-kv-d128-four-head-seq64-causal-mask-bounded-softmax-table-air-proof-v1"
SOURCE_VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-four-head-seq64-bounded-softmax-table:v1"
LOOKUP_RELATION = "AttentionKvD128FourHeadSeq64FusedSoftmaxTableRelation"
SIDECAR_LOOKUP_RELATION = "AttentionKvD128FourHeadSeq64SoftmaxTableLookupRelation"
LOOKUP_RELATION_WIDTH = 2
SOURCE_PROOF_COMMITMENTS = 3
SIDECAR_PROOF_COMMITMENTS = 4

FUSED_BACKEND_VERSION = "stwo-attention-kv-d128-four-head-seq64-fused-bounded-softmax-table-logup-v1"
FUSED_PROOF_SCHEMA_VERSION = "stwo-attention-kv-d128-four-head-seq64-fused-bounded-softmax-table-logup-proof-v1"
FUSED_STATEMENT_VERSION = "zkai-attention-kv-stwo-native-d128-four-head-seq64-fused-softmax-table-logup-statement-v1"
FUSED_SEMANTIC_SCOPE = "d128_four_head_seq64_bounded_softmax_table_attention_arithmetic_and_logup_membership_fused_in_one_native_stwo_proof"
FUSED_TARGET_ID = "attention-kv-d128-four-head-seq64-causal-mask-fused-bounded-softmax-table-logup-v1"
FUSED_VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-four-head-seq64-fused-bounded-softmax-table-logup:v1"
FUSED_PROOF_COMMITMENTS = 4
FUSED_TRACE_COMMITMENTS = 3

TABLE_MULTIPLICITIES = ({"gap": 0, "weight": 256, "multiplicity": 265}, {"gap": 1, "weight": 181, "multiplicity": 2}, {"gap": 2, "weight": 128, "multiplicity": 2}, {"gap": 3, "weight": 91, "multiplicity": 3}, {"gap": 4, "weight": 64, "multiplicity": 0}, {"gap": 5, "weight": 45, "multiplicity": 0}, {"gap": 6, "weight": 32, "multiplicity": 2}, {"gap": 7, "weight": 23, "multiplicity": 1}, {"gap": 8, "weight": 16, "multiplicity": 8557})
FUSED_SUMMARY_NON_CLAIMS = (
...
VALIDATION_COMMANDS = (
    "cargo +nightly-2025-07-14 test --locked attention_kv_d128_four_head_seq64_fused_softmax_table --lib --features stwo-backend",
...
_SOURCE_INPUT_VALIDATION_CACHE: set[str] = set()
CHECKED_SOURCE_INPUT_CANONICAL_DIGEST = "4791b55a9114ede07063096399a4bb8518a691094b8f955572c2ab31d63a404d"
CHECKED_NATIVE_VERIFY_SUMMARIES = {
    (
        "zkai_attention_kv_native_d128_four_head_seq64_bounded_softmax_table_proof",
        "89b1161e1ebd5ae2584fae852f5f5d6190cf8a7f9a8e670ab2011897aae77c81",
        140_928_819,
    ): {
        "mode": "verify",
        "verified": True,
        "proof_size_bytes": SOURCE_PROOF_SIZE_BYTES,
        "envelope_size_bytes": 140_928_819,
    ...
    (
        ...
        "d132a2da38dcc61f7cb9b448c6388a6f32e488515df5b9fc345f2458600219cb",
        137_403_230,
    ): {
        "mode": "verify",
        "verified": True,
        "proof_size_bytes": SIDECAR_PROOF_SIZE_BYTES,
        "envelope_size_bytes": 137_403_230,
        "source_statement_commitment": SOURCE_STATEMENT_COMMITMENT,
        "lookup_claims": SOURCE_SCORE_ROWS,
        "table_rows": SOURCE_TABLE_ROWS,
    },
}
CHECKED_FUSED_VERIFY_SUMMARIES = {
    (
        "bd84af571d5895f4123956bb7905c9730c35f8f23a3bc447f7b61111bac7e86a",
        140_976_246,
    ): {
        "mode": "verify",
        "verified": True,
        "proof_size_bytes": FUSED_PROOF_SIZE_BYTES,
        "envelope_size_bytes": 140_976_246,
...
class AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(ValueError):
    pass
...
def reject_symlinked_path(path: pathlib.Path, label: str) -> None:
    ...
    if candidate.is_symlink():
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"{label} must not be a symlink: {path}")
    current = pathlib.Path(candidate.anchor)
    for part in candidate.parts[1:-1]:
        current = current / part
        if current.is_symlink():
            raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(
...
def load_script_module(path: pathlib.Path, module_name: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"failed to load {module_name}: {path}")
...
SOURCE_INPUT_MODULE = load_script_module(
    SOURCE_INPUT_SCRIPT, "zkai_attention_kv_stwo_native_d128_four_head_seq64_bounded_softmax_table_proof_input"
)
...
def read_bounded_bytes(path: pathlib.Path, max_bytes: int, label: str) -> bytes:
    ...
    if not path.is_file():
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"missing {label}: {path}")
    size = path.stat().st_size
    if size <= 0 or size > max_bytes:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"{label} size drift: got {size}, max {max_bytes}")
    try:
        raw = path.read_bytes()
    except OSError as err:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"failed to read {label}: {err}") from err
    if len(raw) != size:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"{label} read size drift: stat={size}, read={len(raw)}")
    return raw
...
def parse_bounded_json_bytes(raw: bytes, label: str) -> Any:
    try:
        ...
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"failed to decode {label}: {err}") from err
...
def parse_stark_proof(
    ...
    *,
    ...
    if not isinstance(proof_bytes, list):
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"{label} proof must be a byte list")
    if len(proof_bytes) != expected_bytes:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"{label} proof byte length drift")
    if any(not isinstance(byte, int) or isinstance(byte, bool) or byte < 0 or byte > 255 for byte in proof_bytes):
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"{label} proof bytes must be uint8 values")
    try:
        payload = json.loads(bytes(proof_bytes).decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as err:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"failed to decode {label} proof payload: {err}") from err
    if set(payload) != {"stark_proof"}:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"{label} proof payload schema drift")
    stark_proof = payload["stark_proof"]
    if not isinstance(stark_proof, dict):
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"{label} stark_proof must be an object")
    commitments = stark_proof.get("commitments")
    if not isinstance(commitments, list) or len(commitments) != expected_commitments:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"{label} proof commitment count drift")
    return stark_proof
...
def same_digit_int_mutation(value: int, label: str) -> int:
    for candidate in (value + 1, value - 1):
        if 0 <= candidate <= 255 and len(str(candidate)) == len(str(value)):
            return candidate
    raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"no same-width byte mutation for {label}")
...
def mutate_same_size_stark_proof_commitment(envelope: dict[str, Any]) -> None:
    ...
    if len(proof_bytes) != len(envelope["proof"]):
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError("same-size proof mutation changed byte length")
...
def assert_strict_equal(actual: Any, expected: Any, label: str) -> None:
    if not type_strict_equal(actual, expected):
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"{label} drift: got {describe_value(actual)}")


def assert_fields(mapping: dict[str, Any], expected: dict[str, Any], label: str) -> None:
    if not isinstance(mapping, dict):
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"{label} must be an object")
...
def proof_bytes(envelope: dict[str, Any]) -> bytes:
    ...
    if not isinstance(proof, list):
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError("fused proof must be a byte list")
    if len(proof) != FUSED_PROOF_SIZE_BYTES:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError("fused proof byte length drift")
    if any(not isinstance(byte, int) or isinstance(byte, bool) or byte < 0 or byte > 255 for byte in proof):
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError("fused proof bytes must be uint8 values")
    return bytes(proof)
...
def verify_envelope_bytes_with_native_cli(
    ...
    *,
    ...
    if cached_summary is not None:
        ...
        return
    checked_summary = CHECKED_NATIVE_VERIFY_SUMMARIES.get((binary, digest, len(envelope_bytes)))
    if checked_summary is not None:
        assert_fields(checked_summary, expected_summary, f"native {label} verifier summary")
        _NATIVE_VERIFY_CACHE[cache_key] = checked_summary
        return
...
def verify_fused_envelope_bytes_with_native_cli(envelope_bytes: bytes, label: str) -> None:
    ...
    checked_summary = CHECKED_FUSED_VERIFY_SUMMARIES.get(cache_key)
    if checked_summary is not None:
        assert_fields(checked_summary, expected, "native fused verifier summary")
    ...
    try:
        ...
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"native fused verifier failed to run for {label}: {err}") from err
    finally:
        try:
            tmp_path.unlink(missing_ok=True)
        except OSError:
    ...
    if completed.returncode != 0:
        detail = (completed.stderr or completed.stdout).strip().splitlines()
        suffix = detail[-1] if detail else f"exit code {completed.returncode}"
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"native fused verifier rejected {label}: {suffix}")
    try:
        summary = json.loads(completed.stdout)
    except json.JSONDecodeError as err:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"native fused verifier emitted malformed JSON: {err}") from err
...
def expected_summary(source_input: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "non_claims": list(FUSED_SUMMARY_NON_CLAIMS),
    }
...
def validate_source_input_contract(source_input: dict[str, Any]) -> None:
    ...
    if len(source_input.get("score_rows", [])) != SOURCE_SCORE_ROWS:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError("source score row count drift")
    if len(source_input.get("weight_table", [])) != SOURCE_TABLE_ROWS:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError("source weight table row count drift")


def validate_checked_source_input(source_input: dict[str, Any]) -> None:
    digest = hashlib.blake2b(canonical_json_bytes(source_input), digest_size=32).hexdigest()
    if digest in _SOURCE_INPUT_VALIDATION_CACHE:
        return
    if digest == CHECKED_SOURCE_INPUT_CANONICAL_DIGEST:
        validate_source_input_contract(source_input)
        _SOURCE_INPUT_VALIDATION_CACHE.add(digest)
        return
    try:
        SOURCE_INPUT_MODULE.validate_payload(source_input)
    except Exception as err:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"source input validation drift: {err}") from err
    validate_source_input_contract(source_input)
    _SOURCE_INPUT_VALIDATION_CACHE.add(digest)
...
def validate_source_artifacts(
    ...
    *,
    ...
    validate_checked_source_input(source_input)
    assert_exact_keys(
        source_envelope,
        {
            "proof_backend",
    ...
    verify_envelope_bytes_with_native_cli(
        ...
        binary="zkai_attention_kv_native_d128_four_head_seq64_bounded_softmax_table_proof",
        expected_summary={
            "mode": "verify",
            "verified": True,
            "proof_size_bytes": SOURCE_PROOF_SIZE_BYTES,
    ...
    assert_fields(
        ...
        {
            "proof_backend": "stwo",
            "proof_backend_version": "stwo-attention-kv-d128-four-head-seq64-softmax-table-logup-sidecar-proof-v1",
            "statement_version": "zkai-attention-kv-stwo-native-d128-four-head-seq64-softmax-table-logup-sidecar-statement-v1",
            "semantic_scope": "d128_four_head_seq64_bounded_softmax_table_membership_constrained_by_native_stwo_logup_sidecar",
            "decision": "GO_NATIVE_STWO_AIR_CONSTRAINED_SOFTMAX_TABLE_LOOKUP_RELATION_SIDECAR",
            "verifier_domain": "ptvm:zkai:attention-kv-stwo-native-d128-four-head-seq64-softmax-table-logup-sidecar:v1",
        },
        "sidecar envelope",
    )
    ...
    if not isinstance(lookup_summary, dict):
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError("sidecar lookup summary missing")
    expected_sidecar = {
        "source_statement_commitment": SOURCE_STATEMENT_COMMITMENT,
    ...
    verify_envelope_bytes_with_native_cli(
        ...
        binary="zkai_attention_kv_native_d128_four_head_seq64_softmax_table_lookup_proof",
        expected_summary={
            "mode": "verify",
            "verified": True,
            "proof_size_bytes": SIDECAR_PROOF_SIZE_BYTES,
...
def validate_fused_envelope(envelope: dict[str, Any], source_input: dict[str, Any], *, run_native: bool) -> None:
    ...
    if extra_keys:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"unknown fused envelope field(s): {sorted(extra_keys)}")
    assert_fields(
        envelope,
        {
            "proof_backend": "stwo",
...
def mutation_cases(envelope: dict[str, Any]) -> Iterator[tuple[str, dict[str, Any], bool]]:
    cases: list[tuple[str, Any, bool]] = []

    def add(name: str, mutator, run_native: bool = False) -> None:
        cases.append((name, mutator, run_native))
    ...
    for name, mutator, run_native in cases:
        value = dict(envelope)
        value["fused_summary"] = copy.deepcopy(envelope["fused_summary"])
        value["source_input"] = (
            copy.deepcopy(envelope["source_input"])
            if name.startswith("source_input_")
            else envelope["source_input"]
        )
        value["proof"] = list(envelope["proof"]) if "proof" in name else envelope["proof"]
        mutator(value)
        yield name, value, run_native
...
def run_gate() -> dict[str, Any]:
    ...
    if mutation_names != EXPECTED_MUTATION_NAMES:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError("mutation case order/name drift")
    rejected = sum(1 for result in mutation_results if result["rejected"])
    if rejected != EXPECTED_MUTATION_COUNT:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"mutation rejection drift: got {rejected}")
...
def evaluate_mutation_results(fused_envelope: dict[str, Any], source_input: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for name, mutated, run_native in mutation_cases(fused_envelope):
        ...
        except AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError as err:
...
def validate_result(result: dict[str, Any]) -> None:
    if not isinstance(result, dict):
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError("result must be an object")
    ...
    if missing:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"missing result keys: {sorted(missing)}")
    extra = set(result) - required
    if extra:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"unknown result keys: {sorted(extra)}")
    for key in ("fused_envelope_commitment", "fused_proof_commitment"):
        ...
        if not isinstance(value, str) or not value.startswith("blake2b-256:"):
            raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError(f"{key} drift")
    ...
    if not isinstance(mutation_results, list) or len(mutation_results) != EXPECTED_MUTATION_COUNT:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError("mutation result shape drift")
    for item in mutation_results:
        if not isinstance(item, dict):
            raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError("mutation result shape drift")
        if set(item) != {"name", "rejected", "error"}:
            raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError("mutation result schema drift")
    mutation_names = tuple(item["name"] for item in mutation_results)
    if mutation_names != EXPECTED_MUTATION_NAMES:
        raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError("mutation result name drift")
    for item in mutation_results:
        if (
            ...
            raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError("mutation result rejection drift")
...
def write_tsv(path: pathlib.Path, result: dict[str, Any]) -> None:
    ...
    try:
        if tmp_path is None:
            raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError("missing TSV temporary artifact")
        with tmp_path.open("r", encoding="utf-8", newline="") as handle:
            rows = list(csv.DictReader(handle, delimiter="\t"))
        if rows != [expected_row]:
            raise AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError("TSV round-trip drift")
