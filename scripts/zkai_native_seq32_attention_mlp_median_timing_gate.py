...
def validate_raw_timing(payload: dict[str, Any]) -> None:
    ...
    for key in ("os", "arch", "family", "logical_cpus", "cargo_profile", "privacy_policy"):
        if key not in host:
            raise Seq32TimingGateError(f"host metadata missing {key}", layer="timing_metrics")
    expect_equal(host["privacy_policy"], "hostnames_usernames_and_absolute_local_paths_are_not_recorded", "host metadata privacy policy", layer="timing_metrics")
...
def build_payload() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "host_metadata_policy": {
            "included": ["os", "arch", "family", "logical_cpus", "cargo_profile"],
            "excluded": ["hostname", "username", "absolute_local_paths"],
            "raw_host_metadata": raw["host_metadata"],
        },
        "interpretation": [
...
def validate_payload(payload: dict[str, Any], *, check_mutations: bool = True) -> None:
    ...
    if payload["host_metadata_policy"]["excluded"] != ["hostname", "username", "absolute_local_paths"]:
