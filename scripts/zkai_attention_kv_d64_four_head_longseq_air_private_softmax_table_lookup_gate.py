#!/usr/bin/env python3
"""Checked AIR-constrained d64 four-head seq16 Softmax-table lookup sidecar gate for issue #715.

This gate records the narrow positive result: the existing native d64 four-head seq16
...
LOOKUP_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d64-four-head-longseq-softmax-table-logup-sidecar-proof-2026-05.envelope.json"
...
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d64-four-head-longseq-softmax-table-logup-sidecar-gate-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d64-four-head-longseq-softmax-table-logup-sidecar-gate-2026-05.tsv"

MAX_SOURCE_INPUT_JSON_BYTES = 33_554_432
MAX_LOOKUP_ENVELOPE_JSON_BYTES = 33_554_432

SCHEMA = "zkai-attention-kv-stwo-native-d64-four-head-longseq-softmax-table-logup-sidecar-gate-v1"
ISSUE = 715
SOURCE_ISSUE = 715
DECISION = "GO_NATIVE_STWO_AIR_CONSTRAINED_D64_FOUR_HEAD_LONGSEQ_SOFTMAX_TABLE_LOOKUP_RELATION_SIDECAR"
CLAIM_BOUNDARY = (
    "NATIVE_STWO_D64_FOUR_HEAD_LONGSEQ_BOUNDED_SOFTMAX_TABLE_LOOKUP_MEMBERSHIP_CONSTRAINED_BY_LOGUP_SIDECAR_"
...
NEXT_BACKEND_STEP = (
    "use this source-plus-sidecar comparator as the matched control for the d64 four-head seq16 fused native route and crossing-grid row"
)
TIMING_POLICY = "no_new_timing_proof_existence_and_relation_gate_only"
LOOKUP_VERIFY_TIMEOUT_SECONDS = 420

SOURCE_DECISION = "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_D64_FOUR_HEAD_LONGSEQ_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
...
LOOKUP_PROOF_VERSION = "stwo-attention-kv-d64-four-head-longseq-softmax-table-logup-sidecar-proof-v1"
LOOKUP_STATEMENT_VERSION = "zkai-attention-kv-stwo-native-d64-four-head-longseq-softmax-table-logup-sidecar-statement-v1"
LOOKUP_SEMANTIC_SCOPE = "d64_four_head_longseq_bounded_softmax_table_membership_constrained_by_native_stwo_logup_sidecar"
LOOKUP_VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d64-four-head-longseq-softmax-table-logup-sidecar:v1"
LOOKUP_TARGET_ID = "attention-kv-d64-four-head-longseq-causal-mask-bounded-softmax-table-logup-sidecar-v1"
LOOKUP_RELATION = "AttentionKvD64FourHeadLongseqSoftmaxTableLookupRelation"
LOOKUP_RELATION_WIDTH = 2
LOOKUP_PROOF_SIZE_BYTES = 27_694
LOOKUP_ENVELOPE_SIZE_BYTES = 5_910_774
LOOKUP_PROOF_COMMITMENTS = 4
LOOKUP_TRACE_COMMITMENTS = 3
LOOKUP_TABLE_MULTIPLICITIES = (
    {"gap": 0, "weight": 256, "multiplicity": 70},
    ...
    {"gap": 8, "weight": 16, "multiplicity": 600},
)
EXPECTED_LOOKUP_TABLE_MULTIPLICITIES = LOOKUP_TABLE_MULTIPLICITIES

# Baseline: checked d64 two-head longseq LogUp sidecar row.
FIXED_D64_TWO_HEAD_LOOKUP_CLAIMS = 336
FIXED_D64_TWO_HEAD_LOOKUP_PROOF_SIZE_BYTES = 27_037
FIXED_D64_TWO_HEAD_LOOKUP_ENVELOPE_SIZE_BYTES = 3_054_256
CLAIM_COUNT_RATIO = "2.000000"
PROOF_SIZE_RATIO = "1.024300"
ENVELOPE_SIZE_RATIO = "1.935258"
...
FUSED_TO_SOURCE_PLUS_SIDECAR_RATIO = "0.911430"
...
EXPECTED_MUTATION_NAMES = (
    ...
    "fixed_d64_two_head_comparison_metric_smuggling",
...
TSV_COLUMNS = (
    ...
    "lookup_claims",
    "fixed_d64_two_head_lookup_claims",
    "claim_count_ratio",
    "lookup_proof_size_bytes",
    "fixed_d64_two_head_lookup_proof_size_bytes",
    "proof_size_ratio",
    "lookup_envelope_size_bytes",
    "fixed_d64_two_head_lookup_envelope_size_bytes",
    "envelope_size_ratio",
...
def verify_lookup_envelope_bytes_with_native_cli(envelope_bytes: bytes, label: str) -> None:
    ...
    command = [
        ...
        "stwo-backend",
        "--bin",
        "zkai_attention_kv_native_d64_four_head_longseq_softmax_table_lookup_proof",
        "--",
        "verify",
        str(tmp_path),
    ]
...
def fixed_d64_two_head_comparison() -> dict[str, Any]:
    return {
        "fixed_d64_two_head_lookup_claims": FIXED_D64_TWO_HEAD_LOOKUP_CLAIMS,
        "fixed_d64_two_head_lookup_proof_size_bytes": FIXED_D64_TWO_HEAD_LOOKUP_PROOF_SIZE_BYTES,
        "fixed_d64_two_head_lookup_envelope_size_bytes": FIXED_D64_TWO_HEAD_LOOKUP_ENVELOPE_SIZE_BYTES,
...
def build_payload() -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {
        ...
        "fixed_d64_two_head_comparison": fixed_d64_two_head_comparison(),
    ...
    payload["gate_commitment"] = blake2b_commitment(
        {
            ...
            "proof_size_ratio": payload["fixed_d64_two_head_comparison"]["proof_size_ratio"],
        ...
        "ptvm:zkai:attention-kv-d64-four-head-longseq-softmax-table-logup-sidecar-gate:v1",
    )
...
def mutate_payload(payload: dict[str, Any], name: str) -> dict[str, Any]:
    ...
    elif name == "fixed_d64_two_head_comparison_metric_smuggling":
        mutated["fixed_d64_two_head_comparison"]["proof_size_ratio"] = "1.000000"
...
def to_tsv(payload: dict[str, Any]) -> str:
    ...
    comparison = payload["fixed_d64_two_head_comparison"]
    row = {
        "decision": payload["decision"],
        "lookup_status": payload["lookup_status"],
        "fused_component_status": payload["fused_component_status"],
        "lookup_claims": receipt["lookup_claims"],
        "fixed_d64_two_head_lookup_claims": comparison["fixed_d64_two_head_lookup_claims"],
        "claim_count_ratio": comparison["claim_count_ratio"],
        "lookup_proof_size_bytes": receipt["lookup_proof_size_bytes"],
        "fixed_d64_two_head_lookup_proof_size_bytes": comparison["fixed_d64_two_head_lookup_proof_size_bytes"],
        "proof_size_ratio": comparison["proof_size_ratio"],
        "lookup_envelope_size_bytes": receipt["lookup_envelope_size_bytes"],
        "fixed_d64_two_head_lookup_envelope_size_bytes": comparison["fixed_d64_two_head_lookup_envelope_size_bytes"],
