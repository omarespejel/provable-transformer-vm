#!/usr/bin/env python3
"""Checked AIR-constrained d128 four-head seq32 Softmax-table lookup sidecar gate for issue #715.

This gate records the narrow positive result: the existing native d128 four-head seq32
...
LOOKUP_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d128-four-head-seq32-softmax-table-logup-sidecar-proof-2026-05.envelope.json"
SOURCE_INPUT_SCRIPT = ROOT / "scripts" / "zkai_attention_kv_stwo_native_d128_four_head_seq32_bounded_softmax_table_proof_input.py"
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d128-four-head-seq32-softmax-table-logup-sidecar-gate-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d128-four-head-seq32-softmax-table-logup-sidecar-gate-2026-05.tsv"

MAX_SOURCE_INPUT_JSON_BYTES = 33_554_432
MAX_LOOKUP_ENVELOPE_JSON_BYTES = 67_108_864

SCHEMA = "zkai-attention-kv-stwo-native-d128-four-head-seq32-softmax-table-logup-sidecar-gate-v1"
ISSUE = 715
SOURCE_ISSUE = 715
DECISION = "GO_NATIVE_STWO_AIR_CONSTRAINED_D128_FOUR_HEAD_SEQ32_SOFTMAX_TABLE_LOOKUP_RELATION_SIDECAR"
CLAIM_BOUNDARY = (
    "NATIVE_STWO_D128_FOUR_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_LOOKUP_MEMBERSHIP_CONSTRAINED_BY_LOGUP_SIDECAR_"
...
NEXT_BACKEND_STEP = (
    "use this source-plus-sidecar comparator as the matched control for the d128 four-head seq32 fused native route and crossing-grid row"
)
TIMING_POLICY = "no_new_timing_proof_existence_and_relation_gate_only"
LOOKUP_VERIFY_TIMEOUT_SECONDS = 420

SOURCE_DECISION = "GO_INPUT_FOR_STWO_NATIVE_ATTENTION_KV_D128_FOUR_HEAD_SEQ32_BOUNDED_SOFTMAX_TABLE_AIR_PROOF"
...
LOOKUP_PROOF_VERSION = "stwo-attention-kv-d128-four-head-seq32-softmax-table-logup-sidecar-proof-v1"
LOOKUP_STATEMENT_VERSION = "zkai-attention-kv-stwo-native-d128-four-head-seq32-softmax-table-logup-sidecar-statement-v1"
LOOKUP_SEMANTIC_SCOPE = "d128_four_head_seq32_bounded_softmax_table_membership_constrained_by_native_stwo_logup_sidecar"
LOOKUP_VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-four-head-seq32-softmax-table-logup-sidecar:v1"
LOOKUP_TARGET_ID = "attention-kv-d128-four-head-seq32-causal-mask-bounded-softmax-table-logup-sidecar-v1"
LOOKUP_RELATION = "AttentionKvD128FourHeadSeq32SoftmaxTableLookupRelation"
LOOKUP_RELATION_WIDTH = 2
LOOKUP_PROOF_SIZE_BYTES = 41_524
LOOKUP_ENVELOPE_SIZE_BYTES = 37_658_403
LOOKUP_PROOF_COMMITMENTS = 4
LOOKUP_TRACE_COMMITMENTS = 3
LOOKUP_TABLE_MULTIPLICITIES = (
    {"gap": 0, "weight": 256, "multiplicity": 136},
    {"gap": 1, "weight": 181, "multiplicity": 1},
    {"gap": 2, "weight": 128, "multiplicity": 0},
    {"gap": 3, "weight": 91, "multiplicity": 2},
    {"gap": 4, "weight": 64, "multiplicity": 2},
    {"gap": 5, "weight": 45, "multiplicity": 1},
    {"gap": 6, "weight": 32, "multiplicity": 0},
    {"gap": 7, "weight": 23, "multiplicity": 2},
    {"gap": 8, "weight": 16, "multiplicity": 2224},
)
EXPECTED_LOOKUP_TABLE_MULTIPLICITIES = LOOKUP_TABLE_MULTIPLICITIES

# Baseline: checked d128 two-head seq32 LogUp sidecar row.
FIXED_D128_TWO_HEAD_LOOKUP_CLAIMS = 1184
FIXED_D128_TWO_HEAD_LOOKUP_PROOF_SIZE_BYTES = 35_010
FIXED_D128_TWO_HEAD_LOOKUP_ENVELOPE_SIZE_BYTES = 18_862_726
CLAIM_COUNT_RATIO = "2.000000"
PROOF_SIZE_RATIO = "1.186061"
ENVELOPE_SIZE_RATIO = "1.996445"
...
FUSED_TO_SOURCE_PLUS_SIDECAR_RATIO = "0.922160"
...
EXPECTED_MUTATION_NAMES = (
    ...
    "fixed_d128_two_head_comparison_metric_smuggling",
...
TSV_COLUMNS = (
    ...
    "lookup_claims",
    "fixed_d128_two_head_lookup_claims",
    "claim_count_ratio",
    "lookup_proof_size_bytes",
    "fixed_d128_two_head_lookup_proof_size_bytes",
    "proof_size_ratio",
    "lookup_envelope_size_bytes",
    "fixed_d128_two_head_lookup_envelope_size_bytes",
    "envelope_size_ratio",
...
_PAYLOAD_CACHE: tuple[tuple[Any, ...], dict[str, Any]] | None = None
...
def verify_lookup_envelope_bytes_with_native_cli(envelope_bytes: bytes, label: str) -> None:
    ...
    command = [
        ...
        "stwo-backend",
        "--bin",
        "zkai_attention_kv_native_d128_four_head_seq32_softmax_table_lookup_proof",
        "--",
        "verify",
        str(tmp_path),
    ]
...
def validate_source_input(source_input: Any) -> None:
    ...
    if digest not in _SOURCE_INPUT_VALIDATION_CACHE:
        try:
...
def build_payload_cache_key() -> tuple[Any, ...]:
    source_size = bounded_file_size(SOURCE_INPUT_JSON, MAX_SOURCE_INPUT_JSON_BYTES, "source four-head bounded Softmax-table input")
    envelope_size = bounded_file_size(LOOKUP_ENVELOPE_JSON, MAX_LOOKUP_ENVELOPE_JSON_BYTES, "lookup sidecar envelope")
    return (
        SOURCE_INPUT_JSON.stat().st_mtime_ns,
        source_size,
        LOOKUP_ENVELOPE_JSON.stat().st_mtime_ns,
        envelope_size,
        tuple(EXPECTED_MUTATION_NAMES),
    )
...
def fixed_d128_two_head_comparison() -> dict[str, Any]:
    return {
        "fixed_d128_two_head_lookup_claims": FIXED_D128_TWO_HEAD_LOOKUP_CLAIMS,
        "fixed_d128_two_head_lookup_proof_size_bytes": FIXED_D128_TWO_HEAD_LOOKUP_PROOF_SIZE_BYTES,
        "fixed_d128_two_head_lookup_envelope_size_bytes": FIXED_D128_TWO_HEAD_LOOKUP_ENVELOPE_SIZE_BYTES,
...
def build_payload() -> dict[str, Any]:
    ...
    if _PAYLOAD_CACHE is not None and _PAYLOAD_CACHE[0] == cache_key:
    ...
    payload: dict[str, Any] = {
        ...
        "fixed_d128_two_head_comparison": fixed_d128_two_head_comparison(),
    ...
    payload["gate_commitment"] = blake2b_commitment(
        {
            ...
            "proof_size_ratio": payload["fixed_d128_two_head_comparison"]["proof_size_ratio"],
        ...
        "ptvm:zkai:attention-kv-d128-four-head-seq32-softmax-table-logup-sidecar-gate:v1",
    )
...
def mutate_payload(payload: dict[str, Any], name: str) -> dict[str, Any]:
    ...
    elif name == "fixed_d128_two_head_comparison_metric_smuggling":
        mutated["fixed_d128_two_head_comparison"]["proof_size_ratio"] = "1.000000"
...
def validate_payload(payload: Any, *, allow_missing_mutation_summary: bool = False) -> None:
    ...
    allowed = {
        ...
        "timing_policy",
        "lookup_receipt",
        "fixed_d128_two_head_comparison",
        "fused_comparator",
        "non_claims",
        "validation_commands",
        "gate_commitment",
...
def to_tsv(payload: dict[str, Any]) -> str:
    ...
    comparison = payload["fixed_d128_two_head_comparison"]
    row = {
        "decision": payload["decision"],
        "lookup_status": payload["lookup_status"],
        "fused_component_status": payload["fused_component_status"],
        "lookup_claims": receipt["lookup_claims"],
        "fixed_d128_two_head_lookup_claims": comparison["fixed_d128_two_head_lookup_claims"],
        "claim_count_ratio": comparison["claim_count_ratio"],
        "lookup_proof_size_bytes": receipt["lookup_proof_size_bytes"],
        "fixed_d128_two_head_lookup_proof_size_bytes": comparison["fixed_d128_two_head_lookup_proof_size_bytes"],
        "proof_size_ratio": comparison["proof_size_ratio"],
        "lookup_envelope_size_bytes": receipt["lookup_envelope_size_bytes"],
        "fixed_d128_two_head_lookup_envelope_size_bytes": comparison["fixed_d128_two_head_lookup_envelope_size_bytes"],
