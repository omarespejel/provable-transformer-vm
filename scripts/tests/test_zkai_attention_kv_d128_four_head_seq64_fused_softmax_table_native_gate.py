...
from scripts import zkai_attention_kv_d128_four_head_seq64_fused_softmax_table_native_gate as gate


class AttentionKvD128FourHeadSeq64FusedSoftmaxTableNativeGateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        missing = [
            str(path.relative_to(gate.ROOT))
            for path in (
                gate.SOURCE_INPUT_JSON,
        ...
        if missing:
            raise unittest.SkipTest(
                "regenerate oversized local d128_four_head_seq64 artifacts before running this gate: "
                + ", ".join(missing)
            )
    ...
    def test_gate_records_fused_go_and_byte_delta(self):
        ...
        self.assertEqual(payload["lookup_claims"], gate.SOURCE_SCORE_ROWS)
        self.assertEqual(payload["table_rows"], gate.SOURCE_TABLE_ROWS)
        self.assertEqual(payload["source_head_count"], gate.SOURCE_HEAD_COUNT)
        self.assertEqual(payload["source_proof_size_bytes"], gate.SOURCE_PROOF_SIZE_BYTES)
        self.assertEqual(payload["sidecar_proof_size_bytes"], gate.SIDECAR_PROOF_SIZE_BYTES)
        self.assertEqual(payload["source_plus_sidecar_raw_proof_bytes"], gate.SOURCE_PLUS_SIDECAR_RAW_PROOF_BYTES)
        self.assertEqual(payload["fused_proof_size_bytes"], gate.FUSED_PROOF_SIZE_BYTES)
        self.assertEqual(payload["fused_over_source_proof_bytes"], gate.FUSED_OVER_SOURCE_PROOF_BYTES)
        self.assertEqual(payload["fused_saves_vs_source_plus_sidecar_bytes"], gate.FUSED_SAVES_VS_SOURCE_PLUS_SIDECAR_BYTES)
        self.assertEqual(payload["fused_to_source_plus_sidecar_ratio"], gate.FUSED_TO_SOURCE_PLUS_SIDECAR_RATIO)
        self.assertLess(payload["fused_to_source_plus_sidecar_ratio"], 0.93)
    ...
    def test_fused_summary_counts_table_multiplicities(self):
        ...
        self.assertEqual(sum(row["multiplicity"] for row in summary["table_multiplicities"]), gate.SOURCE_SCORE_ROWS)
        self.assertEqual(summary["table_multiplicities"][-1]["gap"], 8)
        self.assertEqual(summary["table_multiplicities"][-1]["multiplicity"], 8557)

    def test_all_declared_mutations_reject(self):
        for name, mutated, run_native in gate.mutation_cases(self.fused_envelope):
            if run_native:
                continue
            with self.assertRaises(gate.AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError, msg=name):
    ...
    def test_rejects_sidecar_or_source_proof_injection(self):
        for key in ("sidecar_proof", "source_proof"):
            ...
            mutated[key] = []
            with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError, "unknown fused envelope field"):
                gate.validate_fused_envelope(mutated, self.source_input, run_native=False)

    def test_rejects_source_input_split_brain(self):
        mutated = copy.deepcopy(self.fused_envelope)
        mutated["source_input"]["score_rows"][0]["attention_weight"] = 255
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError, "source input split-brain"):
            gate.validate_fused_envelope(mutated, self.source_input, run_native=False)
    ...
    def test_native_verifier_rejects_same_size_tampered_proof_payload(self):
        ...
        try:
            with self.assertRaisesRegex(
                gate.AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError,
    ...
    def test_gate_checks_source_sidecar_and_fused_envelope_byte_sizes(self):
        ...
        for label, path, expected_size in cases:
            ...
            with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError, f"{label} size drift"):
                gate.expect_artifact_size(raw + b" ", expected_size, label)

    def test_write_json_and_tsv_round_trip(self):
        with tempfile.TemporaryDirectory(dir=gate.EVIDENCE_DIR, prefix=".tmp-d128-h4-fused-test-") as tmp_dir:
            ...
            self.assertIn(str(gate.FUSED_PROOF_SIZE_BYTES), tsv_path.read_text(encoding="utf-8"))
    ...
    def test_input_helpers_reject_symlink_paths(self):
        with tempfile.TemporaryDirectory(dir=gate.ROOT) as tmp_dir:
            ...
            with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError, "symlink"):
    ...
    def test_write_json_rejects_metric_drift(self):
        ...
        with tempfile.TemporaryDirectory(dir=gate.EVIDENCE_DIR, prefix=".tmp-d128-h4-fused-test-") as tmp:
            with self.assertRaisesRegex(
                gate.AttentionKvD128FourHeadSeq64FusedSoftmaxTableGateError,
