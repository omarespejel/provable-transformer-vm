...
from scripts import zkai_attention_kv_d64_single_head_longseq_air_private_softmax_table_lookup_gate as gate
...
class AttentionKvAirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["lookup_claims"], 168)
        ...
        self.assertEqual(receipt["lookup_proof_size_bytes"], 22960)
        self.assertEqual(receipt["lookup_envelope_size_bytes"], 1601867)
        self.assertEqual(receipt["lookup_proof_commitments"], 4)
        self.assertEqual(receipt["lookup_trace_commitments"], 3)
        self.assertEqual(sum(row["multiplicity"] for row in receipt["table_multiplicities"]), 168)
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 114)
    ...
    def test_rejects_float_encoded_lookup_claim_count(self):
        payload = self.strip_mutation_summary(gate.build_payload())
        payload["lookup_receipt"]["lookup_claims"] = 168.0
        self.assert_rejects(payload, "lookup_receipt drift")
