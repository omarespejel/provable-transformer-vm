...
from scripts import zkai_attention_kv_d64_four_head_longseq_air_private_softmax_table_lookup_gate as gate


class AttentionKvD64FourHeadLongseqAirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["lookup_proof_size_bytes"], 27694)
        self.assertEqual(receipt["lookup_envelope_size_bytes"], 5910774)
        ...
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 600)
        self.assertEqual(payload["fixed_d64_two_head_comparison"]["fixed_d64_two_head_lookup_claims"], 336)
        self.assertEqual(payload["fixed_d64_two_head_comparison"]["proof_size_ratio"], "1.024300")
        self.assertEqual(payload["fused_comparator"]["source_plus_sidecar_raw_proof_bytes"], 260685)
        self.assertEqual(payload["fused_comparator"]["fused_saves_vs_source_plus_sidecar_bytes"], 23089)
        self.assertEqual(payload["fused_comparator"]["fused_to_source_plus_sidecar_ratio"], "0.911430")
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn("27694", tsv)
        self.assertIn("260685", tsv)
        self.assertIn("23089", tsv)
        self.assertIn("1.024300", tsv)
