...
from scripts import zkai_attention_kv_d32_two_head_longseq_air_private_softmax_table_lookup_gate as gate


class AttentionKvD32TwoHeadLongseqAirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["lookup_proof_size_bytes"], 27075)
        self.assertEqual(receipt["lookup_envelope_size_bytes"], 1753808)
        ...
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 288)
        self.assertEqual(payload["fixed_d32_two_head_comparison"]["fixed_d32_two_head_lookup_claims"], 104)
        self.assertEqual(payload["fixed_d32_two_head_comparison"]["proof_size_ratio"], "1.492805")
        self.assertEqual(payload["fused_comparator"]["source_plus_sidecar_raw_proof_bytes"], 162138)
        self.assertEqual(payload["fused_comparator"]["fused_saves_vs_source_plus_sidecar_bytes"], 29595)
        self.assertEqual(payload["fused_comparator"]["fused_to_source_plus_sidecar_ratio"], "0.817470")
    ...
    def test_rejects_float_encoded_lookup_claim_count(self):
        payload = self.strip_mutation_summary(gate.build_payload())
        payload["lookup_receipt"]["lookup_claims"] = 336.0
        self.assert_rejects(payload, "lookup_receipt drift")
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn("27075", tsv)
        self.assertIn("162138", tsv)
        self.assertIn("29595", tsv)
        self.assertIn("1.492805", tsv)
