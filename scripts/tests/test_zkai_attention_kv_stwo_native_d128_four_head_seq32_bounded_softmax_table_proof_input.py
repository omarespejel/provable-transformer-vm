...
from scripts import zkai_attention_kv_stwo_native_d128_four_head_seq32_bounded_softmax_table_proof_input as gate


class AttentionKvD128FourHeadSeq32BoundedSoftmaxTableInputTests(unittest.TestCase):
    def test_payload_builds_checked_weighted_attention_surface(self):
        ...
        self.assertEqual(payload["score_row_count"], 2368)
        self.assertEqual(payload["trace_row_count"], 4096)
        self.assertEqual(
            payload["attention_outputs"][0],
            [-4, -1, 4, -2, 2, 6, 1, -5, -1, -6, -2, 2, -4, 1, 4, -1, 3, 7, -8, -4, 0, -5, -1, 2, -2, 1, 5, 0, 4, -2, -7, -3, 0, -3, 2, 6, -5, 1, 5, 1, -4, 1, -3, -5, 0, -4, 0, 6, 1, 6, 4, -10, -5, 0, -4, 1, 5, -6, 0, 4, 1, 5, 0, -4, -3, 1, -1, 4, 3, -6, -1, 5, 2, -2, 3, -6, -7, -1, -4, 2, 7, 4, 3, 2, -10, -5, 1, -2, 4, 2, -7, -2, 4, 1, 7, 2, -7, -4, 1, -2, 2, 7, 3, 1, -4, -8, -3, 2, -2, 3, 0, -3, 1, 6, 2, -3, 2, -9, -4, 1, -3, 1, 7, 2, 0, 5, -9, -4],
        )
        self.assertEqual(
            payload["attention_outputs"][32],
            [-1, -1, 4, -4, 1, 1, -2, -3, 3, -7, -1, 4, -3, 1, 1, -1, -2, 3, -6, 0, -2, -4, 2, 2, -1, -1, 4, -4, 1, -1, -2, -4, 2, -4, 1, 1, -5, 1, 1, 0, -2, 5, -5, -2, -1, -3, -3, 5, -2, 3, 0, -4, -2, 2, -5, 1, 1, -5, 1, 1, -1, 0, 4, -3, -1, 0, -1, -1, 0, -2, -1, 4, -3, 1, 2, -5, -4, 2, -5, 1, 2, 0, -3, 0, -3, -1, 3, -3, 3, 0, -5, -4, 3, -4, 4, 3, -1, -4, 2, -5, 1, 2, 0, -3, -1, -2, -3, 4, -3, 3, 0, -2, -1, 4, -3, 1, 2, -5, -4, 2, -5, 0, 6, -2, 0, 0, -3, -1],
        )
        self.assertEqual(
            payload["attention_outputs"][-1],
            [-4, 0, 2, -5, 0, 4, -7, 0, 4, -9, 1, 6, -6, -3, 6, -4, -1, 2, -6, 0, 2, -3, 0, 3, -8, 1, 6, -7, -4, 5, -4, -2, 5, -5, -3, 5, -5, -1, 5, -6, -1, 1, -4, 1, 5, -7, 0, 5, -6, -3, 4, -4, -2, 6, -5, -2, 2, -3, 2, 4, -9, -1, 5, -8, -1, 4, -8, 0, 5, -5, 1, 3, -7, -4, 6, -3, 0, 4, -6, -1, 4, -8, -1, 6, -6, 0, 4, -7, -3, 7, -3, 0, 0, -6, 0, 2, -4, 1, 3, -6, -1, 3, -7, 0, 5, -7, 1, 6, -6, -4, 6, -4, 0, 2, -6, 0, 2, -3, 1, 4, -9, 1, 6, -7, -3, 5, -3, -3],
        )
    ...
    def test_rejects_weight_policy_drift(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq32BoundedSoftmaxTableInputError, "weight_policy drift"):
            gate.validate_payload(payload)

    def test_rejects_weight_relabeling(self):
        payload = gate.build_payload()
        payload["score_rows"][0]["attention_weight"] = 15
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq32BoundedSoftmaxTableInputError, "score rows drift"):
            gate.validate_payload(payload)

    def test_rejects_head_relabeling(self):
        payload = gate.build_payload()
        payload["input_steps"][1]["head_index"] = 0
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq32BoundedSoftmaxTableInputError, "input steps drift"):
            gate.validate_payload(payload)

    def test_rejects_output_relabeling(self):
        payload = gate.build_payload()
        payload["attention_outputs"][0][0] = 99
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq32BoundedSoftmaxTableInputError, "attention outputs drift"):
            gate.validate_payload(payload)

    def test_rejects_commitment_relabeling(self):
        payload = gate.build_payload()
        payload["statement_commitment"] = "blake2b-256:" + "55" * 32
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq32BoundedSoftmaxTableInputError, "statement commitment drift"):
            gate.validate_payload(payload)

    def test_rejects_unknown_top_level_field(self):
        payload = gate.build_payload()
        payload["unexpected"] = "claim smuggling"
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq32BoundedSoftmaxTableInputError, "unknown field"):
            gate.validate_payload(payload)
    ...
    def test_write_json_rejects_symlinked_parent_before_create(self):
        with tempfile.TemporaryDirectory(dir=gate.EVIDENCE_DIR, prefix=".tmp-d64-h4-source-test-") as tmp:
            ...
            with self.assertRaisesRegex(
                gate.AttentionKvD128FourHeadSeq32BoundedSoftmaxTableInputError,
                "symlinked parent",
            ):
    ...
    def test_rejects_source_payload_identity_drift(self):
        ...
        with mock.patch.object(gate.SOURCE, "build_payload", return_value=payload):
            with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq32BoundedSoftmaxTableInputError, "source payload head_count drift"):
                gate.build_payload()
    ...
    def test_rejects_source_payload_commitment_drift(self):
        ...
        with mock.patch.object(gate.SOURCE, "build_payload", return_value=payload):
            with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq32BoundedSoftmaxTableInputError, "source payload commitment drift"):
                gate.build_payload()
    ...
    def test_build_score_rows_rejects_malformed_input_step_shape(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq32BoundedSoftmaxTableInputError, r"input_steps\[0\]\.query width drift"):
    ...
    def test_build_score_rows_rejects_malformed_candidate_shape(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq32BoundedSoftmaxTableInputError, r"initial_kv\[0\]\.value width drift"):
            gate.build_score_rows(initial, steps)
    ...
    def test_build_score_rows_rejects_invalid_head_index(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq32BoundedSoftmaxTableInputError, r"input_steps\[0\]\.head_index outside head range"):
            gate.build_score_rows(initial, steps)
    ...
    def test_build_score_rows_rejects_token_position_drift(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq32BoundedSoftmaxTableInputError, r"input_steps\[0\]\.token_position drift"):
            gate.build_score_rows(initial, steps)

    def test_build_score_rows_rejects_missing_per_head_steps(self):
        initial = gate.fixture_initial_kv()
        steps = gate.fixture_input_steps()[:-1]
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq32BoundedSoftmaxTableInputError, "per-head input step count drift"):
            gate.build_score_rows(initial, steps)
    ...
    def test_build_score_rows_rejects_score_gap_bit_overflow(self):
        ...
        initial[0]["key"][0] = 1000
        initial[1]["key"] = [0 for _ in range(gate.KEY_WIDTH)]
        steps[0]["query"] = [0 for _ in range(gate.KEY_WIDTH)]
        steps[0]["query"][0] = 70
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq32BoundedSoftmaxTableInputError, r"score_gap\[1\] outside 16-bit range"):
            gate.build_score_rows(initial, steps)
    ...
    def test_rejects_output_remainder_bit_overflow(self):
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq32BoundedSoftmaxTableInputError, "outside 16-bit range"):
