...
from scripts import zkai_attention_kv_d64_single_head_longseq_fused_softmax_table_native_gate as gate


class AttentionKvD64SingleHeadLongseqFusedSoftmaxTableNativeGateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_gate_records_fused_go_and_byte_delta(self):
        ...
        self.assertEqual(payload["source_proof_size_bytes"], 231415)
        self.assertEqual(payload["sidecar_proof_size_bytes"], 22960)
        self.assertEqual(payload["source_plus_sidecar_raw_proof_bytes"], 254375)
        self.assertEqual(payload["fused_proof_size_bytes"], 237725)
        self.assertEqual(payload["fused_over_source_proof_bytes"], 6310)
        self.assertEqual(payload["fused_saves_vs_source_plus_sidecar_bytes"], 16650)
        self.assertEqual(payload["fused_to_source_plus_sidecar_ratio"], "0.934545")
    ...
    def test_fused_summary_counts_table_multiplicities(self):
        ...
        self.assertEqual(summary["table_multiplicities"][-1]["multiplicity"], 114)

    def test_all_declared_mutations_reject(self):
        for name, mutated, run_native in gate.mutation_cases(self.fused_envelope):
            with self.assertRaises(gate.AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError, msg=name):
    ...
    def test_rejects_sidecar_or_source_proof_injection(self):
        for key in ("sidecar_proof", "source_proof"):
            ...
            mutated[key] = []
            with self.assertRaisesRegex(gate.AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError, "unknown fused envelope field"):
    ...
    def test_rejects_non_object_fused_envelope_without_crashing(self):
        with self.assertRaisesRegex(gate.AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError, "fused envelope must be an object"):
    ...
    def test_rejects_source_input_split_brain(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError, "source input split-brain"):
            gate.validate_fused_envelope(mutated, self.source_input, run_native=False)
    ...
    def test_rejects_matching_mutated_source_denominator_pair(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError, "source input validation drift"):
    ...
    def test_rejects_summary_type_relabeling(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError, "fused summary drift"):
            gate.validate_fused_envelope(mutated, self.source_input, run_native=False)
        ...
        for key, value in (("lookup_relation", "OtherRelation"), ("lookup_relation_width", 2.0)):
            with self.subTest(key=key):
                ...
                with self.assertRaisesRegex(
                    gate.AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError,
    ...
    def test_rejects_sidecar_target_id_relabeling(self):
        sidecar = copy.deepcopy(self.sidecar_envelope)
        sidecar["target_id"] = "attention-kv-d64-single-head-longseq-forged-sidecar"
        with self.assertRaisesRegex(gate.AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError, "sidecar target_id drift"):
            gate.validate_source_artifacts(self.source_input, self.source_envelope, sidecar)
    ...
    def test_gate_checks_source_sidecar_and_fused_envelope_byte_sizes(self):
        ...
        for label, path, expected_size in cases:
            ...
            with self.assertRaisesRegex(gate.AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError, f"{label} size drift"):
    ...
    def test_write_json_and_tsv_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn("237725", tsv_path.read_text(encoding="utf-8"))
