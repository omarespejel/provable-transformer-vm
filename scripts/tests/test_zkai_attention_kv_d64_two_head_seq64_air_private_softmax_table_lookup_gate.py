...
from scripts import zkai_attention_kv_d64_two_head_seq64_air_private_softmax_table_lookup_gate as gate


class AttentionKvD64TwoHeadSeq64AirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["lookup_claims"], 4416)
        ...
        self.assertEqual(receipt["lookup_proof_size_bytes"], 42567)
        self.assertEqual(receipt["lookup_envelope_size_bytes"], 35881481)
        self.assertEqual(receipt["lookup_proof_commitments"], 4)
        self.assertEqual(receipt["lookup_trace_commitments"], 3)
        self.assertEqual(sum(row["multiplicity"] for row in receipt["table_multiplicities"]), 4416)
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 4265)
        self.assertEqual(payload["baseline_d64_two_head_seq32_comparison"]["baseline_d64_two_head_seq32_lookup_claims"], 1184)
        self.assertEqual(payload["baseline_d64_two_head_seq32_comparison"]["proof_size_ratio"], "1.169423")
        self.assertEqual(payload["fused_comparator"]["source_plus_sidecar_raw_proof_bytes"], 306970)
        self.assertEqual(payload["fused_comparator"]["fused_saves_vs_source_plus_sidecar_bytes"], 34334)
        self.assertEqual(payload["fused_comparator"]["fused_to_source_plus_sidecar_ratio"], "0.888152")
    ...
    def test_rejects_float_encoded_lookup_claim_count(self):
        payload = self.strip_mutation_summary(gate.build_payload())
        payload["lookup_receipt"]["lookup_claims"] = 4416.0
        self.assert_rejects(payload, "lookup_receipt drift")
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn("42567", tsv)
        self.assertIn("306970", tsv)
        self.assertIn("34334", tsv)
        self.assertIn("1.169423", tsv)
