...
from scripts import zkai_attention_kv_d128_two_head_seq32_fused_softmax_table_native_gate as gate


class AttentionKvD128TwoHeadSeq32FusedSoftmaxTableNativeGateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_gate_records_fused_go_and_byte_delta(self):
        ...
        self.assertEqual(payload["source_proof_size_bytes"], 443266)
        self.assertEqual(payload["sidecar_proof_size_bytes"], 35010)
        self.assertEqual(payload["source_plus_sidecar_raw_proof_bytes"], 478276)
        self.assertEqual(payload["fused_proof_size_bytes"], 445888)
        self.assertEqual(payload["fused_over_source_proof_bytes"], 2622)
        self.assertEqual(payload["fused_saves_vs_source_plus_sidecar_bytes"], 32388)
    ...
    def test_fused_envelope_commitment_rejects_bytes_dict_split_brain(self):
        ...
        with self.assertRaisesRegex(
            gate.AttentionKvD128TwoHeadSeq32FusedSoftmaxTableGateError,
    ...
    def test_fused_summary_counts_table_multiplicities(self):
        ...
        self.assertEqual(summary["table_multiplicities"][-1]["multiplicity"], 1113)

    def test_all_declared_mutations_reject(self):
        for name, mutated, run_native in gate.mutation_cases(self.fused_envelope):
            with self.assertRaises(gate.AttentionKvD128TwoHeadSeq32FusedSoftmaxTableGateError, msg=name):
    ...
    def test_rejects_sidecar_or_source_proof_injection(self):
        for key in ("sidecar_proof", "source_proof"):
            mutated = copy.deepcopy(self.fused_envelope)
            mutated[key] = []
            with self.assertRaisesRegex(gate.AttentionKvD128TwoHeadSeq32FusedSoftmaxTableGateError, "unknown fused envelope field"):
    ...
    def test_rejects_source_input_split_brain(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD128TwoHeadSeq32FusedSoftmaxTableGateError, "source input split-brain"):
            gate.validate_fused_envelope(mutated, self.source_input, run_native=False)
    ...
    def test_gate_checks_source_sidecar_and_fused_envelope_byte_sizes(self):
        ...
        for label, path, expected_size in cases:
            ...
            with self.assertRaisesRegex(gate.AttentionKvD128TwoHeadSeq32FusedSoftmaxTableGateError, f"{label} size drift"):
    ...
    def test_write_json_and_tsv_round_trip(self):
        with self.evidence_tempdir() as tmp_dir:
            ...
            self.assertIn("445888", tsv_path.read_text(encoding="utf-8"))
