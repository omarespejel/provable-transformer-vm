...
from scripts import zkai_attention_kv_d128_four_head_seq64_air_private_softmax_table_lookup_gate as gate


class AttentionKvD128FourHeadSeq64AirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        missing = [
            str(path.relative_to(gate.ROOT))
            for path in (gate.SOURCE_INPUT_JSON, gate.LOOKUP_ENVELOPE_JSON)
            if not path.exists()
        ]
        if missing:
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["lookup_claims"], 8832)
        self.assertEqual(receipt["table_rows"], 9)
        self.assertEqual(receipt["source_head_count"], gate.SOURCE_HEAD_COUNT)
        self.assertEqual(receipt["lookup_relation"], gate.LOOKUP_RELATION)
        self.assertEqual(receipt["lookup_relation_width"], 2)
        self.assertEqual(receipt["lookup_proof_size_bytes"], gate.LOOKUP_PROOF_SIZE_BYTES)
        self.assertEqual(receipt["lookup_envelope_size_bytes"], gate.LOOKUP_ENVELOPE_SIZE_BYTES)
        self.assertEqual(receipt["lookup_proof_commitments"], 4)
        self.assertEqual(receipt["lookup_trace_commitments"], 3)
        self.assertEqual(sum(row["multiplicity"] for row in receipt["table_multiplicities"]), 8832)
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 8557)
        self.assertEqual(payload["fixed_d64_four_head_comparison"]["fixed_d64_four_head_lookup_claims"], 8832)
        self.assertEqual(
            payload["fused_comparator"]["source_plus_sidecar_raw_proof_bytes"],
            gate.SOURCE_PLUS_SIDECAR_RAW_PROOF_BYTES,
        )
        self.assertEqual(
            payload["fused_comparator"]["fused_saves_vs_source_plus_sidecar_bytes"],
            gate.FUSED_SAVES_VS_SOURCE_PLUS_SIDECAR_BYTES,
        )
        self.assertEqual(
            payload["fused_comparator"]["fused_to_source_plus_sidecar_ratio"],
            gate.format_ratio(gate.FUSED_PROOF_SIZE_BYTES, gate.SOURCE_PLUS_SIDECAR_RAW_PROOF_BYTES),
        )
    ...
    def test_rejects_float_encoded_lookup_claim_count(self):
        payload = self.strip_mutation_summary(gate.build_payload())
        payload["lookup_receipt"]["lookup_claims"] = 8832.0
        self.assert_rejects(payload, "lookup_receipt drift")
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn(str(gate.LOOKUP_PROOF_SIZE_BYTES), tsv)
        self.assertIn(str(gate.SOURCE_PLUS_SIDECAR_RAW_PROOF_BYTES), tsv)
        self.assertIn(str(gate.FUSED_SAVES_VS_SOURCE_PLUS_SIDECAR_BYTES), tsv)
        self.assertIn(gate.format_ratio(gate.LOOKUP_PROOF_SIZE_BYTES, gate.FIXED_D64_FOUR_HEAD_LOOKUP_PROOF_SIZE_BYTES), tsv)
