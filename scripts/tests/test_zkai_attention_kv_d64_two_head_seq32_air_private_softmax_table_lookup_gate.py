...
from scripts import zkai_attention_kv_d64_two_head_seq32_air_private_softmax_table_lookup_gate as gate


class AttentionKvD64TwoHeadSeq32AirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["lookup_proof_size_bytes"], 36400)
        self.assertEqual(receipt["lookup_envelope_size_bytes"], 9978658)
        ...
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 1107)
        self.assertEqual(payload["fixed_d64_two_head_comparison"]["fixed_d64_two_head_lookup_claims"], 104)
        self.assertEqual(payload["fixed_d64_two_head_comparison"]["proof_size_ratio"], "2.006947")
        self.assertEqual(payload["fused_comparator"]["source_plus_sidecar_raw_proof_bytes"], 285102)
        self.assertEqual(payload["fused_comparator"]["fused_saves_vs_source_plus_sidecar_bytes"], 31845)
        self.assertEqual(payload["fused_comparator"]["fused_to_source_plus_sidecar_ratio"], "0.888303")
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn("36400", tsv)
        self.assertIn("285102", tsv)
        self.assertIn("31845", tsv)
        self.assertIn("2.006947", tsv)
