...
class AttentionKvFullerCrossingGridGateTests(unittest.TestCase):
    ...
    def test_records_45_cell_grid_with_13_proved_and_32_missing(self):
        result = self.result
        ...
        self.assertEqual(result["summary"]["proved_cell_count"], 13)
        self.assertEqual(result["summary"]["missing_cell_count"], 32)
        self.assertEqual(result["summary"]["coverage_share"], 0.288889)
    ...
    def test_missing_cells_carry_no_proof_metrics_or_evidence_paths(self):
        ...
        by_cell = {row["cell_id"]: row for row in self.result["grid_rows"]}
        self.assertEqual(by_cell["d32_h2_seq32"]["status"], gate.MISSING_STATUS)
    ...
    def test_summary_keeps_go_and_no_go_boundary_sharp(self):
        ...
        self.assertEqual(summary["proved_crossing_cell_count"], 6)
        self.assertEqual(summary["proved_all_axis_cell_count"], 2)
        self.assertEqual(summary["missing_all_axis_cell_count"], 14)
        self.assertEqual(summary["proved_counts_by_width"], {"16": 3, "32": 3, "8": 7})
        self.assertEqual(summary["proved_counts_by_head_count"], {"1": 3, "16": 1, "2": 7, "4": 1, "8": 1})
        self.assertEqual(summary["proved_counts_by_steps_per_head"], {"16": 3, "32": 1, "8": 9})
    ...
    def test_validate_rejects_overclaims_and_metric_smuggling(self):
        ...
        bad = copy.deepcopy(self.result)
        missing = next(row for row in bad["grid_rows"] if row["cell_id"] == "d32_h2_seq32")
