...
class AttentionKvFullerCrossingGridGateTests(unittest.TestCase):
    ...
    def test_records_60_cell_grid_with_15_proved_and_45_missing(self):
        result = self.result
        ...
        self.assertEqual(result["summary"]["proved_cell_count"], 15)
        self.assertEqual(result["summary"]["missing_cell_count"], 45)
    ...
    def test_missing_cells_carry_no_proof_metrics_or_evidence_paths(self):
        ...
        self.assertEqual(len(missing), 45)
        for row in missing:
        ...
        by_cell = {row["cell_id"]: row for row in self.result["grid_rows"]}
        self.assertEqual(by_cell["d16_h2_seq32"]["status"], gate.MISSING_STATUS)
        ...
        self.assertEqual(by_cell["d64_h2_seq16"]["status"], gate.MISSING_STATUS)
    ...
    def test_summary_keeps_go_and_no_go_boundary_sharp(self):
        ...
        self.assertEqual(summary["proved_crossing_cell_count"], 8)
        self.assertEqual(summary["proved_all_axis_cell_count"], 4)
        self.assertEqual(summary["missing_all_axis_cell_count"], 20)
        self.assertEqual(summary["proved_counts_by_width"], {"16": 3, "32": 4, "64": 1, "8": 7})
        self.assertEqual(summary["proved_counts_by_head_count"], {"1": 3, "16": 1, "2": 9, "4": 1, "8": 1})
        self.assertEqual(summary["proved_counts_by_steps_per_head"], {"16": 3, "32": 3, "8": 9})
        self.assertIn("full factorial proof-grid claim", self.result["no_go_criteria"][0])
        self.assertEqual(self.result["next_low_risk_profiles"][0]["profile_id"], "d64_two_head_seq16")
    ...
    def test_validate_rejects_overclaims_and_metric_smuggling(self):
        ...
        bad = copy.deepcopy(self.result)
        missing = next(row for row in bad["grid_rows"] if row["cell_id"] == "d16_h2_seq32")
