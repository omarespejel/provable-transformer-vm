...
class AttentionKvFullerCrossingGridGateTests(unittest.TestCase):
    ...
    def test_records_60_cell_grid_with_19_proved_and_41_missing(self):
        result = self.result
        ...
        self.assertEqual(result["summary"]["proved_cell_count"], 19)
        self.assertEqual(result["summary"]["missing_cell_count"], 41)
        self.assertEqual(result["summary"]["coverage_share"], 0.316667)
    ...
    def test_missing_cells_carry_no_proof_metrics_or_evidence_paths(self):
        ...
        self.assertEqual(len(missing), 41)
        for row in missing:
    ...
    def test_summary_keeps_go_and_no_go_boundary_sharp(self):
        ...
        self.assertEqual(summary["proved_crossing_cell_count"], 12)
        self.assertEqual(summary["proved_all_axis_cell_count"], 8)
        self.assertEqual(summary["missing_all_axis_cell_count"], 16)
        self.assertEqual(summary["proved_counts_by_width"], {"16": 4, "32": 6, "64": 2, "8": 7})
        self.assertEqual(summary["proved_counts_by_head_count"], {"1": 3, "16": 1, "2": 11, "4": 3, "8": 1})
        self.assertEqual(summary["proved_counts_by_steps_per_head"], {"16": 5, "32": 5, "8": 9})
        self.assertIn("full factorial proof-grid claim", self.result["no_go_criteria"][0])
        self.assertEqual(self.result["next_low_risk_profiles"][0]["profile_id"], "d64_four_head_seq32")
