...
from scripts import zkai_attention_kv_stwo_native_d32_two_head_longseq_bounded_softmax_table_proof_input as gate


class AttentionKvD32TwoHeadLongseqBoundedSoftmaxTableInputTests(unittest.TestCase):
    def test_payload_builds_checked_weighted_attention_surface(self):
        ...
        self.assertEqual(payload["attention_outputs"][0], [2, -4, 1, -5, 0, 4, -1, 3, -2, 2, -3, 2, -4, 1, -5, 0, 4, -1, 3, -2, 2, -3, 2, -4, 1, -5, 0, 4, -1, 3, -2, 2])
        self.assertEqual(payload["attention_outputs"][8], [3, -1, 2, -2, 2, -3, 1, -4, 0, -5, -1, 3, -1, 2, -2, 2, -3, 1, -4, 0, -5, -1, 3, -1, 2, -2, 2, -3, 1, -4, 0, -5])
        self.assertEqual(payload["attention_outputs"][-1], [0, 1, -2, 2, 0, -2, -1, -3, 1, -3, 2, -2, 1, -2, 2, 1, -4, -1, -2, 1, -2, 0, -2, 1, -1, 3, -1, -3, -1, -2, 2, -4])
    ...
    def test_rejects_weight_policy_drift(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD32TwoHeadLongseqBoundedSoftmaxTableInputError, "weight_policy drift"):
            gate.validate_payload(payload)

    def test_rejects_weight_relabeling(self):
        payload = gate.build_payload()
        payload["score_rows"][0]["attention_weight"] = 15
        with self.assertRaisesRegex(gate.AttentionKvD32TwoHeadLongseqBoundedSoftmaxTableInputError, "score rows drift"):
            gate.validate_payload(payload)

    def test_rejects_head_relabeling(self):
        payload = gate.build_payload()
        payload["input_steps"][1]["head_index"] = 0
        with self.assertRaisesRegex(gate.AttentionKvD32TwoHeadLongseqBoundedSoftmaxTableInputError, "input steps drift"):
            gate.validate_payload(payload)

    def test_rejects_output_relabeling(self):
        payload = gate.build_payload()
        payload["attention_outputs"][0][0] = 99
        with self.assertRaisesRegex(gate.AttentionKvD32TwoHeadLongseqBoundedSoftmaxTableInputError, "attention outputs drift"):
            gate.validate_payload(payload)

    def test_rejects_commitment_relabeling(self):
        payload = gate.build_payload()
        payload["statement_commitment"] = "blake2b-256:" + "55" * 32
        with self.assertRaisesRegex(gate.AttentionKvD32TwoHeadLongseqBoundedSoftmaxTableInputError, "statement commitment drift"):
            gate.validate_payload(payload)

    def test_rejects_unknown_top_level_field(self):
        payload = gate.build_payload()
        payload["unexpected"] = "claim smuggling"
        with self.assertRaisesRegex(gate.AttentionKvD32TwoHeadLongseqBoundedSoftmaxTableInputError, "unknown field"):
            gate.validate_payload(payload)
    ...
    def test_rejects_source_payload_identity_drift(self):
        ...
        with mock.patch.object(gate.SOURCE, "build_payload", return_value=payload):
            with self.assertRaisesRegex(gate.AttentionKvD32TwoHeadLongseqBoundedSoftmaxTableInputError, "source payload head_count drift"):
                gate.build_payload()
    ...
    def test_rejects_source_payload_commitment_drift(self):
        ...
        with mock.patch.object(gate.SOURCE, "build_payload", return_value=payload):
            with self.assertRaisesRegex(gate.AttentionKvD32TwoHeadLongseqBoundedSoftmaxTableInputError, "source payload commitment drift"):
                gate.build_payload()
    ...
    def test_build_score_rows_rejects_malformed_input_step_shape(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD32TwoHeadLongseqBoundedSoftmaxTableInputError, r"input_steps\[0\]\.query width drift"):
    ...
    def test_build_score_rows_rejects_malformed_candidate_shape(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD32TwoHeadLongseqBoundedSoftmaxTableInputError, r"initial_kv\[0\]\.value width drift"):
            gate.build_score_rows(initial, steps)
    ...
    def test_build_score_rows_rejects_invalid_head_index(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD32TwoHeadLongseqBoundedSoftmaxTableInputError, r"input_steps\[0\]\.head_index outside head range"):
            gate.build_score_rows(initial, steps)
    ...
    def test_build_score_rows_rejects_token_position_drift(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD32TwoHeadLongseqBoundedSoftmaxTableInputError, r"input_steps\[0\]\.token_position drift"):
            gate.build_score_rows(initial, steps)

    def test_build_score_rows_rejects_missing_per_head_steps(self):
        initial = gate.fixture_initial_kv()
        steps = gate.fixture_input_steps()[:-1]
        with self.assertRaisesRegex(gate.AttentionKvD32TwoHeadLongseqBoundedSoftmaxTableInputError, "per-head input step count drift"):
            gate.build_score_rows(initial, steps)
    ...
    def test_build_score_rows_rejects_score_gap_bit_overflow(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD32TwoHeadLongseqBoundedSoftmaxTableInputError, r"score_gap\[1\] outside 16-bit range"):
            gate.build_score_rows(initial, steps)
    ...
    def test_build_score_rows_rejects_derived_dot_product_overflow(self):
        ...
        with self.assertRaisesRegex(
            gate.AttentionKvD32TwoHeadLongseqBoundedSoftmaxTableInputError,
    ...
    def test_rejects_output_remainder_bit_overflow(self):
        with self.assertRaisesRegex(gate.AttentionKvD32TwoHeadLongseqBoundedSoftmaxTableInputError, "outside 16-bit range"):
