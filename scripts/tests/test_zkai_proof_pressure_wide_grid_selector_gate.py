...
class ProofPressureWideGridSelectorGateTests(unittest.TestCase):
    ...
    def test_records_wide_grid_as_falsification_target_not_result(self):
        ...
        self.assertIn("D128_HAS_TWO_SOURCE_BACKED_TWO_HEAD_ROWS", payload["claim_boundary"])
        ...
        self.assertEqual(requested["source_backed_requested_cell_count"], 9)
        self.assertEqual(requested["missing_requested_cell_count"], 18)
        self.assertEqual(
        ...
        self.assertEqual(requested["selector_status"], "PARTIAL_D64_AND_TWO_D128_SOURCE_BACKED_D256_MISSING")
    ...
    def test_binds_current_route_matrix_signal(self):
        ...
        self.assertEqual(current["checked_attention_route_rows"], 26)
        self.assertEqual(current["checked_widths"], [8, 16, 32, 64, 128])
        self.assertEqual(current["checked_sequences"], [8, 16, 32, 64])
        self.assertEqual(current["raw_fused_bytes_total"], 4234408)
        self.assertEqual(current["raw_split_bytes_total"], 4871057)
        self.assertEqual(current["raw_saving_bytes_total"], 636649)
        self.assertEqual(
        ...
        self.assertEqual(accounting["attention_raw_proof_savings_bytes_total"], 636649)
    ...
    def test_selects_d128_first_after_single_head_anchor_landed(self):
        ...
        first = candidates[0]
        self.assertEqual(first["selector_status"], "NEXT_D128_HEAD_AXIS_AFTER_D128_SEQ64_GO")
        self.assertIn("d128 two-head rows", first["why_this_row"])
    ...
    def test_validate_rejects_wide_row_smuggling(self):
        ...
        payload["requested_grid_signal"]["source_backed_requested_cell_count"] = 10
    ...
    def test_write_json_and_tsv_round_trip(self):
        with self.evidence_tempdir() as tmp:
            ...
            self.assertIn("d128_h4_seq32", tsv)
            self.assertIn("NEXT_D128_HEAD_AXIS_AFTER_D128_SEQ64_GO", tsv)
