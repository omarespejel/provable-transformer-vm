...
from scripts import zkai_attention_kv_d32_two_head_fused_softmax_table_native_gate as gate


class AttentionKvD32TwoHeadFusedSoftmaxTableNativeGateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_gate_records_fused_go_and_byte_delta(self):
        ...
        self.assertEqual(payload["source_proof_size_bytes"], 123926)
        self.assertEqual(payload["sidecar_proof_size_bytes"], 18137)
        self.assertEqual(payload["source_plus_sidecar_raw_proof_bytes"], 142063)
        self.assertEqual(payload["fused_proof_size_bytes"], 125756)
        self.assertEqual(payload["fused_over_source_proof_bytes"], 1830)
        self.assertEqual(payload["fused_saves_vs_source_plus_sidecar_bytes"], 16307)
    ...
    def test_fused_summary_counts_table_multiplicities(self):
        ...
        self.assertEqual(summary["table_multiplicities"][-1]["multiplicity"], 79)

    def test_all_declared_mutations_reject(self):
        for name, mutated, run_native in gate.mutation_cases(self.fused_envelope):
            with self.assertRaises(gate.AttentionKvD32TwoHeadFusedSoftmaxTableGateError, msg=name):
    ...
    def test_rejects_sidecar_or_source_proof_injection(self):
        for key in ("sidecar_proof", "source_proof"):
            ...
            mutated[key] = []
            with self.assertRaisesRegex(gate.AttentionKvD32TwoHeadFusedSoftmaxTableGateError, "unknown fused envelope field"):
    ...
    def test_rejects_source_input_split_brain(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD32TwoHeadFusedSoftmaxTableGateError, "source input split-brain"):
            gate.validate_fused_envelope(mutated, self.source_input, run_native=False)
    ...
    def test_native_verifier_rejects_same_size_tampered_proof_payload(self):
        ...
        try:
            with self.assertRaisesRegex(
                gate.AttentionKvD32TwoHeadFusedSoftmaxTableGateError,
    ...
    def test_gate_checks_source_sidecar_and_fused_envelope_byte_sizes(self):
        ...
        for label, path, expected_size in cases:
            ...
            with self.assertRaisesRegex(gate.AttentionKvD32TwoHeadFusedSoftmaxTableGateError, f"{label} size drift"):
    ...
    def test_write_json_and_tsv_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("125756", tsv_path.read_text(encoding="utf-8"))
