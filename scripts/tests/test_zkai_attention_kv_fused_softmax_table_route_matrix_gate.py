...
class AttentionKvFusedSoftmaxTableRouteMatrixGateTests(unittest.TestCase):
    ...
    def test_records_controlled_axis_matrix_with_matched_eight_head_comparator(self):
        ...
        self.assertEqual(result["profiles_checked"], 16)
        self.assertEqual(result["matched_comparator_profiles"], 16)
    ...
    def test_aggregate_metrics_are_checked(self):
        ...
        self.assertEqual(metrics["total_lookup_claims"], 6820)
        self.assertEqual(metrics["total_trace_rows"], 10560)
        self.assertEqual(metrics["total_fused_proof_size_bytes"], 1636934)
        self.assertEqual(metrics["max_fused_proof_size_bytes"], 253257)
        self.assertEqual(metrics["matched_source_plus_sidecar_raw_proof_bytes_total"], 1954325)
        self.assertEqual(metrics["matched_fused_proof_size_bytes_total"], 1636934)
        self.assertEqual(metrics["matched_fused_savings_bytes_total"], 317391)
    ...
    def test_write_json_and_tsv_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(rows[12]["profile_id"], "d32_two_head_seq16")
            self.assertEqual(rows[12]["source_plus_sidecar_raw_proof_bytes"], "162138")
            self.assertEqual(rows[12]["fused_to_source_plus_sidecar_ratio"], "0.81747")
            self.assertEqual(rows[13]["profile_id"], "d32_two_head_seq32")
            self.assertEqual(rows[13]["source_plus_sidecar_raw_proof_bytes"], "176473")
            self.assertEqual(rows[13]["fused_to_source_plus_sidecar_ratio"], "0.850821")
            self.assertEqual(rows[14]["profile_id"], "d64_two_head_seq16")
            self.assertEqual(rows[14]["source_plus_sidecar_raw_proof_bytes"], "257725")
            self.assertEqual(rows[14]["fused_to_source_plus_sidecar_ratio"], "0.925421")
            self.assertEqual(rows[15]["profile_id"], "d64_two_head_seq32")
            self.assertEqual(rows[15]["source_plus_sidecar_raw_proof_bytes"], "285102")
            self.assertEqual(rows[15]["fused_to_source_plus_sidecar_ratio"], "0.888303")
