...
class AttentionKvFusedSoftmaxTableRouteMatrixGateTests(unittest.TestCase):
    ...
    def test_records_controlled_axis_matrix_with_matched_eight_head_comparator(self):
        ...
        self.assertEqual(result["profiles_checked"], 23)
        self.assertEqual(result["matched_comparator_profiles"], 23)
    ...
    def test_axis_summary_separates_width_head_and_sequence_effects(self):
        ...
        self.assertEqual(d64_seq16_head["profile_ids"], ["d64_two_head_seq16", "d64_four_head_seq16"])
        self.assertEqual(d64_seq16_head["head_counts"], [2, 4])
        self.assertEqual(d64_seq16_head["lookup_claims"], [336, 672])
        self.assertEqual(d64_seq16_head["trace_rows"], [512, 1024])
        self.assertEqual(d64_seq16_head["source_proof_size_bytes"], [230688, 232991])
        self.assertEqual(d64_seq16_head["sidecar_proof_size_bytes"], [27037, 27694])
        self.assertEqual(d64_seq16_head["fused_proof_size_bytes"], [238504, 237596])
        self.assertEqual(d64_seq16_head["source_plus_sidecar_raw_proof_bytes"], [257725, 260685])
        self.assertEqual(d64_seq16_head["fused_to_source_plus_sidecar_ratios"], [0.925421, 0.91143])
    ...
    def test_aggregate_metrics_are_checked(self):
        ...
        self.assertEqual(metrics["total_lookup_claims"], 27332)
        self.assertEqual(metrics["total_trace_rows"], 47424)
        self.assertEqual(metrics["total_fused_proof_size_bytes"], 3068925)
        self.assertEqual(metrics["max_fused_proof_size_bytes"], 276503)
        self.assertEqual(metrics["matched_source_plus_sidecar_raw_proof_bytes_total"], 3616219)
        self.assertEqual(metrics["matched_fused_proof_size_bytes_total"], 3068925)
        self.assertEqual(metrics["matched_fused_savings_bytes_total"], 547294)
    ...
    def test_write_json_and_tsv_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(rows[18]["profile_id"], "d64_four_head_seq16")
            self.assertEqual(rows[18]["source_plus_sidecar_raw_proof_bytes"], "260685")
            self.assertEqual(rows[18]["fused_to_source_plus_sidecar_ratio"], "0.91143")
            self.assertEqual(rows[19]["profile_id"], "d64_two_head_seq32")
            self.assertEqual(rows[19]["source_plus_sidecar_raw_proof_bytes"], "285102")
            self.assertEqual(rows[19]["fused_to_source_plus_sidecar_ratio"], "0.888303")
            self.assertEqual(rows[20]["profile_id"], "d64_two_head_seq64")
            self.assertEqual(rows[20]["source_plus_sidecar_raw_proof_bytes"], "306970")
            self.assertEqual(rows[20]["fused_to_source_plus_sidecar_ratio"], "0.888152")
            self.assertEqual(rows[21]["profile_id"], "d64_four_head_seq32")
            self.assertEqual(rows[21]["source_plus_sidecar_raw_proof_bytes"], "288292")
            self.assertEqual(rows[21]["fused_to_source_plus_sidecar_ratio"], "0.887604")
            self.assertEqual(rows[22]["profile_id"], "d64_four_head_seq64")
            self.assertEqual(rows[22]["source_plus_sidecar_raw_proof_bytes"], "315785")
            self.assertEqual(rows[22]["fused_to_source_plus_sidecar_ratio"], "0.875605")
