...
class AttentionKvFusedSoftmaxTableRouteMatrixGateTests(unittest.TestCase):
    ...
    def test_records_controlled_axis_matrix_with_matched_eight_head_comparator(self):
        ...
        self.assertEqual(result["profiles_checked"], 30)
        self.assertEqual(result["matched_comparator_profiles"], 30)
    ...
    def test_route_rows_match_expected_dimensions_and_existing_gate_metrics(self):
        ...
        self.assertEqual(
            rows["d128_single_head_seq16"]["axis_role"],
            "combined_width_sequence_axis_d128_single_head_seq16_width_anchor",
        )
        self.assertEqual(rows["d128_single_head_seq16"]["key_width"], 128)
        self.assertEqual(rows["d128_single_head_seq16"]["head_count"], 1)
        self.assertEqual(rows["d128_single_head_seq16"]["steps_per_head"], 16)
        self.assertEqual(rows["d128_single_head_seq16"]["lookup_claims"], 168)
        self.assertEqual(rows["d128_single_head_seq16"]["trace_rows"], 256)
        self.assertEqual(rows["d128_single_head_seq16"]["source_proof_size_bytes"], 374261)
        self.assertEqual(rows["d128_single_head_seq16"]["sidecar_proof_size_bytes"], 23052)
        self.assertEqual(rows["d128_single_head_seq16"]["source_plus_sidecar_raw_proof_bytes"], 397313)
        self.assertEqual(rows["d128_single_head_seq16"]["fused_proof_size_bytes"], 380342)
        self.assertEqual(rows["d128_single_head_seq16"]["fused_over_source_proof_bytes"], 6081)
        self.assertEqual(rows["d128_single_head_seq16"]["fused_saves_vs_source_plus_sidecar_bytes"], 16971)
        self.assertEqual(rows["d128_single_head_seq16"]["fused_to_source_plus_sidecar_ratio"], 0.957286)
        ...
        self.assertEqual(
            rows["d128_two_head_seq64"]["axis_role"],
            "combined_width_head_sequence_axis_d128_seq64_sequence_frontier",
        )
        self.assertEqual(rows["d128_two_head_seq64"]["key_width"], 128)
        self.assertEqual(rows["d128_two_head_seq64"]["head_count"], 2)
        self.assertEqual(rows["d128_two_head_seq64"]["steps_per_head"], 64)
        self.assertEqual(rows["d128_two_head_seq64"]["lookup_claims"], 4416)
        self.assertEqual(rows["d128_two_head_seq64"]["trace_rows"], 8192)
        self.assertEqual(rows["d128_two_head_seq64"]["source_proof_size_bytes"], 476773)
        self.assertEqual(rows["d128_two_head_seq64"]["sidecar_proof_size_bytes"], 45414)
        self.assertEqual(rows["d128_two_head_seq64"]["source_plus_sidecar_raw_proof_bytes"], 522187)
        self.assertEqual(rows["d128_two_head_seq64"]["fused_proof_size_bytes"], 481870)
        self.assertEqual(rows["d128_two_head_seq64"]["fused_over_source_proof_bytes"], 5097)
        self.assertEqual(rows["d128_two_head_seq64"]["fused_saves_vs_source_plus_sidecar_bytes"], 40317)
        self.assertEqual(rows["d128_two_head_seq64"]["fused_to_source_plus_sidecar_ratio"], 0.922792)

        self.assertEqual(
            rows["d128_four_head_seq32"]["axis_role"],
            "combined_width_head_sequence_axis_d128_seq32_head_frontier",
        )
        self.assertEqual(rows["d128_four_head_seq32"]["key_width"], 128)
        self.assertEqual(rows["d128_four_head_seq32"]["head_count"], 4)
        self.assertEqual(rows["d128_four_head_seq32"]["steps_per_head"], 32)
        self.assertEqual(rows["d128_four_head_seq32"]["lookup_claims"], 2368)
        self.assertEqual(rows["d128_four_head_seq32"]["trace_rows"], 4096)
        self.assertEqual(rows["d128_four_head_seq32"]["source_proof_size_bytes"], 463410)
        self.assertEqual(rows["d128_four_head_seq32"]["sidecar_proof_size_bytes"], 41524)
        self.assertEqual(rows["d128_four_head_seq32"]["source_plus_sidecar_raw_proof_bytes"], 504934)
        self.assertEqual(rows["d128_four_head_seq32"]["fused_proof_size_bytes"], 465630)
        self.assertEqual(rows["d128_four_head_seq32"]["fused_over_source_proof_bytes"], 2220)
        self.assertEqual(rows["d128_four_head_seq32"]["fused_saves_vs_source_plus_sidecar_bytes"], 39304)
        self.assertEqual(rows["d128_four_head_seq32"]["fused_to_source_plus_sidecar_ratio"], 0.92216)
    ...
    def test_axis_summary_separates_width_head_and_sequence_effects(self):
        ...
        d256_seq32 = summary["combined_width_head_sequence_axis_d256_seq32_width_stress"]
        self.assertEqual(d256_seq32["profile_ids"], ["d64_two_head_seq32", "d128_two_head_seq32", "d256_two_head_seq32"])
        self.assertEqual(d256_seq32["key_widths"], [64, 128, 256])
        self.assertEqual(d256_seq32["lookup_claims"], [1184, 1184, 1184])
        self.assertEqual(d256_seq32["trace_rows"], [2048, 2048, 2048])
        self.assertEqual(d256_seq32["source_proof_size_bytes"], [248702, 443266, 816627])
        self.assertEqual(d256_seq32["sidecar_proof_size_bytes"], [36400, 35010, 34914])
        self.assertEqual(d256_seq32["fused_proof_size_bytes"], [253257, 445888, 821398])
        self.assertEqual(d256_seq32["source_plus_sidecar_raw_proof_bytes"], [285102, 478276, 851541])
        self.assertEqual(d256_seq32["fused_to_source_plus_sidecar_ratios"], [0.888303, 0.932282, 0.964602])
        self.assertEqual(d256_seq32["d128_to_d256_key_width_ratio"], 2.0)
        self.assertEqual(d256_seq32["d128_to_d256_lookup_claim_ratio"], 1.0)
        self.assertEqual(d256_seq32["d128_to_d256_trace_row_ratio"], 1.0)
        self.assertEqual(d256_seq32["d128_to_d256_source_proof_size_ratio"], 1.842296)
        self.assertEqual(d256_seq32["d128_to_d256_sidecar_proof_size_ratio"], 0.997258)
        self.assertEqual(d256_seq32["d128_to_d256_fused_proof_size_ratio"], 1.842162)
        self.assertEqual(d256_seq32["d128_to_d256_source_plus_sidecar_ratio"], 1.780438)
        self.assertEqual(d256_seq32["d128_to_d256_savings_ratio"], 0.930684)
        self.assertEqual(d256_seq32["matched_comparator_status"], gate.MATCHED_COMPARATOR_STATUS)

        d128_sequence = summary["combined_width_head_sequence_axis_d128_sequence_frontier"]
        self.assertEqual(d128_sequence["profile_ids"], ["d128_two_head_seq32", "d128_two_head_seq64"])
        self.assertEqual(d128_sequence["steps_per_head"], [32, 64])
        self.assertEqual(d128_sequence["lookup_claims"], [1184, 4416])
        self.assertEqual(d128_sequence["trace_rows"], [2048, 8192])
        self.assertEqual(d128_sequence["source_proof_size_bytes"], [443266, 476773])
        self.assertEqual(d128_sequence["sidecar_proof_size_bytes"], [35010, 45414])
        self.assertEqual(d128_sequence["fused_proof_size_bytes"], [445888, 481870])
        self.assertEqual(d128_sequence["source_plus_sidecar_raw_proof_bytes"], [478276, 522187])
        self.assertEqual(d128_sequence["fused_to_source_plus_sidecar_ratios"], [0.932282, 0.922792])
        self.assertEqual(d128_sequence["seq32_to_seq64_lookup_claim_ratio"], 3.72973)
        self.assertEqual(d128_sequence["seq32_to_seq64_trace_row_ratio"], 4.0)
        self.assertEqual(d128_sequence["seq32_to_seq64_source_proof_size_ratio"], 1.075591)
        self.assertEqual(d128_sequence["seq32_to_seq64_sidecar_proof_size_ratio"], 1.297172)
        self.assertEqual(d128_sequence["seq32_to_seq64_fused_proof_size_ratio"], 1.080697)
        self.assertEqual(d128_sequence["seq32_to_seq64_source_plus_sidecar_ratio"], 1.091811)
        self.assertEqual(d128_sequence["seq32_to_seq64_savings_ratio"], 1.244813)

        d128_seq32_head = summary["combined_width_head_sequence_axis_d128_seq32_head_frontier"]
        self.assertEqual(d128_seq32_head["profile_ids"], ["d128_two_head_seq32", "d128_four_head_seq32"])
        self.assertEqual(d128_seq32_head["head_counts"], [2, 4])
        self.assertEqual(d128_seq32_head["lookup_claims"], [1184, 2368])
        self.assertEqual(d128_seq32_head["trace_rows"], [2048, 4096])
        self.assertEqual(d128_seq32_head["source_proof_size_bytes"], [443266, 463410])
        self.assertEqual(d128_seq32_head["sidecar_proof_size_bytes"], [35010, 41524])
        self.assertEqual(d128_seq32_head["fused_proof_size_bytes"], [445888, 465630])
        self.assertEqual(d128_seq32_head["source_plus_sidecar_raw_proof_bytes"], [478276, 504934])
        self.assertEqual(d128_seq32_head["fused_to_source_plus_sidecar_ratios"], [0.932282, 0.92216])
        self.assertEqual(d128_seq32_head["two_to_four_head_ratio"], 2.0)
        self.assertEqual(d128_seq32_head["two_to_four_lookup_claim_ratio"], 2.0)
        self.assertEqual(d128_seq32_head["two_to_four_trace_row_ratio"], 2.0)
        self.assertEqual(d128_seq32_head["two_to_four_source_proof_size_ratio"], 1.045444)
        self.assertEqual(d128_seq32_head["two_to_four_sidecar_proof_size_ratio"], 1.186061)
        self.assertEqual(d128_seq32_head["two_to_four_fused_proof_size_ratio"], 1.044276)
        self.assertEqual(d128_seq32_head["two_to_four_source_plus_sidecar_ratio"], 1.055738)
        self.assertEqual(d128_seq32_head["two_to_four_savings_ratio"], 1.213536)

        d128_seq64_width = summary["combined_width_head_sequence_axis_d128_seq64_width_frontier"]
        self.assertEqual(d128_seq64_width["profile_ids"], ["d64_two_head_seq64", "d128_two_head_seq64"])
        self.assertEqual(d128_seq64_width["key_widths"], [64, 128])
        self.assertEqual(d128_seq64_width["lookup_claims"], [4416, 4416])
        self.assertEqual(d128_seq64_width["trace_rows"], [8192, 8192])
        self.assertEqual(d128_seq64_width["source_proof_size_bytes"], [264403, 476773])
        self.assertEqual(d128_seq64_width["sidecar_proof_size_bytes"], [42567, 45414])
        self.assertEqual(d128_seq64_width["fused_proof_size_bytes"], [272636, 481870])
        self.assertEqual(d128_seq64_width["source_plus_sidecar_raw_proof_bytes"], [306970, 522187])
        self.assertEqual(d128_seq64_width["fused_to_source_plus_sidecar_ratios"], [0.888152, 0.922792])
        self.assertEqual(d128_seq64_width["d64_to_d128_key_width_ratio"], 2.0)
        self.assertEqual(d128_seq64_width["d64_to_d128_lookup_claim_ratio"], 1.0)
        self.assertEqual(d128_seq64_width["d64_to_d128_trace_row_ratio"], 1.0)
        self.assertEqual(d128_seq64_width["d64_to_d128_source_proof_size_ratio"], 1.803206)
        self.assertEqual(d128_seq64_width["d64_to_d128_sidecar_proof_size_ratio"], 1.066883)
        ...
        d128_single_width_anchor = summary["combined_width_sequence_axis_d128_single_head_seq16_width_anchor"]
        self.assertEqual(
            d128_single_width_anchor["profile_ids"],
            ["d64_single_head_seq16", "d128_single_head_seq16"],
        )
        self.assertEqual(d128_single_width_anchor["key_widths"], [64, 128])
        self.assertEqual(d128_single_width_anchor["lookup_claims"], [168, 168])
        self.assertEqual(d128_single_width_anchor["trace_rows"], [256, 256])
        self.assertEqual(d128_single_width_anchor["source_proof_size_bytes"], [231415, 374261])
        self.assertEqual(d128_single_width_anchor["sidecar_proof_size_bytes"], [22960, 23052])
        self.assertEqual(d128_single_width_anchor["fused_proof_size_bytes"], [237725, 380342])
        self.assertEqual(d128_single_width_anchor["source_plus_sidecar_raw_proof_bytes"], [254375, 397313])
        self.assertEqual(d128_single_width_anchor["fused_to_source_plus_sidecar_ratios"], [0.934545, 0.957286])
        self.assertEqual(d128_single_width_anchor["d64_to_d128_key_width_ratio"], 2.0)
        self.assertEqual(d128_single_width_anchor["d64_to_d128_lookup_claim_ratio"], 1.0)
        self.assertEqual(d128_single_width_anchor["d64_to_d128_trace_row_ratio"], 1.0)
        self.assertEqual(d128_single_width_anchor["d64_to_d128_source_proof_size_ratio"], 1.617272)
        self.assertEqual(d128_single_width_anchor["d64_to_d128_sidecar_proof_size_ratio"], 1.004007)
        self.assertEqual(d128_single_width_anchor["d64_to_d128_fused_proof_size_ratio"], 1.599924)
        self.assertEqual(d128_single_width_anchor["d64_to_d128_source_plus_sidecar_ratio"], 1.561918)
        self.assertEqual(d128_single_width_anchor["d64_to_d128_savings_ratio"], 1.019279)

    def test_aggregate_metrics_are_checked(self):
        metrics = self.result["aggregate_metrics"]
        self.assertEqual(metrics["total_lookup_claims"], 45652)
        self.assertEqual(metrics["total_trace_rows"], 80704)
        self.assertEqual(metrics["total_fused_proof_size_bytes"], 6397632)
        self.assertEqual(metrics["max_fused_proof_size_bytes"], 821398)
        self.assertEqual(metrics["matched_source_plus_sidecar_raw_proof_bytes_total"], 7164515)
        self.assertEqual(metrics["matched_fused_proof_size_bytes_total"], 6397632)
        self.assertEqual(metrics["matched_fused_savings_bytes_total"], 766883)
        self.assertEqual(metrics["min_matched_fused_to_source_plus_sidecar_ratio"], 0.676723)
        self.assertEqual(metrics["max_matched_fused_to_source_plus_sidecar_ratio"], 0.964602)
    ...
    def test_write_json_and_tsv_round_trip(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(rows[24]["profile_id"], "d128_single_head_seq16")
            self.assertEqual(rows[24]["source_plus_sidecar_raw_proof_bytes"], "397313")
            self.assertEqual(rows[24]["fused_to_source_plus_sidecar_ratio"], "0.957286")
            self.assertEqual(rows[25]["profile_id"], "d128_two_head_seq32")
            self.assertEqual(rows[25]["source_plus_sidecar_raw_proof_bytes"], "478276")
            self.assertEqual(rows[25]["fused_to_source_plus_sidecar_ratio"], "0.932282")
            self.assertEqual(rows[26]["profile_id"], "d128_two_head_seq64")
            self.assertEqual(rows[26]["source_plus_sidecar_raw_proof_bytes"], "522187")
            self.assertEqual(rows[26]["fused_to_source_plus_sidecar_ratio"], "0.922792")
            self.assertEqual(rows[27]["profile_id"], "d128_four_head_seq32")
            self.assertEqual(rows[27]["source_plus_sidecar_raw_proof_bytes"], "504934")
            self.assertEqual(rows[27]["fused_to_source_plus_sidecar_ratio"], "0.92216")
            self.assertEqual(rows[28]["profile_id"], "d128_four_head_seq64")
            self.assertEqual(rows[28]["source_plus_sidecar_raw_proof_bytes"], "539670")
            self.assertEqual(rows[28]["fused_to_source_plus_sidecar_ratio"], "0.91881")
            self.assertEqual(rows[29]["profile_id"], "d256_two_head_seq32")
            self.assertEqual(rows[29]["source_plus_sidecar_raw_proof_bytes"], "851541")
            self.assertEqual(rows[29]["fused_to_source_plus_sidecar_ratio"], "0.964602")
