...
class AttentionKvD256TwoHeadSeq32AirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    def evidence_tempdir(self):
        return tempfile.TemporaryDirectory(dir=gate.EVIDENCE_DIR, prefix=".tmp-d256-lookup-gate-test-")
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 1109)
        self.assertEqual(payload["fixed_d64_two_head_comparison"]["fixed_d64_two_head_lookup_claims"], 1184)
        self.assertEqual(payload["fixed_d64_two_head_comparison"]["proof_size_ratio"], "0.959176")
        self.assertEqual(payload["fused_comparator"]["source_plus_sidecar_raw_proof_bytes"], 851541)
        self.assertEqual(payload["fused_comparator"]["fused_saves_vs_source_plus_sidecar_bytes"], 30143)
        self.assertEqual(payload["fused_comparator"]["fused_to_source_plus_sidecar_ratio"], "0.964602")
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn("34914", tsv)
        self.assertIn("851541", tsv)
        self.assertIn("30143", tsv)
        self.assertIn("0.959176", tsv)
        self.assertIn("0.964602", tsv)
