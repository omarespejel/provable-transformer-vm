...
from scripts import zkai_attention_kv_d128_two_head_seq32_air_private_softmax_table_lookup_gate as gate
...
class AttentionKvD128TwoHeadSeq32AirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["lookup_proof_size_bytes"], 35010)
        self.assertEqual(receipt["lookup_envelope_size_bytes"], 18862726)
        ...
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 1113)
        self.assertEqual(payload["fixed_d64_two_head_comparison"]["fixed_d64_two_head_lookup_claims"], 1184)
        self.assertEqual(payload["fixed_d64_two_head_comparison"]["proof_size_ratio"], "0.961813")
        self.assertEqual(payload["fused_comparator"]["source_plus_sidecar_raw_proof_bytes"], 478276)
        self.assertEqual(payload["fused_comparator"]["fused_saves_vs_source_plus_sidecar_bytes"], 32388)
        self.assertEqual(payload["fused_comparator"]["fused_to_source_plus_sidecar_ratio"], "0.932282")
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn("478276", tsv)
        self.assertIn("32388", tsv)
        self.assertIn("0.961813", tsv)
