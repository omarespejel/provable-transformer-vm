...
from scripts import zkai_attention_kv_d64_four_head_seq64_fused_softmax_table_native_gate as gate


class AttentionKvD64FourHeadSeq64FusedSoftmaxTableNativeGateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_gate_records_fused_go_and_byte_delta(self):
        ...
        self.assertEqual(payload["lookup_claims"], 8832)
        self.assertEqual(payload["table_rows"], 9)
        self.assertEqual(payload["source_head_count"], 4)
        self.assertEqual(payload["source_proof_size_bytes"], 272638)
        self.assertEqual(payload["sidecar_proof_size_bytes"], 43147)
        self.assertEqual(payload["source_plus_sidecar_raw_proof_bytes"], 315785)
        self.assertEqual(payload["fused_proof_size_bytes"], 276503)
        self.assertEqual(payload["fused_over_source_proof_bytes"], 3865)
        self.assertEqual(payload["fused_saves_vs_source_plus_sidecar_bytes"], 39282)
        self.assertLess(payload["fused_to_source_plus_sidecar_ratio"], 0.88)
    ...
    def test_fused_summary_counts_table_multiplicities(self):
        ...
        self.assertEqual(sum(row["multiplicity"] for row in summary["table_multiplicities"]), 8832)
        self.assertEqual(summary["table_multiplicities"][-1]["gap"], 8)
        self.assertEqual(summary["table_multiplicities"][-1]["multiplicity"], 8538)

    def test_all_declared_mutations_reject(self):
        for name, mutated, run_native in gate.mutation_cases(self.fused_envelope):
            with self.assertRaises(gate.AttentionKvD64FourHeadSeq64FusedSoftmaxTableGateError, msg=name):
    ...
    def test_rejects_sidecar_or_source_proof_injection(self):
        for key in ("sidecar_proof", "source_proof"):
            ...
            mutated[key] = []
            with self.assertRaisesRegex(gate.AttentionKvD64FourHeadSeq64FusedSoftmaxTableGateError, "unknown fused envelope field"):
    ...
    def test_rejects_source_input_split_brain(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD64FourHeadSeq64FusedSoftmaxTableGateError, "source input split-brain"):
            gate.validate_fused_envelope(mutated, self.source_input, run_native=False)
    ...
    def test_native_verifier_rejects_same_size_tampered_proof_payload(self):
        ...
        try:
            with self.assertRaisesRegex(
                gate.AttentionKvD64FourHeadSeq64FusedSoftmaxTableGateError,
    ...
    def test_gate_checks_source_sidecar_and_fused_envelope_byte_sizes(self):
        ...
        for label, path, expected_size in cases:
            ...
            with self.assertRaisesRegex(gate.AttentionKvD64FourHeadSeq64FusedSoftmaxTableGateError, f"{label} size drift"):
    ...
    def test_write_json_and_tsv_round_trip(self):
        with tempfile.TemporaryDirectory(dir=gate.EVIDENCE_DIR, prefix=".tmp-d64-h4-fused-test-") as tmp_dir:
            ...
            self.assertIn("276503", tsv_path.read_text(encoding="utf-8"))
    ...
    def test_input_helpers_reject_symlink_paths(self):
        with tempfile.TemporaryDirectory(dir=gate.ROOT) as tmp_dir:
            ...
            with self.assertRaisesRegex(gate.AttentionKvD64FourHeadSeq64FusedSoftmaxTableGateError, "symlink"):
