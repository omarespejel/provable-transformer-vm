...
from scripts import zkai_attention_kv_d64_four_head_seq32_air_private_softmax_table_lookup_gate as gate


class AttentionKvD64FourHeadSeq32AirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["lookup_proof_size_bytes"], 34147)
        self.assertEqual(receipt["lookup_envelope_size_bytes"], 19731274)
        ...
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 2221)
        self.assertEqual(payload["fixed_d64_two_head_comparison"]["fixed_d64_two_head_lookup_claims"], 1184)
        self.assertEqual(payload["fixed_d64_two_head_comparison"]["proof_size_ratio"], "0.938104")
        self.assertEqual(payload["fused_comparator"]["source_plus_sidecar_raw_proof_bytes"], 288292)
        self.assertEqual(payload["fused_comparator"]["fused_saves_vs_source_plus_sidecar_bytes"], 32403)
        self.assertEqual(payload["fused_comparator"]["fused_to_source_plus_sidecar_ratio"], "0.887604")
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn("34147", tsv)
        self.assertIn("288292", tsv)
        self.assertIn("32403", tsv)
        self.assertIn("0.938104", tsv)
