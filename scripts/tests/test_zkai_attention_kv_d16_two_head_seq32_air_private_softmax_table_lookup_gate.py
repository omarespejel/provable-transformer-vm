...
from scripts import zkai_attention_kv_d16_two_head_seq32_air_private_softmax_table_lookup_gate as gate


class AttentionKvD16TwoHeadSeq32AirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["lookup_proof_size_bytes"], 36453)
        self.assertEqual(receipt["lookup_envelope_size_bytes"], 3317663)
        ...
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 1084)
        self.assertEqual(payload["fixed_d16_two_head_comparison"]["fixed_d16_two_head_lookup_claims"], 104)
        self.assertEqual(payload["fixed_d16_two_head_comparison"]["proof_size_ratio"], "2.015314")
        self.assertEqual(payload["fused_comparator"]["source_plus_sidecar_raw_proof_bytes"], 127207)
        self.assertEqual(payload["fused_comparator"]["fused_saves_vs_source_plus_sidecar_bytes"], 34844)
        self.assertEqual(payload["fused_comparator"]["fused_to_source_plus_sidecar_ratio"], "0.726084")
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn("36453", tsv)
        self.assertIn("127207", tsv)
        self.assertIn("34844", tsv)
        self.assertIn("0.726084", tsv)
