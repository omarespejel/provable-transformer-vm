...
from scripts import zkai_attention_kv_d64_two_head_longseq_air_private_softmax_table_lookup_gate as gate


class AttentionKvD64TwoHeadLongseqAirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["lookup_claims"], 336)
        ...
        self.assertEqual(receipt["lookup_proof_size_bytes"], 27037)
        self.assertEqual(receipt["lookup_envelope_size_bytes"], 3054256)
        ...
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 297)
        self.assertEqual(payload["fixed_d64_two_head_comparison"]["fixed_d64_two_head_lookup_claims"], 104)
        self.assertEqual(payload["fixed_d64_two_head_comparison"]["proof_size_ratio"], "1.490710")
        self.assertEqual(payload["fused_comparator"]["source_plus_sidecar_raw_proof_bytes"], 257725)
        self.assertEqual(payload["fused_comparator"]["fused_saves_vs_source_plus_sidecar_bytes"], 19221)
        self.assertEqual(payload["fused_comparator"]["fused_to_source_plus_sidecar_ratio"], "0.925421")
    ...
    def test_rejects_float_encoded_lookup_claim_count(self):
        payload = self.strip_mutation_summary(gate.build_payload())
        payload["lookup_receipt"]["lookup_claims"] = 1184.0
        self.assert_rejects(payload, "lookup_receipt drift")
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn("27037", tsv)
        self.assertIn("257725", tsv)
        self.assertIn("19221", tsv)
        self.assertIn("1.490710", tsv)
