...
from scripts import zkai_attention_kv_d64_four_head_seq64_air_private_softmax_table_lookup_gate as gate


class AttentionKvD64FourHeadSeq64AirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["lookup_proof_size_bytes"], 43147)
        self.assertEqual(receipt["lookup_envelope_size_bytes"], 71779632)
        ...
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 8538)
        self.assertEqual(payload["baseline_d64_four_head_seq32_comparison"]["baseline_d64_four_head_seq32_lookup_claims"], 2368)
        self.assertEqual(payload["baseline_d64_four_head_seq32_comparison"]["proof_size_ratio"], "1.263566")
        self.assertEqual(payload["fused_comparator"]["source_plus_sidecar_raw_proof_bytes"], 315785)
        self.assertEqual(payload["fused_comparator"]["fused_saves_vs_source_plus_sidecar_bytes"], 39282)
        self.assertEqual(payload["fused_comparator"]["fused_to_source_plus_sidecar_ratio"], "0.875605")
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn("43147", tsv)
        self.assertIn("315785", tsv)
        self.assertIn("39282", tsv)
        self.assertIn("1.263566", tsv)
