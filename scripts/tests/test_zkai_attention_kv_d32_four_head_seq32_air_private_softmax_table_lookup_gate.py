...
from scripts import zkai_attention_kv_d32_four_head_seq32_air_private_softmax_table_lookup_gate as gate


class AttentionKvD32FourHeadSeq32AirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["lookup_proof_size_bytes"], 41628)
        self.assertEqual(receipt["lookup_envelope_size_bytes"], 10862447)
        ...
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 2177)
        self.assertEqual(payload["fixed_d32_two_head_comparison"]["fixed_d32_two_head_lookup_claims"], 1184)
        self.assertEqual(payload["fixed_d32_two_head_comparison"]["proof_size_ratio"], "1.343879")
        self.assertEqual(payload["fused_comparator"]["source_plus_sidecar_raw_proof_bytes"], 192937)
        self.assertEqual(payload["fused_comparator"]["fused_saves_vs_source_plus_sidecar_bytes"], 38267)
        self.assertEqual(payload["fused_comparator"]["fused_to_source_plus_sidecar_ratio"], "0.801661")
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn("41628", tsv)
        self.assertIn("192937", tsv)
        self.assertIn("38267", tsv)
        self.assertIn("1.343879", tsv)
