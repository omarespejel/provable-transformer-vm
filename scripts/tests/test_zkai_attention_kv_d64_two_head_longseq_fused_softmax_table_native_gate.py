...
from scripts import zkai_attention_kv_d64_two_head_longseq_fused_softmax_table_native_gate as gate


class AttentionKvD64TwoHeadLongseqFusedSoftmaxTableNativeGateTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def evidence_tempdir(self, prefix=".tmp-d64-longseq-fused-test-"):
    ...
    def test_gate_records_fused_go_and_byte_delta(self):
        ...
        self.assertEqual(payload["lookup_claims"], 336)
        self.assertEqual(payload["table_rows"], 9)
        self.assertEqual(payload["source_head_count"], 2)
        self.assertEqual(payload["source_proof_size_bytes"], 230688)
        self.assertEqual(payload["sidecar_proof_size_bytes"], 27037)
        self.assertEqual(payload["source_plus_sidecar_raw_proof_bytes"], 257725)
        self.assertEqual(payload["fused_proof_size_bytes"], 238504)
        self.assertEqual(payload["fused_over_source_proof_bytes"], 7816)
        self.assertEqual(payload["fused_saves_vs_source_plus_sidecar_bytes"], 19221)
    ...
    def test_fused_envelope_commitment_rejects_bytes_dict_split_brain(self):
        ...
        with self.assertRaisesRegex(
            gate.AttentionKvD64TwoHeadLongseqFusedSoftmaxTableGateError,
    ...
    def test_fused_summary_counts_table_multiplicities(self):
        ...
        self.assertEqual(sum(row["multiplicity"] for row in summary["table_multiplicities"]), 336)
        self.assertEqual(summary["table_multiplicities"][-1]["gap"], 8)
        self.assertEqual(summary["table_multiplicities"][-1]["multiplicity"], 297)

    def test_all_declared_mutations_reject(self):
        for name, mutated, run_native in gate.mutation_cases(self.fused_envelope):
            with self.assertRaises(gate.AttentionKvD64TwoHeadLongseqFusedSoftmaxTableGateError, msg=name):
    ...
    def test_rejects_sidecar_or_source_proof_injection(self):
        for key in ("sidecar_proof", "source_proof"):
            mutated = copy.deepcopy(self.fused_envelope)
            mutated[key] = []
            with self.assertRaisesRegex(gate.AttentionKvD64TwoHeadLongseqFusedSoftmaxTableGateError, "unknown fused envelope field"):
    ...
    def test_rejects_source_input_split_brain(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD64TwoHeadLongseqFusedSoftmaxTableGateError, "source input split-brain"):
            gate.validate_fused_envelope(mutated, self.source_input, run_native=False)
    ...
    def test_gate_checks_source_sidecar_and_fused_envelope_byte_sizes(self):
        ...
        for label, path, expected_size in cases:
            ...
            with self.assertRaisesRegex(gate.AttentionKvD64TwoHeadLongseqFusedSoftmaxTableGateError, f"{label} size drift"):
    ...
    def test_write_json_and_tsv_round_trip(self):
        with self.evidence_tempdir() as tmp_dir:
            ...
            self.assertIn("238504", tsv_path.read_text(encoding="utf-8"))
    ...
    def test_write_outputs_reject_leaf_symlink(self):
        with self.evidence_tempdir(prefix=".tmp-d64-longseq-fused-leaf-symlink-") as tmp:
    ...
    def test_write_outputs_reject_parent_symlink(self):
        with self.evidence_tempdir(prefix=".tmp-d64-longseq-fused-symlink-") as tmp:
            tmp_path = gate.pathlib.Path(tmp)
