...
from scripts import zkai_attention_kv_d128_four_head_seq32_air_private_softmax_table_lookup_gate as gate


class AttentionKvD128FourHeadSeq32AirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["lookup_claims"], 2368)
        ...
        self.assertEqual(receipt["lookup_proof_size_bytes"], 41524)
        self.assertEqual(receipt["lookup_envelope_size_bytes"], 37658403)
        self.assertEqual(receipt["lookup_proof_commitments"], 4)
        self.assertEqual(receipt["lookup_trace_commitments"], 3)
        self.assertEqual(sum(row["multiplicity"] for row in receipt["table_multiplicities"]), 2368)
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 2224)
        self.assertEqual(payload["fixed_d128_two_head_comparison"]["fixed_d128_two_head_lookup_claims"], 1184)
        self.assertEqual(payload["fixed_d128_two_head_comparison"]["proof_size_ratio"], "1.186061")
        self.assertEqual(payload["fused_comparator"]["source_plus_sidecar_raw_proof_bytes"], 504934)
        self.assertEqual(payload["fused_comparator"]["fused_saves_vs_source_plus_sidecar_bytes"], 39304)
        self.assertEqual(payload["fused_comparator"]["fused_to_source_plus_sidecar_ratio"], "0.922160")
    ...
    def test_rejects_float_encoded_lookup_claim_count(self):
        payload = self.strip_mutation_summary(gate.build_payload())
        payload["lookup_receipt"]["lookup_claims"] = 2368.0
        self.assert_rejects(payload, "lookup_receipt drift")

    def test_native_verifier_rejects_same_size_tampered_proof_payload(self):
        serialized = self.same_size_tampered_envelope_json()
        with tempfile.TemporaryDirectory(dir=gate.EVIDENCE_DIR, prefix=".tmp-d128-h4-sidecar-input-test-") as tmp:
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn("41524", tsv)
        self.assertIn("504934", tsv)
        self.assertIn("39304", tsv)
        self.assertIn("1.186061", tsv)
    ...
    def test_write_helpers_reject_symlink_targets(self):
        payload = gate.build_payload()
        with tempfile.TemporaryDirectory(dir=gate.EVIDENCE_DIR, prefix=".tmp-d128-h4-sidecar-test-") as tmp:
