...
from scripts import zkai_attention_kv_d32_two_head_seq32_air_private_softmax_table_lookup_gate as gate


class AttentionKvD32TwoHeadSeq32AirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["lookup_claims"], 1184)
        ...
        self.assertEqual(receipt["lookup_proof_size_bytes"], 30976)
        self.assertEqual(receipt["lookup_envelope_size_bytes"], 5492746)
        ...
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 1092)
        self.assertEqual(payload["fixed_d32_two_head_comparison"]["fixed_d32_two_head_lookup_claims"], 104)
        self.assertEqual(payload["fixed_d32_two_head_comparison"]["proof_size_ratio"], "1.707890")
        self.assertEqual(payload["fused_comparator"]["source_plus_sidecar_raw_proof_bytes"], 176473)
        self.assertEqual(payload["fused_comparator"]["fused_saves_vs_source_plus_sidecar_bytes"], 26326)
        self.assertEqual(payload["fused_comparator"]["fused_to_source_plus_sidecar_ratio"], "0.850821")
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn("30976", tsv)
        self.assertIn("176473", tsv)
        self.assertIn("26326", tsv)
        self.assertIn("1.707890", tsv)
