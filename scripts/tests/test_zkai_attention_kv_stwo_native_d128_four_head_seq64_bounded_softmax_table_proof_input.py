...
from scripts import zkai_attention_kv_stwo_native_d128_four_head_seq64_bounded_softmax_table_proof_input as gate


class AttentionKvD128FourHeadSeq64BoundedSoftmaxTableInputTests(unittest.TestCase):
    ...
    def test_rejects_weight_policy_drift(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64BoundedSoftmaxTableInputError, "weight_policy drift"):
            gate.validate_payload(payload)

    def test_rejects_weight_relabeling(self):
        payload = gate.build_payload()
        payload["score_rows"][0]["attention_weight"] = 15
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64BoundedSoftmaxTableInputError, "score rows drift"):
            gate.validate_payload(payload)

    def test_rejects_head_relabeling(self):
        payload = gate.build_payload()
        payload["input_steps"][1]["head_index"] = 0
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64BoundedSoftmaxTableInputError, "input steps drift"):
            gate.validate_payload(payload)

    def test_rejects_output_relabeling(self):
        payload = gate.build_payload()
        payload["attention_outputs"][0][0] = 99
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64BoundedSoftmaxTableInputError, "attention outputs drift"):
            gate.validate_payload(payload)

    def test_rejects_commitment_relabeling(self):
        payload = gate.build_payload()
        payload["statement_commitment"] = "blake2b-256:" + "55" * 32
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64BoundedSoftmaxTableInputError, "statement commitment drift"):
            gate.validate_payload(payload)

    def test_rejects_unknown_top_level_field(self):
        payload = gate.build_payload()
        payload["unexpected"] = "claim smuggling"
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64BoundedSoftmaxTableInputError, "unknown field"):
            gate.validate_payload(payload)
    ...
    def test_write_json_rejects_symlinked_parent_before_create(self):
        with tempfile.TemporaryDirectory(dir=gate.EVIDENCE_DIR, prefix=".tmp-d64-h4-source-test-") as tmp:
            ...
            with self.assertRaisesRegex(
                gate.AttentionKvD128FourHeadSeq64BoundedSoftmaxTableInputError,
                "symlinked parent",
            ):
    ...
    def test_rejects_source_payload_identity_drift(self):
        ...
        with mock.patch.object(gate.SOURCE, "build_payload", return_value=payload):
            with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64BoundedSoftmaxTableInputError, "source payload head_count drift"):
                gate.build_payload()
    ...
    def test_rejects_source_payload_commitment_drift(self):
        ...
        with mock.patch.object(gate.SOURCE, "build_payload", return_value=payload):
            with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64BoundedSoftmaxTableInputError, "source payload commitment drift"):
                gate.build_payload()
    ...
    def test_build_score_rows_rejects_malformed_input_step_shape(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64BoundedSoftmaxTableInputError, r"input_steps\[0\]\.query width drift"):
    ...
    def test_build_score_rows_rejects_malformed_candidate_shape(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64BoundedSoftmaxTableInputError, r"initial_kv\[0\]\.value width drift"):
            gate.build_score_rows(initial, steps)
    ...
    def test_build_score_rows_rejects_invalid_head_index(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64BoundedSoftmaxTableInputError, r"input_steps\[0\]\.head_index outside head range"):
            gate.build_score_rows(initial, steps)
    ...
    def test_build_score_rows_rejects_token_position_drift(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64BoundedSoftmaxTableInputError, r"input_steps\[0\]\.token_position drift"):
            gate.build_score_rows(initial, steps)

    def test_build_score_rows_rejects_missing_per_head_steps(self):
        initial = gate.fixture_initial_kv()
        steps = gate.fixture_input_steps()[:-1]
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64BoundedSoftmaxTableInputError, "per-head input step count drift"):
            gate.build_score_rows(initial, steps)
    ...
    def test_build_score_rows_rejects_score_gap_bit_overflow(self):
        ...
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64BoundedSoftmaxTableInputError, r"score_gap\[1\] outside 16-bit range"):
            gate.build_score_rows(initial, steps)
    ...
    def test_rejects_output_remainder_bit_overflow(self):
        with self.assertRaisesRegex(gate.AttentionKvD128FourHeadSeq64BoundedSoftmaxTableInputError, "outside 16-bit range"):
