...
from scripts import zkai_attention_kv_d128_two_head_seq64_air_private_softmax_table_lookup_gate as gate


class AttentionKvD128TwoHeadSeq64AirPrivateSoftmaxTableLookupGateTests(unittest.TestCase):
    ...
    def test_build_payload_records_logup_sidecar_go(self):
        ...
        self.assertEqual(receipt["lookup_proof_size_bytes"], 45414)
        self.assertEqual(receipt["lookup_envelope_size_bytes"], 68536643)
        ...
        self.assertEqual(receipt["table_multiplicities"][-1]["multiplicity"], 4277)
        self.assertEqual(payload["fixed_d64_two_head_comparison"]["fixed_d64_two_head_lookup_claims"], 4416)
        self.assertEqual(payload["fixed_d64_two_head_comparison"]["proof_size_ratio"], "1.066883")
        self.assertEqual(payload["fused_comparator"]["source_plus_sidecar_raw_proof_bytes"], 522187)
        self.assertEqual(payload["fused_comparator"]["fused_saves_vs_source_plus_sidecar_bytes"], 40317)
        self.assertEqual(payload["fused_comparator"]["fused_to_source_plus_sidecar_ratio"], "0.922792")
    ...
    def test_tsv_summary_matches_payload(self):
        ...
        self.assertIn("45414", tsv)
        self.assertIn("522187", tsv)
        self.assertIn("40317", tsv)
        self.assertIn("1.066883", tsv)
