#!/usr/bin/env python3
"""Checked fused native Stwo d64 single-head seq16 bounded Softmax-table gate for issue #715.
...
SOURCE_INPUT_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d64-single-head-longseq-bounded-softmax-table-proof-2026-05.json"
SOURCE_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d64-single-head-longseq-bounded-softmax-table-proof-2026-05.envelope.json"
SIDECAR_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d64-single-head-longseq-softmax-table-logup-sidecar-proof-2026-05.envelope.json"
FUSED_ENVELOPE_JSON = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d64-single-head-longseq-fused-softmax-table-proof-2026-05.envelope.json"
SOURCE_INPUT_SCRIPT = ROOT / "scripts" / "zkai_attention_kv_stwo_native_d64_single_head_longseq_bounded_softmax_table_proof_input.py"
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d64-single-head-longseq-fused-softmax-table-gate-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d64-single-head-longseq-fused-softmax-table-gate-2026-05.tsv"
...
SCHEMA = "zkai-attention-kv-stwo-native-d64-single-head-longseq-fused-softmax-table-gate-v1"
ISSUE = 715
SOURCE_ISSUE = 715
SIDECAR_ISSUE = 715
DECISION = "GO_NATIVE_STWO_FUSED_ATTENTION_ARITHMETIC_AND_SOFTMAX_TABLE_LOGUP_MEMBERSHIP"
ROUTE_ID = "local_stwo_attention_kv_d64_single_head_longseq_fused_bounded_softmax_table_logup_proof"
CLAIM_BOUNDARY = (
    "ONE_NATIVE_STWO_D64_SINGLE_HEAD_LONGSEQ_BOUNDED_SOFTMAX_TABLE_ATTENTION_PROOF_WITH_LOGUP_TABLE_MEMBERSHIP_"
...
NON_FUSED_STATUS = "GO_MATCHED_D64_SINGLE_HEAD_LONGSEQ_SOURCE_PLUS_LOGUP_SIDECAR_COMPARATOR_RECORDED"
TIMING_POLICY = "proof_existence_and_byte_accounting_only_not_public_benchmark"

SOURCE_PROOF_SIZE_BYTES = 231_415
SOURCE_ENVELOPE_SIZE_BYTES = 3_267_678
SIDECAR_PROOF_SIZE_BYTES = 22_960
SIDECAR_ENVELOPE_SIZE_BYTES = 1_601_867
SOURCE_PLUS_SIDECAR_RAW_PROOF_BYTES = SOURCE_PROOF_SIZE_BYTES + SIDECAR_PROOF_SIZE_BYTES
FUSED_PROOF_SIZE_BYTES = 237_725
FUSED_ENVELOPE_SIZE_BYTES = 3_320_952
...
FUSED_TO_SOURCE_PLUS_SIDECAR_RATIO = "0.934545"

SOURCE_STATEMENT_COMMITMENT = "blake2b-256:8fbe4d65ed56dfb86cb8267645bc12f9ccf56582335c5dab6626b97806ac0168"
SOURCE_PUBLIC_INSTANCE_COMMITMENT = "blake2b-256:c93a2a984047b2ef9c00bd9d7a86979b2d20b898a4e58e8d193abcb4d6c2773e"
SOURCE_SCORE_ROW_COMMITMENT = "blake2b-256:622e8d110cae6c2e19bda9f165116152e38556c6ff21f7bbaa724b77c18e38a1"
SOURCE_WEIGHT_TABLE_COMMITMENT = "blake2b-256:3282ee0fa00e37aa8db87432d039dfb6d59795368ef9ef1e12dbd14069835fb2"
SOURCE_WEIGHT_POLICY = "exp2_half_gap_table_clipped_8_floor_division"
SOURCE_SCORE_GAP_CLIP = 8
SOURCE_SCORE_ROWS = 168
SOURCE_TRACE_ROWS = 256
SOURCE_TABLE_ROWS = 9
LOOKUP_RELATION = "AttentionKvD64SingleHeadLongseqFusedSoftmaxTableRelation"
LOOKUP_RELATION_WIDTH = 2
SIDECAR_LOOKUP_RELATION = "AttentionKvD64SingleHeadLongseqSoftmaxTableLookupRelation"
SIDECAR_LOOKUP_RELATION_WIDTH = 2
SIDECAR_BACKEND_VERSION = "stwo-attention-kv-d64-single-head-longseq-softmax-table-logup-sidecar-proof-v1"
SIDECAR_STATEMENT_VERSION = "zkai-attention-kv-stwo-native-d64-single-head-longseq-softmax-table-logup-sidecar-statement-v1"
SIDECAR_SEMANTIC_SCOPE = "d64_single_head_longseq_bounded_softmax_table_membership_constrained_by_native_stwo_logup_sidecar"
SIDECAR_TARGET_ID = "attention-kv-d64-single-head-longseq-causal-mask-bounded-softmax-table-logup-sidecar-v1"
SIDECAR_VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d64-single-head-longseq-softmax-table-logup-sidecar:v1"

FUSED_BACKEND_VERSION = "stwo-attention-kv-d64-single-head-longseq-fused-bounded-softmax-table-logup-v1"
FUSED_PROOF_SCHEMA_VERSION = "stwo-attention-kv-d64-single-head-longseq-fused-bounded-softmax-table-logup-proof-v1"
FUSED_STATEMENT_VERSION = "zkai-attention-kv-stwo-native-d64-single-head-longseq-fused-softmax-table-logup-statement-v1"
FUSED_SEMANTIC_SCOPE = "d64_single_head_longseq_bounded_softmax_table_attention_arithmetic_and_logup_membership_fused_in_one_native_stwo_proof"
FUSED_TARGET_ID = "attention-kv-d64-single-head-longseq-causal-mask-fused-bounded-softmax-table-logup-v1"
FUSED_VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d64-single-head-longseq-fused-bounded-softmax-table-logup:v1"
FUSED_PROOF_COMMITMENTS = 4
FUSED_TRACE_COMMITMENTS = 3

TABLE_MULTIPLICITIES = (
    {"gap": 0, "weight": 256, "multiplicity": 35},
    {"gap": 1, "weight": 181, "multiplicity": 3},
    {"gap": 2, "weight": 128, "multiplicity": 2},
    {"gap": 3, "weight": 91, "multiplicity": 1},
    {"gap": 4, "weight": 64, "multiplicity": 7},
    {"gap": 5, "weight": 45, "multiplicity": 2},
    {"gap": 6, "weight": 32, "multiplicity": 4},
    {"gap": 7, "weight": 23, "multiplicity": 0},
    {"gap": 8, "weight": 16, "multiplicity": 114},
)
NON_CLAIMS = (
...
VALIDATION_COMMANDS = (
    "cargo +nightly-2025-07-14 test --locked attention_kv_d64_single_head_longseq_fused_softmax_table --lib --features stwo-backend",
    "cargo +nightly-2025-07-14 run --locked --features stwo-backend --bin zkai_attention_kv_native_d64_single_head_longseq_fused_softmax_table_proof -- prove docs/engineering/evidence/zkai-attention-kv-stwo-native-d64-single-head-longseq-bounded-softmax-table-proof-2026-05.json docs/engineering/evidence/zkai-attention-kv-stwo-native-d64-single-head-longseq-fused-softmax-table-proof-2026-05.envelope.json",
...
class AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(ValueError):
    pass
...
def load_script_module(path: pathlib.Path, module_name: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"failed to load {module_name}: {path}")
...
SOURCE_INPUT_MODULE = load_script_module(
    SOURCE_INPUT_SCRIPT, "zkai_attention_kv_stwo_native_d64_single_head_longseq_bounded_softmax_table_proof_input"
)


def read_bounded_bytes(path: pathlib.Path, max_bytes: int, label: str) -> bytes:
    if not path.is_file():
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"missing {label}: {path}")
    size = path.stat().st_size
    if size <= 0 or size > max_bytes:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"{label} size drift: got {size}, max {max_bytes}")
    try:
        raw = path.read_bytes()
    except OSError as err:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"failed to read {label}: {err}") from err
    if len(raw) != size:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"{label} read size drift: stat={size}, read={len(raw)}")
    return raw
...
def parse_bounded_json_bytes(raw: bytes, label: str) -> Any:
    try:
        ...
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"failed to decode {label}: {err}") from err


def expect_artifact_size(raw: bytes, expected_size: int, label: str) -> None:
    if len(raw) != expected_size:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(
...
def parse_stark_proof(proof_bytes: list[Any], expected_bytes: int, label: str) -> dict[str, Any]:
    if not isinstance(proof_bytes, list):
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"{label} proof must be a byte list")
    if len(proof_bytes) != expected_bytes:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"{label} proof byte length drift")
    if any(not isinstance(byte, int) or isinstance(byte, bool) or byte < 0 or byte > 255 for byte in proof_bytes):
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"{label} proof bytes must be uint8 values")
    try:
        payload = json.loads(bytes(proof_bytes).decode("utf-8"))
    except (UnicodeDecodeError, json.JSONDecodeError) as err:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"failed to decode {label} proof payload: {err}") from err
    if set(payload) != {"stark_proof"}:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"{label} proof payload schema drift")
    stark_proof = payload["stark_proof"]
    if not isinstance(stark_proof, dict):
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"{label} stark_proof must be an object")
    commitments = stark_proof.get("commitments")
    if not isinstance(commitments, list) or len(commitments) != FUSED_PROOF_COMMITMENTS:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"{label} proof commitment count drift")
    return stark_proof
...
def same_digit_int_mutation(value: int, label: str) -> int:
    for candidate in (value + 1, value - 1):
        if 0 <= candidate <= 255 and len(str(candidate)) == len(str(value)):
            return candidate
    raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"no same-width byte mutation for {label}")
...
def mutate_same_size_stark_proof_commitment(envelope: dict[str, Any]) -> None:
    ...
    if len(proof_bytes) != len(envelope["proof"]):
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("same-size proof mutation changed byte length")
...
def proof_bytes(envelope: dict[str, Any]) -> bytes:
    ...
    if not isinstance(proof, list) or not all(isinstance(value, int) for value in proof):
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("fused proof bytes drift")
    return bytes(proof)
...
def verify_fused_envelope_bytes_with_native_cli(envelope_bytes: bytes, label: str) -> None:
    ...
    command = [
        ...
        "stwo-backend",
        "--bin",
        "zkai_attention_kv_native_d64_single_head_longseq_fused_softmax_table_proof",
        "--",
        "verify",
        str(tmp_path),
    ]
    try:
        ...
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"native fused verifier failed to run for {label}: {err}") from err
    finally:
        try:
    ...
    if completed.returncode != 0:
        ...
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"native fused verifier rejected {label}: {suffix}")
    try:
        summary = json.loads(completed.stdout)
    except json.JSONDecodeError as err:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"native fused verifier emitted malformed JSON: {err}") from err
    expected = {
        "mode": "verify",
        "verified": True,
...
def validate_source_artifacts(source_input: dict[str, Any], source_envelope: dict[str, Any], sidecar_envelope: dict[str, Any]) -> None:
    try:
        ...
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"source input validation drift: {err}") from err
    if not type_strict_equal(source_envelope.get("input"), source_input):
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("source envelope/input split-brain drift")
    if len(source_envelope.get("proof", [])) != SOURCE_PROOF_SIZE_BYTES:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("source proof size drift")
    if len(sidecar_envelope.get("proof", [])) != SIDECAR_PROOF_SIZE_BYTES:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("sidecar proof size drift")
    ...
    for key, expected_value in expected_sidecar_scalars.items():
        if not type_strict_equal(sidecar_envelope.get(key), expected_value):
            raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"sidecar {key} drift")
    lookup_summary = sidecar_envelope.get("lookup_summary")
    if not isinstance(lookup_summary, dict):
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("sidecar lookup summary missing")
    expected_sidecar = {
        "source_statement_commitment": SOURCE_STATEMENT_COMMITMENT,
    ...
    for key, expected_value in expected_sidecar.items():
        if not type_strict_equal(lookup_summary.get(key), expected_value):
            raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"sidecar summary drift for {key}")


def validate_fused_envelope(envelope: Any, source_input: dict[str, Any], *, run_native: bool) -> None:
    if not isinstance(envelope, dict):
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("fused envelope must be an object")
    try:
        SOURCE_INPUT_MODULE.validate_payload(source_input)
    except Exception as err:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"source input validation drift: {err}") from err
    ...
    if extra_keys:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"unknown fused envelope field(s): {sorted(extra_keys)}")
    if envelope.get("proof_backend") != "stwo":
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("fused proof backend drift")
    if envelope.get("proof_backend_version") != FUSED_BACKEND_VERSION:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("fused proof backend version drift")
    if envelope.get("proof_schema_version") != FUSED_PROOF_SCHEMA_VERSION:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("fused proof schema version drift")
    if envelope.get("statement_version") != FUSED_STATEMENT_VERSION:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("fused statement version drift")
    if envelope.get("semantic_scope") != FUSED_SEMANTIC_SCOPE:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("fused semantic scope drift")
    if envelope.get("decision") != DECISION:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("fused decision drift")
    if envelope.get("target_id") != FUSED_TARGET_ID:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("fused target id drift")
    if envelope.get("verifier_domain") != FUSED_VERIFIER_DOMAIN:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("fused verifier domain drift")
    if not type_strict_equal(envelope.get("source_input"), source_input):
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("fused source input split-brain drift")
    if not type_strict_equal(envelope.get("fused_summary"), expected_summary(source_input)):
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("fused summary drift")
...
def run_gate() -> dict[str, Any]:
    ...
    if mutation_names != EXPECTED_MUTATION_NAMES:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("mutation case order/name drift")
    rejected = sum(1 for result in mutation_results if result["rejected"])
    if rejected != EXPECTED_MUTATION_COUNT:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"mutation rejection drift: got {rejected}")
...
def validate_result(result: dict[str, Any]) -> None:
    if not isinstance(result, dict):
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("result must be an object")
    ...
    if missing:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"missing result keys: {sorted(missing)}")
    extra = set(result) - required
    if extra:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"unknown result keys: {sorted(extra)}")
    for key in ("fused_envelope_commitment", "fused_proof_commitment"):
        ...
        if not isinstance(value, str) or not value.startswith("blake2b-256:"):
            raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"{key} drift")
    for key, expected_value in expected_exact.items():
        if not type_strict_equal(result.get(key), expected_value):
            raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError(f"result drift for {key}")
    ...
    if not isinstance(mutation_results, list) or len(mutation_results) != EXPECTED_MUTATION_COUNT:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("mutation result shape drift")
    for item in mutation_results:
        if not isinstance(item, dict):
            raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("mutation result shape drift")
        if set(item) != {"name", "rejected", "error"}:
            raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("mutation result schema drift")
    mutation_names = tuple(item["name"] for item in mutation_results)
    if mutation_names != EXPECTED_MUTATION_NAMES:
        raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("mutation result name drift")
    for item in mutation_results:
        if (
            ...
            raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("mutation result rejection drift")
...
def write_tsv(path: pathlib.Path, result: dict[str, Any]) -> None:
    ...
    try:
        ...
        if rows != [expected_row]:
            raise AttentionKvD64SingleHeadLongseqFusedSoftmaxTableGateError("TSV round-trip drift")
        tmp_path.replace(path)
    except Exception:
        tmp_path.unlink(missing_ok=True)
        raise
