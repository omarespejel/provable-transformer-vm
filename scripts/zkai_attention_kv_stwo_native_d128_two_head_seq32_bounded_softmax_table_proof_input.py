#!/usr/bin/env python3
"""Build the native Stwo d128 two-head seq32 bounded Softmax-table attention/KV proof input.

This answers issue #715 narrowly. It widens the checked d64 two-head seq32
...
JSON_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d128-two-head-seq32-bounded-softmax-table-proof-2026-05.json"
TSV_OUT = EVIDENCE_DIR / "zkai-attention-kv-stwo-native-d128-two-head-seq32-bounded-softmax-table-proof-2026-05.tsv"
...
ISSUE = 715
SOURCE_ISSUE = 720
TARGET_ID = "attention-kv-d128-two-head-seq32-causal-mask-bounded-softmax-table-v1"
REQUIRED_BACKEND_VERSION = "stwo-attention-kv-d128-two-head-seq32-causal-mask-bounded-softmax-table-v1"
PROOF_VERSION = "stwo-attention-kv-d128-two-head-seq32-causal-mask-bounded-softmax-table-air-proof-v1"
STATEMENT_VERSION = "zkai-attention-kv-stwo-native-d128-two-head-seq32-bounded-softmax-table-statement-v1"
SEMANTIC_SCOPE = "d128_two_head_seq32_bounded_table_softmax_approx_attention_kv_causal_mask_rows_bound_to_statement_receipt"
VERIFIER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-two-head-seq32-bounded-softmax-table:v1"
...
ROW_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-two-head-seq32-bounded-softmax-table-score-rows:v1"
INITIAL_KV_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-two-head-seq32-bounded-softmax-table-initial-kv:v1"
INPUT_STEPS_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-two-head-seq32-bounded-softmax-table-input-steps:v1"
FINAL_KV_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-two-head-seq32-bounded-softmax-table-final-kv:v1"
OUTPUTS_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-two-head-seq32-bounded-softmax-table-outputs:v1"
PUBLIC_INSTANCE_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-two-head-seq32-bounded-softmax-table-public-instance:v1"
PROOF_NATIVE_PARAMETER_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-two-head-seq32-bounded-softmax-table-proof-parameters:v1"
WEIGHT_TABLE_DOMAIN = "ptvm:zkai:attention-kv-stwo-native-d128-two-head-seq32-bounded-softmax-table-weight-table:v1"
...
NEXT_BACKEND_STEP = "fuse the same d128 two-head seq32 bounded Softmax-table arithmetic with LogUp table membership in one native Stwo proof"
...
class AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(ValueError):
    pass
...
def _load_source_module() -> Any:
    ...
    if spec is None or spec.loader is None:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"failed to load source script: {SOURCE_SCRIPT}")
...
def require_bounded_int(value: Any, label: str) -> int:
    if not isinstance(value, int) or isinstance(value, bool):
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"{label} must be an integer")
    if abs(value) > MAX_ABS_VALUE:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"{label} outside bounded fixture range")
    return value
...
def require_nonnegative_bit_bound(value: int, *, bits: int, label: str) -> int:
    ...
    if checked < 0 or checked >= (1 << bits):
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"{label} outside {bits}-bit range")
    return checked


def next_power_of_two(value: int) -> int:
    if value <= 0:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("trace row count input must be positive")
    return 1 << (value - 1).bit_length()


def require_vector(value: Any, *, width: int, label: str) -> list[int]:
    if not isinstance(value, list):
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"{label} must be a list")
    if len(value) != width:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"{label} width drift")
...
def require_head_index(value: Any, label: str) -> int:
    ...
    if head_index < 0 or head_index >= HEAD_COUNT:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"{label} outside head range")
    return head_index


def require_kv_item(value: Any, *, label: str) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"{label} must be an object")
    return {
...
def source_payload() -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("source payload must be an object")
    expected_scalars = {
    ...
    for key, expected in expected_scalars.items():
        if payload.get(key) != expected:
            raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"source payload {key} drift")
    observed = sha256_hex_bytes(canonical_json_bytes(payload))
    if observed != SOURCE_PAYLOAD_SHA256:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("source payload commitment drift")
    return payload


def dot(lhs: list[int], rhs: list[int], *, label: str) -> int:
    if len(lhs) != KEY_WIDTH or len(rhs) != KEY_WIDTH:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("dot-product width mismatch")
    total = 0
...
def weight_from_gap(score_gap: int) -> int:
    if score_gap < 0:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("negative score gap")
...
def rows_commitment(rows: list[dict[str, Any]]) -> str:
    return commitment_from_parts(
        [
            ("encoding", "attention_kv_stwo_native_d128_two_head_seq32_bounded_softmax_table_score_rows_v1"),
...
def outputs_material(steps: list[dict[str, Any]], outputs: list[list[int]]) -> list[list[int]]:
    if len(steps) != len(outputs):
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("output/input step length mismatch")
...
def widen_vector(
    ...
    *,
    ...
    if len(widened) != target_width:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"{label} widened width drift")
    return widened
...
def fixture_initial_kv() -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(source_items):
        if not isinstance(item, dict):
            raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"initial_kv[{index}] must be an object")
    ...
    if len(items) != INITIAL_KV_ITEMS:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("source initial KV length drift")
    return items
...
def fixture_input_steps() -> list[dict[str, Any]]:
    ...
    for index, item in enumerate(require_sequence(steps, label="input_steps")):
        if not isinstance(item, dict):
            raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"input_steps[{index}] must be an object")
    ...
    if len(seed_items) != HEAD_COUNT * SOURCE_SEQUENCE_LENGTH:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("source input step length drift")
    ...
    for head_index, head_steps in enumerate(per_head):
        if len(head_steps) != SOURCE_SEQUENCE_LENGTH:
            raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"source head {head_index} step count drift")
        for local_step, item in enumerate(head_steps):
            ...
            if item["token_position"] != expected_position:
                raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(
    ...
    if len(extended) != HEAD_COUNT * SEQUENCE_LENGTH:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("extended input step length drift")
    return extended
...
def build_score_rows(initial_kv: list[dict[str, Any]], input_steps: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[list[int]]]:
    ...
    for global_step_index, step in enumerate(steps):
        ...
        if step_index >= SEQUENCE_LENGTH:
            raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"input_steps[{global_step_index}] exceeds per-head sequence length")
        ...
        if not allowed:
            raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"input_steps[{global_step_index}] has no allowed candidates")
        scored = [
            (
                candidate,
                dot(
        ...
        if any(weight <= 0 for weight in weights):
            raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"input_steps[{global_step_index}] has zero weight")
        denominator = require_bounded_int(sum(weights), f"input_steps[{global_step_index}].weight_denominator")
        if denominator <= 0:
            raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"input_steps[{global_step_index}] has invalid weight denominator")
    ...
    if local_step_counts != [SEQUENCE_LENGTH for _ in range(HEAD_COUNT)]:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("per-head input step count drift")
    return rows, current, outputs


def validate_payload(payload: dict[str, Any]) -> None:
    if not isinstance(payload, dict):
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("payload must be object")
    expected_scalars = {
        "schema": SCHEMA,
        "decision": DECISION,
        "issue": ISSUE,
    ...
    if extra:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"unknown field(s): {extra}")
    for key, expected in expected_scalars.items():
        if payload.get(key) != expected:
            raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError(f"{key} drift")
    if payload.get("non_claims") != NON_CLAIMS:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("non claims drift")
    if payload.get("proof_verifier_hardening") != PROOF_VERIFIER_HARDENING:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("proof verifier hardening drift")
    if payload.get("validation_commands") != VALIDATION_COMMANDS:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("validation commands drift")
    if payload.get("weight_table_commitment") != weight_table_commitment():
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("weight table commitment drift")
    ...
    if len(initial) != payload.get("initial_kv_items"):
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("derived initial KV count drift")
    if len(final_cache) != payload.get("final_kv_items"):
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("derived final KV count drift")
    if len(rows) != payload.get("score_row_count"):
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("derived score row count drift")
    if next_power_of_two(len(rows)) != payload.get("trace_row_count"):
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("derived trace row count drift")
    if payload.get("initial_kv_cache") != initial:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("initial KV drift")
    if payload.get("input_steps") != steps:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("input steps drift")
    if payload.get("score_rows") != rows:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("score rows drift")
    if payload.get("final_kv_cache") != final_cache:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("final KV drift")
    if payload.get("attention_outputs") != outputs:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("attention outputs drift")
    ...
    if payload.get("statement_commitment") != statement:
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("statement commitment drift")
    if payload.get("public_instance_commitment") != public_instance_commitment(statement):
        raise AttentionKvD128TwoHeadSeq32BoundedSoftmaxTableInputError("public instance commitment drift")
